...
/* questionMeta arrays for each skill (5 questions each) */
const metas = {
...
const skillFiles = Object.keys(metas);
skillFiles.forEach(f => {
    ...
    // Insert questionMeta constant before the return statement
    const insertBefore = '\n    return (\n        <ToyJoyPracticeTemplate';
    const metaBlock = `\n    const questionMeta = ${meta};\n`;

    if (content.includes('const questionMeta')) {
        // replace existing
        content = content.replace(/\n    const questionMeta = \[[\s\S]*?\];\n/, metaBlock);
    } else {
        content = content.replace(insertBefore, metaBlock + insertBefore);
    }

    // Add questionMeta prop to the template call
    content = content.replace(
        /logicProps={logicProps}\s*\/>/g,
