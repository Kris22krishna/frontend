...
const HighPracticeSession = () => {
    ...
    useEffect(() => {
        if (skillsList.length === 0 && skillId) {
            api.getSkillById(skillId).then(async (currentSkill) => {
                if (currentSkill?.grade) setGradeLevel(currentSkill.grade);
    ...
    const handleCheck = async (selectedVal) => {
        ...
        const attempt = {
            ...
            isCorrect // Store explicitly for easier access
        };
        ...
        if (isCorrect) {
            setStats(prev => ({ ...prev, correct: prev.correct + 1, total: prev.total + 1, streak: prev.streak + 1 }));
        } else {
            setStats(prev => ({ ...prev, wrong: prev.wrong + 1, total: prev.total + 1, streak: 0 }));
        }
    };
    ...
    const handleNextStep = async () => {
        ...
        // Ensure we are looking at the attempt for the current question
        const isCorrect = lastAttempt && lastAttempt.isCorrect;

        let nextDiff = currentDifficulty;
        let nextLevelCount = correctCountAtLevel;
        ...
        if (isCorrect) {
            ...
            if (currentDifficulty === 'Medium' || currentDifficulty === 'Hard') {
                ...
                nextWrongStreak = 0; // Reset wrong streak on demotion to give breathing room? Or keep it? Junior keeps it until demotion triggers?
                // Junior resets wrongStreak on difficulty change.
            }

            // If already Easy and failing
    ...
    const loadingMessages = [
        "Preparing your blackboard...", "Calculating variables...", "Fine-tuning the geometry...", "Polishing your scratchpad...", "Setting up the theorem...", "Almost ready for math!"
    ];
    const [msgIndex, setMsgIndex] = useState(0);
    useEffect(() => {
        let timer;
        if (loading) {
            timer = setInterval(() => { setMsgIndex(prev => (prev + 1) % loadingMessages.length); }, 2500);
        }
        return () => clearInterval(timer);
    }, [loading]);
    ...
    if (loading) return (
        <div className="high-practice-page high-loading-screen">
            <div className="high-loading-content">
                <div className="high-loading-icon"><RefreshCw size={48} className="high-spin" /></div>
                <h3 className="high-loading-msg">{loadingMessages[msgIndex]}</h3>
                <div className="high-loading-bar"><div className="high-loading-progress"></div></div>
            </div>
        </div>
    );
        ...
        <div className="high-practice-page">
            <main className="high-main-content" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100%' }}>
                <div className="question-card" style={{ textAlign: 'center', padding: '3rem' }}>
                    ...
                    <h2 style={{ fontFamily: 'Merriweather, serif', marginBottom: '1rem', color: '#31326F', fontSize: '2rem', fontWeight: 'bold' }}>Oops! This content isn’t available yet!!</h2>
                    <p style={{ color: '#64748b', marginBottom: '2rem', fontSize: '1.1rem' }}>But don’t worry — we’re working hard to bring it to you soon! 🚀</p>
                    <Link to={gradeLevel ? `/senior/grade/${gradeLevel}` : '/math'} className="high-btn primary" style={{ textDecoration: 'none', display: 'inline-block', padding: '0.75rem 2rem', fontSize: '1rem' }}>Return to Dashboard</Link>
                </div>
            </main>
        </div>
    );
                    ...
                    <div className="bg-white rounded-[2rem] shadow-xl overflow-hidden border border-slate-200">
                        <div className="bg-slate-900 p-12 text-white text-center">
                            <div className="flex justify-between items-start mb-6">
                                ...
                                <button onClick={() => navigate(gradeLevel ? `/senior/grade/${gradeLevel}` : "/math")} className="back-topics-top px-10 py-3 bg-slate-800 text-white rounded-xl font-bold hover:bg-slate-700 transition-all flex items-center gap-3 text-lg shadow-lg border border-slate-700 relative z-[1001]"><ArrowLeft size={20} /> Back to Topics</button>
                            </div>
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-8 mt-12">
                                <div className="text-center"><p className="text-slate-500 dark:text-slate-400 uppercase tracking-widest text-xs font-bold mb-2">Accuracy</p><p className="text-4xl font-bold dark:text-white">{accuracy}%</p></div>
                                <div className="text-center"><p className="text-slate-500 dark:text-slate-400 uppercase tracking-widest text-xs font-bold mb-2">Time Taken</p><p className="text-4xl font-bold dark:text-white">{formatTime(elapsedTime)}</p></div>
                                <div className="text-center"><p className="text-slate-500 dark:text-slate-400 uppercase tracking-widest text-xs font-bold mb-2">Correct</p><p className="text-4xl font-bold text-emerald-400">{stats.correct}</p></div>
                                <div className="text-center"><p className="text-slate-500 dark:text-slate-400 uppercase tracking-widest text-xs font-bold mb-2">Mastery</p><p className="text-4xl font-bold text-blue-400">{currentDifficulty}</p></div>
                            </div>
                        </div>
                        <div className="p-12">
                            <h2 className="text-2xl font-serif font-bold text-slate-900 mb-8 flex items-center gap-3"><Target className="text-slate-400" />Detailed Performance Breakdown</h2>
                                ...
                                {history.map((item, idx) => (
                                    <div key={idx} className={`p-8 rounded-2xl border-l-4 transition-all ${item.status === 'correct' ? 'border-emerald-500 bg-emerald-50/20' : 'border-rose-500 bg-rose-50/20'}`}>
                                        <div className="flex items-start justify-between gap-6 mb-6">
                                            <div className="flex items-center gap-4"><span className={`w-8 h-8 rounded shrink-0 flex items-center justify-center font-bold font-serif ${item.status === 'correct' ? 'bg-emerald-500 text-white' : 'bg-rose-500 text-white'}`}>{idx + 1}</span><div className="text-lg font-serif text-slate-800 leading-relaxed"><LatexContent html={item.question.text} /></div></div>
                                            {item.status === 'correct' ? (<div className="flex items-center gap-2 text-emerald-600 font-bold bg-emerald-100 px-3 py-1 rounded-full text-sm"><Check size={16} /> CORRECT</div>) : (<div className="flex items-center gap-2 text-rose-600 font-bold bg-rose-100 px-3 py-1 rounded-full text-sm"><X size={16} /> INCORRECT</div>)}
                                        </div>
                                        <div className="grid md:grid-cols-2 gap-8 mb-6 ml-12">
                                            <div className="space-y-2"><p className="text-slate-400 uppercase tracking-widest text-[10px] font-bold">Your Response</p><div className={`p-4 rounded-xl font-medium ${item.status === 'correct' ? 'bg-emerald-100/50 text-emerald-900' : 'bg-rose-100/50 text-rose-900'}`}><LatexContent html={item.userVal} /></div></div>
                                            {!item.status === 'correct' && (<div className="space-y-2"><p className="text-slate-500 uppercase tracking-widest text-[10px] font-bold">Key Solution</p><div className="p-4 rounded-xl bg-slate-100 text-slate-900 font-medium"><LatexContent html={item.question.correctAnswer} /></div></div>)}
                                        </div>
                                        <div className="ml-12 p-6 bg-white rounded-xl border border-slate-100 shadow-sm"><p className="text-slate-500 uppercase tracking-widest text-[10px] font-bold mb-3 flex items-center gap-2"><BookOpen size={12} /> Step-by-Step Explanation</p><div className="text-slate-600 font-serif leading-relaxed"><LatexContent html={item.question.solution} /></div></div>
                                    </div>
                                ))}
                            </div>
                            <div className="flex items-center justify-center gap-6 mt-16 pt-12 border-t border-slate-100">
                                <button onClick={() => window.location.reload()} className="px-12 py-4 border-2 border-slate-200 text-slate-600 rounded-xl font-bold hover:bg-slate-50 transition-all flex items-center gap-2"><RefreshCw size={20} /> Reset Session</button>
                            </div>
                        </div>
                    </div>
                </main>
                ...
                {suggestionModal.isOpen && (
                    <motion.div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                        <motion.div className="bg-white rounded-3xl shadow-2xl p-8 max-w-sm w-full text-center relative overflow-hidden border-4 border-white" initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, y: 20 }}>
                            <div className="absolute top-0 left-0 w-full h-24 bg-gradient-to-br from-indigo-100 to-indigo-200 -z-10 rounded-t-2xl" />
                            ...
                            <h3 className="text-2xl font-serif font-bold text-slate-900 mb-2">
                                {suggestionModal.type === 'next' && "Impressive Work! 🌟"}
                                {suggestionModal.type === 'prev' && "Challenging Topic? 🤔"}
                                {suggestionModal.type === 'mentor' && "Need Clarity? 💡"}
                            </h3>
                            <p className="text-slate-600 mb-6 font-medium leading-relaxed">
                                {suggestionModal.type === 'next' && `You've mastered this! Ready to tackle "${suggestionModal.skill?.skill_name || suggestionModal.skill?.name}"?`}
                                {suggestionModal.type === 'prev' && `This seems tough. Shall we review "${suggestionModal.skill?.skill_name || suggestionModal.skill?.name}" first?`}
                                {suggestionModal.type === 'mentor' && "It looks like you might need some help. Consider asking a mentor for guidance!"}
                            </p>
                            <div className="flex flex-col gap-3">
                                <button onClick={async () => {
                                    if (suggestionModal.type === 'mentor') {
                                        ...
                                        setSuggestionModal({ isOpen: false, type: null, skill: null });
                                        navigate(`/senior/grade/${gradeLevel}/practice/${newSkill.skill_id}`, { state: { skills: skillsList, currentIndex: newIndex, grade: gradeLevel } });
                                    }
                                }} className="w-full py-3 bg-slate-900 text-white rounded-xl font-bold text-lg shadow-lg hover:bg-slate-800 transition-all">
                                    {suggestionModal.type === 'next' ? "Start Next Skill" : suggestionModal.type === 'prev' ? "Go to Previous Skill" : "Understood"}
                                </button>
                                <button onClick={async () => {
                                    setSuggestionModal({ isOpen: false, type: null, skill: null });
                                    if (suggestionModal.type === 'next') setCorrectCountAtLevel(0);
                                    if (suggestionModal.type === 'prev') setWrongStreak(0);
                                    await fetchQuestions(currentDifficulty, false);
                                    setCurrentIndex(prev => prev + 1);
                                }} className="w-full py-3 bg-white text-slate-500 rounded-xl font-bold text-lg hover:bg-slate-50 transition-colors border border-slate-200">
                ...
                <main className="high-main-content">
                    <div className="high-header">
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                            ...
                            {!feedback ? (
                                ...
                                <button className="high-btn primary" onClick={handleNextStep} style={{ padding: '0.5rem 1.5rem', fontSize: '0.9rem' }}>Next Question</button>
                            )}
                            <button className={`high-btn secondary high-mobile-scratchpad-toggle ${showScratchpad ? 'active' : ''}`} onClick={() => setShowScratchpad(!showScratchpad)} title="Open Scratchpad" style={{ padding: '0.5rem' }}><PenTool size={18} /></button>
                            <Link to={gradeLevel ? `/senior/grade/${gradeLevel}` : '/math'} className="high-btn secondary" style={{ padding: '0.5rem 1rem', fontSize: '0.9rem' }} onClick={() => { if (sessionId) api.finishSession(sessionId).catch(console.error); }}>Exit</Link>
                        </div>
                    </div>
                    ...
                    <div className="question-card">
                        ...
                        <div className="question-content">
                            {currentQ?.text && (<LatexContent html={currentQ.text} className="question-text" block={true} />)}
                            {currentQ?.imageUrl && (<img src={currentQ.imageUrl} alt="Question" style={{ maxWidth: '100%', marginBottom: '2rem', borderRadius: '4px' }} />)}
                        </div>
                        ...
                        <div className="high-options-grid">
                            {currentQ?.options?.map((opt, idx) => {
                                ...
                                return (
                                    <button key={idx} className={btnClass} onClick={() => !feedback && handleCheck(opt)} disabled={!!feedback}>
                        ...
                        {(currentQ?.type === 'input' || !currentQ?.options || currentQ?.options.length === 0) && (
                            <div className="high-input-group">
                                <input type="text" className="high-text-input" value={userAnswer} onChange={(e) => setUserAnswer(e.target.value)} placeholder="Enter answer here..." disabled={!!feedback} onKeyDown={(e) => { if (e.key === 'Enter' && !feedback && userAnswer.trim()) { handleCheck(userAnswer); } }} />
                                {!feedback && (<button className="high-submit-btn" onClick={() => handleCheck(userAnswer)} disabled={!userAnswer.trim()}>Submit Answer</button>)}
                            </div>
                        )}
                        ...
                        {feedback && (
                            <div className="high-feedback-container">
                                <div className={`high-feedback-row ${feedback}`}>
                                    <span className="high-feedback-msg">{feedback === 'correct' ? 'Correct Answer!' : 'Incorrect Answer'}</span>
                                </div>
                        ...
                        <div className="high-actions"></div>
                    </div>
                    {showScratchpad && (<FullScreenScratchpad onClose={() => setShowScratchpad(false)} />)}
                </main>
                <aside className="high-tools-col">
                    <div className="high-notebook">
                        <div style={{ flex: 1, position: 'relative' }}><Whiteboard isOpen={true} /></div>
                    </div>
                </aside>
            </div>
        </div>
