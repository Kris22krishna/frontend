    ...
    switch (name) {
        case "Circle":
            return `<svg width="200" height="200" viewBox="0 0 200 200" xmlns="http://www.w3.org/2000/svg">
                <circle cx="100" cy="100" r="80" stroke="${strokeColor}" stroke-width="4" fill="${fillColor}" />
                <line x1="100" y1="20" x2="100" y2="180" stroke="${strokeColor}" stroke-width="2" stroke-dasharray="5,5" />
            </svg>`;
        case "Butterfly":
            return `<svg width="200" height="200" viewBox="0 0 200 200" xmlns="http://www.w3.org/2000/svg">
                <path d="M100,100 Q60,40 20,100 Q60,160 100,100 Q140,40 180,100 Q140,160 100,100" stroke="${strokeColor}" stroke-width="4" fill="${fillColor}" />
                <line x1="100" y1="20" x2="100" y2="180" stroke="${strokeColor}" stroke-width="2" stroke-dasharray="5,5" />
            </svg>`;
        case "Letter F":
            return `<svg width="200" height="200" viewBox="0 0 200 200" xmlns="http://www.w3.org/2000/svg">
                <text x="60" y="160" font-family="Arial" font-size="150" fill="${strokeColor}">F</text>
                <line x1="100" y1="20" x2="100" y2="180" stroke="#f43f5e" stroke-width="2" stroke-dasharray="5,5" />
            </svg>`;
        case "Letter G":
            return `<svg width="200" height="200" viewBox="0 0 200 200" xmlns="http://www.w3.org/2000/svg">
                <text x="50" y="160" font-family="Arial" font-size="150" fill="${strokeColor}">G</text>
                <line x1="100" y1="20" x2="100" y2="180" stroke="#f43f5e" stroke-width="2" stroke-dasharray="5,5" />
            </svg>`;
        default:
            return "";
...
const createFractionSVG = (num, den) => {
    const size = 200;
    const center = size / 2;
    const radius = 80;
    const strokeColor = "#4f46e5";
    ...
    let paths = "";
    for (let i = 0; i < den; i++) {
        const startAngle = (i * 2 * Math.PI) / den;
        const endAngle = ((i + 1) * 2 * Math.PI) / den;
        ...
        const color = i < num ? activeColor : fillColor;

        paths += `<path d="M${center},${center} L${x1},${y1} A${radius},${radius} 0 ${largeArc} 1 ${x2},${y2} Z" fill="${color}" stroke="${strokeColor}" stroke-width="2" />`;
    }
...
export const generateFractions = () => {
    ...
    const question = `What fraction represents the shaded part out of ${denominator} equal parts?`;
    ...
    return {
        ...
        image: createFractionSVG(numerator, denominator),
        options: options,
        answer: answer
    };
};
...
export const generateSymmetry = () => {
    ...
    return {
        ...
        image: createSymmetrySVG(obj.name),
        options: [
            { value: "Yes", label: "Yes" },
            { value: "No", label: "No" }
        ],
...
//         image: `/src/assets/grade3/ClockAt${hour}.png`,
//         options: options,
//         answer: time
//     };
// };
...
const createClockSVG = (hour) => {
    ...
    svg += `<circle cx="${center}" cy="${center}" r="${radius}" stroke="${strokeColor}" stroke-width="4" fill="white" />`;
    ...
    for (let i = 1; i <= 12; i++) {
        ...
        svg += `<line x1="${x1}" y1="${y1}" x2="${x2}" y2="${y2}" stroke="${strokeColor}" stroke-width="2" />`;
    }

    // Hands
    const hourAngle = (hour * 30 - 90) * (Math.PI / 180);
    const minuteAngle = -Math.PI / 2; // Always at 12

    // Hour hand
    svg += `<line x1="${center}" y1="${center}" x2="${center + radius * 0.5 * Math.cos(hourAngle)}" y2="${center + radius * 0.5 * Math.sin(hourAngle)}" stroke="${strokeColor}" stroke-width="6" stroke-linecap="round" />`;
    // Minute hand
    svg += `<line x1="${center}" y1="${center}" x2="${center + radius * 0.8 * Math.cos(minuteAngle)}" y2="${center + radius * 0.8 * Math.sin(minuteAngle)}" stroke="#f43f5e" stroke-width="4" stroke-linecap="round" />`;
    // Center point
    svg += `<circle cx="${center}" cy="${center}" r="4" fill="${strokeColor}" />`;
...
export const generateTimeReading = () => {
    ...
    const wrongHours = [];
    while (wrongHours.length < 3) {
        const w = getRandomInt(1, 12);
        if (w !== hour && !wrongHours.includes(w)) wrongHours.push(w);
    }

    const options = shuffleArray([
        { value: hour.toString(), label: hour.toString() },
        ...wrongHours.map(w => ({ value: w.toString(), label: w.toString() }))
    ]);
