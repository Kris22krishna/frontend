...
export default function EverEvolvingScienceCoreConcepts() {
    ...
    const openSkill = (idx, nextView) => {
        ...
        const allQuestions = [...CORE_CONCEPTS[idx].questionBank];
        for (let i = allQuestions.length - 1; i > 0; i--) {
            const j = Math.floor(Math.random() * (i + 1));
            [allQuestions[i], allQuestions[j]] = [allQuestions[j], allQuestions[i]];
        }
        
        // Deal 20 for practice, 10 for assess
        setPracticeQ(allQuestions.slice(0, 20));
        setAssessQ(allQuestions.slice(20, 30));
    ...
    const NAV = (
        <nav className={styles['chem-nav']}>
            <button className={styles['chem-nav-back']} onClick={() => {
                ...
                else navigate('/middle/grade/7/science/wonderful-world-science');
            }}>
        ...
        return (
            <div className={styles['chem-page']}>
                ...
                <div className={styles['chem-hero']}>
                    ...
                    <p className={styles['chem-hero-sub']}>Master the central pillars of Chapter 1 — the foundation of your Grade 7 science journey.</p>
                </div>
                ...
                <div style={{ maxWidth: 960, margin: '40px auto', padding: '0 24px 60px' }}>
                    ...
                    <div style={{ marginTop: 60, padding: 36, borderRadius: 24, background: 'linear-gradient(135deg, #f59e0b, #ef4444)', color: '#fff', textAlign: 'center', boxShadow: '0 15px 40px rgba(245,158,11,0.25)' }}>
                        ...
                        <h3 style={{ fontFamily: 'Outfit, sans-serif', fontSize: 24, fontWeight: 800, margin: '0 0 8px' }}>See the Big Picture?</h3>
                        <p style={{ opacity: 0.9, maxWidth: 460, margin: '0 auto 24px', fontSize: 15 }}>
                            Head to Connectomics to discover how all branches of science are interconnected.
                        </p>
                        <button onClick={() => navigate('/middle/grade/7/science/wonderful-world-science/connectomics')}
                            ...
                            <main className="chem-details-window-anim chem-details-window" key={`${skill.id}-${selectedRuleIdx}`}>
                                {rule && (
                                    <>
                                        ...
                                        <div style={{ background: `${skill.color}05`, padding: '24px 16px', borderRadius: 20, border: `2px solid ${skill.color}15`, marginBottom: 32, textAlign: 'center', wordWrap: 'break-word', whiteSpace: 'normal' }}>
                                            <div className="formula-text" style={{ fontSize: 'clamp(22px, 5vw, 32px)', fontWeight: 800, color: skill.color, lineHeight: 1.4 }}>
                                        ...
                                        <div className="chem-rule-split" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 32, marginBottom: 32 }}>
                                            ...
                                            <div style={{ minWidth: 0 }}>
                                                ...
                                                <div style={{ background: '#f8fafc', padding: '24px 20px', borderRadius: 20, border: '1px solid rgba(0,0,0,0.03)', wordWrap: 'break-word', whiteSpace: 'normal' }}>
                                                    <div style={{ fontSize: 18, fontWeight: 600, color: 'var(--chem-text, #0f172a)', lineHeight: 1.5 }}>
