...
export default function ScienceAssessmentEngine({ questions, title, onBack, onSecondaryBack, color, nodeId, prefix = 'chemtest' }) {
    ...
    const handlePrev = () => { if (current > 0) setCurrent((index) => index - 1); };
    ...
    const palette = (
        <div className={`${prefix}-assessment-palette ${paletteOpen ? 'is-open' : ''}`}>
            <div className={`${prefix}-assessment-mobile-head`}>
                ...
                <button type="button" className={`${prefix}-palette-close`} onClick={() => setPaletteOpen(false)}>Close</button>
            </div>
            ...
            <div style={{
                ...
                borderRadius: 16, marginBottom: 20, fontWeight: 900, fontSize: 24,
            ...
            <div className={`${prefix}-palette-mobile-stats`}>
                <div><span>Answered</span><strong>{answeredCount}/{questionSet.length}</strong></div>
                <div><span>Current</span><strong>Q{current + 1}</strong></div>
            </div>
            ...
            <div className={`${prefix}-palette-grid`} style={{ display: 'grid', gridTemplateColumns: 'repeat(5, minmax(0, 42px))', justifyContent: 'space-between', gap: 8 }}>
                {questionSet.map((_, index) => {
                    ...
                    if (isMarked) { border = '2px solid #f59e0b'; if (!isAnswered) txt = '#f59e0b'; }
                    if (isCurrent) border = `2px solid var(--${prefix}-text, #0f172a)`;
                    ...
                    return (
                        <button
                            ...
                            style={{ width: '42px', height: '42px', borderRadius: 8, fontSize: 13, fontWeight: 700, background: bg, color: txt, border, cursor: 'pointer', padding: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}
                        >
                            {index + 1}
                        </button>
                    );
            ...
            <div style={{ marginTop: 20, fontSize: 12, color: `var(--${prefix}-muted, #64748b)` }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}><div style={{ width: 14, height: 14, background: color, borderRadius: 3 }} /> Answered</div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}><div style={{ width: 14, height: 14, background: '#fff', border: '1px solid #cbd5e1', borderRadius: 3 }} /> Not Answered</div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}><div style={{ width: 14, height: 14, background: '#fff', border: '2px solid #f59e0b', borderRadius: 3 }} /> Marked for Review</div>
            </div>
        ...
        <div className={`${prefix}-quiz-active ${prefix}-assessment-layout`}>
            <div style={{ flex: 1 }} ref={topRef}>
                <div className={`${prefix}-score-header`} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                    ...
                    <button
                        onClick={() => { if (window.confirm('Are you sure you want to exit? Your progress will be lost.')) onBack(); }}
                        style={{ display: 'flex', alignItems: 'center', gap: '6px', background: '#fee2e2', color: '#ef4444', border: '1px solid #fca5a5', padding: '6px 14px', borderRadius: '8px', fontSize: '14px', fontWeight: '700', cursor: 'pointer' }}
                    >
                        Exit
                    </button>
                </div>
                    ...
                    {getQuestionType(q) === 'text' ? (
                        <div style={{ display: 'grid', gap: 12 }}>
                            ...
                            <input
                                ...
                                style={{ width: '100%', padding: '14px 16px', borderRadius: 12, border: `2px solid ${isAnswerComplete(q, answers[current]) ? color : 'rgba(0,0,0,0.08)'}`, background: '#fff', color: `var(--${prefix}-text, #1e293b)`, fontSize: 15, fontWeight: 600, outline: 'none' }}
                            />
                        </div>
                    ) : (
                                ...
                                return (
                                    <button
                                        ...
                                        style={{
                                            ...
                                            color: `var(--${prefix}-text, #1e293b)`, width: '100%', minHeight: 78, lineHeight: 1.55
                                        }}
                                    >
                                        <div style={{ width: 8, height: 8, borderRadius: '50%', background: isSelected ? color : '#f1f5f9', flexShrink: 0, marginTop: 6 }} />
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 24, gap: 16 }}>
                    <button onClick={handlePrev} disabled={current === 0} className={`${prefix}-btn-secondary`}
                    ...
                    <button onClick={toggleMarkForReview} className={`${prefix}-btn-secondary`}
                        style={{ padding: '12px 28px', fontSize: 15, borderRadius: 100, flex: 1, maxWidth: 200, margin: '0 auto', background: '#fff', border: `1px solid ${markedForReview[current] ? '#f59e0b' : '#e2e8f0'}`, color: markedForReview[current] ? '#d97706' : `var(--${prefix}-text)`, fontWeight: 600 }}>
                        {markedForReview[current] ? 'Marked for Review' : 'Mark for Review'}
                    </button>
                    {current + 1 === questionSet.length ? (
                        <button onClick={handleSubmit} className={`${prefix}-btn-primary`}
                        ...
                        <button onClick={handleNext} className={`${prefix}-btn-primary`}
                            style={{ background: `var(--${prefix}-blue, #2563eb)`, border: 'none', color: '#fff', padding: '12px 28px', fontSize: 15, borderRadius: 100, flex: 1, maxWidth: 200, fontWeight: 600 }}>
                            Next →
                        </button>
                    )}
