...
export default function ScienceQuizEngine({ questions, title, onBack, onSecondaryBack, color, nodeId, prefix = 'chemtest' }) {
    // v4 Logging
            ...
            <div className={`${prefix}-quiz-card`}>
                ...
                <div className={`${prefix}-quiz-options`}>
                    {q.options.map((opt, oi) => {
                        ...
                        let bgColor = '#fff';
                        let textColor = `var(--${prefix}-text, #1e293b)`;
                        let dotColor = '#f1f5f9';

                        if (answered) {
                            if (oi === q.correct) {
                                borderColor = `var(--${prefix}-teal, #0d9488)`;
                                bgColor = 'rgba(16,185,129,0.05)';
                                textColor = `var(--${prefix}-teal, #0d9488)`;
                                dotColor = `var(--${prefix}-teal, #0d9488)`;
                            } else if (oi === selected) {
