...
export default function MatterAssessmentEngine({ questions, title, onBack, onSecondaryBack, color, prefix = 'mattertest' }) {
                                ...
                                <div className={`${prefix}-summary-split`}>
                                    <div className={`${prefix}-summary-item`}>
                                        ...
                                        <div style={{ marginTop: 6 }}>
                                            <MathRenderer text={correctOptText} />
                                        </div>
                                    </div>
                                    <div className={`${prefix}-summary-item user-ans`}>
                                        ...
                                        <div style={{ marginTop: 6 }}>
                                            {userOptText === 'Not Answered' ? 'Not Answered' : <MathRenderer text={userOptText} />}
                                        </div>
                                    </div>
                                </div>
                            </div>
    ...
    const palette = (
        <div className={`${prefix}-assessment-palette ${paletteOpen ? 'is-open' : ''}`}>
            ...
            <div className={`${prefix}-palette-grid`} style={{ display: 'grid', gridTemplateColumns: 'repeat(5, minmax(0, 42px))', gap: 8 }}>
                {questionSet.map((_, index) => {
                    ...
                    if (isMarked) border = '2px solid #f59e0b';
                    if (isCurrent) border = `2px solid var(--${prefix}-text, #0f172a)`;

                    return (
                        <button key={index} onClick={() => setCurrent(index)} className={`${prefix}-palette-cell`} style={{ width: '42px', height: '42px', borderRadius: 8, fontSize: 13, fontWeight: 700, background: bg, color: txt, border: border, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                            {index + 1}
                        </button>
                    );
                })}
            ...
            <div style={{ flex: 1 }} ref={topRef}>
                ...
                <div className={`${prefix}-quiz-card`}>
                    ...
                    {getQuestionType(q) === 'text' ? (
                        <input type="text" value={answers[current] ?? ''} onChange={(e) => handleTextAnswerChange(e.target.value)} style={{ width: '100%', padding: '14px 16px', borderRadius: 12, border: `2px solid ${color}`, background: '#fff', fontSize: 15, fontWeight: 600, outline: 'none' }} />
                    ) : (
                        <div className={`${prefix}-quiz-options`}>
                            {(q.options ?? []).map((opt, optIndex) => {
                                const isSelected = getQuestionType(q) === 'msq' ? Array.isArray(answers[current]) && answers[current].includes(optIndex) : answers[current] === optIndex;
                                return (
                                    <button key={optIndex} onClick={() => getQuestionType(q) === 'msq' ? handleMsqToggle(optIndex) : handleSelect(optIndex)} className={`${prefix}-quiz-option`} style={{ display: 'flex', alignItems: 'flex-start', gap: 12, padding: '14px 16px', borderRadius: 16, border: `2.5px solid ${isSelected ? color : 'rgba(0,0,0,0.04)'}`, background: isSelected ? `${color}05` : '#fff', cursor: 'pointer', fontSize: 14, color: `var(--${prefix}-text, #1e293b)`, width: '100%', minHeight: 78, lineHeight: 1.55 }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 40, gap: 20 }}>
                    ...
                    <button 
                        ...
                        className={`${prefix}-nav-btn-mark ${markedForReview[current] ? 'is-marked' : ''}`}
                    >
                        {markedForReview[current] ? '⭐ Marked' : '☆ Mark for Review'}
                    </button>

                    <button 
                        onClick={current + 1 === questionSet.length ? handleSubmit : handleNext} 
                        className={`${prefix}-nav-btn-next`}
                    >
                        <span>{current + 1 === questionSet.length ? 'Finish & Submit' : 'Next Question →'}</span>
                    </button>
                </div>
            </div>
            <button type="button" className={`${prefix}-palette-trigger`} onClick={() => setPaletteOpen(true)}>Palette ({answeredCount}/{questionSet.length})</button>
