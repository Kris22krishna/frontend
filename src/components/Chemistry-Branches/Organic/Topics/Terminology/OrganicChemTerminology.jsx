...
import { TERMS, KEY_IDENTITIES, VOCAB_QUIZ } from './OrganicChemTerminologyData';
...
export default function OrganicChemTerminology() {
    ...
    const handleTabChange = (tab) => {
        ...
        if (tab === 'quiz' && !sessionStarted.current) {
            startSession({ nodeId: NODE_IDS.chemOrganicTerminologyQuiz, sessionType: 'practice' });
            ...
            <div className="chemTermContainer">
                <div style={{ textAlign: 'center', marginBottom: 36 }}>
                    <h1 style={{ fontFamily: 'Outfit, sans-serif', fontSize: 'clamp(2rem,4vw,3rem)', fontWeight: 900, color: '#0f172a', margin: '0 0 10px' }}>
                        Organic <span style={{ color: ACCENT }}>Lexicon</span>
                    </h1>
                    <p style={{ fontSize: 16, color: '#64748b', margin: 0 }}>
                        {activeTab === 'quiz' ? 'Test your vocabulary!' : `Explore ${activeTab === 'terms' ? '8 key terms' : '3 key identities'}.`}
                    </p>
                </div>
                            ...
                            <>
                                ...
                                <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 28 }}>
                                    {activeQ.options.map((opt, i) => {
                                        ...
                                        if (quizAnswered) {
                                            if (isCorrect) { bg = '#fffbeb'; border = '2px solid #f59e0b'; }
                                            else if (isSelected) { bg = '#fef2f2'; border = '2px solid #ef4444'; }
                                        } else if (isSelected) { bg = '#fffbeb'; border = `2px solid ${ACCENT}`; }
                                        return (
                                            <button key={i} disabled={quizAnswered} onClick={() => handleQuizSelect(i)} style={{
                                                ...
                                                {quizAnswered && isCorrect && <span style={{ color: '#f59e0b' }}>✓</span>}
                ...
                <div style={{ marginTop: 36, textAlign: 'center' }}>
                    <button onClick={() => navigate('/chemistry/organic/skills')} style={{ padding: '12px 32px', fontSize: 15, background: '#fff', color: '#0f172a', border: '1px solid #e2e8f0', borderRadius: 100, fontWeight: 800, cursor: 'pointer', display: 'inline-flex', alignItems: 'center', gap: 8, boxShadow: '0 2px 4px rgba(0,0,0,0.05)' }}>
