...
import { ORGANIC_CHEM_SKILLS } from './OrganicChemSkillsData';
...
const useShuffledQuestions = (sourceQuestions, amount) => {
    const [questions, setQuestions] = useState([]);
    useEffect(() => {
        if (!sourceQuestions) return;
        let qs = shuffleArray(sourceQuestions).slice(0, amount);
        qs = qs.map(q => {
            if (q.type === 'mcq') {
                const correctOpt = q.options[q.answer];
                const shuffled = shuffleArray(q.options);
                return { ...q, options: shuffled, answer: shuffled.indexOf(correctOpt) };
            }
            return q;
        });
        setQuestions(qs);
            ...
            <div style={{ display: 'grid', gap: 10 }}>
                {options.map((opt, i) => {
                    ...
                    if (disabled && showCorrect) {
                        ...
                        if (i === selectedOption) { bg = '#fffbeb'; bdr = BRANCH_COLOR; }
                    } else if (i === selectedOption) {
                        bg = `${BRANCH_COLOR}10`; bdr = BRANCH_COLOR;
                    }
                    return (
                        <button key={i} onClick={() => onAnswer(i)} disabled={disabled} style={{
                            ...
                            <span style={{
                                ...
                                background: i === selectedOption ? (showCorrect ? (i === answer ? '#10b981' : '#ef4444') : BRANCH_COLOR) : '#f1f5f9',
...
function LearnMode({ skill, onBack }) {
    ...
    const renderCard = (sec, idx) => {
        if (!sec) return null;
        const cardColor = topColors[idx % topColors.length];
        let keyIcon = '💡', keyTitle = 'Key Fact:', keyBg = '#f8fafc', keyBorder = '#cbd5e1', keyColor = '#0f172a';
        if (sec?.keyLabel === 'neet-note') { keyIcon = '📌'; keyTitle = 'Note:'; keyBg = '#f0fdf4'; keyBorder = '#10b981'; keyColor = '#065f46'; }
        if (sec?.keyLabel === 'neet-trap') { keyIcon = '⚠️'; keyTitle = 'Watch Out:'; keyBg = '#fff1f2'; keyBorder = '#f43f5e'; keyColor = '#9f1239'; }

        const parts = sec.heading.split('—').map(s => s.trim());
        const smallCaps = parts.length > 1 ? parts[0] : `Topic ${idx + 1}`;
        const mainHeading = parts.length > 1 ? parts[1] : sec.heading;

        return (
            <div style={{ background: '#fff', borderRadius: 24, padding: 40, boxShadow: '0 10px 40px rgba(0,0,0,0.06)', borderTop: `8px solid ${cardColor}`, border: '1px solid #f1f5f9', borderTopColor: cardColor, display: 'flex', flexDirection: 'column', gap: 20 }}>
                        ...
                        <div key={bi} style={{ marginBottom: 16 }}>
                            {block.split('\n').map((line, li) => {
                                ...
                                if ((t.endsWith(':') || t.endsWith('):')) && t.length < 100) {
                                    return <h4 key={li} style={{ fontFamily: 'Outfit, sans-serif', fontSize: 19, fontWeight: 800, color: '#1e293b', margin: '20px 0 10px', borderBottom: `2px solid ${cardColor}20`, paddingBottom: 6, display: 'inline-block' }}>{t}</h4>;
                                }
                                if (t.startsWith('•')) {
                                    return (
                                ...
                                return <p key={li} style={{ margin: '0 0 10px', fontSize: 16, color: '#475569', lineHeight: 1.7 }}>{t}</p>;
                            })}
                        </div>
                    ));
                })()}
                ...
                {sec?.table && (
                    <div style={{ overflowX: 'auto', margin: '20px 0' }}>
                        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 15, background: '#fff', borderRadius: 12, overflow: 'hidden', border: '1px solid #e2e8f0' }}>
                            <thead>
                                <tr>{sec.table.headers.map((h, hi) => (
                                    <th key={hi} style={{ padding: '14px 18px', textAlign: 'left', background: `${cardColor}15`, color: cardColor, fontWeight: 800, fontSize: 14, textTransform: 'uppercase', borderBottom: `2px solid ${cardColor}40` }}>{h}</th>
                                ))}</tr>
                            </thead>
                            <tbody>
                                {sec.table.rows.map((row, ri) => (
                                    <tr key={ri} style={{ borderBottom: '1px solid #f1f5f9', background: ri % 2 === 0 ? '#fff' : '#f8fafc' }}>
                                        {row.map((cell, ci) => (
                                            <td key={ci} style={{ padding: '14px 18px', color: ci === 0 ? '#0f172a' : '#475569', fontWeight: ci === 0 ? 700 : 400 }}>{cell}</td>
                                        ))}
                                    </tr>
                                ))}
                            </tbody>
                ...
                {sec?.example && (
                    <div style={{ background: keyBg, borderLeft: `5px solid ${keyBorder}`, padding: '20px 24px', borderRadius: '0 12px 12px 0', marginTop: 'auto' }}>
                        <div style={{ fontWeight: 800, fontSize: 15, color: keyColor, marginBottom: 8 }}>{keyIcon} {keyTitle}</div>
                        <p style={{ margin: 0, fontSize: 15, color: keyColor, lineHeight: 1.6 }}>{sec.example}</p>
                    </div>
                )}
            </div>
        );
            ...
            <div style={{ display: 'flex', gap: 32, alignItems: 'flex-start' }}>
                <div style={{ flex: '0 0 280px', position: 'sticky', top: 80 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 24, padding: '0 12px' }}>
                        ...
                        <h2 style={{ fontFamily: 'Outfit, sans-serif', fontSize: 24, fontWeight: 900, color: '#0f172a', margin: 0, lineHeight: 1.2 }}>{skill.title}</h2>
                    </div>
                    <div style={{ background: '#fff', borderRadius: 20, padding: 16, boxShadow: '0 4px 20px rgba(0,0,0,0.04)', border: '1px solid #f1f5f9' }}>
                        {skill.learnSections && skill.learnSections.map((s, i) => {
                            ...
                            return (
                                <button key={i} onClick={() => setActiveIdx(i)} style={{
                                    ...
                                    background: itemColor, color: active ? '#fff' : '#475569',
                ...
                <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', gap: 24 }}>
                    {renderCard(sec, activeIdx)}
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 16 }}>
                        <button onClick={() => setActiveIdx(Math.max(0, activeIdx - 1))} disabled={activeIdx === 0}
                            style={{ padding: '14px 28px', borderRadius: 100, border: `2px solid ${activeIdx === 0 ? '#e2e8f0' : skill.color}`, background: '#fff', color: activeIdx === 0 ? '#cbd5e1' : skill.color, fontWeight: 800, fontSize: 15, cursor: activeIdx === 0 ? 'default' : 'pointer' }}>← Previous</button>
                        {activeIdx < skill.learnSections.length - 1 ? (
                            <button onClick={() => setActiveIdx(activeIdx + 1)}
                                style={{ padding: '14px 32px', borderRadius: 100, border: 'none', background: skill.color, color: '#fff', fontWeight: 800, fontSize: 16, cursor: 'pointer' }}>Next Topic →</button>
                        ) : (
                            <button onClick={onBack}
                                style={{ padding: '14px 32px', borderRadius: 100, border: 'none', background: BRANCH_COLOR, color: '#fff', fontWeight: 800, fontSize: 16, cursor: 'pointer' }}>Got it! ✓</button>
                        )}
                    </div>
                </div>
            </div>
...
function PracticeMode({ skill, onBack, onAssess, nodeId }) {
    const questions = useShuffledQuestions(skill.practice, 10);
    ...
    const handleAnswer = async (val) => {
        ...
        if (nodeId) {
            const d = { question_index: qIdx + 1, answer_json: { selected: val, text: q.options[val] }, is_correct: correct ? 1.0 : 0.0, marks_awarded: correct ? 1 : 0, marks_possible: 1, time_taken_ms: 0 };
    ...
    if (finished) {
        ...
        let msg = 'Keep Learning!', emoji = '💪';
        if (pct >= 90) { msg = 'Excellent!'; emoji = '🏆'; }
        ...
        return (
            <div style={{ maxWidth: 600, margin: '0 auto', textAlign: 'center', padding: '40px 24px', background: '#fff', borderRadius: 24, boxShadow: '0 10px 40px rgba(0,0,0,0.05)' }}>
                ...
                <ScoreRing score={score} total={questions.length} color={skill.color} />
                <div style={{ margin: '16px 0 8px', padding: '8px 24px', background: '#f8fafc', borderRadius: 100, display: 'inline-flex', alignItems: 'center', gap: 8, fontWeight: 700, color: '#64748b', fontSize: 14 }}>⏱️ Time: {fmtTime(totalTime)}</div>
                ...
                <div style={{ display: 'flex', justifyContent: 'center', gap: 12, marginTop: 20 }}>
                    ...
                    <button onClick={onAssess} style={{ padding: '12px 28px', borderRadius: 100, border: 'none', background: skill.color, color: '#fff', fontWeight: 800, fontSize: 15, cursor: 'pointer' }}>Take Assessment 🏆</button>
                </div>
            </div>
        );
    }
    ...
    return (
        <div style={{ maxWidth: 700, margin: '0 auto', background: '#fff', padding: 32, borderRadius: 24, boxShadow: '0 10px 40px rgba(0,0,0,0.05)' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                {qIdx > 0 ? (
                    <button onClick={() => setQIdx(qIdx - 1)} style={{ padding: '12px 32px', borderRadius: 100, fontWeight: 800, fontSize: 16, border: `2px solid ${skill.color}`, background: '#fff', color: skill.color, cursor: 'pointer' }}>← Previous</button>
                ) : <div />}
...
function AssessMode({ skill, onBack, nodeId }) {
    const questions = useShuffledQuestions(skill.assess, 10);
    const [qIdx, setQIdx] = useState(0);
    const [answersMap, setAnswersMap] = useState({});
    ...
    const handleAnswer = async (val) => {
        const now = Date.now(), timeSpent = now - qStartRef.current;
        setAnswersMap(prev => ({ ...prev, [qIdx]: val }));
        setQTimes(prev => ({ ...prev, [qIdx]: (prev[qIdx] || 0) + timeSpent }));
        qStartRef.current = now;
        if (nodeId) {
            const correct = val === q.answer;
            const d = { question_index: qIdx + 1, answer_json: { selected: val, text: q.options[val] }, is_correct: correct ? 1.0 : 0.0, marks_awarded: correct ? 1 : 0, marks_possible: 1, time_taken_ms: timeSpent };
    ...
    const goTo = (idx) => {
        ...
        qStartRef.current = now; setQIdx(idx);
    };
    ...
    if (finished) {
        ...
        let score = 0;
        questions.forEach((qq, i) => { if (answersMap[i] !== undefined && answersMap[i] === qq.answer) score++; });
        const accuracy = Math.round((score / questions.length) * 100);
        return (
            <div style={{ maxWidth: 900, margin: '0 auto', padding: '32px 24px', background: '#fff', borderRadius: 24 }}>
                <h1 style={{ fontFamily: 'Outfit, sans-serif', fontSize: 32, fontWeight: 900, color: '#0f172a', margin: '0 0 28px' }}>📊 Assessment Report</h1>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16, marginBottom: 32 }}>
                    {[['Total Score', `${score} / ${questions.length}`, skill.color], ['Accuracy', `${accuracy}%`, accuracy >= 70 ? '#059669' : '#dc2626'], ['Time', `⏱️ ${fmtTime(totalTime)}`, '#d97706']].map(([label, val, color]) => (
                        <div key={label} style={{ textAlign: 'center', padding: '20px 16px', borderRadius: 16, border: '1px solid #e2e8f0', background: '#f8fafc' }}>
                            <div style={{ fontSize: 13, fontWeight: 700, color: '#64748b', marginBottom: 8, textTransform: 'uppercase', letterSpacing: 1 }}>{label}</div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                    {questions.map((qq, i) => {
                        const userAns = answersMap[i], skipped = userAns === undefined, correct = !skipped && userAns === qq.answer;
                            ...
                            <div key={i} style={{ background: '#fff', border: `1px solid ${skipped ? '#fde68a' : correct ? '#bbf7d0' : '#fecaca'}`, borderRadius: 16, padding: '20px 24px' }}>
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12 }}>
                                    <div style={{ display: 'flex', gap: 12, alignItems: 'flex-start', flex: 1 }}>
                                        ...
                                        <p style={{ margin: 0, fontWeight: 700, fontSize: 15, color: '#0f172a', lineHeight: 1.5 }}><MathRenderer text={qq.question} /></p>
                                    </div>
                                    <div style={{ textAlign: 'right', flexShrink: 0, marginLeft: 12 }}>
                                        <div style={{ fontSize: 13, fontWeight: 700, color: skipped ? '#d97706' : correct ? '#059669' : '#dc2626' }}>{skipped ? '⏩ Skipped' : correct ? '✅ Correct' : '❌ Wrong'}</div>
                                        <div style={{ fontSize: 12, color: '#94a3b8', marginTop: 2 }}>⏱️ {qTimes[i] ? Math.round(qTimes[i] / 1000) : 0}s</div>
                                    </div>
                                </div>
                                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 8 }}>
                                    {qq.options.map((opt, oi) => {
                                        const isUserPick = userAns === oi, isCorrectOpt = oi === qq.answer;
                                        let bg = '#f8fafc', bdr = '#e2e8f0';
                                        if (isCorrectOpt) { bg = '#f0fdf4'; bdr = '#10b981'; }
                                        else if (isUserPick) { bg = '#fef2f2'; bdr = '#ef4444'; }
                                        return (
                                            <div key={oi} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 14px', borderRadius: 10, border: `2px solid ${bdr}`, background: bg, fontSize: 14, fontWeight: 600, color: '#334155' }}>
                                                <span style={{ width: 24, height: 24, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 800, flexShrink: 0, background: isCorrectOpt ? '#10b981' : isUserPick ? '#ef4444' : '#e2e8f0', color: (isCorrectOpt || isUserPick) ? '#fff' : '#64748b' }}>{String.fromCharCode(65 + oi)}</span>
                                    ...
                                    <div>
                                        ...
                                        {showSol && (
                                            <div style={{ marginTop: 8, padding: '12px 16px', background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: 12 }}>
                                                <div style={{ fontWeight: 700, color: skill.color, marginBottom: 4 }}>💡 Explanation</div>
    ...
    return (
        <div style={{ display: 'flex', gap: 24, maxWidth: 1100, margin: '0 auto', alignItems: 'flex-start' }}>
            <div style={{ flex: '1 1 60%', background: '#fff', borderRadius: 24, padding: 32, boxShadow: '0 10px 40px rgba(0,0,0,0.05)' }}>
                ...
                <QuestionCard key={qIdx} question={q.question} options={q.options} answer={q.answer} onAnswer={handleAnswer} disabled={false} selectedOption={answersMap[qIdx] ?? null} showCorrect={false} />
                <div style={{ display: 'flex', gap: 12, justifyContent: 'center', marginTop: 16 }}>
                    <button onClick={() => goTo(Math.max(0, qIdx - 1))} disabled={qIdx === 0} style={{ padding: '10px 24px', borderRadius: 100, border: '2px solid #e2e8f0', background: '#fff', fontWeight: 700, fontSize: 14, color: qIdx === 0 ? '#cbd5e1' : '#334155', cursor: qIdx === 0 ? 'default' : 'pointer' }}>← Previous</button>
                    <button onClick={() => setMarked(prev => ({ ...prev, [qIdx]: !prev[qIdx] }))} style={{ padding: '10px 24px', borderRadius: 100, border: `2px solid ${marked[qIdx] ? '#f59e0b' : '#e2e8f0'}`, background: marked[qIdx] ? '#fef3c7' : '#fff', fontWeight: 700, fontSize: 14, color: marked[qIdx] ? '#92400e' : '#334155', cursor: 'pointer' }}>
                        {marked[qIdx] ? '★ Marked' : 'Mark for Review'}
                    </button>
                    <button onClick={() => goTo(Math.min(questions.length - 1, qIdx + 1))} disabled={qIdx === questions.length - 1} style={{ padding: '10px 24px', borderRadius: 100, border: 'none', background: BRANCH_COLOR, fontWeight: 700, fontSize: 14, color: '#fff', cursor: qIdx === questions.length - 1 ? 'default' : 'pointer', opacity: qIdx === questions.length - 1 ? 0.5 : 1 }}>Next →</button>
                </div>
            </div>
            <div style={{ flex: '0 0 260px', background: '#f8fafc', borderRadius: 24, padding: 24, position: 'sticky', top: 80, border: '1px solid #e2e8f0' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 8, marginBottom: 20 }}>
                    {questions.map((_, i) => {
                        const isAnswered = answersMap[i] !== undefined, isMarked = !!marked[i], isCurrent = i === qIdx;
                        let bg = '#fff', bdr = '#e2e8f0', clr = '#64748b';
                        if (isMarked) { bg = '#fef3c7'; bdr = '#f59e0b'; clr = '#92400e'; }
                        if (isAnswered) { bg = BRANCH_COLOR; bdr = BRANCH_COLOR; clr = '#fff'; }
                        if (isCurrent) bdr = '#0f172a';
                        return (
                            <button key={i} onClick={() => goTo(i)} style={{ width: 40, height: 40, borderRadius: 8, border: `2px solid ${bdr}`, background: bg, color: clr, fontWeight: 800, fontSize: 14, cursor: 'pointer', boxShadow: isCurrent ? '0 0 0 2px #0f172a' : 'none' }}>{i + 1}</button>
                        );
                    })}
                </div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginBottom: 20, fontSize: 13 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}><div style={{ width: 16, height: 16, borderRadius: 4, background: BRANCH_COLOR }} /><span style={{ fontWeight: 600, color: '#334155' }}>Answered</span></div>
                ...
                <button onClick={submitAssessment} style={{ width: '100%', padding: '14px 0', borderRadius: 12, border: 'none', background: '#ef4444', color: '#fff', fontFamily: 'Outfit, sans-serif', fontWeight: 800, fontSize: 16, cursor: 'pointer' }}>Submit Assessment</button>
            </div>
        </div>
    );
}
...
const SKILL_NODE_ID_MAP = {
    'skill-1': NODE_IDS.chemOrganicClassification,
    'skill-2': NODE_IDS.chemOrganicMoleculeTypes,
    'skill-3': NODE_IDS.chemOrganicCarbonProperties,
};

export default function OrganicChemSkills() {
    ...
    const activeSkill = ORGANIC_CHEM_SKILLS.find(s => s.id === activeSkillId);
    const activeNodeId = activeSkillId ? SKILL_NODE_ID_MAP[activeSkillId] : null;
    const openMode = (skill, mode) => { setActiveSkillId(skill.id); setView(mode); };

    return (
        <div className="chemPage">
            <nav className="chemNav">
                {view === 'list' ? (
                    <button className="chemNavBack" onClick={() => navigate('/chemistry/organic')}>← Back to Dashboard</button>
                ) : (
                    <button className="chemNavBack" onClick={() => setView('list')}>← Back to Skills</button>
                )}
                <div className="chemNavLinks">
                    <button className="chemNavLink" onClick={() => navigate('/chemistry/organic/introduction')}>🌟 Intro</button>
                    <button className="chemNavLink" onClick={() => navigate('/chemistry/organic/terminology')}>📖 Terminology</button>
                    <button className="chemNavLink chemNavLinkActive">🎯 Skills</button>
                </div>
            </nav>
                ...
                {view === 'list' && (
                    <div>
                        <div style={{ textAlign: 'center', marginBottom: 40 }}>
                            <h1 style={{ fontFamily: 'Outfit, sans-serif', fontSize: 36, fontWeight: 900, color: '#0f172a', margin: '0 0 12px' }}>Core <span style={{ color: BRANCH_COLOR }}>Skills</span></h1>
                            <p style={{ fontSize: 18, color: '#64748b', margin: 0, maxWidth: 600, marginInline: 'auto' }}>Choose a skill. Read the lesson, practice to build confidence, then take the assessment!</p>
                        </div>
                        <div className="chemSkillsList">
                            {ORGANIC_CHEM_SKILLS.map((skill) => (
                                <div key={skill.id} className="chemSkillCard">
                                    <div className="chemSkillInfo">
                                        <div className="chemSkillIcon" style={{ background: `${skill.color}15`, color: skill.color }}>{skill.icon}</div>
                                        <div>
                                            <div style={{ fontFamily: 'Outfit, sans-serif', fontSize: 18, fontWeight: 800, color: '#0f172a', marginBottom: 4 }}>{skill.title}</div>
                                            <div style={{ fontSize: 14, color: '#64748b' }}>{skill.desc}</div>
                                        </div>
                                    </div>
                                    <div className="chemSkillActions">
                                        <button onClick={() => openMode(skill, 'learn')} className="chemSkillBtnOutline" style={{ '--btn-color': skill.color }}>📖 Learn</button>
                                        <button onClick={() => openMode(skill, 'practice')} className="chemSkillBtnOutline" style={{ '--btn-color': skill.color }}>✏️ Practice</button>
                                        <button onClick={() => openMode(skill, 'assess')} className="chemSkillBtnFilled" style={{ '--btn-color': skill.color }}>🏆 Assess</button>
                                    </div>
                                </div>
                            ))}
                        </div>
