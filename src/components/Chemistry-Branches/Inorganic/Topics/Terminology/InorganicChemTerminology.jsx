...
import { TERMS, KEY_IDENTITIES, VOCAB_QUIZ } from './InorganicChemTerminologyData';
...
export default function InorganicChemTerminology() {
    ...
    const handleTabChange = (tab) => {
        ...
        if (tab === 'quiz' && !sessionStarted.current) {
            startSession({ nodeId: NODE_IDS.chemInorganicTerminologyQuiz, sessionType: 'practice' });
        ...
        <div className="chemPage">
            <nav className="chemNav">
                ...
                <div className="chemNavLinks">
                    ...
                    <button className="chemNavLink" onClick={() => navigate('/chemistry/inorganic/skills')}>🎯 Skills</button>
                </div>
            </nav>
            ...
            <div className="chemTermContainer">
                <div style={{ textAlign: 'center', marginBottom: 36 }}>
                    <h1 style={{ fontFamily: 'Outfit, sans-serif', fontSize: 'clamp(2rem,4vw,3rem)', fontWeight: 900, color: '#0f172a', margin: '0 0 10px' }}>
                        Inorganic <span style={{ color: ACCENT }}>Lexicon</span>
                    </h1>
                    <p style={{ fontSize: 16, color: '#64748b', margin: 0 }}>
                        {activeTab === 'quiz' ? 'Test your vocabulary!' : `Explore ${activeTab === 'terms' ? '9 key terms' : '5 key identities'}.`}
                    </p>
                </div>
                            ...
                            {(activeTab === 'terms' ? TERMS : KEY_IDENTITIES).map((item, i) => {
                                ...
                                return (
                                    <button key={i} onClick={() => activeTab === 'terms' ? setSelectedIdx(i) : setSelectedIdxB(i)} style={{
                                        ...
                                        <span>{activeTab === 'terms' ? item.name : item.name}</span>
                                    </button>
                                );
                            })}
                        </aside>
                            ...
                            {activeTab === 'terms' ? (
                                ...
                                <>
                                    ...
                                    <div className="chemFormulaBox">
                                        <div className="chemFormulaLabel">Formula</div>
                            ...
                            <>
                                ...
                                <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 28 }}>
                                    {activeQ.options.map((opt, i) => {
                                        ...
                                        } else if (isSelected) { bg = '#ecfdf5'; border = `2px solid ${ACCENT}`; }
                                        return (
                ...
                <div style={{ marginTop: 36, textAlign: 'center' }}>
                    <button onClick={() => navigate('/chemistry/inorganic/skills')} style={{ padding: '12px 32px', fontSize: 15, background: '#fff', color: '#0f172a', border: '1px solid #e2e8f0', borderRadius: 100, fontWeight: 800, cursor: 'pointer', display: 'inline-flex', alignItems: 'center', gap: 8, boxShadow: '0 2px 4px rgba(0,0,0,0.05)' }}>
