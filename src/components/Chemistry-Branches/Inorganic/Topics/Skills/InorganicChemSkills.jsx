...
import { INORGANIC_CHEM_SKILLS } from './InorganicChemSkillsData';
import '../../../chemistry.css';
import MathRenderer from '../../../../MathRenderer';
import { useSessionLogger } from '@/hooks/useSessionLogger';
import { NODE_IDS } from '@/lib/curriculumIds';

const BRANCH_COLOR = '#10b981';
...
function LearnMode({ skill, onBack }) {
    ...
    const renderCard = (sec, idx) => {
        ...
        else if (sec?.keyLabel === 'neet-note') { keyIcon = '📌'; keyTitle = 'Note:'; keyBg = '#f0fdf4'; keyBorder = '#10b981'; keyColor = '#065f46'; }
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 16 }}>
                        ...
                        {activeIdx < skill.learnSections.length - 1 ? (
                            <button onClick={() => setActiveIdx(activeIdx + 1)}
                                style={{ padding: '14px 32px', borderRadius: 100, border: 'none', background: skill.color, color: '#fff', fontWeight: 800, fontSize: 16, cursor: 'pointer', boxShadow: `0 4px 14px ${skill.color}40` }}>Next Topic →</button>
                        ) : (
                            <button onClick={onBack}
                                style={{ padding: '14px 32px', borderRadius: 100, border: 'none', background: '#10b981', color: '#fff', fontWeight: 800, fontSize: 16, cursor: 'pointer' }}>Got it! ✓</button>
                        )}
                    </div>
                </div>
            </div>
...
const SKILL_NODE_ID_MAP = {
    'skill-1': NODE_IDS.chemInorganicAtomicStructure,
    'skill-2': NODE_IDS.chemInorganicElectronicConfig,
    'skill-3': NODE_IDS.chemInorganicBondIdentification,
    'skill-4': NODE_IDS.chemInorganicElementClassification,
};

export default function InorganicChemSkills() {
    ...
    const activeSkill = INORGANIC_CHEM_SKILLS.find(s => s.id === activeSkillId);
    ...
    return (
        <div className="chemPage">
            <nav className="chemNav">
                {view === 'list' ? (
                    <button className="chemNavBack" onClick={() => navigate('/chemistry/inorganic')}>← Back to Dashboard</button>
                ) : (
                    <button className="chemNavBack" onClick={() => setView('list')}>← Back to Skills</button>
                )}
                <div className="chemNavLinks">
                    <button className="chemNavLink" onClick={() => navigate('/chemistry/inorganic/introduction')}>🌟 Intro</button>
                    <button className="chemNavLink" onClick={() => navigate('/chemistry/inorganic/terminology')}>📖 Terminology</button>
                ...
                {view === 'list' && (
                    <div>
                        ...
                        <div className="chemSkillsList">
                            {INORGANIC_CHEM_SKILLS.map((skill) => (
