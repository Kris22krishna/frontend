...
import { cellIntroData } from './TheCellIntroData';
...
function W1HCard({ card }) {
    ...
    return (
        <div
            className={`cell-intro-card${open ? ' cell-intro-card--open' : ''}`}
            onClick={() => setOpen(o => !o)}
            style={{
        ...
        >
            {/* Gradient top strip */}
            <div
                className="cell-card-strip"
                style={{ background: `linear-gradient(90deg, ${card.gradFrom}, ${card.gradTo})` }}
            />

            {/* Header row */}
            <div className="cell-intro-card-header">
                {/* Icon */}
                <div
                    className="cell-intro-card-icon"
                    style={{
            ...
            {/* Expanded content */}
            {open && (
                <div className="cell-intro-card-body">
                    <p className="cell-intro-card-content">{card.content}</p>
                    <div className="cell-intro-card-fact">
...
/* ── Main page ───────────────────────────────────── */
export default function TheCellIntro() {
    ...
    return (
        <div className="cell-page">
            ...
            <div className="cell-hero">
                <h1 className="cell-hero-title">Dive into <span style={{ color: '#8bde4f' }}>The Cell</span></h1>
                <p className="cell-hero-sub">Get started with the 6 big questions and check your prerequisites.</p>
            </div>

            <main style={{ maxWidth: '1200px', margin: '0 auto', padding: '40px 24px' }}>
                {/* Prerequisites */}
                <section style={{ marginBottom: '60px' }}>
                    <h2 style={{ fontFamily: 'Outfit, sans-serif', fontSize: '28px', fontWeight: 900, marginBottom: '24px', textAlign: 'center', color: 'var(--cell-indigo)' }}>
                        Prerequisites
                    </h2>
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '24px' }}>
                        {cellIntroData.prerequisites.map((p, idx) => (
                            <div key={idx} style={{ background: '#fff', padding: '32px 24px', borderRadius: '24px', border: '1px solid #e2e8f0', textAlign: 'center', boxShadow: '0 4px 6px -1px rgba(0, 0, 0, 0.05)', transition: 'all 0.3s ease' }}>
                                ...
                                <h3 style={{ fontSize: '20px', fontWeight: 900, margin: '0 0 8px', color: 'var(--cell-text)' }}>{p.title}</h3>
                                <p style={{ fontSize: '15px', color: 'var(--cell-muted)', margin: 0, lineHeight: 1.6 }}>{p.desc}</p>
                            </div>
                        ))}
                    </div>
                </section>
                ...
                {/* 5W1H Cards Grid */}
                <h2 style={{ fontFamily: 'Outfit, sans-serif', fontSize: '28px', fontWeight: 900, marginBottom: '32px', textAlign: 'center', color: 'var(--cell-indigo)' }}>
                    6 Big Questions
                </h2>
                <div className="cell-grid" style={{ padding: 0 }}>
                    {cellIntroData.cards5W1H.map((card, idx) => (
                ...
                <div style={{ 
                    ...
                    <div>
                        ...
                        <p style={{ margin: 0, opacity: 0.8 }}>Next up: Key terms and the Cell Theory Rules.</p>
                    </div>
                    <button 
                        onClick={() => navigate('/senior/grade/11/biology/the-cell/terminology')}
