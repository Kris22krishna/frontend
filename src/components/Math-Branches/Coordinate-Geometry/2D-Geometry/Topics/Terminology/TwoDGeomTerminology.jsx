...
import { TERMS, KEY_IDENTITIES, VOCAB_QUIZ } from './TwoDGeomTerminologyData';

export default function TwoDGeomTerminology() {
    ...
    return (
        <div className={styles.arithPage}>
            <nav className={styles.introNav}>
                <button className={styles.arithBackBtn} onClick={() => navigate('/coordinate-geometry/2d-geometry')} style={{ marginBottom: 0 }}>← Back to Branch</button>
                <div className={styles.arithIntroNavLinks}>
                    <button className={styles.arithIntroNavLink} onClick={() => navigate('/coordinate-geometry/2d-geometry/introduction')}>🌟 Introduction</button>
                    <button className={`${styles.arithIntroNavLink} ${styles.arithIntroNavLinkActive}`}>📖 Terminology</button>
                    <button className={styles.arithIntroNavLink} onClick={() => navigate('/coordinate-geometry/2d-geometry/skills')}>🎯 Skills</button>
                </div>
            </nav>
            ...
            <div className={styles.arithTermContainer}>
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', marginBottom: 20, paddingTop: 32 }}>
                    <h1 style={{ fontFamily: '"Outfit", sans-serif', fontSize: '2.8rem', fontWeight: 900, color: '#0f172a', margin: '0 0 8px' }}>
                        Coordinate Geometry{' '}
                        <span style={{ background: 'linear-gradient(135deg, #10b981, #0ea5e9)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>Vocabulary</span>
                    </h1>
                    <div style={{ fontSize: 16, fontWeight: 700, color: '#64748b', letterSpacing: 0.5 }}>
                        {activeTab === 'quiz'
                            ? 'Test your knowledge with interactive questions!'
                            : `Select any ${activeTab === 'terms' ? 'term' : 'formula'} below to explore details.`}
                    </div>
                </div>
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            <div style={{ display: 'flex', flexDirection: 'column', textAlign: 'left' }}>
                                                <span style={{ fontWeight: 800, fontSize: 13, color: isActive ? '#fff' : '#1e293b', lineHeight: 1.2 }}>Formula {i + 1}</span>
                                ...
                                <>
                                    ...
                                    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                                        ...
                                        {activeTerm.realLifeExample && (
                                            <div>
                                                <h4 style={{ textTransform: 'uppercase', fontSize: 11, letterSpacing: 1, color: '#0ea5e9', marginBottom: 8 }}>Real World Application</h4>
                                                <div style={{ background: 'rgba(14,165,233,0.04)', padding: 14, borderRadius: 12, border: '1px solid rgba(14,165,233,0.15)' }}>
                                                    <div style={{ fontSize: 15, color: '#0c4a6e', fontWeight: 500, lineHeight: 1.5 }}>
                                                        🌍 {activeTerm.realLifeExample}
                                                    </div>
                                                </div>
                                            </div>
                            ...
                            {!showResults ? (
                                ...
                                <div>
                                    ...
                                    <div className={styles.arithActionRow}>
                                        ...
                                        <button onClick={() => navigate('/coordinate-geometry/2d-geometry/skills')} style={{ padding: '12px 24px', background: accentColor, color: '#fff', border: 'none', borderRadius: 100, fontSize: 15, fontWeight: 700, cursor: 'pointer' }}>Go to Skills →</button>
                                    </div>
                                </div>
                            )}
                        </div>
