...
import { TERMS, FIVE_RULES, VOCAB_QUIZ } from './LimitsTerminologyData';

export default function LimitsTerminology() {
    ...
    const accentColor = 'var(--calc-violet)';
    const accentBg = 'var(--calc-violet-10)';
        ...
        <div className="calc-page">
            ...
            <nav className="intro-nav">
                ...
                <div className="intro-nav-links">
                    ...
                    <button className="intro-nav-link" onClick={() => navigate('/calculus/limits/skills')}>🎯 Skills</button>
                </div>
            </nav>
            ...
            <div className="calc-lexicon-container" style={{ maxWidth: 1100, margin: '40px auto 20px', padding: '0 24px' }}>
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', marginBottom: 20 }}>
                    <h1 style={{ fontFamily: 'Outfit, sans-serif', fontSize: '2.8rem', fontWeight: 900, color: 'var(--calc-text)', margin: '0 0 8px' }}>
                        Limits <span style={{ background: 'linear-gradient(135deg, var(--calc-violet), var(--calc-rose))', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>Vocabulary</span>
                    </h1>
                ...
                <div style={{ display: 'flex', justifyContent: 'center', gap: 8, marginBottom: 16 }}>
                    ...
                    <button className={`calc-term-tab ${activeTab === 'rules' ? 'active' : ''}`} onClick={() => setActiveTab('rules')}>⚡ Limit Laws</button>
                ...
                {activeTab !== 'quiz' ? (
                    <div className="calc-lexicon-grid" style={{ display: 'grid', gridTemplateColumns: 'minmax(300px, 360px) 1fr', gap: 16, alignItems: 'start' }}>
                            ...
                            {activeTab === 'terms' ? (
                                TERMS.map((term, i) => {
                                    ...
                                    return (
                                        <button key={i} className={`calc-term-btn ${isActive ? 'active' : ''}`}
                                            ...
                                            style={{
                                                background: isActive ? `linear-gradient(135deg, #8b5cf6, #8b5cf6dd)` : `linear-gradient(135deg, rgba(139,92,246,0.08), rgba(139,92,246,0.02))`,
                                                borderColor: isActive ? '#8b5cf6' : 'rgba(139,92,246,0.15)',
                                        ...
                                        >
                                            ...
                                            <span style={{ fontWeight: 800, fontSize: 16, color: isActive ? '#fff' : 'var(--calc-text)' }}>{term.word}</span>
                                        </button>
                                    );
                                })
                            ) : (
                                FIVE_RULES.map((rule, i) => {
                                    ...
                                    return (
                                        ...
                                        >
                                            <div style={{ width: 36, height: 36, borderRadius: 10, background: isActive ? 'rgba(255,255,255,0.25)' : '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20, color: isActive ? '#fff' : '#8b5cf6', fontWeight: 900 }}>{i + 1}</div>
                        ...
                        <main className="calc-detail-anim" key={activeTab === 'terms' ? selectedIdx : `r${selectedRuleIdx}`} style={{
                            ...
                            border: '2px solid rgba(139,92,246,0.08)', minHeight: 330
                        }}>
                            {activeTab === 'terms' ? (
                                <>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
                                        ...
                                        <h2 style={{ fontFamily: 'Outfit, sans-serif', fontSize: 28, fontWeight: 900, color: '#8b5cf6', margin: 0 }}>{activeTerm.word}</h2>
                                    </div>
                                    <p style={{ fontSize: 17, color: 'var(--calc-text)', lineHeight: 1.6, margin: '0 0 24px' }}>{activeTerm.def}</p>
                                    <div>
                                        <h4 style={{ textTransform: 'uppercase', fontSize: 11, letterSpacing: 1, color: '#8b5cf6', marginBottom: 10 }}>Example</h4>
                                        <div style={{ background: 'rgba(139,92,246,0.04)', padding: 16, borderRadius: 16, border: '1px solid rgba(139,92,246,0.1)' }}>
                                ...
                                <>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
                                        <div style={{ width: 44, height: 44, borderRadius: 12, background: accentBg, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 24, color: '#8b5cf6', fontWeight: 900 }}>⚡</div>
                                        <h2 style={{ fontFamily: 'Outfit, sans-serif', fontSize: 28, fontWeight: 900, color: '#8b5cf6', margin: 0 }}>{activeRule.name}</h2>
                                    </div>
                                    <p style={{ fontSize: 17, color: 'var(--calc-text)', lineHeight: 1.6, margin: '0 0 24px' }}>{activeRule.desc}</p>
                                    <div>
                                        <h4 style={{ textTransform: 'uppercase', fontSize: 11, letterSpacing: 1, color: '#8b5cf6', marginBottom: 10 }}>Formula</h4>
                                        <div style={{ background: 'rgba(139,92,246,0.04)', padding: 20, borderRadius: 16, border: '1px solid rgba(139,92,246,0.1)', textAlign: 'center' }}>
                    ...
                    <div className="calc-detail-anim" style={{ maxWidth: 700, margin: '0 auto', background: '#fff', borderRadius: 24, padding: '32px 40px', boxShadow: '0 15px 40px rgba(0,0,0,0.04)', border: '1px solid rgba(0,0,0,0.04)' }}>
                        ...
                        <div style={{ marginTop: 40, textAlign: 'center', paddingTop: 32, borderTop: '1px solid #e2e8f0' }}>
                            {!showResults ? (
                                <button onClick={handleCheckAnswers} style={{ padding: '14px 32px', background: '#8b5cf6', color: '#fff', border: 'none', borderRadius: 100, fontSize: 16, fontWeight: 700, cursor: 'pointer', boxShadow: '0 8px 20px rgba(139,92,246,0.2)' }}>Check Answers</button>
                            ) : (
                                <div>
                                    ...
                                    <div style={{ display: 'flex', gap: 16, justifyContent: 'center' }}>
                                        ...
                                        <button onClick={() => navigate('/calculus/limits/skills')} style={{ padding: '12px 24px', background: '#8b5cf6', color: '#fff', border: 'none', borderRadius: 100, fontSize: 15, fontWeight: 700, cursor: 'pointer', boxShadow: '0 8px 20px rgba(139,92,246,0.2)' }}>Go to Skills →</button>
                                    </div>
                                </div>
                            )}
                        </div>
                ...
                <div style={{ marginTop: 16, textAlign: 'center' }}>
                    <button onClick={() => navigate('/calculus/limits/skills')} style={{ padding: '10px 28px', fontSize: 13, borderRadius: 100, border: 'none', background: '#8b5cf6', color: '#fff', fontWeight: 800, cursor: 'pointer', boxShadow: '0 4px 14px rgba(139,92,246,0.3)' }}>Ready to Solve! 🎯</button>
                </div>
            </div>
        </div>
    );
