...
import { TERMS, FIVE_RULES, VOCAB_QUIZ } from './IntegersTerminologyData';
    ...
    return (
        <div className={styles.arithPage}>
            <nav className={styles.introNav}>
                <button className={styles.arithBackBtn} onClick={() => navigate('/arithmetic/integers')} style={{ marginBottom: 0 }}>← Back to Branch</button>
                <div className={styles.arithIntroNavLinks}>
                    <button className={styles.arithIntroNavLink} onClick={() => navigate('/arithmetic/integers/introduction')}>🌟 Introduction</button>
                    <button className={`${styles.arithIntroNavLink} ${styles.arithIntroNavLinkActive}`}>📖 Terminology</button>
                    <button className={styles.arithIntroNavLink} onClick={() => navigate('/arithmetic/integers/skills')}>🎯 Skills</button>
                </div>
            </nav>
            ...
            <div className={styles.arithTermContainer}>
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', marginBottom: 20 }}>
                    <h1 style={{ fontFamily: '"Outfit", sans-serif', fontSize: '2.8rem', fontWeight: 900, color: '#0f172a', margin: '0 0 8px' }}>
                        Integers <span style={{ background: 'linear-gradient(135deg, #ef4444, #3b82f6)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>Vocabulary</span>
                    </h1>
                ...
                <div className={styles.arithTabSwitcher}>
                    ...
                    >📚 Terminology</button>
                    <button 
                        style={{ padding: '8px 16px', borderRadius: 100, border: 'none', background: activeTab === 'rules' ? accentColor : '#e2e8f0', color: activeTab === 'rules' ? '#fff' : '#475569', fontWeight: 700, cursor: 'pointer' }}
                        onClick={() => setActiveTab('rules')}
                    >⚡ Integer Rules</button>
                    <button 
                ...
                {activeTab !== 'quiz' ? (
                    <div className={styles.arithTermLayout}>
                                ...
                                TERMS.map((term, i) => {
                                    ...
                                    return (
                                        <button key={i} 
                                            ...
                                            style={{
                                                background: isActive ? `linear-gradient(135deg, ${accentColor}, ${accentColor}dd)` : `linear-gradient(135deg, rgba(59,130,246,0.08), rgba(59,130,246,0.02))`,
                                                borderColor: isActive ? accentColor : 'rgba(59,130,246,0.15)',
                                        ...
                                        >
                                            ...
                                            <span style={{ fontWeight: 800, fontSize: 16, color: isActive ? '#fff' : '#1e293b' }}>{term.word}</span>
                                        </button>
                                    );
                                })
                            ) : (
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            <div style={{ display: 'flex', flexDirection: 'column', textAlign: 'left' }}>
                                                <span style={{ fontWeight: 800, fontSize: 16, color: isActive ? '#fff' : '#1e293b', lineHeight: 1 }}>Rule {i + 1}</span>
                                                <span style={{ fontSize: 11, fontWeight: 700, color: isActive ? 'rgba(255,255,255,0.8)' : '#64748b', textTransform: 'uppercase', letterSpacing: 0.5, marginTop: 4 }}>{rule.name.split('(')[0]}</span>
                                            </div>
                                        </button>
                                    );
                                })
                        ...
                        <main className={styles.arithTermPanel} key={activeTab === 'terms' ? selectedIdx : `r${selectedRuleIdx}`} style={{
                            ...
                            boxShadow: '0 8px 24px rgba(0,0,0,0.03)', border: '2px solid rgba(59,130,246,0.08)', minHeight: 330
                        }}>
                            {activeTab === 'terms' ? (
                                <>
                                ...
                                <>
                                    ...
                                    <div>
                                        <h4 style={{ textTransform: 'uppercase', fontSize: 11, letterSpacing: 1, color: accentColor, marginBottom: 10 }}>Formula View</h4>
                                        <div style={{ background: 'rgba(59,130,246,0.04)', padding: 20, borderRadius: 16, border: '1px solid rgba(59,130,246,0.1)', textAlign: 'center' }}>
                                        ...
                                        {q.options.map((opt, optIdx) => {
                                            ...
                                            return (
                                                <button key={optIdx} onClick={() => handleQuizSelect(q.id, optIdx)}
                                                    style={{ padding: '16px 20px', borderRadius: 12, background: bg, border, color: color, textAlign: 'left', fontSize: 15, cursor: showResults ? 'default' : 'pointer', transition: 'all 0.2s ease', fontWeight: isSelected ? 600 : 400 }}
                                                >
                                                    {opt}
                            ...
                            {!showResults ? (
                                ...
                                <div>
                                    ...
                                    <div className={styles.arithActionRow}>
                                        ...
                                        <button onClick={() => navigate('/arithmetic/integers/skills')} style={{ padding: '12px 24px', background: '#3b82f6', color: '#fff', border: 'none', borderRadius: 100, fontSize: 15, fontWeight: 700, cursor: 'pointer' }}>Go to Skills →</button>
                                    </div>
                                </div>
                            )}
                        </div>
