...
import { SKILLS } from './SolidGeoSkillsData';
import { SOLID_SKILL_DIAGRAMS } from '../../../SkillDiagrams';

export default function SolidGeoSkills() {
            ...
            <div className={styles.page} style={{ background: '#f8fafc', minHeight: '100vh', paddingBottom: 60 }}>
                <nav className={styles.introNav}>
                    ...
                    <div className={styles.introNavLinks}>
                        <button className={styles.introNavLink} onClick={() => navigate('/geometry/3d-geometry/introduction')}>🌟 Introduction</button>
                        <button className={styles.introNavLink} onClick={() => navigate('/geometry/3d-geometry/terminology')}>📖 Terminology</button>
                                ...
                                <main key={`${skill.id}-${selectedLearnIdx}`} style={{ background: '#fff', borderRadius: 20, padding: '20px 28px', boxShadow: '0 8px 24px rgba(0,0,0,0.03)', border: `2px solid ${skill.color}15`, minHeight: 330, minWidth: 0, animation: 'geoSlideIn 0.4s cubic-bezier(0.4,0,0.2,1)' }}>
                                    ...
                                    <div className={styles.learnContentGrid}>
                                        <div>
                                            ...
                                            <div style={{ marginTop: 24, background: 'rgba(236,72,153,0.05)', padding: 16, borderRadius: 16, border: '1px solid rgba(236,72,153,0.1)' }}>
                                                <p style={{ margin: 0, fontSize: 15, lineHeight: 1.6, color: '#64748b' }}><span style={{ fontWeight: 800, color: '#ec4899' }}>Tip: </span><MathRenderer text={skill.learn.rules[selectedLearnIdx].tip} /></p>
                                            </div>
                                        </div>
                                        <div>
                                            {SOLID_SKILL_DIAGRAMS[skill.id]?.[selectedLearnIdx] && (
                                                <div style={{ marginBottom: 16 }}>
                                                    ...
                                                    <div style={{ background: '#f8fafc', borderRadius: 16, border: `1px solid ${skill.color}20`, padding: '10px 8px', display: 'flex', justifyContent: 'center', overflow: 'hidden' }}
                                                        dangerouslySetInnerHTML={{ __html: SOLID_SKILL_DIAGRAMS[skill.id][selectedLearnIdx] }} />
                                                </div>
                                            )}
    ...
    return (
        <div className={styles.page} style={{ background: '#f8fafc', minHeight: '100vh', paddingBottom: 60 }}>
            <nav className={styles.introNav}>
                <button className={styles.backBtn} onClick={() => navigate('/geometry/3d-geometry')} style={{ marginBottom: 0 }}>← Back to 3D Geometry</button>
                <div className={styles.introNavLinks}>
                    <button className={styles.introNavLink} onClick={() => navigate('/geometry/3d-geometry/introduction')}>🌟 Introduction</button>
                    <button className={styles.introNavLink} onClick={() => navigate('/geometry/3d-geometry/terminology')}>📖 Terminology</button>
                    <button className={`${styles.introNavLink} ${styles.introNavLinkActive}`}>🎯 Skills</button>
            ...
            <div className={styles.lexiconContainer} style={{ maxWidth: 1100, margin: '40px auto 20px', padding: '0 24px' }}>
                <div style={{ textAlign: 'center', marginBottom: 40 }}>
                    <h1 style={{ fontFamily: 'Outfit, sans-serif', fontSize: '2.8rem', fontWeight: 900, color: '#0f172a', margin: '0 0 8px' }}>
                        3D Geometry <span style={{ background: 'linear-gradient(135deg, #ec4899, #f472b6)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>Skills</span>
                    </h1>
                    <p style={{ color: '#64748b', fontWeight: 600, fontSize: 16, margin: 0 }}>Master volume and surface area of cylinders, spheres, and cones.</p>
                </div>
                ...
                <div style={{ marginTop: 60, padding: 32, borderRadius: 24, background: 'linear-gradient(135deg, #ec4899, #f472b6)', color: '#fff', textAlign: 'center', boxShadow: '0 15px 40px rgba(236,72,153,0.15)' }}>
                    <div style={{ fontSize: 32, marginBottom: 12 }}>🌐</div>
                    <h3 style={{ fontSize: 24, fontWeight: 800, margin: '0 0 8px' }}>Ready for More?</h3>
                    <p style={{ opacity: 0.9, maxWidth: 500, margin: '0 auto 24px' }}>You've mastered 3D solids! Explore transformations and symmetry next.</p>
                    <button className={styles.skillBtnOutline} style={{ background: '#fff', color: '#ec4899', fontWeight: 800, borderColor: '#fff' }} onClick={() => navigate('/geometry')}>Explore More Topics →</button>
                </div>
            </div>
        </div>
    );
