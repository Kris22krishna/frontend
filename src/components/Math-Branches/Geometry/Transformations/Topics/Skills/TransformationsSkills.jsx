...
import { SKILLS } from './TransformationsSkillsData';
import { TRANS_SKILL_DIAGRAMS } from '../../../SkillDiagrams';

export default function TransformationsSkills() {
    ...
    const openSkill = (index, nextView) => { setActiveSkill(index); setSelectedLearnIdx(0); setView(nextView); };
            ...
            <div className={styles.page} style={{ background: '#f8fafc', minHeight: '100vh', paddingBottom: 60 }}>
                <nav className={styles.introNav}>
                    ...
                    <div className={styles.introNavLinks}>
                        <button className={styles.introNavLink} onClick={() => navigate('/geometry/transformations/introduction')}>🌟 Introduction</button>
                        <button className={styles.introNavLink} onClick={() => navigate('/geometry/transformations/terminology')}>📖 Terminology</button>
                            ...
                            <div className={styles.learnGrid}>
                                <aside style={{ background: 'rgba(255,255,255,0.7)', padding: 14, borderRadius: 20, border: '1px solid rgba(0,0,0,0.05)', display: 'flex', flexDirection: 'column', gap: 8, backdropFilter: 'blur(10px)' }}>
                                    {skill.learn.rules.map((rule, index) => {
                                        ...
                                        return (
                                            <button key={rule.title} onClick={() => setSelectedLearnIdx(index)}
                                                style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '12px 14px', borderRadius: 12, border: `1.5px solid ${isActive ? skill.color : 'rgba(0,0,0,0.06)'}`, background: isActive ? `linear-gradient(135deg, ${skill.color}, ${skill.color}dd)` : `linear-gradient(135deg, ${skill.color}08, ${skill.color}03)`, cursor: 'pointer', textAlign: 'left', transition: 'all 0.25s', fontFamily: 'Outfit, sans-serif', transform: isActive ? 'translateY(-1px)' : 'none', boxShadow: isActive ? '0 8px 20px rgba(0,0,0,0.1)' : 'none' }}>
                                                <div style={{ width: 32, height: 32, borderRadius: 10, background: isActive ? 'rgba(255,255,255,0.25)' : '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 14, fontWeight: 900, color: isActive ? '#fff' : skill.color, flexShrink: 0 }}>{index + 1}</div>
                                                <span style={{ fontWeight: 700, fontSize: 14, color: isActive ? '#fff' : '#0f172a', lineHeight: 1.3 }}>{rule.title}</span>
                                            </button>
                                        );
                                    })}
                                </aside>
                                <main key={`${skill.id}-${selectedLearnIdx}`} style={{ background: '#fff', borderRadius: 20, padding: '20px 28px', boxShadow: '0 8px 24px rgba(0,0,0,0.03)', border: `2px solid ${skill.color}15`, minHeight: 330, minWidth: 0, animation: 'geoSlideIn 0.4s cubic-bezier(0.4,0,0.2,1)' }}>
                                    ...
                                    {TRANS_SKILL_DIAGRAMS[skill.id]?.[selectedLearnIdx] && (
                                        <div style={{ marginBottom: 32 }}>
                                            <h4 style={{ textTransform: 'uppercase', fontSize: 12, letterSpacing: 1, color: skill.color, marginBottom: 10 }}>VISUAL</h4>
                                            <div style={{ background: '#f8fafc', borderRadius: 16, padding: 12, border: '1px solid rgba(0,0,0,0.04)' }}
                                                dangerouslySetInnerHTML={{ __html: TRANS_SKILL_DIAGRAMS[skill.id][selectedLearnIdx] }} />
                                        </div>
                                    )}
                                    <div className={styles.learnContentGrid}>
                                        <div>
                                            ...
                                            <div style={{ marginTop: 24, background: 'rgba(6,182,212,0.05)', padding: 16, borderRadius: 16, border: '1px solid rgba(6,182,212,0.1)' }}>
                                                <p style={{ margin: 0, fontSize: 15, lineHeight: 1.6, color: '#64748b' }}><span style={{ fontWeight: 800, color: '#06b6d4' }}>Tip: </span><MathRenderer text={skill.learn.rules[selectedLearnIdx].tip} /></p>
                                            </div>
                                        </div>
                                        <div>
    ...
    return (
        <div className={styles.page} style={{ background: '#f8fafc', minHeight: '100vh', paddingBottom: 60 }}>
            <nav className={styles.introNav}>
                <button className={styles.backBtn} onClick={() => navigate('/geometry/transformations')} style={{ marginBottom: 0 }}>← Back to Transformations</button>
                <div className={styles.introNavLinks}>
                    <button className={styles.introNavLink} onClick={() => navigate('/geometry/transformations/introduction')}>🌟 Introduction</button>
                    <button className={styles.introNavLink} onClick={() => navigate('/geometry/transformations/terminology')}>📖 Terminology</button>
                    <button className={`${styles.introNavLink} ${styles.introNavLinkActive}`}>🎯 Skills</button>
            ...
            <div className={styles.lexiconContainer} style={{ maxWidth: 1100, margin: '40px auto 20px', padding: '0 24px' }}>
                <div style={{ textAlign: 'center', marginBottom: 40 }}>
                    <h1 style={{ fontFamily: 'Outfit, sans-serif', fontSize: '2.8rem', fontWeight: 900, color: '#0f172a', margin: '0 0 8px' }}>
                        Transformations <span style={{ background: 'linear-gradient(135deg, #06b6d4, #22d3ee)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>Skills</span>
                    </h1>
                    <p style={{ color: '#64748b', fontWeight: 600, fontSize: 16, margin: 0 }}>Master translation, reflection, rotation, and dilation with coordinate practice.</p>
                </div>
                ...
                <div style={{ marginTop: 60, padding: 32, borderRadius: 24, background: 'linear-gradient(135deg, #06b6d4, #22d3ee)', color: '#fff', textAlign: 'center', boxShadow: '0 15px 40px rgba(6,182,212,0.15)' }}>
                    <div style={{ fontSize: 32, marginBottom: 12 }}>🔄</div>
                    <h3 style={{ fontSize: 24, fontWeight: 800, margin: '0 0 8px' }}>You've Completed Transformations!</h3>
                    <p style={{ opacity: 0.9, maxWidth: 500, margin: '0 auto 24px' }}>Excellent work! You've covered all the core geometry branches. Keep exploring!</p>
                    <button className={styles.skillBtnOutline} style={{ background: '#fff', color: '#06b6d4', fontWeight: 800, borderColor: '#fff' }} onClick={() => navigate('/geometry')}>Back to Geometry →</button>
                </div>
            </div>
        </div>
    );
