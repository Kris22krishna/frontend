...
import { transformationsCards5W1H } from './TransformationsIntroData';
...
export default function TransformationsIntro5W1H() {
    ...
    return (
        <div className={styles.page}>
            ...
            <div className={styles.moduleHero}><div style={{ maxWidth: 1100, margin: '0 auto', textAlign: 'center' }}><h1 className={styles.sectionTitle}>Discover Transformations Through <span className={styles.sectionTitleAccent}>6 Big Questions</span></h1><p className={styles.sectionSubtitle} style={{ marginBottom: 0 }}>Tap each card to explore the world of motion and symmetry ✨</p></div></div>
            <div className={styles.section} style={{ paddingTop: 32 }}>
                <div className={styles.w1hGrid}>{transformationsCards5W1H.map((card, idx) => <W1HCard key={idx} card={card} />)}</div>
                <div style={{ marginTop: 40, padding: 24, background: '#fff', borderRadius: 20, border: '1px solid #e2e8f0', display: 'flex', alignItems: 'center', justifyContent: 'space-between', boxShadow: '0 4px 20px rgba(0,0,0,0.04)' }}>
                    <p style={{ margin: 0, fontSize: 16, fontWeight: 600, color: '#475569' }}>Up next: Translation, rotation, reflection, and dilation vocabulary!</p>
                    <button onClick={() => navigate('/geometry/transformations/terminology')} style={{ padding: '10px 24px', background: '#06b6d4', color: '#fff', border: 'none', borderRadius: 100, fontWeight: 700, cursor: 'pointer' }}>Terminology →</button>
                </div>
            </div>
        </div>
    );
