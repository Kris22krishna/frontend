...
import { polygonsCards5W1H } from './PolygonsIntroData';
...
export default function PolygonsIntro5W1H() {
    ...
    return (
        <div className={styles.page}>
            ...
            <div className={styles.moduleHero}><div style={{ maxWidth: 1100, margin: '0 auto', textAlign: 'center' }}><h1 className={styles.sectionTitle}>Discover Polygons Through <span className={styles.sectionTitleAccent}>6 Big Questions</span></h1><p className={styles.sectionSubtitle} style={{ marginBottom: 0 }}>Tap each card to explore quadrilaterals, special shapes, and properties ✨</p></div></div>
            <div className={styles.section} style={{ paddingTop: 32 }}>
                <div className={styles.w1hGrid}>{polygonsCards5W1H.map((card, idx) => <W1HCard key={idx} card={card} />)}</div>
                <div style={{ marginTop: 40, padding: 24, background: '#fff', borderRadius: 20, border: '1px solid #e2e8f0', display: 'flex', alignItems: 'center', justifyContent: 'space-between', boxShadow: '0 4px 20px rgba(0,0,0,0.04)' }}>
                    <p style={{ margin: 0, fontSize: 16, fontWeight: 600, color: '#475569' }}>Up next: Quadrilateral types, properties, and formulas!</p>
                    <button onClick={() => navigate('/geometry/polygons/terminology')} style={{ padding: '10px 24px', background: '#10b981', color: '#fff', border: 'none', borderRadius: 100, fontWeight: 700, cursor: 'pointer' }}>Terminology →</button>
                </div>
            </div>
        </div>
    );
