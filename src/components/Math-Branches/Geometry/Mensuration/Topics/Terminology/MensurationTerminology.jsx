...
import { TERMS, FIVE_RULES, VOCAB_QUIZ } from './MensurationTerminologyData';

const accent = '#8b5cf6';

export default function MensurationTerminology() {
    ...
    return (
        <div className={styles.page}>
            <style>{`.men-anim{animation:menSlide 0.4s cubic-bezier(0.4,0,0.2,1)} @keyframes menSlide{from{opacity:0;transform:translateX(20px)}to{opacity:1;transform:translateX(0)}} .men-btn{display:flex;align-items:center;gap:10px;padding:10px 14px;border-radius:12px;border:1.5px solid rgba(0,0,0,0.06);cursor:pointer;transition:all 0.25s;text-align:left;font-family:'Outfit',sans-serif;background:#fff} .men-btn:hover{transform:translateY(-2px);box-shadow:0 6px 15px rgba(0,0,0,0.08)} @media(max-width:1024px){.men-grid{grid-template-columns:1fr !important}}`}</style>
            <nav className={styles.introNav}>
                ...
                <div className={styles.introNavLinks}>
                    <button className={styles.introNavLink} onClick={() => navigate('/geometry/mensuration/introduction')}>🌟 Introduction</button>
            ...
            <div style={{ maxWidth: 1100, margin: '40px auto 20px', padding: '0 24px' }}>
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', marginBottom: 20 }}>
                    <h1 style={{ fontFamily: 'Outfit,sans-serif', fontSize: '2.8rem', fontWeight: 900, color: '#0f172a', margin: '0 0 8px' }}>Mensuration <span style={{ background: `linear-gradient(135deg,${accent},#a78bfa)`, WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>Vocabulary</span></h1>
                ...
                {activeTab !== 'quiz' ? (
                    <div className="men-grid" style={{ display:'grid', gridTemplateColumns:'minmax(220px,280px) 1fr', gap:16, alignItems:'start' }}>
                        <aside style={{ background:'rgba(255,255,255,0.7)', padding:14, borderRadius:20, border:'1px solid rgba(0,0,0,0.05)', display:'grid', gridTemplateColumns:'1fr', gap:10, backdropFilter:'blur(10px)' }}>
                            {activeTab === 'terms' ? TERMS.map((term,i) => { const isA=selectedIdx===i; return (
                                <button key={i} className="men-btn" onClick={()=>setSelectedIdx(i)} style={{ background:isA?`linear-gradient(135deg,${accent},${accent}dd)`:`linear-gradient(135deg,${accent}10,${accent}05)`, borderColor:isA?accent:`${accent}20` }}>
                                ...
                                <button key={i} className="men-btn" onClick={()=>setSelectedRuleIdx(i)} style={{ background:isA?`linear-gradient(135deg,${accent},${accent}dd)`:`linear-gradient(135deg,${accent}10,${accent}05)`, borderColor:isA?accent:`${accent}20`, padding:'12px 16px' }}>
                        ...
                        <main className="men-anim" key={activeTab==='terms'?selectedIdx:`r${selectedRuleIdx}`} style={{ background:'#fff', borderRadius:20, padding:'20px 28px', boxShadow:'0 8px 24px rgba(0,0,0,0.03)', border:`2px solid ${accent}10`, minHeight:330, minWidth:0 }}>
                            {activeTab === 'terms' ? (<>
                    ...
                    <div className="men-anim" style={{ maxWidth:700, margin:'0 auto', background:'#fff', borderRadius:24, padding:'32px 40px', boxShadow:'0 15px 40px rgba(0,0,0,0.04)', border:'1px solid rgba(0,0,0,0.04)' }}>
                        ...
                        <div style={{marginTop:40,textAlign:'center',paddingTop:32,borderTop:'1px solid #e2e8f0'}}>
                            {!showResults ? <button onClick={handleCheck} style={{padding:'14px 32px',background:accent,color:'#fff',border:'none',borderRadius:100,fontSize:16,fontWeight:700,cursor:'pointer'}}>Check Answers</button> : (
                                <div><div style={{fontSize:24,fontWeight:700,color:'#0f172a',marginBottom:16}}>You scored {Object.entries(quizAnswers).filter(([id,ans])=>VOCAB_QUIZ.find(q=>q.id===parseInt(id)).correct===ans).length} out of {VOCAB_QUIZ.length}! 🎉</div><div style={{display:'flex',gap:16,justifyContent:'center'}}><button onClick={handleRetry} style={{padding:'12px 24px',background:'#fff',color:'#475569',border:'1.5px solid #e2e8f0',borderRadius:100,fontSize:15,fontWeight:600,cursor:'pointer'}}>Try Again</button><button onClick={()=>navigate('/geometry/mensuration/skills')} style={{padding:'12px 24px',background:accent,color:'#fff',border:'none',borderRadius:100,fontSize:15,fontWeight:700,cursor:'pointer'}}>Go to Skills →</button></div></div>
                            )}
                        </div>
                    </div>
                )}
                <div style={{marginTop:16,textAlign:'center'}}><button onClick={()=>navigate('/geometry/mensuration/skills')} style={{padding:'10px 28px',fontSize:13,borderRadius:100,border:'none',background:accent,color:'#fff',fontWeight:800,cursor:'pointer'}}>Ready to Solve! 🎯</button></div>
            </div>
        </div>
    );
}
