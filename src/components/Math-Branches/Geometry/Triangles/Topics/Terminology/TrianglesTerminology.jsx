...
export default function TrianglesTerminology() {
    ...
    const handleQuizSelect = (qId, optionIdx) => { if (!showResults) setQuizAnswers(p => ({ ...p, [qId]: optionIdx })); };
    ...
    return (
        <div className={styles.page}>
            <style>{`
                .tri-detail-anim { animation: triSlide 0.4s cubic-bezier(0.4,0,0.2,1); }
                @keyframes triSlide { from { opacity:0; transform:translateX(20px); } to { opacity:1; transform:translateX(0); } }
                .tri-term-btn { display:flex; align-items:center; gap:10px; padding:10px 14px; border-radius:12px; border:1.5px solid rgba(0,0,0,0.06); cursor:pointer; transition:all 0.25s; text-align:left; font-family:'Outfit',sans-serif; background:#fff; }
                .tri-term-btn:hover { transform:translateY(-2px); box-shadow:0 6px 15px rgba(0,0,0,0.08); }
                @media(max-width:1024px){ .tri-lex-grid{ grid-template-columns:1fr !important; } }
            `}</style>

            <nav className={styles.introNav}>
                <button className={styles.backBtn} onClick={() => navigate('/geometry/triangles')} style={{ marginBottom: 0 }}>← Back to Triangles</button>
                <div className={styles.introNavLinks}>
                    <button className={styles.introNavLink} onClick={() => navigate('/geometry/triangles/introduction')}>🌟 Introduction</button>
                    <button className={`${styles.introNavLink} ${styles.introNavLinkActive}`}>📖 Terminology</button>
                    <button className={styles.introNavLink} onClick={() => navigate('/geometry/triangles/skills')}>🎯 Skills</button>
                </div>
            </nav>
            ...
            <div style={{ maxWidth: 1100, margin: '40px auto 20px', padding: '0 24px' }}>
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', marginBottom: 20 }}>
                    <h1 style={{ fontFamily: 'Outfit, sans-serif', fontSize: '2.8rem', fontWeight: 900, color: '#0f172a', margin: '0 0 8px' }}>
                        Triangles <span style={{ background: `linear-gradient(135deg, ${accent}, #fcd34d)`, WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>Vocabulary</span>
                    </h1>
                ...
                <div style={{ display: 'flex', justifyContent: 'center', gap: 8, marginBottom: 16 }}>
                    {[['terms','📚 Terminology'], ['rules','⚡ Key Theorems'], ['quiz','🧪 Vocab Check']].map(([key, label]) => (
                        <button key={key} className={styles.termTab + (activeTab === key ? ` ${styles.termTabActive}` : '')} onClick={() => setActiveTab(key)}>{label}</button>
                    ))}
                </div>

                {activeTab !== 'quiz' ? (
                    <div className="tri-lex-grid" style={{ display: 'grid', gridTemplateColumns: 'minmax(220px,280px) 1fr', gap: 16, alignItems: 'start' }}>
                        <aside style={{ background: 'rgba(255,255,255,0.7)', padding: 14, borderRadius: 20, border: '1px solid rgba(0,0,0,0.05)', display: 'grid', gridTemplateColumns: '1fr', gap: 10, backdropFilter: 'blur(10px)' }}>
                            {activeTab === 'terms' ? TERMS.map((term, i) => {
                                ...
                                return (
                                    <button key={i} className="tri-term-btn" onClick={() => setSelectedIdx(i)}
                                        style={{
                                        ...
                                        <span style={{ fontWeight: 800, fontSize: 16, color: isActive ? '#fff' : '#0f172a' }}>{term.word}</span>
                                    </button>
                                );
                            }) : FIVE_RULES.map((rule, i) => {
                                const isActive = selectedRuleIdx === i;
                                return (
                                    <button key={i} className="tri-term-btn" onClick={() => setSelectedRuleIdx(i)}
                                        style={{ background: isActive ? `linear-gradient(135deg, ${accent}, ${accent}dd)` : `linear-gradient(135deg, ${accent}10, ${accent}05)`, borderColor: isActive ? accent : `${accent}20`, padding: '12px 16px' }}>
                                        <div style={{ width: 36, height: 36, borderRadius: 10, background: isActive ? 'rgba(255,255,255,0.25)' : '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16, fontWeight: 900, color: isActive ? '#fff' : accent }}>{i + 1}</div>
                                        <div style={{ display: 'flex', flexDirection: 'column' }}>
                                            <span style={{ fontWeight: 800, fontSize: 16, color: isActive ? '#fff' : '#0f172a', lineHeight: 1 }}>Rule {i + 1}</span>
                                            <span style={{ fontSize: 11, fontWeight: 700, color: isActive ? 'rgba(255,255,255,0.8)' : '#64748b', textTransform: 'uppercase', letterSpacing: 0.5, marginTop: 4 }}>{rule.name}</span>
                                        </div>
                                    </button>
                                );
                            })}
                        ...
                        <main className="tri-detail-anim" key={activeTab === 'terms' ? selectedIdx : `r${selectedRuleIdx}`}
                            style={{ background: '#fff', borderRadius: 20, padding: '20px 28px', boxShadow: '0 8px 24px rgba(0,0,0,0.03)', border: `2px solid ${accent}10`, minHeight: 330, minWidth: 0 }}>
                            {activeTab === 'terms' ? (
                                <>
                                    ...
                                    <div style={{ background: `${accent}05`, padding: 16, borderRadius: 16, border: `1px solid ${accent}10` }}>
                                        <div style={{ fontSize: 15, color: '#0f172a' }}><MathRenderer text={activeTerm.example} /></div>
                                    </div>
                                </>
                            ) : (
                                <>
                                    ...
                                    <div style={{ background: `${accent}05`, padding: 20, borderRadius: 16, border: `1px solid ${accent}10`, textAlign: 'center' }}>
                                        <div style={{ fontSize: 18, fontWeight: 700, color: accent }}><MathRenderer text={activeRule.formula} /></div>
                                    </div>
                                </>
                            )}
                        </main>
                    ...
                    <div className="tri-detail-anim" style={{ maxWidth: 700, margin: '0 auto', background: '#fff', borderRadius: 24, padding: '32px 40px', boxShadow: '0 15px 40px rgba(0,0,0,0.04)', border: '1px solid rgba(0,0,0,0.04)' }}>
                                ...
                                <div key={q.id}>
                                    ...
                                    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                                        {q.options.map((opt, optIdx) => {
                                            const isSel = quizAnswers[q.id] === optIdx;
                                            const isCorrect = optIdx === q.correct;
                                            let bg = '#f8fafc', border = '1px solid #e2e8f0', color = '#475569';
                                            if (showResults) {
                                                if (isCorrect) { bg = '#ecfdf5'; border = '1px solid #10b981'; color = '#10b981'; }
                                                else if (isSel) { bg = '#fef2f2'; border = '1px solid #ef4444'; color = '#ef4444'; }
                                            } else if (isSel) { bg = `${accent}10`; border = `1px solid ${accent}`; color = accent; }
                                            return (
                                                <button key={optIdx} onClick={() => handleQuizSelect(q.id, optIdx)}
                                                    ...
                                                    {showResults && isSel && !isCorrect && <span style={{ float: 'right' }}>❌</span>}
                                                </button>
                                            );
                                        })}
                                    </div>
                        ...
                        <div style={{ marginTop: 40, textAlign: 'center', paddingTop: 32, borderTop: '1px solid #e2e8f0' }}>
                            {!showResults ? (
                                <button onClick={handleCheck} style={{ padding: '14px 32px', background: accent, color: '#fff', border: 'none', borderRadius: 100, fontSize: 16, fontWeight: 700, cursor: 'pointer' }}>Check Answers</button>
                            ) : (
                                <div>
                                    <div style={{ fontSize: 24, fontWeight: 700, color: '#0f172a', marginBottom: 16 }}>
                                        You scored {Object.entries(quizAnswers).filter(([id, ans]) => VOCAB_QUIZ.find(q => q.id === parseInt(id)).correct === ans).length} out of {VOCAB_QUIZ.length}! 🎉
                                    </div>
                                    <div style={{ display: 'flex', gap: 16, justifyContent: 'center' }}>
                                        ...
                                        <button onClick={() => navigate('/geometry/triangles/skills')} style={{ padding: '12px 24px', background: accent, color: '#fff', border: 'none', borderRadius: 100, fontSize: 15, fontWeight: 700, cursor: 'pointer' }}>Go to Skills →</button>
                                    </div>
                                </div>
                            )}
                        </div>
                ...
                <div style={{ marginTop: 16, textAlign: 'center' }}>
                    <button onClick={() => navigate('/geometry/triangles/skills')} style={{ padding: '10px 28px', fontSize: 13, borderRadius: 100, border: 'none', background: accent, color: '#fff', fontWeight: 800, cursor: 'pointer' }}>Ready to Solve! 🎯</button>
                </div>
            </div>
        </div>
    );
