...
import { TERMS, KEY_IDENTITIES, VOCAB_QUIZ } from './InverseTrigGr12TerminologyData';
import { useSessionLogger } from '@/hooks/useSessionLogger';
import { curriculumPathToNodeId } from '@/lib/curriculumIds';

export default function InverseTrigGr12Terminology() {
    ...
    const handleRetry = () => {
        ...
        const nodeId = curriculumPathToNodeId['inverse-trig-terminology'];
        if (nodeId) {
    ...
    return (
        <div className={styles.arithPage}>
            <nav className={styles.introNav}>
                <button className={styles.arithBackBtn} onClick={() => navigate('/trigonometry/grade-12-inverse')} style={{ marginBottom: 0 }}>← Back to Branch</button>
                <div className={styles.arithIntroNavLinks}>
                    <button className={styles.arithIntroNavLink} onClick={() => navigate('/trigonometry/grade-12-inverse/introduction')}>🌟 Introduction</button>
                    <button className={`${styles.arithIntroNavLink} ${styles.arithIntroNavLinkActive}`}>📖 Terminology</button>
                    <button className={styles.arithIntroNavLink} onClick={() => navigate('/trigonometry/grade-12-inverse/skills')}>🎯 Skills</button>
                </div>
            </nav>
            ...
            <div className={styles.arithTermContainer}>
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', marginBottom: 20, paddingTop: 32 }}>
                    <h1 style={{ fontFamily: '"Outfit", sans-serif', fontSize: '2.8rem', fontWeight: 900, color: '#0f172a', margin: '0 0 8px' }}>
                        Inverse Trig{' '}
                        <span style={{ background: 'linear-gradient(135deg, #ec4899, #f472b6)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>Vocabulary</span>
                    </h1>
                    <div style={{ fontSize: 16, fontWeight: 700, color: '#64748b', letterSpacing: 0.5 }}>
                        {activeTab === 'quiz'
                            ? 'Test your knowledge with interactive questions!'
                            : `Select any ${activeTab === 'terms' ? 'term' : 'identity'} below to explore details.`}
                    </div>
                </div>
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            <div style={{ display: 'flex', flexDirection: 'column', textAlign: 'left' }}>
                                                <span style={{ fontWeight: 800, fontSize: 13, color: isActive ? '#fff' : '#1e293b', lineHeight: 1.2 }}>Identity {i + 1}</span>
                                ...
                                <>
                                    ...
                                    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                                        <div>
                                            ...
                                            <div style={{ background: 'rgba(236,72,153,0.04)', padding: 14, borderRadius: 12, border: '1px solid rgba(236,72,153,0.1)' }}>
                                        ...
                                        {activeTerm.realLifeExample && (
                                            <div>
                                                <h4 style={{ textTransform: 'uppercase', fontSize: 11, letterSpacing: 1, color: '#be185d', marginBottom: 8 }}>Real World Application</h4>
                                                <div style={{ background: 'rgba(190,24,93,0.04)', padding: 14, borderRadius: 12, border: '1px solid rgba(190,24,93,0.15)' }}>
                                                    <div style={{ fontSize: 15, color: '#831843', fontWeight: 500, lineHeight: 1.5 }}>
                                                        🌍 {activeTerm.realLifeExample}
                                                    </div>
                                                </div>
                                            </div>
                            ...
                            {!showResults ? (
                                ...
                                <div>
                                    ...
                                    <div className={styles.arithActionRow}>
                                        ...
                                        <button onClick={() => navigate('/trigonometry/grade-12-inverse/skills')} style={{ padding: '12px 24px', background: accentColor, color: '#fff', border: 'none', borderRadius: 100, fontSize: 15, fontWeight: 700, cursor: 'pointer' }}>Go to Skills →</button>
                                    </div>
                                </div>
                            )}
                        </div>
