...
export default function TrigPracticeEngine({ questionPool, sampleSize, title, color, onBack, nodeId }) {
    ...
    useEffect(() => {
        if (finished) return;
        const timer = setInterval(() => setTimeTaken((s) => s + 1), 1000);
    ...
    if (finished) {
        ...
        const sub = pct >= 75 ? 'Excellent understanding of the concept!' : 'Review the concepts and try again.';
        ...
        return (
            <div style={{ textAlign: 'center', padding: '40px 0' }}>
                ...
                <div style={{ display: 'flex', gap: 12, justifyContent: 'center' }}>
                    <button className={styles['ccr-btn-primary']} onClick={handleRetry} style={{ background: color }}>New Questions</button>
                    <button className={styles['ccr-btn-secondary']} onClick={onBack}>Return to Skills</button>
                </div>
            </div>
        );
    }
        ...
        <div className={styles['ccr-quiz-container']}>
            ...
            <div className={styles['ccr-quiz-card']}>
                <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, background: `${color}15`, padding: '4px 12px', borderRadius: 8, fontSize: 12, fontWeight: 800, color, marginBottom: 14 }}>
                    {q.type === 'fill' ? 'FILL IN THE BLANK' : q.type === 'truefalse' ? 'TRUE OR FALSE' : 'QUESTION'} {current + 1}
                </div>
                ...
                {isFill ? (
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
                        <input
                            type="number"
                            className={styles['ccr-fill-input']}
                            value={fillValue}
                        ...
                        {answered && (
                            <div style={{ padding: '12px 18px', borderRadius: 12, background: fillCorrect ? 'rgba(16,185,129,0.06)' : 'rgba(239,68,68,0.06)', border: `2px solid ${fillCorrect ? '#10b981' : '#ef4444'}`, fontSize: 15, fontWeight: 700, color: fillCorrect ? '#059669' : '#ef4444' }}>
                                <MathRenderer text={fillCorrect ? `Correct! Answer: $${q.correctValue}$` : `Wrong. Correct answer: $${q.correctValue}$`} />
                            </div>
                        )}
                    </div>
                ) : renderOptions()}
                ...
                {answered && q.explanation && (
                    <div style={{ marginTop: 20, padding: '14px 18px', borderRadius: 12, background: 'rgba(15,118,110,0.05)', border: '1px solid rgba(15,118,110,0.1)', color: '#64748b', fontSize: 13.5, lineHeight: 1.65, whiteSpace: 'pre-line' }}>
                        <strong style={{ color }}>Explanation: </strong><MathRenderer text={q.explanation} />
                    </div>
                )}
            </div>
            ...
            <div style={{ display: 'flex', justifyContent: 'center', marginTop: 20 }}>
                <button
                    onClick={handleNext}
                    disabled={!answered}
                    style={{ padding: '13px 44px', background: answered ? color : '#f1f5f9', color: answered ? '#fff' : '#94a3b8', cursor: answered ? 'pointer' : 'not-allowed', border: 'none', borderRadius: 100, fontSize: 15, fontWeight: 800, boxShadow: answered ? `0 8px 20px ${color}35` : 'none', transition: 'all 0.2s', fontFamily: 'Open Sans, sans-serif' }}
                >
