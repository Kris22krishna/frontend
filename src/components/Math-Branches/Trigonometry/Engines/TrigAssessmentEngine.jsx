...
export default function TrigAssessmentEngine({ questionPool, sampleSize = 10, title, color, onBack, nodeId }) {
    ...
    const normalised = safePool.map(q => ({
        ...q,
    ...
    const mcqOnly = normalised.filter((q) => allowedType(q) && validQuestion(q));
    const source = mcqOnly.length ? mcqOnly : normalised.filter(validQuestion);

    const [questions] = useState(() => shuffle(source).slice(0, Math.min(sampleSize, source.length)));
    const [current, setCurrent] = useState(0);
    const [answers, setAnswers] = useState(() => Array(Math.min(sampleSize, source.length)).fill(null));
    const [reviewed, setReviewed] = useState(() => Array(Math.min(sampleSize, source.length)).fill(false));
        ...
        return (
            <div style={{ maxWidth: 860, margin: '0 auto', padding: '20px' }}>
                <div style={{ textAlign: 'center', marginBottom: 40 }}>
                    <div style={{ fontSize: 13, fontWeight: 800, color, textTransform: 'uppercase', letterSpacing: 1.5, marginBottom: 8 }}>{title} — Assessment Complete</div>
                        ...
                        return (
                            ...
                            >
                                <div style={{ display: 'flex', gap: 10, alignItems: 'flex-start', marginBottom: 12 }}>
                                    ...
                                    <div style={{ fontSize: 15, fontWeight: 600, color: '#1e293b', lineHeight: 1.55 }}>
                                ...
                                <div className={styles['ccr-quiz-options']}>
                                    <div style={{ background: 'rgba(16,185,129,0.08)', borderRadius: 10, padding: '10px 14px' }}>
                                        ...
                                        <div style={{ fontSize: 14, fontWeight: 600, color: '#1e293b' }}><MathRenderer text={item.options[item.correct]} /></div>
                                    </div>
                                    <div style={{ background: isCorrect ? 'rgba(16,185,129,0.08)' : 'rgba(239,68,68,0.08)', borderRadius: 10, padding: '10px 14px' }}>
                                        ...
                                        <div style={{ fontSize: 14, fontWeight: 600, color: '#1e293b' }}>
                                            {userAns !== null ? <MathRenderer text={item.options[userAns]} /> : '— Not Answered —'}
                                        </div>
                                    </div>
                                </div>
                                ...
                                {item.explanation && (
                                    <div style={{ marginTop: 12, fontSize: 13, color: '#64748b', lineHeight: 1.6, background: 'rgba(255,255,255,0.5)', padding: '10px', borderRadius: 8 }}>
                                        <strong style={{ color }}>Explanation: </strong><MathRenderer text={item.explanation} />
                                    </div>
                                )}
                            </div>
                        );
        ...
        <div className={styles['ccr-assessment-layout']}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div className={styles['ccr-quiz-card']}>
                    ...
                    <div style={{ fontSize: 17, fontWeight: 600, color: '#1e293b', lineHeight: 1.65, marginBottom: 20, whiteSpace: 'pre-line' }}>
                        ...
                        {(q?.options || []).map((opt, oi) => {
                            ...
                            return (
                                <button
                                    ...
                                    style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '14px 16px', borderRadius: 12, border: `2.5px solid ${isSelected ? color : 'rgba(0,0,0,0.06)'}`, background: isSelected ? `${color}08` : '#fff', cursor: 'pointer', fontSize: 15, color: isSelected ? color : '#1e293b', textAlign: 'left', transition: 'all 0.2s', fontWeight: isSelected ? 700 : 500, justifyContent: isTF ? 'center' : 'flex-start', fontFamily: 'Open Sans, sans-serif', width: '100%' }}
                                >
