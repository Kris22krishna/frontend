...
export default function AssessmentEngine({ 
    ...
    const normalizePlainTextSafe = (value) => normalizeTextAnswer(value)
        .replace(/[\u2212\u2013\u2014]/g, '-');

    const splitTopLevelItems = (value) => {
        const items = [];
        let current = '';
        let depth = 0;
    ...
    const normalizeSetExpressionSafe = (value) => {
        const normalized = normalizePlainTextSafe(value);
        ...
        const items = splitTopLevelItems(inner)
            .map((item) => normalizeSetExpressionSafe(item))
            .sort();

        return `{${items.join(',')}}`;
    };

    const normalizeComparableTextAnswer = (value) => {
        const normalized = normalizePlainTextSafe(value);
        return normalized.startsWith('{') && normalized.endsWith('}')
            ? normalizeSetExpressionSafe(normalized)
            : normalized;
    };

    const getAcceptedTextAnswers = (question) => [question.answer, ...(question.acceptedAnswers ?? [])]
        .filter((answer) => answer !== undefined && answer !== null);

    const validateQuestionBank = (questionList) => {
        questionList.forEach((question, index) => {
            const type = getQuestionType(question);
            if ((type === 'mcq' || type === 'msq') && !Array.isArray(question.options)) {
                console.warn(`Assessment question ${index + 1} is missing options.`, question);
            }
            if (type === 'mcq' && (!Number.isInteger(question.correct) || question.correct < 0 || question.correct >= (question.options?.length ?? 0))) {
                console.warn(`Assessment question ${index + 1} has an invalid correct index.`, question);
            }
            if (type === 'msq') {
                const isValid = Array.isArray(question.correct) && question.correct.every(
                    (answerIndex) => Number.isInteger(answerIndex) && answerIndex >= 0 && answerIndex < (question.options?.length ?? 0)
                );
                if (!isValid) {
                    console.warn(`Assessment question ${index + 1} has invalid MSQ answers.`, question);
                }
            }
            if (type === 'text' && getAcceptedTextAnswers(question).length === 0) {
                console.warn(`Assessment question ${index + 1} is missing a text answer.`, question);
            }
        });
    };
    ...
    const isAnswerComplete = (question, answer) => {
        const type = getQuestionType(question);
        if (type === 'text') return normalizeComparableTextAnswer(answer).length > 0;
    ...
    const isAnswerCorrect = (question, answer) => {
        const type = getQuestionType(question);
        if (type === 'text') {
            const normalizedAnswer = normalizeComparableTextAnswer(answer);
            return getAcceptedTextAnswers(question).some(
                (acceptedAnswer) => normalizedAnswer === normalizeComparableTextAnswer(acceptedAnswer)
            );
        }
        if (type === 'msq') {
                            ...
                            {(q.options ?? []).map((opt, optIndex) => {
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        />
                                        <span style={{ display: 'block', minWidth: 0, maxWidth: '100%', lineHeight: 1.55, color: 'inherit', overflowWrap: 'anywhere', wordBreak: 'break-word' }}>
