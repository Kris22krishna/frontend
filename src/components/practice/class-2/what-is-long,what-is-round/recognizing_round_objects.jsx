...
const RecognizingRoundObjects = () => {
    const { user } = useAuth();
    const navigate = useNavigate();
    const location = useLocation();
    const queryParams = new URLSearchParams(location.search);
    const skillId = queryParams.get('skillId') || '1003';
    ...
    const getTopicInfo = () => {
        ...
        return { topicName: 'What is Long, What is Round?', skillName: 'Recognizing round objects' };
    };
    ...
    useEffect(() => {
        const init = async () => {
            ...
            try {
                const session = await api.createPracticeSession(user?.id, 'recognizing-round-objects');
...
export default RecognizingRoundObjects;
