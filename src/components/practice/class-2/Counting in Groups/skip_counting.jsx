...
const DynamicVisual = ({ sequence, missingIndex, color }) => {
    return (
        <motion.div
            ...
            {sequence.map((num, i) => (
                <div
                    ...
                    style={{
                        ...
                        fontSize: '1.5rem',
                        fontWeight: 'bold',
                        color: i === missingIndex ? color : 'white',
...
const SkipCounting = () => {
    ...
    const skillId = queryParams.get('skillId') || '1005';
    ...
    const getTopicInfo = () => {
        ...
        return { topicName: 'Counting in Groups', skillName: 'Skip counting (2s, 5s, 10s)' };
    };
    ...
    const generateQuestions = () => {
        ...
        for (let i = 0; i < TOTAL_QUESTIONS; i++) {
            ...
            while (wrongOptions.length < 2) {
                const wrong = (start + Math.floor(Math.random() * 10) * (step / 2 || 1)).toString();
    ...
    useEffect(() => {
        const init = async () => {
            ...
            try {
                const session = await api.createPracticeSession(user?.id, 'skip-counting');
