...
const CountingInPairs = () => {
    const { user } = useAuth();
    const navigate = useNavigate();
    const location = useLocation();
    const queryParams = new URLSearchParams(location.search);
    const skillId = queryParams.get('skillId') || '1004';
    ...
    const getTopicInfo = () => {
        ...
        return { topicName: 'Counting in Groups', skillName: 'Counting in pairs' };
    };
    ...
    useEffect(() => {
        const init = async () => {
            ...
            try {
                const session = await api.createPracticeSession(user?.id, 'counting-in-pairs');
...
export default CountingInPairs;
