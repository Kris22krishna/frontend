...
import { TERMS, FIVE_RULES, VOCAB_QUIZ } from './WorkEnergyPowerTerminologyData';
import { useSessionLogger } from '@/hooks/useSessionLogger';
import { NODE_IDS } from '@/lib/curriculumIds';

export default function WorkEnergyPowerTerminology() {
    ...
    // Start session lazily when user enters Quiz tab
    const handleTabChange = (tab) => {
        setActiveTab(tab);
        if (tab === 'quiz' && !quizSessionStarted.current) {
            startSession({ nodeId: NODE_IDS.g11PhysWEPTerminologyQuiz, sessionType: 'practice' });
    ...
    const resetQuiz = () => {
        ...
        quizSessionStarted.current = false; // allow fresh session on re-entry
    };
        ...
        <div className="wep-page">
            ...
            <nav className="wep-nav">
                ...
                <div className="wep-nav-links">
                    ...
                    <button className="wep-nav-link active" onClick={() => navigate('/senior/grade/11/physics/work-energy-power/terminology')}>📖 Terminology</button>
            ...
            {/* ── MAIN CONTENT ──────────────────────────────── */}
            <div className="wep-lexicon-container">
                ...
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', marginBottom: 40 }}>
                    <h1 style={{ fontFamily: 'Outfit, sans-serif', fontSize: 'clamp(2.2rem, 8vw, 3.5rem)', fontWeight: 900, color: 'var(--rn-text)', margin: '0 0 12px', letterSpacing: '-0.02em' }}>
                        Physics <span style={{ background: 'linear-gradient(135deg, var(--rn-indigo), var(--rn-violet))', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>Lexicon</span>
                    </h1>
                ...
                <div className="rn-subtabs" style={{ display: 'flex', justifyContent: 'center', gap: 12, marginBottom: 32 }}>
                    <button className={`wep-nav-link ${activeTab === 'terms' ? 'active' : ''}`} onClick={() => handleTabChange('terms')}>📚 Key Terms</button>
                    <button className={`wep-nav-link ${activeTab === 'rules' ? 'active' : ''}`} onClick={() => handleTabChange('rules')}>📏 5 Golden Rules</button>
                    <button className={`wep-nav-link ${activeTab === 'quiz' ? 'active' : ''}`} onClick={() => handleTabChange('quiz')}>🧪 Quiz Time</button>
                </div>

                {activeTab !== 'quiz' ? (
                    /* ── MASTER-DETAIL VIEW (Terms or Rules) ── */
                    <div className="wep-lexicon-grid">
                        <aside className="wep-selector-container">
                            {activeTab === 'terms' ? (
                                TERMS.map((term, i) => {
                                    const isActive = selectedIdx === i;
                                    return (
                                        <button key={i} className={`wep-term-btn-mini ${isActive ? 'active' : ''}`} onClick={() => setSelectedIdx(i)}>
                                ...
                                FIVE_RULES.map((rule, i) => {
                                    ...
                                    return (
                                        <button key={i} className={`wep-term-btn-mini ${isActive ? 'active' : ''}`} onClick={() => setSelectedRuleIdx(i)}>
                        ...
                        <main className="wep-details-window" key={activeTab === 'terms' ? selectedIdx : selectedRuleIdx}>
                            {activeTab === 'terms' ? (
                                <>
                                    ...
                                    <div className="wep-responsive-grid-2" style={{ gap: 24 }}>
                                        <div>
                    ...
                    <div className="wep-quiz-window">
                        {!quizFinished ? (
                            <>
                                ...
                                <div className="wep-responsive-grid-2" style={{ gap: 16, marginBottom: 32 }}>
                                    {activeQuiz.options.map((opt, oi) => {
                                        ...
                                        return (
                                            <button 
                                                ...
                                                className={`wep-quiz-option ${statusClass}`}
                            ...
                            <div style={{ textAlign: 'center', padding: '24px 0' }}>
                                ...
                                <div style={{ display: 'flex', gap: 16, justifyContent: 'center' }}>
                                    <button className="wep-btn-outline" onClick={resetQuiz} style={{ padding: '14px 32px' }}>Try Again</button>
                                    <button className="wep-btn-filled" onClick={() => navigate('/senior/grade/11/physics/work-energy-power/skills')} style={{ padding: '14px 32px', '--skill-color': 'var(--rn-indigo)' }}>Practical Skills →</button>
                                </div>
                            </div>
                        )}
                    </div>
                ...
                <div style={{ marginTop: 40, textAlign: 'center' }}>
                    <button className="wep-btn-filled" onClick={() => navigate('/senior/grade/11/physics/work-energy-power/skills')} style={{ padding: '12px 32px', fontSize: 15, '--skill-color': 'var(--rn-indigo)' }}>
