...
import { SKILLS } from './WorkEnergyPowerSkillsData';
import '../../WorkEnergyPowerBranch.css';
import MathRenderer from '../../../../../../MathRenderer';
import { useSessionLogger } from '@/hooks/useSessionLogger';
import { NODE_IDS } from '@/lib/curriculumIds';

// ── Canonical node ID map (skill.id → UUID) ──────────────────────────────
const SKILL_NODE_ID_MAP = {
    'work-calculation':       NODE_IDS.g11PhysWEPWorkCalc,
    'work-energy-theorem':    NODE_IDS.g11PhysWEPWorkEnergyTheorem,
    'potential-energy':       NODE_IDS.g11PhysWEPPotentialEnergy,
    'conservation-of-energy': NODE_IDS.g11PhysWEPConservation,
};

export default function WorkEnergyPowerSkills() {
            ...
            <div className="wep-page">
                <nav className="wep-nav">
                    ...
                    <div className="wep-nav-links">
                        ...
                        <button className="wep-nav-link active" onClick={() => setView('list')}>🎯 Skills</button>
                ...
                <div className="wep-hero">
                    <h1 className="wep-hero-title">Core <span style={{ color: '#818cf8' }}>Skills</span></h1>
                    <p className="wep-hero-sub">Practice makes perfect. Choose a skill to start your journey.</p>
                </div>

                <div className="wep-skills-grid">
                    {SKILLS.map(s => (
                        <div key={s.id} className="wep-skill-card" style={{ '--skill-color': s.color }}>
                            <div className="wep-skill-icon" style={{ background: `${s.color}15`, color: s.color }}>
                                {s.icon}
                            </div>
                            <div className="wep-skill-info">
                                <h3 className="wep-skill-title">{s.title}</h3>
                                <p className="wep-skill-desc"><MathRenderer text={s.desc} /></p>
                            </div>
                            <div className="wep-skill-actions">
                                <button className="wep-btn wep-btn-outline" onClick={() => { setActiveSkillId(s.id); setView('learn'); }}>Learn</button>
                                <button className="wep-btn wep-btn-outline" onClick={() => { setActiveSkillId(s.id); setView('practice'); }}>Practice</button>
                                <button className="wep-btn wep-btn-filled" style={{ '--skill-color': s.color }} onClick={() => { setActiveSkillId(s.id); setView('assess'); }}>Assess</button>
                            </div>
                        </div>
                    ))}
                </div>
    ...
    return (
        <div className="wep-page">
            <nav className="wep-nav">
                <button className="wep-nav-back" onClick={() => {
                ...
                <div className="wep-nav-links">
                    <button className={`wep-nav-link ${view === 'learn' ? 'active' : ''}`} onClick={() => setView('learn')}>📖 Learn</button>
                    <button className={`wep-nav-link ${view === 'practice' ? 'active' : ''}`} onClick={() => setView('practice')}>🎯 Practice</button>
                    <button className={`wep-nav-link ${view === 'assess' ? 'active' : ''}`} onClick={() => setView('assess')}>🏆 Assess</button>
                </div>
            </nav>
            ...
            <main style={{ maxWidth: '1000px', margin: '40px auto', padding: '0 24px' }}>
                ...
                {view === 'practice' && <PracticeView skill={skill} onAssess={() => setView('assess')} nodeId={activeNodeId} />}
                {view === 'assess' && <AssessView skill={skill} onComplete={() => setView('list')} nodeId={activeNodeId} />}
            </main>
        </div>
    );
}
...
function LearnView({ skill, onPractice, mobileLearnDetailView, setMobileLearnDetailView, selectedCategory, setSelectedCategory }) {
    ...
    return (
        <>
            {/* Desktop View */}
            <div className="wep-details-window skills-desktop-only">
                ...
                <div style={{ fontSize: 18, color: 'var(--wep-text)', lineHeight: 1.7, marginBottom: 32, whiteSpace: 'pre-line' }}>
                    <MathRenderer text={skill.learn.content} />
                </div>

                <div className="wep-responsive-grid-2">
                    <div>
                        ...
                        {skill.learn.rules.map((rule, idx) => (
                            <div key={idx} style={{ display: 'flex', gap: 12, marginBottom: 16 }}>
                                ...
                                <p style={{ margin: 0, fontSize: 15, color: 'var(--wep-muted)' }}><MathRenderer text={rule} /></p>
                            </div>
                        ))}
                    </div>
                    <div>
                        <h4 style={{ textTransform: 'uppercase', fontSize: 12, letterSpacing: 2, color: 'var(--wep-teal)', marginBottom: 16, fontWeight: 800 }}>Quick Examples</h4>
                        {skill.learn.examples.map((ex, idx) => (
                            <div key={idx} style={{ background: '#f8fafc', padding: 16, borderRadius: 14, border: '1px solid #e2e8f0', marginBottom: 16 }}>
                                <div style={{ color: 'var(--wep-indigo)', fontWeight: 800, fontSize: 14, marginBottom: 4 }}>Q: <MathRenderer text={ex.q} /></div>
                                <div style={{ color: 'var(--wep-text)', fontSize: 15 }}>A: <MathRenderer text={ex.a} /></div>
                            </div>
                        ))}
                    </div>
                </div>
                ...
                <div style={{ marginTop: 40, textAlign: 'center' }}>
                    <button className="wep-btn-filled" style={{ '--skill-color': skill.color, padding: '14px 40px' }} onClick={onPractice}>
...
/* ── Practice View Component (Probability Chapter Style) ── */
function PracticeView({ skill, onAssess, nodeId }) {
    ...
    const handleSelect = async (optIdx) => {
        ...
        setResponses(prev => ({ ...prev, [current]: { selected: optIdx, isCorrect: correct } }));
        ...
        return (
            <div className="practice-report-card" style={{
                ...
                <div style={{ display: 'flex', gap: '12px', justifyContent: 'center' }}>
                    <button className="wep-btn-outline" style={{ padding: '10px 24px', borderRadius: '12px', fontSize: '14px' }} onClick={resetPractice}>Try Again</button>
                    <button className="wep-btn-filled" style={{ '--skill-color': color, padding: '10px 24px', borderRadius: '12px', fontSize: '14px' }} onClick={onAssess}>Take Assessment🏆</button>
                </div>
            </div>
        );
    }
    ...
    return (
        <div className="quiz-engine-practice" style={{ maxWidth: '900px', margin: '0 auto', padding: '10px 0' }}>
            <div className="wep-assess-main" style={{
                ...
                <div className="wep-responsive-grid-2" style={{ marginBottom: '20px' }}>
                    {q.options.map((opt, i) => {
...
/* ── Assess View Component ────────────────────────────── */
/* ── Assess View Component (Probability Chapter Style) ── */
function AssessView({ skill, onComplete, nodeId }) {
    ...
    const handleSelect = async (optIdx) => {
        ...
        setSelected(optIdx);
        const correct = optIdx === q.correct;
        setResponses(prev => ({ ...prev, [qIdx]: { selected: optIdx, isCorrect: correct } }));
    ...
    const handleNext = async () => {
        if (qIdx + 1 < questions.length) {
            ...
            if (nodeId) {
                ...
                await finishSession({ totalQuestions: questions.length, questionsAnswered: fPayload.length, answersPayload: fPayload });
            }
        }
    };
        ...
        return (
            <div className="exam-report-container" style={{ maxWidth: '1000px', margin: '0 auto', padding: '20px' }}>
                    ...
                    {questions.map((question, idx) => {
                        ...
                        return (
                            <div key={idx} style={{
                                ...
                                <div className="wep-responsive-grid-2" style={{ marginBottom: '20px', marginLeft: '40px' }}>
                ...
                <div style={{ textAlign: 'center', marginTop: '40px', display: 'flex', gap: '16px', justifyContent: 'center' }}>
                    <button className="wep-btn-outline" onClick={() => window.location.reload()} style={{ padding: '12px 30px' }}>Retake Test</button>
                    <button className="wep-btn-filled" onClick={onComplete} style={{ '--skill-color': skill.color, padding: '12px 40px' }}>Return to Skills</button>
                </div>
            </div>
        );
    }
    ...
    return (
        <div className="quiz-engine-exam" style={{ maxWidth: '1300px', margin: '0 auto', padding: '10px 0' }}>
            ...
            {/* Layout */}
            <div className="wep-assess-layout">
                {/* Left Side */}
                <div className="wep-assess-main">
                    <div style={{
                        ...
                        <div className="wep-responsive-grid-2" style={{ marginBottom: '16px' }}>
                            {q.options.map((opt, i) => {
                                const isSelected = selected === i;
                                let borderColor = '#f1f5f9';
                                let bgColor = '#fff';
                ...
                {/* Right Side Palette */}
                <div className="wep-assess-sidebar">
                    <div className="wep-palette-window" style={{
