...
const CellStructureTest = () => {
    ...
    // v4 Session logging (runs alongside legacy api calls during migration)
    const { startSession: v4Start, logAnswer: v4Log, finishSession: v4Finish } = useSessionLogger();
    ...
    const recordAttempt = async (q, sel, cor) => {
        ...
        api.recordAttempt({ user_id: parseInt(uid), session_id: sessionId, skill_id: SKILL_ID, template_id: null, difficulty_level: q.difficulty_level || 'Medium', question_text: q.text, correct_answer: q.correctAnswer, student_answer: sel || '', is_correct: cor, solution_text: q.solution || '', time_spent_seconds: sec }).catch(console.error);

        // v4 Log answer alongside legacy
        const answerData = { question_index: qIndex + 1, answer_json: { selected: sel }, is_correct: cor ? 1.0 : 0.0, marks_awarded: cor ? 1 : 0, marks_possible: 1, time_taken_ms: sec * 1000 };
        v4AnswersPayload.current.push(answerData);
        await v4Log({ questionIndex: answerData.question_index, answerJson: answerData.answer_json, isCorrect: answerData.is_correct });
    };
    ...
    const handleFinalSubmit = async () => {
        // Legacy: finish old session + create report
        if (sessionId) await api.finishSession(sessionId).catch(console.error);
        const uid = sessionStorage.getItem('userId') || localStorage.getItem('userId');
        if (uid) { const tc = Object.values(answers).filter(v => v.isCorrect).length; await api.createReport({ title: SKILL_NAME, type: 'practice', score: (tc / questions.length) * 100, parameters: { skill_id: SKILL_ID, skill_name: SKILL_NAME, total_questions: questions.length, correct_answers: tc, timestamp: new Date().toISOString(), time_taken_seconds: timeElapsed }, user_id: parseInt(uid) }).catch(console.error) }

        // v4: finish parallel session
        v4IsFinishedRef.current = true;
        await v4Finish({ totalQuestions: questions.length, questionsAnswered: v4AnswersPayload.current.length, answersPayload: v4AnswersPayload.current });

        setIsFinished(true);
    };
    const handleExit = async () => {
        if (!isFinished && !v4IsFinishedRef.current) {
            v4IsFinishedRef.current = true;
            await v4Finish({ totalQuestions: questions.length, questionsAnswered: v4AnswersPayload.current.length, answersPayload: v4AnswersPayload.current, status: 'abandoned' });
        }
        navigate(-1);
    };
