...
import { TERMS, FIVE_RULES, VOCAB_QUIZ } from './ComplexTerminologyData';
import { useSessionLogger } from '@/hooks/useSessionLogger';
import { NODE_IDS } from '@/lib/curriculumIds';

const TERMINOLOGY_NODE_ID = NODE_IDS.g11MathCNTerminologyQuiz;
const BASE = '/senior/grade/11/maths/complex-numbers';

export default function ComplexTerminology() {
        ...
        <div className="sets-page">
            <nav className="sets-nav">
                ...
                <div className="sets-nav-links">
                    ...
                    <button className="sets-nav-link active" onClick={() => navigate(`${BASE}/terminology`)}>📖 Terminology</button>
            ...
            <div className="sets-lexicon-container">
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', marginBottom: 40 }}>
                    <h1 style={{ fontFamily: 'Outfit, sans-serif', fontSize: 'clamp(2.2rem, 8vw, 3.5rem)', fontWeight: 900, color: 'var(--sets-text)', margin: '0 0 12px', letterSpacing: '-0.02em' }}>
                        Complex Numbers <span style={{ background: 'linear-gradient(135deg, #6366f1, #7c3aed)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>Lexicon</span>
                    </h1>
                    <p style={{ fontSize: 18, fontWeight: 500, color: 'var(--sets-muted)', maxWidth: '650px', lineHeight: 1.6 }}>
                        {activeTab === 'quiz' ? 'Test your vocabulary and rules knowledge!' : `Explore the foundations with ${activeTab === 'terms' ? '7 key terms' : '5 golden rules'}.`}
                    </p>
                </div>
                        ...
                        {!quizFinished ? (
                            ...
                            <div style={{ textAlign: 'center', padding: '24px 0' }}>
                                ...
                                <div className="sets-finish-actions" style={{ display: 'flex', gap: 16, justifyContent: 'center' }}>
                                    ...
                                    <button className="sets-btn-filled" onClick={() => navigate(`${BASE}/skills`)} style={{ padding: '14px 32px', '--skill-color': 'var(--sets-indigo)' }}>Practical Skills →</button>
                                </div>
                            </div>
                        )}
                    </div>
                ...
                <div style={{ marginTop: 40, textAlign: 'center' }}>
                    <button className="sets-btn-filled" onClick={() => navigate(`${BASE}/skills`)} style={{ padding: '12px 32px', fontSize: 15, '--skill-color': 'var(--sets-indigo)' }}>
