...
export default function LAScenarioAssessmentEngine({ scenarios = [], title, color = "#0f4c81", onBack, nodeId }) {
    const TOTAL_QUESTIONS = 20;
    const TIME_LIMIT_SEC = 20 * 60; // 20 minutes

    const [questions, setQuestions] = useState([]);
    const [currentStep, setCurrentStep] = useState(0);
    // Answers: store the user's answer (angle or selected opt index)
    ...
    if (finished && questions.length > 0) {
        questions.forEach((q, i) => {
            ...
            if (userAns !== null) {
                if (q.type === 'mcq' && userAns === q.ans) {
                    finalScore++;
                    scorePerStep[i] = 1;
                } else if (q.type === 'protractor' && Math.abs(userAns - q.ans) <= 2) {
                    finalScore++;
                    scorePerStep[i] = 1;
                }
            }
                                    ...
                                    <div style={{ flex: 1, fontFamily: 'Open Sans, sans-serif' }}>
                                        ...
                                        {q.type === 'mcq' ? (
                                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginTop: 12 }}>
                                             ...
                                             <div style={{ background: 'rgba(0,0,0,0.03)', padding: 12, borderRadius: 8, fontSize: 14 }}>
                                                <div style={{ marginBottom: 8 }}>
                                                    <strong>Your Answer: </strong>
                                                    {userAns === null ? 'None' : `${userAns}°`}
                                                </div>
                                                {!isCorrect && (
                    ...
                    <div style={{ maxWidth: 700, margin: '0 auto' }}>
                        ...
                        {cq.svg && (
                            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', marginBottom: 32 }}>
                                <LAGraphMini 
                                    config={cq.svg} 
                                    onProtractorChange={(val) => {
                                        if (cq.type === 'protractor') handleAnswerUpdate(val);
                                    }} 
                                />
                                {cq.type === 'protractor' && (
                                    <div style={{ marginTop: 12, textAlign: 'center' }}>
                                        <div style={{ fontSize: 18, fontWeight: 900, color: '#0f172a' }}>Your Angle: <span style={{ color }}>{answers[currentStep] || 0}°</span></div>
