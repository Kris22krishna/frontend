...
export default function StatisticsScenarioAssessmentEngine({ scenarios = [], title, color = "#0f4c81", onBack, nodeId }) {
    const TOTAL_QUESTIONS = 20;
    const TIME_LIMIT_SEC = 20 * 60; // 20 minutes

    const [questions, setQuestions] = useState([]);
    const [currentStep, setCurrentStep] = useState(0);
    // Answers: store the user's answer (selected opt index)
    ...
    // Calculate score
    let finalScore = 0;
    let scorePerStep = Array(TOTAL_QUESTIONS).fill(0); // 1 = correct, 0 = wrong
    if (finished && questions.length > 0) {
        questions.forEach((q, i) => {
            const userAns = answers[i];
            if (userAns !== null && userAns === q.ans) {
                finalScore++;
                scorePerStep[i] = 1;
            }
        });
            ...
            <div style={{ maxWidth: 860, margin: '0 auto', padding: '20px 0', fontFamily: 'Outfit, sans-serif' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 16, marginBottom: 32 }}>
                    <div style={{ background: '#fff', borderRadius: 16, padding: '20px', textAlign: 'center', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.05)', border: '1px solid #e2e8f0' }}>
                        ...
                        <div style={{ fontSize: 36, fontWeight: 900, color: '#0f4c81' }}>{finalScore}<span style={{ fontSize: 20, color: '#94a3b8' }}>/{TOTAL_QUESTIONS}</span></div>
                    </div>
                    <div style={{ background: '#fff', borderRadius: 16, padding: '20px', textAlign: 'center', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.05)', border: '1px solid #e2e8f0' }}>
                        <div style={{ fontSize: 13, color: '#64748b', fontWeight: 700, textTransform: 'uppercase', letterSpacing: 1, marginBottom: 8 }}>Accuracy</div>
                        <div style={{ fontSize: 36, fontWeight: 900, color: pct >= 70 ? '#0f4c81' : pct >= 40 ? '#d97706' : '#ef4444' }}>{pct}%</div>
                    </div>
                    <div style={{ background: '#fff', borderRadius: 16, padding: '20px', textAlign: 'center', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.05)', border: '1px solid #e2e8f0' }}>
                        ...
                        return (
                            <div key={i} style={{ padding: '20px', borderBottom: i < TOTAL_QUESTIONS - 1 ? '1px solid #e2e8f0' : 'none', background: isCorrect ? '#f8fafc' : '#fff' }}>
                                <div style={{ display: 'flex', gap: 16 }}>
                                    <div style={{ width: 40, height: 40, borderRadius: '50%', background: isCorrect ? '#0f4c81' : userAns === null ? '#f59e0b' : '#ef4444', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16, fontWeight: 800, flexShrink: 0 }}>
                                        {i + 1}
                                    </div>
                                    <div style={{ flex: 1, fontFamily: 'Open Sans, sans-serif' }}>
                                        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
                                            ...
                                            <div style={{ display: 'inline-flex', alignItems: 'center', gap: 4, fontSize: 12, fontWeight: 800, color: isCorrect ? '#0f4c81' : userAns === null ? '#b45309' : '#dc2626', background: isCorrect ? 'rgba(15, 76, 129, 0.1)' : userAns === null ? '#fffbeb' : '#fef2f2', padding: '4px 10px', borderRadius: 20, whiteSpace: 'nowrap', alignSelf: 'flex-start' }}>
                                                {isCorrect ? '✅ Correct' : userAns === null ? '⏭️ Skipped' : '❌ Incorrect'}
                                            </div>
                                        </div>
                                        {q.svg && (
                                            <div style={{ marginBottom: 12 }}>
                                                <StatisticsGraphMini config={q.svg} />
                                            </div>
                                        )}
                                        ...
                                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginTop: 12 }}>
                                            {q.opts.map((opt, oi) => {
                                                ...
                                                if (isActualAns) { border = '#0f4c81'; bg = 'rgba(15, 76, 129, 0.1)'; }
                                                ...
                                                return (
                                                    <div key={oi} style={{ padding: '10px 14px', borderRadius: 8, border: `2px solid ${border}`, background: bg, fontSize: 14, display: 'flex', alignItems: 'center', gap: 8 }}>
                                                        <div style={{ width: 16, height: 16, borderRadius: '50%', background: isActualAns ? '#0f4c81' : (isUserAns && !isCorrect) ? '#ef4444' : '#e2e8f0', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, color: '#fff' }}>
                ...
                <div style={{ flex: 1, overflowY: 'auto', padding: '32px', fontFamily: 'Open Sans, sans-serif' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
                        <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: '#f1f5f9', padding: '6px 14px', borderRadius: 50, fontSize: 13, fontWeight: 800, color: '#475569' }}>
                            <span style={{ color: color }}>● {cq.type.toUpperCase()}</span>
                        </div>
