...
import CirclesGraphMini from './CirclesGraphMini';

export default function CirclesScenarioAssessmentEngine({ scenarios = [], title, color = "#0f4c81", onBack, nodeId }) {
    const TOTAL_QUESTIONS = 20;
    const TIME_LIMIT_SEC = 20 * 60; // 20 minutes

    const [questions, setQuestions] = useState([]);
    const [currentStep, setCurrentStep] = useState(0);
    // Answers: store the user's answer
                ...
                <div style={{ flex: 1, overflowY: 'auto', padding: '32px', fontFamily: 'Open Sans, sans-serif' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
                        <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: '#f1f5f9', padding: '6px 14px', borderRadius: 50, fontSize: 13, fontWeight: 800, color: '#475569' }}>
                            <span style={{ color: color }}>● Multiple Choice</span>
                        </div>
                    ...
                    <div style={{ maxWidth: 700, margin: '0 auto' }}>
                        ...
                        {cq.svg && (
                            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', marginBottom: 32 }}>
                                <CirclesGraphMini config={cq.svg} />
                            </div>
                        )}
