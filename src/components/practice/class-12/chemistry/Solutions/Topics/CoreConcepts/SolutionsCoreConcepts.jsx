...
import { CORE_CONCEPTS } from './SolutionsCoreConceptsData';
import styles from '../../SolutionsDashboard.module.css';
import MathRenderer from '../../../../../../MathRenderer';

import QuizEngine from '../../../../../../science/grade10/ChemicalReactions/Engines/ChemQuizEngine';
import AssessmentEngine from '../../../../../../science/grade10/ChemicalReactions/Engines/ChemAssessmentEngine';
import '../../../../../../science/grade10/ChemicalReactions/Engines/ChemTestLayout.css';
...
const verifyQuestion = (question) => {
    if (!question?.question || !Array.isArray(question.options)) return null;

    const rawOptions = question.options.map((option) => String(option));
    const correctFromAnswer = question.answer ?? rawOptions[question.correct];
    const correctKey = normalizeAnswerText(correctFromAnswer);
    if (!correctKey) return null;

    const correctOption = rawOptions.find((option) => normalizeAnswerText(option) === correctKey);
    if (!correctOption) return null;

    const seen = new Set([normalizeAnswerText(correctOption)]);
    const distractors = rawOptions.filter((option) => {
        const key = normalizeAnswerText(option);
...
const shuffleQuestionOptions = (question) => {
    const verified = verifyQuestion(question);
    if (!verified) return null;

    const shuffledOptions = shuffleArray(verified.options);
    const correct = shuffledOptions.findIndex(
        (option) => normalizeAnswerText(option) === normalizeAnswerText(verified.answer)
    );
...
const buildQuestionSession = (questionPool, count) =>
...
/* ── Main Component ─────────────────────────────────────────────── */
export default function SolutionsCoreConcepts() {
    ...
    const NAV = (
        <nav className={styles['sol-nav']}>
            ...
            <div className={styles['sol-nav-links']}>
                ...
                <button className={`${styles['sol-nav-link']} ${styles['active']}`} onClick={() => { setView('list'); setActiveSkillIdx(null); }}>🎯 Core Concepts</button>
        ...
        return (
            <div className={styles['sol-page']}>
                ...
                <div className={styles['sol-hero']}>
                    <h1 className={styles['sol-hero-title']}>Core <span style={{ color: '#f59e0b' }}>Concepts</span></h1>
                    <p className={styles['sol-hero-sub']}>Master the central pillars of Solutions — the physical chemistry foundations.</p>
                </div>
                ...
                <div style={{ maxWidth: 960, margin: '40px auto', padding: '0 24px 60px' }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
                        {CORE_CONCEPTS.map((concept, idx) => (
                            <div key={concept.id}
                                className={styles['sol-concept-card']}
                                style={{ '--skill-color': concept.color, '--skill-color-15': concept.color + '15', '--skill-color-40': concept.color + '40' }}>
                                
                                <div className={styles['sol-concept-header']}>
                                    <div className={styles['sol-concept-icon']}>{concept.icon}</div>

                                    <div className={styles['sol-concept-info']}>
                                ...
                                <div className={styles['sol-concept-actions']}>
                                    <button onClick={() => openSkill(idx, 'learn')} className={styles['sol-concept-btn']}>
                                        Learn
                                    </button>
                                    <button onClick={() => openSkill(idx, 'practice')} className={styles['sol-concept-btn']}>
                                        Practice
                                    </button>
                                    <button onClick={() => openSkill(idx, 'assess')} className={`${styles['sol-concept-btn']} ${styles['assess']}`}>
                                        Assess
                                    </button>
                                </div>
                            </div>
                    ...
                    <div style={{ marginTop: 60, padding: 36, borderRadius: 24, background: 'linear-gradient(135deg, #f59e0b, #ef4444)', color: '#fff', textAlign: 'center', boxShadow: '0 15px 40px rgba(245,158,11,0.25)' }}>
                        ...
                        <p style={{ opacity: 0.9, maxWidth: 460, margin: '0 auto 24px', fontSize: 15 }}>
                            Head to the Virtual Lab to see these reactions in action with live animations. (Coming Soon)
                        </p>
                    </div>
                </div>
            </div>
        ...
        return (
            <div style={{ background: '#f8fafc', minHeight: '100vh', overflowY: 'auto' }}>
                ...
                <div className="chemtest-practice-shell" style={{ maxWidth: 860, margin: '0 auto', padding: '24px 16px 80px' }}>
                    <QuizEngine
                        questions={() => buildQuestionSession(skill.practice, PRACTICE_QUESTION_COUNT)}
