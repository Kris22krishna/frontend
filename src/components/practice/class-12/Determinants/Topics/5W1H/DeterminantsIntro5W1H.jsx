...
function W1HCard({ card }) {
  ...
  return (
    <div
      className={`det-intro-card${open ? " det-intro-card--open" : ""}`}
      onClick={() => setOpen((o) => !o)}
      style={{
    ...
    >
      <div
        className="det-intro-card-strip"
        style={{
          background: `linear-gradient(90deg, ${card.gradFrom}, ${card.gradTo})`,
        }}
      />
      <div className="det-intro-card-header">
        <div
          className="det-intro-card-icon"
          style={{
        ...
        <div className="det-intro-card-title-block">
          <div className="det-intro-card-q" style={{ color: card.gradFrom }}>
            {card.q}
          </div>
          <div className="det-intro-card-sublabel">{card.label}</div>
        </div>
        <div
          className="det-intro-card-chevron"
          style={{
      ...
      {!open && <div className="det-intro-card-hint">Tap to explore →</div>}
      {open && (
        <div className="det-intro-card-body">
          <div className="det-intro-card-content">
            <MathRenderer text={card.content} />
          </div>
          <div
            className="det-intro-card-fact"
            style={{
...
export default function DeterminantsIntro5W1H() {
  ...
  return (
    <div className="det-intro-page">
      ...
      <div className="det-intro-content" style={{ padding: "10px 24px 20px" }}>
        <div className="det-intro-grid">
          {cards5W1H.map((card, idx) => (
            <W1HCard key={idx} card={card} />
          ))}
        </div>
        <div className="det-intro-cta-strip">
          <p className="det-intro-cta-sub" style={{ margin: 0 }}>
            Up next: 8 key terms &amp; 6 golden rules
          </p>
          <button
            className="det-intro-cta-btn"
            onClick={() =>
              navigate("/senior/grade/12/determinants/terminology")
            }
          >
