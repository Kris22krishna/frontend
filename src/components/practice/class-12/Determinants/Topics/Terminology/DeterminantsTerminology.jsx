...
const TERMS = [
    {
        ...
        examples: ['$|A|$', '$\\det(A)$', '$\\begin{vmatrix} 1 & 2 \\\\ 3 & 4 \\end{vmatrix} = -2$'],
        inUse: '$\\begin{vmatrix} 1 & 2 \\\\ 3 & 4 \\end{vmatrix} = 1 \\times 4 - 2 \\times 3 = -2$.',
    ...
    },
    {
        ...
        examples: ['$|A| = 0$', '$\\begin{bmatrix} 2 & 4 \\\\ 1 & 2 \\end{bmatrix}$'],
        inUse: '$\\begin{vmatrix} 2 & 4 \\\\ 1 & 2 \\end{vmatrix} = 4 - 4 = 0$, so the matrix is singular.',
    ...
    },
    {
        ...
        inUse: '$\\begin{vmatrix} 3 & 1 \\\\ 2 & 4 \\end{vmatrix} = 12 - 2 = 10 \\neq 0$, so the matrix is non-singular.',
    ...
    },
    {
        ...
        inUse: 'In $\\begin{bmatrix} 1 & 2 & 3 \\\\ 4 & 5 & 6 \\\\ 7 & 8 & 9 \\end{bmatrix}$, $M_{11} = \\begin{vmatrix} 5 & 6 \\\\ 8 & 9 \\end{vmatrix} = 45 - 48 = -3$.',
        memory: 'Delete the row and column of the element, then find the determinant of what remains!'
    },
    {
        name: 'Cofactor',
        color: '#f59e0b',
        ...
        examples: ['$A_{11} = +M_{11}$', '$A_{12} = -M_{12}$', '$A_{21} = -M_{21}$'],
        inUse: 'Sign pattern for $3 \\times 3$: $\\begin{bmatrix} + & - & + \\\\ - & + & - \\\\ + & - & + \\end{bmatrix}$.',
    ...
    },
    {
        ...
        examples: ['$\\text{adj}(A) = C^T$', '$A \\cdot \\text{adj}(A) = |A| \\cdot I$'],
        inUse: 'For $A = \\begin{bmatrix} 1 & 2 \\\\ 3 & 4 \\end{bmatrix}$, $\\text{adj}(A) = \\begin{bmatrix} 4 & -2 \\\\ -3 & 1 \\end{bmatrix}$.',
    ...
    },
    {
        ...
        examples: ['Unique solution: $|A| \\neq 0$', 'Infinite solutions: $|A| = 0$ but $(\\text{adj } A) \\cdot B = O$'],
    ...
    },
    {
        ...
        memory: 'Inconsistent = no solution exists. The equations contradict each other!'
    },
];
...
const SIX_RULES = [
    {
        ...
        emoji: '↔️',
        color: '#6366f1',
        detail: 'Choose the row or column with the most zeros for easiest computation. Expand: $|A| = \\sum_{j=1}^{n} a_{ij} A_{ij}$ along row $i$.',
        examples: ['Expand along $R_1$: $|A| = a_{11}A_{11} + a_{12}A_{12} + a_{13}A_{13}$', 'Choose the row/column with more zeros!'],
    ...
    },
    {
        ...
        emoji: '✖️',
        color: '#0891b2',
        detail: 'For an $n \\times n$ matrix: $|kA| = k^n |A|$. One row × $k$ means $|A_{\\text{new}}| = k|A|$, but scaling the full matrix raises $k$ to the $n$th power.',
        examples: ['$|kA| = k^n|A|$ for $n \\times n$', 'One row scaled: $|A| \\to k|A|$'],
    ...
    },
    {
        ...
        emoji: '🔗',
        color: '#f59e0b',
        detail: 'This beautiful property makes computations much simpler. Instead of multiplying matrices first, you can find individual determinants and multiply them.',
    ...
    },
    {
        ...
        emoji: '=',
        color: '#10b981',
        detail: 'This is because swapping identical rows changes the sign but not the matrix, so $|A| = -|A|$, which means $|A| = 0$.',
        examples: ['$\\begin{vmatrix} 1 & 2 \\\\ 1 & 2 \\end{vmatrix} = 0$', 'Two equal rows → $|A| = 0$'],
    ...
    },
    {
        ...
        emoji: '🔄',
        color: '#ec4899',
        detail: '$R_i \\to R_i + kR_j$ does not change $|A|$. But swapping two rows changes the sign: $R_i \\leftrightarrow R_j$ gives $-|A|$.',
        examples: ['$R_1 \\to R_1 + 3R_2$: determinant unchanged', '$R_1 \\leftrightarrow R_2$: determinant changes sign'],
    ...
    },
    {
        ...
        emoji: '🔓',
        color: '#7c3aed',
        detail: 'The inverse formula is $A^{-1} = \\frac{1}{|A|} \\text{adj}(A)$. Division by zero is impossible, hence $|A| = 0$ means no inverse.',
        examples: ['$|A| = 5 \\Rightarrow A^{-1}$ exists', '$|A| = 0 \\Rightarrow$ singular (no inverse)'],
...
const VOCAB_QUIZ = [
    {
        question: "What is the determinant of a $2 \\times 2$ matrix $\\begin{bmatrix} a & b \\\\ c & d \\end{bmatrix}$?",
        options: ["$ab - cd$", "$ad - bc$", "$ac - bd$", "$a + d - b - c$"],
        correct: 1,
        explanation: "For a $2 \\times 2$ matrix, $|A| = ad - bc$ (main diagonal minus cross diagonal)."
    },
    {
    ...
    },
    {
        ...
        options: ["$(-1)^{i \\cdot j}$", "$(-1)^{i+j}$", "$(-1)^{i-j}$", "Always positive"],
        correct: 1,
        explanation: "Cofactor sign is $(-1)^{i+j}$, creating a checkerboard pattern."
    },
    {
    ...
    },
    {
        ...
        correct: 2,
        explanation: "$|kA| = k^n|A|$ where $n$ is the order. For $3 \\times 3$, $|kA| = k^3|A|$."
    },
    {
        question: "A consistent system of equations has:",
        options: ["No solution", "At least one solution", "Exactly two solutions", "Infinite solutions only"],
        correct: 1,
        explanation: "Consistent means at least one solution exists (could be unique or infinite)."
    },
    {
        question: "The formula for inverse using adjoint is:",
        options: ["$A^{-1} = |A| \\cdot \\text{adj}(A)$", "$A^{-1} = \\frac{1}{|A|} \\text{adj}(A)$", "$A^{-1} = \\text{adj}(A)$", "$A^{-1} = \\frac{\\text{adj}(A)}{A}$"],
        correct: 1,
        explanation: "$A^{-1} = \\frac{1}{|A|} \\text{adj}(A)$, provided $|A| \\neq 0$."
    },
    {
        question: "Swapping two rows of a determinant:",
        options: ["Doubles it", "Does not change it", "Changes its sign", "Makes it zero"],
        correct: 2,
    ...
    return (
        <div className="terminology-page">
            <style>{`
                ...
                @media (max-width: 1024px) { .alg-lexicon-grid { grid-template-columns: 1fr !important; } .selector-container { max-width: 600px; margin: 0 auto 16px; } }
            `}</style>
            ...
            <nav className="intro-nav">
                ...
                <div className="intro-nav-links">
                    ...
                    <button className="intro-nav-link intro-nav-link--active" onClick={() => navigate('/senior/grade/12/determinants/terminology')}>📖 Terminology</button>
            ...
            <div className="alg-lexicon-container" style={{ maxWidth: 1100, margin: '40px auto 20px', padding: '0 24px' }}>
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', marginBottom: 20 }}>
                    <h1 style={{ fontFamily: 'Outfit, sans-serif', fontSize: '2.8rem', fontWeight: 900, color: 'var(--alg-text)', margin: '0 0 8px' }}>
                        Determinants <span style={{ background: 'linear-gradient(135deg, var(--alg-teal), var(--alg-indigo))', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>Vocabulary</span>
                    </h1>
                ...
                {activeTab !== 'quiz' ? (
                    <div className="alg-lexicon-grid" style={{ display: 'grid', gridTemplateColumns: 'minmax(300px, 360px) 1fr', gap: 16, alignItems: 'start' }}>
                        ...
                        <main className="details-window-anim" key={activeTab === 'terms' ? selectedIdx : selectedRuleIdx} style={{ background: '#ffffff', borderRadius: 20, padding: '20px 28px', boxShadow: '0 8px 24px rgba(0,0,0,0.03)', border: `2px solid ${(activeTab === 'terms' ? activeTerm : activeRule).color}15`, minHeight: 330 }}>
                            {activeTab === 'terms' ? (
                                <>
                                ...
                                <>
                                    ...
                                    <p style={{ fontSize: 17, color: 'var(--alg-text)', lineHeight: 1.6, margin: '0 0 24px' }}><MathRenderer text={activeRule.detail} /></p>
                                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 20 }}>
                                        <div>
                        ...
                        {!quizFinished ? (
                            <>
                                ...
                                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 28 }}>
                                    {activeQuiz.options.map((opt, oi) => {
                                        let bCol = 'rgba(0,0,0,0.05)'; let bgCol = '#fff'; let txtCol = 'var(--alg-text)';
                                        if (quizAnswered) {
                            ...
                            <div style={{ textAlign: 'center', padding: '20px 0' }}>
                                ...
                                <div style={{ display: 'flex', gap: 12, justifyContent: 'center' }}>
                                    ...
                                    <button className="alg-btn-secondary" onClick={() => navigate('/senior/grade/12/determinants/skills')}>Go to Skills 🎯</button>
                                </div>
                            </div>
                        )}
                    </div>
                ...
                <div style={{ marginTop: 16, textAlign: 'center' }}>
                    <button className="alg-btn-primary" onClick={() => navigate('/senior/grade/12/determinants/skills')} style={{ padding: '10px 28px', fontSize: 13 }}>Ready to Solve! 🎯</button>
                </div>
            </div>
        </div>
    );
