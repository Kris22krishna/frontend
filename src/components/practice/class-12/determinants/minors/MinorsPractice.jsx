...
const SKILL_ID = 12308;
const SKILL_NAME = "Minors";
const TOTAL_QUESTIONS = 10;

const MinorsPractice = () => {
    ...
    const generateQuestion = (index) => {
        let q = { text: "<div class='question-container'><p>Sample question for Minors</p></div>", correctAnswer: "$1$", solution: "Sample solution", options: ["$1$", "$2$", "$3$", "$4$"] };
            ...
            <footer className="junior-bottom-bar">
                <div className="desktop-footer-controls">
                    ...
                    <div className="bottom-center" style={{display:'flex',gap:'8px',alignItems:'center'}}>
                        ...
                        {!isSubmitted ? (
                            <>
                                <button className="bg-indigo-50 text-indigo-600 px-4 py-2 rounded-xl border-2 border-indigo-100 font-bold hover:bg-indigo-100 transition-colors flex items-center gap-2 text-sm" onClick={()=>navigate('/senior/grade/12/determinants/minors/5w1h')}><BookOpen size={16}/> Learn 5W1H</button>
                                <button className="bg-amber-50 text-amber-600 px-4 py-2 rounded-xl border-2 border-amber-100 font-bold hover:bg-amber-100 transition-colors flex items-center gap-2 text-sm" onClick={()=>navigate('/senior/grade/12/determinants/minors/examples')}><Lightbulb size={16}/> Learn by Example</button>
                            </>
                        ) : (
                        ...
                        <div style={{position:'relative'}}>
                            ...
                            {showLearnMenu && (
                                <div style={{position:'absolute',bottom:'110%',left:0,background:'#fff',border:'2px solid #E2E8F0',borderRadius:'12px',boxShadow:'0 8px 24px rgba(0,0,0,0.12)',padding:'8px',zIndex:50,minWidth:'160px'}}>
                                    <button style={{display:'flex',alignItems:'center',gap:'8px',padding:'10px 14px',borderRadius:'8px',border:'none',background:'transparent',cursor:'pointer',width:'100%',fontWeight:600,fontSize:'0.8rem',color:'#4F46E5'}} onClick={()=>{setShowLearnMenu(false);navigate('/senior/grade/12/determinants/minors/5w1h');}}><BookOpen size={14}/> 5W1H</button>
                                    <button style={{display:'flex',alignItems:'center',gap:'8px',padding:'10px 14px',borderRadius:'8px',border:'none',background:'transparent',cursor:'pointer',width:'100%',fontWeight:600,fontSize:'0.8rem',color:'#D97706'}} onClick={()=>{setShowLearnMenu(false);navigate('/senior/grade/12/determinants/minors/examples');}}><Lightbulb size={14}/> Examples</button>
                                </div>
                            )}
                        </div>
                    </div>
