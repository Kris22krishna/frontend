...
const MatrixOrderTest = () => {
  ...
  useEffect(() => {
      ...
      while (unique.length < 4) {
        const candidate =
          typeof correct === "number"
            ? `${correct + fallback * 10}`
            : `Option ${String.fromCharCode(64 + fallback)}`;
    ...
    (() => {
      const r = rand(2, 4),
        c = rand(2, 5);
      const vals = Array.from({ length: r }, () =>
        Array.from({ length: c }, () => rand(-9, 9)),
      );
    ...
    (() => {
      ...
      const factors = [];
      for (let i = 1; i <= n; i++)
        if (n % i === 0) factors.push(`$${i} \\times ${n / i}$`);
    ...
    (() => {
      ...
      qs.push({
        ...
        matrix: matrix,
        options: makeOptions(isRow ? "Row matrix" : "Column matrix", [
          "Square matrix",
          "Zero matrix",
          "Diagonal matrix",
        ]),
  ...
  useEffect(() => {
    ...
    if (userId && !sessionId) {
      api
        .createPracticeSession(userId, SKILL_ID)
        .then((s) => {
  ...
  const handleCheck = () => {
    ...
    setAnswers((p) => ({
      ...p,
      [qIndex]: { selectedOption, isCorrect: right },
    }));
