...
const MatrixTransposeTest = () => {
  ...
  useEffect(() => {
    ...
    (() => {
      ...
      const c = `$\\begin{bmatrix}${AT[0][0]}&${AT[0][1]}\\\\${AT[1][0]}&${AT[1][1]}\\\\${AT[2][0]}&${AT[2][1]}\\end{bmatrix}$`;
      const w1 = `$\\begin{bmatrix}${A[0][0]}&${A[0][1]}&${A[0][2]}\\\\${A[1][0]}&${A[1][1]}&${A[1][2]}\\end{bmatrix}$`;
      qs.push({
        text: `Find $A'$ (transpose) of $A = \\begin{bmatrix}${A[0][0]}&${A[0][1]}&${A[0][2]}\\\\${A[1][0]}&${A[1][1]}&${A[1][2]}\\end{bmatrix}$.`,
        matrix: A,
        options: shuffle([
          c,
          w1,
          `$\\begin{bmatrix}${AT[2][0]}&${AT[2][1]}\\\\${AT[1][0]}&${AT[1][1]}\\\\${AT[0][0]}&${AT[0][1]}\\end{bmatrix}$`,
          `$\\begin{bmatrix}${AT[0][1]}&${AT[0][0]}\\\\${AT[1][1]}&${AT[1][0]}\\\\${AT[2][1]}&${AT[2][0]}\\end{bmatrix}$`,
        ]),
        correctAnswer: c,
        solution:
          "Transpose swaps rows and columns: $(A')_{ij} = a_{ji}$. A $2 \\times 3$ becomes $3 \\times 2$.",
    ...
    qs.push({
      ...
      correctAnswer: "$A$",
      solution: "Transposing twice returns the original matrix: $(A')'= A$.",
      difficulty_level: "Easy",
    });
    ...
    (() => {
      const r = rand(2, 4),
        c2 = rand(2, 5);
      qs.push({
        text: `If $A$ is $${r} \\times ${c2}$, what is the order of $A'$?`,
        options: shuffle([
          `$${c2} \\times ${r}$`,
          `$${r} \\times ${c2}$`,
          `$${r} \\times ${r}$`,
        ...
        correctAnswer: `$${c2} \\times ${r}$`,
        solution: `Transpose swaps dimensions: $m \\times n \\to n \\times m$. So $${r} \\times ${c2} \\to ${c2} \\times ${r}$.`,
        difficulty_level: "Easy",
      });
    })();
    ...
    qs.push({
      text: "$(A + B)'$ equals:",
      options: shuffle(["$A' + B'$", "$A'B'$", "$B'A'$", "$(AB)'$"]),
      correctAnswer: "$A' + B'$",
      solution: "Transpose distributes over addition: $(A+B)' = A' + B'$.",
    ...
    qs.push({
      text: "$(kA)'$ equals:",
      options: shuffle(["$kA'$", "$k'A$", "$kA$", "$A'k'$"]),
      correctAnswer: "$kA'$",
      solution: "Scalar can be taken out: $(kA)' = kA'$.",
      difficulty_level: "Easy",
    });
    ...
    qs.push({
      ...
      correctAnswer: "$B'A'$",
      solution: "The reversal law: $(AB)' = B'A'$.",
      difficulty_level: "Medium",
    });
    ...
    (() => {
      ...
      qs.push({
        text: "Is the matrix shown symmetric ($A = A'$)?",
        matrix: M,
        options: shuffle([
          "Yes, it is symmetric",
          "No, it is skew-symmetric",
          "No, it is neither",
          "Cannot determine",
        ]),
        correctAnswer: "Yes, it is symmetric",
        solution: `A matrix is symmetric if $a_{ij} = a_{ji}$. Here every element above the diagonal equals the element below: confirmed symmetric.`,
        difficulty_level: "Medium",
      });
    })();
    ...
    (() => {
      ...
      qs.push({
        ...
        solution:
          "A skew-symmetric matrix has $A' = -A$, which means $a_{ij} = -a_{ji}$ and diagonal entries are 0.",
        difficulty_level: "Medium",
      });
    })();
    ...
    // Q9: A + A' is always
    qs.push({
      text: "For any square matrix $A$, $A + A'$ is always:",
      options: shuffle(["Symmetric", "Skew-symmetric", "Diagonal", "Zero"]),
      correctAnswer: "Symmetric",
      solution:
        "Let $B = A + A'$. Then $B' = A' + A = B$. So $B$ is symmetric.",
    ...
    qs.push({
      ...
      correctAnswer: "Skew-symmetric",
      solution:
        "Let $C = A - A'$. Then $C' = A' - A = -(A - A') = -C$. So $C$ is skew-symmetric.",
      difficulty_level: "Hard",
    });
  ...
  if (questions.length === 0) return <div className="flex items-center justify-center min-h-screen">Loading Matrix Transpose Test...</div>;
