...
const MatrixTypesTest = () => {
  ...
  useEffect(() => {
    ...
    qs.push({
      text: "$A = [a_{ij}]_{m \\times n}$ is a square matrix if:",
      options: shuffle(["$m = n$", "$m > n$", "$m < n$", "$m + n = 1$"]),
      correctAnswer: "$m = n$",
      solution:
        "A matrix is square when the number of rows equals the number of columns ($m = n$).",
    ...
    (() => {
      const n = rand(3, 5);
      const vals = [Array.from({ length: n }, () => rand(-9, 9))];
      qs.push({
        text: "What type is the matrix shown below?",
        matrix: vals,
        options: shuffle([
          "Row matrix",
          "Column matrix",
        ...
        solution: `A $1 \\times ${n}$ matrix (single row) is a row matrix.`,
        difficulty_level: "Easy",
      });
    })();
    ...
    (() => {
      ...
      qs.push({
        text: "The matrix shown is an example of:",
        matrix: I3,
        options: shuffle([
          "Identity matrix",
          "Zero matrix",
          "Scalar matrix (k≠1)",
          "Diagonal matrix (non-identity)",
        ]),
    ...
    // Q4: Diagonal matrix
    (() => {
      const d = [rand(2, 8), rand(1, 7), rand(3, 9)];
      const M = [
        [d[0], 0, 0],
        [0, d[1], 0],
        [0, 0, d[2]],
      ...
      qs.push({
        ...
        solution:
          "All non-diagonal entries are 0, with diagonal entries not all equal to 1. This is a diagonal matrix.",
        difficulty_level: "Easy",
      });
    })();
    ...
    (() => {
      ...
      qs.push({
        ...
        options: shuffle([
          ...
          "Identity matrix",
          "Zero matrix",
          "Diagonal matrix (non-scalar)",
        ]),
        correctAnswer: `Scalar matrix (k=${k})`,
        solution: `A scalar matrix is a diagonal matrix where all diagonal entries are equal to the same scalar $k = ${k}$.`,
    ...
    qs.push({
      text: "A scalar matrix becomes the identity matrix when the scalar $k$ equals:",
      options: shuffle(["$1$", "$0$", "$-1$", "$2$"]),
      correctAnswer: "$1$",
      solution:
        "When $k = 1$, every diagonal entry is 1 and off-diagonal entries are 0, which is the definition of $I$.",
      difficulty_level: "Easy",
    });
    ...
    (() => {
      const n = rand(3, 4);
      const vals = Array.from({ length: n }, () => [rand(-5, 10)]);
      qs.push({
        text: "Identify the type of matrix shown:",
        matrix: vals,
        options: shuffle([
          "Column matrix",
          "Row matrix",
        ...
        solution: `A matrix with order $${n} \\times 1$ (single column) is a column matrix.`,
        difficulty_level: "Easy",
      });
    })();
    ...
    (() => {
      ...
      qs.push({
        text: "The matrix shown is called a:",
        matrix: M,
        options: shuffle([
          "Zero (Null) matrix",
          "Identity matrix",
        ...
        correctAnswer: "Zero (Null) matrix",
        solution:
          "A matrix with all entries equal to 0 is called a zero (null) matrix.",
        difficulty_level: "Easy",
      });
    })();
    ...
    // Q9: Point as matrix
    qs.push({
      text: "The point $P(0, 1)$ can be represented as a matrix of type:",
      options: shuffle([
        "Column matrix ($2 \\times 1$)",
        "Row matrix ($1 \\times 2$)",
        "Both are valid",
        "Square matrix ($2 \\times 2$)",
      ]),
      correctAnswer: "Both are valid",
      solution:
        "A point $(x, y)$ can be written as a row matrix $\\begin{bmatrix} x & y \\end{bmatrix}$ or column matrix $\\begin{bmatrix} x \\\\ y \\end{bmatrix}$.",
      difficulty_level: "Medium",
    });

    // Q10: Find A if A + B = B
    qs.push({
      text: "If $A + B = B$ for all matrices $B$ of the same order, then $A$ is:",
      options: shuffle([
        "Zero matrix",
        "Identity matrix",
        "$B$ itself",
      ...
      solution:
        "$A + B = B \\Rightarrow A = B - B = O$. So $A$ must be the zero matrix (additive identity).",
      difficulty_level: "Medium",
    });
  ...
  if (questions.length === 0) return <div className="flex items-center justify-center min-h-screen">Loading Matrix Types Test...</div>;
                  ...
                  <div key={idx} className="p-4 rounded-xl border-2 border-gray-100 bg-white shadow-sm">
                    ...
                    {q.matrix && (
                      <div style={{ display: "flex", justifyContent: "center", marginBottom: 8 }}>
                        <InteractiveMatrixBuilder rows={q.matrix.length} cols={q.matrix[0].length} values={q.matrix.map((r) => r.map(String))} compact />
                      </div>
                    )}
              ...
              {cq.matrix && (
                <div style={{ display: "flex", justifyContent: "center", margin: "20px 0" }}>
                  <InteractiveMatrixBuilder
                    rows={cq.matrix.length}
                    cols={cq.matrix[0].length}
                    values={cq.matrix.map((r) => r.map(String))}
                    compact={cq.matrix.length > 3 || cq.matrix[0].length > 3}
                  />
                </div>
              )}
