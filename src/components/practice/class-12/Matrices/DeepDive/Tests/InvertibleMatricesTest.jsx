...
const InvertibleMatricesTest = () => {
  ...
  useEffect(() => {
    ...
    qs.push({
      ...
      correctAnswer: "$|A| \\neq 0$",
      solution:
        "A matrix is invertible (non-singular) when its determinant is non-zero.",
    ...
    (() => {
      ...
      const det = a * d - b * c;
      qs.push({
        text: `Find $|A|$ for $A = \\begin{bmatrix}${a}&${b}\\\\${c}&${d}\\end{bmatrix}$.`,
        matrix: [
          [a, b],
          [c, d],
        ],
        ...
        correctAnswer: `$${det}$`,
        solution: `$|A| = ${a}(${d}) - ${b}(${c}) = ${a * d} - ${b * c} = ${det}$.`,
        difficulty_level: "Easy",
      });
    })();
    ...
    (() => {
      ...
      const det = a * d - b * c2;
      qs.push({
        text: `Find $A^{-1}$ for $A = \\begin{bmatrix}${a}&${b}\\\\${c2}&${d}\\end{bmatrix}$.`,
        matrix: [
          [a, b],
          [c2, d],
        ],
        options: shuffle([
          `$\\begin{bmatrix}${d}&${-b}\\\\${-c2}&${a}\\end{bmatrix}$`,
          `$\\begin{bmatrix}${a}&${b}\\\\${c2}&${d}\\end{bmatrix}$`,
          `$\\begin{bmatrix}${-d}&${b}\\\\${c2}&${-a}\\end{bmatrix}$`,
          `$\\begin{bmatrix}${d}&${c2}\\\\${b}&${a}\\end{bmatrix}$`,
        ]),
        correctAnswer: `$\\begin{bmatrix}${d}&${-b}\\\\${-c2}&${a}\\end{bmatrix}$`,
        solution: `$A^{-1} = \\frac{1}{${det}}\\begin{bmatrix}${d}&${-b}\\\\${-c2}&${a}\\end{bmatrix}$. Since $|A| = ${det}$, $A^{-1} = \\begin{bmatrix}${d}&${-b}\\\\${-c2}&${a}\\end{bmatrix}$.`,
    ...
    qs.push({
      ...
      solution: "By definition of inverse: $AA^{-1} = A^{-1}A = I$.",
      difficulty_level: "Easy",
    });
    ...
    qs.push({
      ...
      solution: "The reversal law for inverse: $(AB)^{-1} = B^{-1}A^{-1}$.",
      difficulty_level: "Medium",
    });

    // Q6: Rectangular matrix inverse
    qs.push({
      text: "Can a rectangular (non-square) matrix have an inverse?",
      options: shuffle([
        "No, inverse requires a square matrix",
        "Yes, always",
        "Only if it has more rows than columns",
        "Only for 2×3 matrices",
      ]),
      correctAnswer: "No, inverse requires a square matrix",
      solution:
        "Only square matrices can have inverses. Rectangular matrices do not have two-sided inverses.",
      difficulty_level: "Easy",
    });

    // Q7: Uniqueness
    qs.push({
      text: "The inverse of a matrix, if it exists, is:",
      options: shuffle([
        "Unique",
        "Not unique",
        "Always the identity",
      ...
      solution:
        "The inverse of a matrix is unique. If $B$ and $C$ are both inverses of $A$, then $B = BI = B(AC) = (BA)C = IC = C$.",
      difficulty_level: "Medium",
    });

    // Q8: Solve system using inverse
    qs.push({
      text: "To solve $AX = B$ using inverse method, $X$ equals:",
      options: shuffle(["$A^{-1}B$", "$BA^{-1}$", "$AB^{-1}$", "$B^{-1}A$"]),
      correctAnswer: "$A^{-1}B$",
      solution:
        "Pre-multiply both sides by $A^{-1}$: $A^{-1}AX = A^{-1}B \\Rightarrow X = A^{-1}B$.",
      difficulty_level: "Medium",
    });

    // Q9: Adjoint definition
    qs.push({
      text: "The adjoint of a matrix is:",
      options: shuffle([
      ...
      correctAnswer: "Transpose of the cofactor matrix",
      solution:
        "adj$(A)$ is obtained by transposing the matrix of cofactors of each element of $A$.",
      difficulty_level: "Medium",
    });

    // Q10: A · adj(A) =
    qs.push({
      text: "$A \\cdot \\text{adj}(A)$ equals:",
      options: shuffle(["$|A| \\cdot I$", "$A^{-1}$", "$I$", "$O$"]),
      correctAnswer: "$|A| \\cdot I$",
      solution:
        "$A \\cdot \\text{adj}(A) = |A| \\cdot I$. This is a fundamental property used to derive $A^{-1} = \\frac{1}{|A|} \\text{adj}(A)$.",
  ...
  if (questions.length === 0) return <div className="flex items-center justify-center min-h-screen">Loading Invertible Matrices Test...</div>;
