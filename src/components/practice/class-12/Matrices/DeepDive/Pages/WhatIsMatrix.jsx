...
3. Therefore, the order is $2 \\times 3$ (rows × columns).`,
    hints: [
      ...
      "Order is written as rows × columns.",
    ],
  },
  {
    id: "wm2",
...
3. This matches $\\begin{bmatrix} 1 & 0 \\\\ 0 & 1 \\end{bmatrix}$.`,
    hints: [
      "Identity matrix has 1s on the diagonal.",
...
3. Note: $I_2$ is technically a scalar matrix too (k=1), but $\\begin{bmatrix} 3 & 0 \\\\ 0 & 3 \\end{bmatrix}$ is the best answer.`,
    hints: [
      "A scalar matrix is a special diagonal matrix.",
      "All diagonal entries must be the same value.",
    ],
  },
  {
    id: "wm8",
...
const WhatIsMatrix = () => {
  ...
  const currentBuilderValues = builderValues
    .slice(0, builderRows)
    .map((row) => row.slice(0, builderCols));
  const detectedTypes = detectMatrixType(
    currentBuilderValues,
    builderRows,
    builderCols,
  );
      ...
      <div style={{ maxWidth: 800, margin: "0 auto", padding: "40px 20px" }}>
        <div style={{ marginBottom: 32 }}>
          ...
          <p style={{ fontSize: "1.05rem", color: "#64748B", lineHeight: 1.6 }}>
            Learn the definition, notation, and types of matrices — the
          ...
          >
            {MATRIX_TYPES.map((type) => (
              <div
                ...
                style={{
                  background:
                    expandedType === type.name ? "#EEF2FF" : "#F8FAFC",
        ...
        <Section title="🛠️ Interactive Matrix Builder">
          <p style={{ color: "#475569", lineHeight: 1.6, marginBottom: 16 }}>
            Choose the number of rows and columns, fill in the values, and the
            system will identify the matrix type!
          </p>
