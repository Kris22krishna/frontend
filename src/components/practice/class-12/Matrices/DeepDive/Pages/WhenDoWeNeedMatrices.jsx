...
const SCENARIOS = [
  {
    ...
    description:
      "Instead of solving equations one by one, matrices let us group all coefficients together and solve the entire system simultaneously. This becomes mandatory when you have hundreds or thousands of variables.",
    example: [
      ["3x", "+", "2y", "=", "7"],
      ["-x", "+", "4y", "=", "5"],
    ],
  },
  {
    ...
    description:
      "A spreadsheet is essentially a large matrix. Data scientists use matrices to filter, sort, and perform operations on millions of rows of data in a single step.",
    example: [
      ["Age", "Income", "Score"],
      ["24", "$50k", "0.8"],
      ["31", "$80k", "0.9"],
  ...
  },
  {
    ...
    description:
      "In probability and statistics (like Markov chains), matrices are used to predict the future state of a system based on its current state—like predicting tomorrow's weather based on today's.",
    example: [
      ["Sunny", "Rainy"],
      ["0.8", "0.2"],
      ["0.4", "0.6"],
...
3. Arranging them in rows gives exactly $\\begin{bmatrix} 2 & 3 \\\\ 1 & -1 \\end{bmatrix}$.`,
    hints: [
      "Look at the numbers in front of variables x and y for both equations.",
      "Equation 1 forms Row 1, Equation 2 forms Row 2.",
    ],
  },
  {
    id: "when3",
    ...
    if (score / total >= 0.5) {
      try {
        const progress = JSON.parse(
          localStorage.getItem("matrices_progress") || "{}",
        );
    ...
    >
      ...
      <div style={{ maxWidth: 800, margin: "0 auto", padding: "40px 20px" }}>
        <div style={{ marginBottom: 40, textAlign: "center" }}>
          ...
          >
            You've spent years solving equations step-by-step. Matrices are the
            ultimate shortcut for dealing with hundreds, thousands, or billions
        ...
        >
          {SCENARIOS.map((scenario, idx) => (
            ...
            >
              ...
              >
                <MatrixGrid
                  rows={scenario.example.length}
                  cols={scenario.example[0].length}
                  values={scenario.example}
                  compact={true}
                />
              </div>
            </div>
        ...
        <div
          style={{ display: "flex", justifyContent: "flex-between", gap: 20 }}
        >
          <button
            onClick={() =>
          ...
          <button
            onClick={() =>
              navigate("/senior/grade/12/matrices/deep-dive/how-operations")
            }
            style={{
              background: "#4F46E5",
              color: "#fff",
