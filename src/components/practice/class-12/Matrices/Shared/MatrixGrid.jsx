...
const MatrixGrid = ({
  ...
  const getCellClass = (i, j) => {
    ...
    if (highlightCell && highlightCell.i === i && highlightCell.j === j)
      classes.push("highlight");
    if (highlightRow !== null && highlightRow === i)
      classes.push("highlight-row");
    if (highlightCol !== null && highlightCol === j)
      classes.push("highlight-col");
    if (highlightResult && highlightResult.i === i && highlightResult.j === j)
      classes.push("highlight-result");
    return classes.join(" ");
  };
      ...
      <div className="matrix-grid-wrapper">
        ...
        <div className="matrix-bracket left">
          <svg
            width="10"
            height={bracketH}
            viewBox={`0 0 10 ${bracketH}`}
            fill="none"
          >
            <path
              d={`M9 1 Q1 1 1 ${bracketH / 2} Q1 ${bracketH - 1} 9 ${bracketH - 1}`}
              stroke="#31326F"
              strokeWidth="2"
              fill="none"
            />
        ...
        >
          {Array.from({ length: rows }, (_, i) =>
            Array.from({ length: cols }, (_, j) => {
              const val =
                values && values[i] && values[i][j] !== undefined
                  ? values[i][j]
                  : "";
              return (
                <div
        ...
        <div className="matrix-bracket right">
          ...
          >
            <path
              d={`M1 1 Q9 1 9 ${bracketH / 2} Q9 ${bracketH - 1} 1 ${bracketH - 1}`}
              stroke="#31326F"
              strokeWidth="2"
              fill="none"
            />
