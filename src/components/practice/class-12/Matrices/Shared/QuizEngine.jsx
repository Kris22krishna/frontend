...
const QuizEngine = ({
  ...
  useEffect(() => {
    ...
    if (userId && !sessionId && skillId) {
      api
        .createPracticeSession(userId, skillId)
        .then((sess) => {
    ...
    if (userId && skillId) {
      ...
      api
        .recordAttempt({
          ...
          is_correct: isRight,
          solution_text:
            currentQ.solution || "No detailed explanation available.",
          ...
          <div style={{ marginTop: 16 }}>
            <button
              ...
              style={{
                ...
                fontSize: "0.85rem",
                cursor:
                  hintIndex >= currentHints.length - 1 ? "default" : "pointer",
                opacity: hintIndex >= currentHints.length - 1 ? 0.5 : 1,
              }}
            >
              <Lightbulb size={16} /> Hint ({hintIndex + 1}/
              {currentHints.length})
            </button>
            {hintIndex >= 0 && (
              <div
