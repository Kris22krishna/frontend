...
import { SKILLS } from './TheTriangleAndItsPropertiesSkillsData.js';

export default function TheTriangleAndItsPropertiesSkills() {
    ...
    if (view !== 'list' && skill) {
        return (
            <div className="tri-skills-page" style={{ background: '#f8fafc', minHeight: '100vh', paddingBottom: '60px' }}>
                ...
                <div style={{ padding: '40px 24px 0' }}>
                    {view === 'learn' ? (
                        <div className="tri-lexicon-container">
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: 'minmax(260px, 300px) 1fr', gap: 24, alignItems: 'start', maxWidth: 1100, margin: '0 auto' }}>
                                ...
                                {/* Detailed Window */}
                                <main className="tri-details-window-anim" key={selectedLearnIdx} style={{ background: '#fff', borderRadius: 24, padding: 32, border: '1px solid #e2e8f0', minHeight: 400 }}>
    ...
    return (
        <div className="tri-skills-page" style={{ background: '#f8fafc', minHeight: '100vh', paddingBottom: '60px' }}>
            ...
            {/* ── MAIN CONTENT ──────────────────────────────── */}
            <div className="tri-lexicon-container" style={{ padding: '40px 24px 80px', maxWidth: 1000, margin: '0 auto' }}>

                {/* Centered Heading */}
                <div style={{ textAlign: 'center', marginBottom: 40 }}>
                    <h1 style={{ fontFamily: 'Outfit, sans-serif', fontSize: '2.8rem', fontWeight: 900, color: '#0f172a', margin: '0 0 8px' }}>
                        Triangle <span style={{ color: '#10b981' }}>Skills</span>
                    </h1>
                ...
                <div className="tri-skills-grid">
                    {SKILLS.map((s, i) => (
                        <div key={s.id} className="tri-skill-card" style={{ '--skill-color': s.color }}>
                            <div className="tri-skill-icon" style={{ background: `${s.color}10`, color: s.color }}>
                                {s.icon}
                            </div>

                            <div className="tri-skill-content">
                                <div className="tri-skill-text-stack">
                            ...
                            <div className="tri-skill-actions">
                                <button className="tri-skill-btn-outline" onClick={() => { setActiveSkill(i); setView('learn'); }}>Learn</button>
                                <button className="tri-skill-btn-outline" onClick={() => { setActiveSkill(i); setView('practice'); }}>Practice</button>
                                <button className="tri-skill-btn-filled" onClick={() => { setActiveSkill(i); setView('assessment'); }}>Assess</button>
                            </div>
                        </div>
                    ))}
                </div>
