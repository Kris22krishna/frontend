...
const CategorizedPracticeEngine = ({ skill, onBack }) => {
    ...
    return (
        <div className="eq-practice-layout">
            <aside className="eq-learn-sidebar">
                <button onClick={onBack} style={{ marginBottom: 16, background: 'none', border: 'none', cursor: 'pointer', color: 'var(--eq-muted)', textAlign: 'left', fontWeight: 'bold' }}>← Back to Skills</button>
                ...
                {skill.practiceCategories.map((c, i) => (
                    <button key={i} onClick={() => setActiveCat(i)} className={`eq-sidebar-btn ${activeCat === i ? 'active' : ''}`} style={{
