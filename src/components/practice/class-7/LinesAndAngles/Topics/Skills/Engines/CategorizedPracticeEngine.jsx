...
const CategorizedPracticeEngine = ({ skill, onBack }) => {
    ...
    return (
        <div className="la-practice-layout">
            <aside className="la-learn-sidebar">
                <button onClick={onBack} style={{ marginBottom: 16, background: 'none', border: 'none', cursor: 'pointer', color: 'var(--la-muted)', textAlign: 'left', fontWeight: 'bold' }}>← Back to Skills</button>
                ...
                {skill.practiceCategories.map((c, i) => (
                    <button key={i} onClick={() => setActiveCat(i)} className={`la-sidebar-btn ${activeCat === i ? 'active' : ''}`} style={{
