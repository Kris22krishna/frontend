...
const DivisionPracticeTwoDigit = () => {
    ...
    const SKILL_NAME = "Divide by 2 digit number";
    ...
    useEffect(() => {
        ...
        startSession({ nodeId: 'a4051013-0007-0000-0000-000000000000', sessionType: 'practice' });
    ...
    const generateQuestion = (level) => {
        ...
        divisor = randomInt(11, 49); // Divisor is 2-digit

        if (level === 0) {
            // Easy: 3-digit dividend
            quotient = randomInt(5, 20);
            dividend = quotient * divisor;
            while (dividend < 100 || dividend > 999) {
                divisor = randomInt(11, 49);
                quotient = randomInt(5, 20);
                dividend = quotient * divisor;
            }
            difficultyLabel = "Easy";
        } else if (level === 1) {
            // Medium: 4-digit dividend
            quotient = randomInt(21, 99);
            dividend = quotient * divisor;
            while (dividend < 1000 || dividend > 9999) {
            ...
            // Hard: 5-digit dividend
            quotient = randomInt(101, 400);
            dividend = quotient * divisor;
            while (dividend < 10000 || dividend > 99999) {
                divisor = randomInt(25, 99);
                quotient = randomInt(101, 400);
                dividend = quotient * divisor;
            }
        ...
        let solution = `To divide ${dividend} by ${divisor}:<br/><br/>`;
        solution += `We can use long division to find how many times ${divisor} goes into ${dividend}.<br/><br/>`;
        solution += `<strong>Solution:</strong><br/>`;
        solution += `${dividend} ÷ ${divisor} = <strong>${quotient}</strong><br/><br/>`;
        solution += `<em>Check your work:</em><br/>`;
        solution += `${divisor} × ${quotient} = ${dividend}`;

        const correctAnswer = quotient.toString();
        const options = [correctAnswer];

        while (options.length < 4) {
            const distractor = (quotient + randomInt(-10, 10)).toString();
            ...
            <div className="junior-practice-page results-view overflow-y-auto" style={{ fontFamily: '"Open Sans", sans-serif' }}>
                <header className="junior-practice-header results-header relative">
                    ...
                    <div className="title-area">
                        <h1 className="results-title">Division Report</h1>
                    </div>
                </header>

                <main className="practice-content results-content max-w-5xl mx-auto w-full px-4">
                    <div className="results-hero-section flex flex-col items-center mb-8">
                        <h2 className="text-4xl font-black text-[#31326F] mb-2">Masterful Division! 🎉</h2>
                    ...
                    <div className="detailed-breakdown w-full mb-12">
                        <h3 className="text-2xl font-black text-[#31326F] mb-6 px-4">Division Log 📜</h3>
                        <div className="space-y-4">
                            {Array.from({ length: TOTAL_QUESTIONS }).map((_, idx) => {
                                const ans = answers[idx];
                                if (!ans) return null;
                                // Note: For dynamic question generations where we don't store the question object,
                                // we might need to store the question text/solution in the 'answers' state or elsewhere.
                                // However, many of these components generate questions on the fly based on index.
                                // For now, I'll store the question data in answers when submitted.
                                return (
                                    <motion.div
                                        key={idx}
...
export default DivisionPracticeTwoDigit;
