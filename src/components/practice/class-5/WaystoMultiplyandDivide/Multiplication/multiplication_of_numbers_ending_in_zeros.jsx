...
const MultiplicationEndingZero = () => {
    ...
    const SKILL_NAME = "Multiply number ending in zeroes";
    ...
    useEffect(() => {
        ...
        startSession({ nodeId: 'a4051013-0003-0000-0000-000000000000', sessionType: 'practice' });
    ...
    const generateQuestion = (level) => {
        let num1, num2, difficultyLabel;

        const generateNumberEndingInZero = (digits) => {
            const base = randomInt(Math.pow(10, digits - 2), Math.pow(10, digits - 1) - 1);
            const zeros = randomInt(1, Math.min(digits - 1, 3));
            return base * Math.pow(10, zeros);
        };

        if (level === 0) {
            // Easy: 2-digit range (e.g., 20 x 30, 40 x 5)
            num1 = randomInt(2, 9) * 10;
            num2 = Math.random() > 0.5 ? randomInt(2, 9) * 10 : randomInt(2, 9);
            difficultyLabel = "Easy";
        } else if (level === 1) {
            // Medium: 3-digit range (e.g., 200 x 30, 400 x 50)
            num1 = randomInt(2, 9) * 100;
            num2 = randomInt(2, 9) * 10;
            difficultyLabel = "Medium";
        } else {
            // Hard: 4-digit range (e.g., 2000 x 30, 5000 x 400)
            num1 = randomInt(11, 49) * 100;
            num2 = randomInt(2, 9) * 100;
        ...
        const z1 = countZeros(num1Str);
        const z2 = countZeros(num2Str);
        const base1 = parseInt(num1Str.slice(0, num1Str.length - z1));
        const base2 = parseInt(num2Str.slice(0, num2Str.length - z2));
        ...
        let solution = `Shortcut to multiply numbers ending in zeros:<br/><br/>`;
        solution += `1. Multiply the numbers without the trailing zeros:<br/>`;
        solution += `&nbsp;&nbsp;&nbsp;<strong>${base1} × ${base2} = ${base1 * base2}</strong><br/><br/>`;
        solution += `2. Count the total number of zeros at the end of both numbers:<br/>`;
        solution += `&nbsp;&nbsp;&nbsp;${num1} has ${z1} zero(s), and ${num2} has ${z2} zero(s).<br/>`;
        solution += `&nbsp;&nbsp;&nbsp;Total zeros = <strong>${z1 + z2}</strong><br/><br/>`;
        solution += `3. Add the zeros to the result of Step 1:<br/>`;
        solution += `&nbsp;&nbsp;&nbsp;<strong>${product}</strong>`;

        const correctAnswer = product.toString();
        const options = [correctAnswer];

        while (options.length < 4) {
            const wrongBase = (base1 * base2) + randomInt(-2, 2);
            const wrongZeros = (z1 + z2) + (Math.random() > 0.5 ? 1 : -1);
            const distractor = (wrongBase * Math.pow(10, Math.max(0, wrongZeros))).toString();
            ...
            <div className="junior-practice-page results-view overflow-y-auto" style={{ fontFamily: '"Open Sans", sans-serif' }}>
                <header className="junior-practice-header results-header relative">
                    ...
                    <div className="title-area">
                        <h1 className="results-title">Multiplication Report</h1>
                    </div>
                </header>
                ...
                <main className="practice-content results-content max-w-5xl mx-auto w-full px-4">
                    ...
                    <div className="detailed-breakdown w-full mb-12">
                        <h3 className="text-2xl font-black text-[#31326F] mb-6 px-4">Multiplication Log 📜</h3>
...
export default MultiplicationEndingZero;
