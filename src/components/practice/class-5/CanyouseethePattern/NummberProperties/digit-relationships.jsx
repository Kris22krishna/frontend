...
const DigitRelationships = () => {
    ...
    useEffect(() => {
        const generateQuestion = (idx) => {
            const types = ['placeValueRelation', 'digitSum', 'digitSwap'];
            const type = types[idx % types.length];
            let questionText, correctAnswer, solution, options;

            if (type === 'placeValueRelation') {
                const val = randomInt(1, 9);
                questionText = `In the number $${val}${val}0$, how many times larger is the $${val}$ in the hundreds place than the $${val}$ in the tens place?`;
                correctAnswer = "10";
                solution = `The hundreds place is 10 times the tens place. So, $${val}00$ is <strong>10</strong> times larger than $${val}0$.`;
                options = ["10", "100", "5", "2"];
            } else if (type === 'digitSum') {
                const a = randomInt(1, 4);
                const b = randomInt(1, 4);
                const num = `${a}${b}`;
                questionText = `What is the sum of digits in the number $${num}$?`;
                correctAnswer = (a + b).toString();
                solution = `$${a} + ${b} = <strong>${correctAnswer}</strong>$.`;
                options = [correctAnswer, (a * b).toString(), num, (a + b + 1).toString()];
            } else {
                const a = randomInt(1, 9);
                const b = randomInt(1, 9);
                while (a === b) { b = randomInt(1, 9); }
                questionText = `If you swap the digits of $${a}${b}$, by how much does the value change?`;
                const diff = Math.abs((a * 10 + b) - (b * 10 + a));
                correctAnswer = diff.toString();
                solution = `Original: $${a}${b}$. Swapped: $${b}${a}$. Difference: $|${a * 10 + b} - ${b * 10 + a}| = <strong>${diff}</strong>$.`;
                options = [diff.toString(), "10", "1", "0"];
            }
