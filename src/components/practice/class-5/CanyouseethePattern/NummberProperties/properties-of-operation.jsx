...
const PropertiesOfOperation = () => {
    ...
    useEffect(() => {
        const generateQuestion = (idx) => {
            const types = ['commutative', 'distributive', 'identity'];
            ...
            if (type === 'commutative') {
                ...
                questionText = `According to the Commutative Property, $${a} + ${b}$ is equal to:`;
                correctAnswer = `$${b} + ${a}$`;
                solution = `The Commutative Property states that the order of numbers doesn't change the sum: $a + b = b + a$. So $${a} + ${b} = <strong>${b} + ${a}</strong>$.`;
                options = [`$${b} + ${a}$`, `$${a} - ${b}$`, `$${a} \\times ${b}$`, `$0$`];
            } else if (type === 'distributive') {
                const a = randomInt(2, 5);
                const b = randomInt(10, 20);
                const c = randomInt(1, 9);
                questionText = `Solve using properties: $${a} \\times (${b} + ${c})$ is the same as:`;
                correctAnswer = `$(${a} \\times ${b}) + (${a} \\times ${c})$`;
                solution = `The Distributive Property: $a(b + c) = ab + ac$. Thus, $${a}(${b} + ${c}) = <strong>(${a} \\times ${b}) + (${a} \\times ${c})</strong>$.`;
                options = [`$(${a} \\times ${b}) + (${a} \\times ${c})$`, `$${a} \\times ${b} + ${c}$`, `$${a} + ${b} + ${c}$`, `$${a} \\times (${b} \\times ${c})$`];
            } else {
                const a = randomInt(10, 100);
                questionText = `What number is the additive identity? (What can you add to $${a}$ to keep it the same?)`;
                correctAnswer = "0";
                solution = `Adding <strong>0</strong> to any number doesn't change it. $${a} + 0 = ${a}$.`;
                options = ["0", "1", "10", a.toString()];
            }
...
export default PropertiesOfOperation;
