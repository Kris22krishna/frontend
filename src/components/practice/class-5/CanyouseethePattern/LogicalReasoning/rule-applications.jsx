...
const RuleApplications = () => {
    ...
    useEffect(() => {
        const generateQuestion = (idx) => {
            const types = ['logicTable', 'symbolCode', 'numberChain'];
            const type = types[idx % types.length];
            let questionText, correctAnswer, solution, options;

            if (type === 'logicTable') {
                questionText = `If $\\triangle = 5$ and $\\square = 10$, what is the value of $\\triangle + \\square + \\triangle$?`;
                correctAnswer = "20";
                solution = `$5 + 10 + 5 = <strong>20</strong>$.`;
                options = ["20", "15", "25", "10"];
            } else if (type === 'symbolCode') {
                questionText = `In a secret code, A=1, B=2, C=3. What is the sum of the code for "CAB"?`;
                correctAnswer = "6";
                solution = `C=3, A=1, B=2. Sum: $3 + 1 + 2 = <strong>6</strong>$.`;
                options = ["6", "5", "7", "312"];
            } else {
                const start = 10;
                questionText = `Follow the chain: Start with $${start}$, double it, add 5, then subtract 3. What is the result?`;
                correctAnswer = "22";
                solution = `$10 \\times 2 = 20 \\to 20 + 5 = 25 \\to 25 - 3 = <strong>22</strong>$.`;
                options = ["22", "20", "25", "17"];
            }
...
export default RuleApplications;
