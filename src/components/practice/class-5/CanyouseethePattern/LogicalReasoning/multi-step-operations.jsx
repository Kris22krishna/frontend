...
const MultiStepOperations = () => {
    ...
    useEffect(() => {
        const generateQuestion = (idx) => {
            const types = ['pemdasLite', 'chainOfOps', 'bracketsFirst'];
            const type = types[idx % types.length];
            let questionText, correctAnswer, solution, options;

            if (type === 'pemdasLite') {
                questionText = `Evaluate: $10 + 2 \\times 5$. (Remember to multiply first!)`;
                correctAnswer = "20";
                solution = `$2 \\times 5 = 10$. Then $10 + 10 = <strong>20</strong>$.`;
                options = ["20", "60", "15", "25"];
            } else if (type === 'chainOfOps') {
                questionText = `Start with 50, subtract 20, then add 10, then divide by 2. What is the result?`;
                correctAnswer = "20";
                solution = `$50 - 20 = 30 \\to 30 + 10 = 40 \\to 40 \\div 2 = <strong>20</strong>$.`;
                options = ["20", "15", "25", "10"];
            } else {
                questionText = `Calculate: $(15 + 5) \\div 4$.`;
                correctAnswer = "5";
                solution = `First brackets: $15 + 5 = 20$. Then $20 \\div 4 = <strong>5</strong>$.`;
                options = ["5", "4", "20", "6"];
            }
