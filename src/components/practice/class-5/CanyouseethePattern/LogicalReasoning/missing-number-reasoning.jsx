...
const MissingNumberReasoning = () => {
    ...
    useEffect(() => {
        const generateQuestion = (idx) => {
            const types = ['balanceScale', 'magicTriangle', 'missingInSequence'];
            const type = types[idx % types.length];
            let questionText, correctAnswer, solution, options;

            if (type === 'balanceScale') {
                questionText = `A balance is equal on both sides. Left side: $15 + x$. Right side: $25$. What is the value of $x$?`;
                correctAnswer = "10";
                solution = `$15 + x = 25 \\to x = 25 - 15 = <strong>10</strong>$.`;
                options = ["10", "15", "5", "40"];
            } else if (type === 'magicTriangle') {
                questionText = `In a triangle, numbers along each side sum to 12. If one side has 4 and 5 already, what is the third number for that side?`;
                correctAnswer = "3";
                solution = `$4 + 5 = 9$. To reach a sum of 12, we need $12 - 9 = <strong>3</strong>$.`;
                options = ["3", "4", "2", "5"];
            } else {
                questionText = `What number is missing in the pattern: $2, 6, \\dots, 14, 18$?`;
                correctAnswer = "10";
                solution = `The numbers increase by 4 each time. $6 + 4 = 10$ and $10 + 4 = 14$. So the missing number is <strong>10</strong>.`;
                options = ["10", "12", "8", "9"];
            }
