...
const GridPatternRecognition = () => {
    ...
    useEffect(() => {
        const generateQuestion = (idx) => {
            const types = ['magicSquare', 'sudokuLite', 'colorGrid'];
            const type = types[idx % types.length];
            let questionText, correctAnswer, solution, options;

            if (type === 'magicSquare') {
                questionText = `In a magic square, each row sums to 15. If a row has numbers 8 and 3, what is the third number?`;
                correctAnswer = "4";
                solution = `$8 + 3 = 11$. To reach a sum of 15, we need $15 - 11 = <strong>4</strong>$.`;
                options = ["4", "5", "1", "7"];
            } else if (type === 'sudokuLite') {
                questionText = `In a $2 \\times 2$ grid, each row and column must have numbers 1 and 2 once. If the first row is [1, 2] and the first item of the second row is 2, what is the next item?`;
                correctAnswer = "1";
                solution = `The second row starts with 2. Since 2 is already in the first column, and each row must have 1 and 2, the next item must be <strong>1</strong>.`;
                options = ["1", "2", "3", "0"];
            } else {
                questionText = `A pattern in a $3 \\times 3$ grid goes: Red, Blue, Red (Row 1); Blue, Red, Blue (Row 2). What should be the first color in Row 3?`;
                correctAnswer = "Red";
                solution = `The colors alternate like a checkerboard. Since Row 1 started with Red and Row 2 with Blue, Row 3 should start with <strong>Red</strong>.`;
