...
const CORRECT_MESSAGES = [
    ...
    "🎉 Correct! You've decoded the grid! 🎉",
    "✨ Fantastic work on the patterns! ✨",
    "🚀 Super! Grids are no match for you! 🚀"
];

const PatternGrid = ({ grid }) => {
    return (
        <div className="flex justify-center items-center py-6">
            <div className="grid gap-3 p-4 bg-white rounded-2xl shadow-inner border-4 border-gray-50" style={{ gridTemplateColumns: `repeat(${grid[0].length}, 60px)` }}>
                {grid.map((row, rIdx) =>
                    row.map((cell, cIdx) => (
                        <div
                            key={`${rIdx}-${cIdx}`}
                            className={`w-[60px] h-[60px] flex items-center justify-center rounded-xl border-2 font-black text-2xl transition-all shadow-sm
                                ${cell === '?' ? 'bg-red-50 border-red-200 text-red-500' : 'bg-white border-gray-100 text-[#31326F]'}`}
                        >
                            {cell}
                        </div>
                    ))
...
const GridPatternRecognition = () => {
    ...
    useEffect(() => {
        ...
        const generateQuestions = () => {
            ...
            for (let i = 0; i < 3; i++) {
                const diff = 2 + i;
                const start = 5 + i;
                const grid = [[start, start + diff, start + (diff * 2)], [start + 10, start + 10 + diff, '?'], [start + 20, start + 20 + diff, start + 20 + (diff * 2)]];
                const ans = (start + 10 + (diff * 2)).toString();
                qs.push({
                    text: "What number completes the pattern in the grid?",
                    visual: <PatternGrid grid={grid} />,
                    correctAnswer: ans,
                    options: [ans, (start + 11 + (diff * 2)).toString(), (start + 15).toString(), (start + 25).toString()],
                    solution: `The rule is <b>Add ${diff}</b> horizontally across rows. Row 2: ${start + 10} + ${diff} = ${start + 10 + diff}, then ${start + 10 + diff} + ${diff} = <b>${ans}</b>.`
                });
            }

            for (let i = 0; i < 3; i++) {
                const grid = [[8, '?', 6], [3, 5, 7], [4, 9, 2]];
                if (i === 1) { grid[0][1] = 1; grid[1][1] = '?'; grid[1][2] = 7; }
                if (i === 2) { grid[2][0] = '?'; grid[2][1] = 9; grid[2][2] = 2; }
                const ans = i === 0 ? "1" : i === 1 ? "5" : "4";
                qs.push({
                    text: "In this 'Magic Square', every row, column, and diagonal adds up to 15. Find the missing number.",
                    visual: <PatternGrid grid={grid} />,
                    correctAnswer: ans,
                    options: ["1", "5", "4", "3"],
                    solution: `Calculating the sum: the row or column with '?' must total 15. The missing number is <b>${ans}</b>.`
                });
            }

            const hardData = [
                { grid: [['A', 'B', 'C'], ['B', 'C', 'A'], ['C', 'A', '?']], ans: "B", rule: "Each row and column contains A, B, and C once.", opts: ["A", "B", "C", "D"] },
                { grid: [[2, 4, 8], [3, 9, 27], [4, 16, '?']], ans: "64", rule: "Each term is position-based: $n, n^2, n^3$. $4^3 = 64$.", opts: ["64", "32", "20", "48"] },
                { grid: [[5, 10, 15], [10, 20, 30], [15, 30, '?']], ans: "45", rule: "Multiplication table of 5. $3 \\times 3 \\times 5 = 45$.", opts: ["45", "40", "50", "60"] },
                { grid: [[1, 1, 2], [1, 2, 3], [2, 3, '?']], ans: "5", rule: "The third cell is the sum of the first two.", opts: ["5", "3", "6", "1"] }
            ];

            hardData.forEach(d => {
                qs.push({
                    text: "Find the missing value in the grid.",
                    visual: <PatternGrid grid={d.grid} />,
                    correctAnswer: d.ans,
                    options: d.opts,
                    solution: d.rule
                });
    ...
    if (!currentQuestion && !showResults) return <div className="flex justify-center items-center h-screen text-[#31326F] font-bold">Grid logic loading...</div>;
        ...
        return (
            <div className="junior-practice-page results-view overflow-y-auto" style={{ fontFamily: '"Open Sans", sans-serif' }}>
                <header className="junior-practice-header results-header relative">
                    ...
                    <div className="title-area"><h1 className="results-title">Grid Champion</h1></div>
                </header>
                <main className="practice-content results-content max-w-5xl mx-auto w-full px-4">
                    <div className="results-hero-section flex flex-col items-center mb-8">
                        <h2 className="text-4xl font-black text-[#31326F] mb-2">Grid Quest Complete! ⬛</h2>
                    ...
                    <div className="detailed-breakdown w-full mb-12">
                        <h3 className="text-2xl font-black text-[#31326F] mb-6 px-4">Grid Log 📜</h3>
                    ...
                    <div className="results-actions flex flex-col md:flex-row justify-center gap-4 py-8 border-t-4 border-dashed border-gray-100">
                        <button className="magic-pad-btn play-again px-12 py-4 rounded-2xl bg-[#31326F] text-white font-black text-xl shadow-xl hover:-translate-y-1 transition-all" onClick={() => window.location.reload()}><RefreshCw size={24} /> New Grid</button>
...
export default GridPatternRecognition;
