...
const CORRECT_MESSAGES = [
    "✨ You're a pattern creator! ✨",
    "🌟 Perfect rule application! 🌟",
    "🎉 Correct! You've mastered the rules! 🎉",
    "✨ Excellent work! ✨",
    "🚀 Way to go! Patterns are your specialty! 🚀"
];
...
const RuleBasedPatternCreation = () => {
    ...
    useEffect(() => {
        ...
        startSession({ nodeId: 'a4051007-0007-0000-0000-000000000000', sessionType: 'practice' });
        ...
        const generateQuestions = () => {
            const qs = [];
            const easyRules = [
                { start: 2, step: 5, op: 'add', text: "Start with 2 and add 5 each time." },
                { start: 10, step: 2, op: 'subtract', text: "Start with 10 and subtract 2 each time." },
                { start: 1, step: 3, op: 'multiply', text: "Start with 1 and multiply by 3 each time." }
            ];

            easyRules.forEach(r => {
                let seq = [r.start];
                for (let i = 1; i < 4; i++) {
                    if (r.op === 'add') seq.push(seq[i - 1] + r.step);
                    else if (r.op === 'subtract') seq.push(seq[i - 1] - r.step);
                    else if (r.op === 'multiply') seq.push(seq[i - 1] * r.step);
                }
                const nextVal = r.op === 'add' ? seq[3] + r.step : r.op === 'subtract' ? seq[3] - r.step : seq[3] * r.step;
                qs.push({
                    text: `Following the rule: "${r.text}", what are the first 5 numbers?`,
                    correctAnswer: `${seq.join(', ')}, ${nextVal}`,
                    options: [
                        `${seq.join(', ')}, ${nextVal}`,
                        `${seq.join(', ')}, ${nextVal + 1}`,
                        `${seq.slice(0, 3).join(', ')}, ${nextVal - 2}, ${nextVal}`,
                        `${r.start}, ${r.start + 1}, ${r.start + 2}, ${r.start + 3}, ${r.start + 4}`
                    ],
                    solution: `Starting with ${r.start}: <br/>1st: ${r.start} <br/>2nd: ${seq[1]} <br/>3rd: ${seq[2]} <br/>4th: ${seq[3]} <br/>5th: <b>${nextVal}</b>`
                });
            });
            ...
            for (let i = 0; i < 3; i++) {
                ...
                let currentStep = inc;
                for (let j = 1; j < 5; j++) {
                    seq.push(seq[j - 1] + currentStep);
                    currentStep += inc;
                }
                qs.push({
                    text: `Starting at 1, if the amount added increases by ${inc} each time, what is the 5th number?`,
                    correctAnswer: seq[4].toString(),
                    options: [seq[4].toString(), (seq[4] + 2).toString(), (seq[4] - inc).toString(), (seq[4] + inc).toString()],
                    solution: `Start: 1 <br/>Add ${inc}: 1 + ${inc} = ${seq[1]} <br/>Add ${inc * 2}: ${seq[1]} + ${inc * 2} = ${seq[2]} <br/>Add ${inc * 3}: ${seq[2]} + ${inc * 3} = ${seq[3]} <br/>Add ${inc * 4}: ${seq[3]} + ${inc * 4} = <b>${seq[4]}</b>`
                });
            }

            const hardRules = [
                { text: "Add 2, then multiply by 2 (starting at 1).", seq: [1, 3, 6, 8, 16], ans: "16" },
                { text: "Subtract 3, then add 5 (starting at 10).", seq: [10, 7, 12, 9, 14], ans: "14" },
                { text: "Divide by 2, then add 10 (starting at 40).", seq: [40, 20, 30, 15, 25], ans: "25" },
                { text: "Square position number and add 1.", seq: [2, 5, 10, 17, 26], ans: "26" }
            ];

            hardRules.forEach(r => {
                qs.push({
                    text: `Apply the rule: "${r.text}". What is the 5th number in the sequence?`,
                    correctAnswer: r.ans,
                    options: [r.ans, (parseInt(r.ans) + 5).toString(), (parseInt(r.ans) - 2).toString(), (parseInt(r.ans) * 2).toString()],
                    solution: `Applying the rule step-by-step: <br/>1st: ${r.seq[0]} <br/>2nd: ${r.seq[1]} <br/>3rd: ${r.seq[2]} <br/>4th: ${r.seq[3]} <br/>5th: <b>${r.ans}</b>`
                });
            });
    ...
    if (!currentQuestion && !showResults) return <div className="flex justify-center items-center h-screen text-[#31326F] font-bold">Loading Creator Mode...</div>;
        ...
        return (
            <div className="junior-practice-page results-view overflow-y-auto" style={{ fontFamily: '"Open Sans", sans-serif' }}>
                <header className="junior-practice-header results-header relative">
                    ...
                    <div className="title-area"><h1 className="results-title">Creation Mastery</h1></div>
                </header>
                <main className="practice-content results-content max-w-5xl mx-auto w-full px-4">
                    <div className="results-hero-section flex flex-col items-center mb-8">
                        <h2 className="text-4xl font-black text-[#31326F] mb-2">Designer Complete! 🎨</h2>
                    ...
                    <div className="detailed-breakdown w-full mb-12">
                        <h3 className="text-2xl font-black text-[#31326F] mb-6 px-4">Creation Log 📜</h3>
                    ...
                    <div className="results-actions flex flex-col md:flex-row justify-center gap-4 py-8 border-t-4 border-dashed border-gray-100">
                        ...
                        <button className="px-12 py-4 rounded-2xl border-4 border-[#31326F] text-[#31326F] font-black text-xl hover:bg-gray-50 transition-all flex items-center justify-center gap-3" onClick={() => navigate(-1)}>Exit</button>
                    </div>
                </main>
            </div>
        );
