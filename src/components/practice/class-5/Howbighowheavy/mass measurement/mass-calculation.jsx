...
const CORRECT_MESSAGES = [
    ...
    "🎉 Correct! You've balanced the village accounts! 🎉",
    "✨ Fantastic work! ✨",
    "🚀 Super! Precision calculation! 🚀",
    "🌿 Perfect! Every gram is accounted for! 🌿",
    "🎊 Great job! Moving on... 🎊",
...
const MassCalculation = () => {
    ...
    const SKILL_NAME = "Mass Calculation";
    ...
    useEffect(() => {
        ...
        startSession({ nodeId: 'a4051014-0008-0000-0000-000000000000', sessionType: 'practice' });
        ...
        for (let i = 0; i < TOTAL_QUESTIONS; i++) {
            ...
            if (i < 3) {
                ...
                const unitWeight = randomInt(1, 5) * 50;
                const total = count * unitWeight;
                question = {
                    text: `A small apple weighs <strong>${unitWeight} g</strong>. What is the total mass of <strong>${count}</strong> such apples?`,
                    correctAnswer: `${total} g`,
                    options: [`${total} g`, `${total - unitWeight} g`, `${total + 100} g`, `${unitWeight} g`],
                    solution: `Multiply unit weight by quantity:<br/>
                               ${unitWeight} g × ${count} = <strong>${total} g</strong>.`
                };
            } else if (i < 6) {
                // Medium: Addition and Subtraction
                const w1 = randomInt(1, 4);
                const w2 = randomInt(5, 9) / 2; // e.g. 2.5, 3, 3.5
                const total = w1 + w2;
                question = {
                    text: `Rani bought <strong>${w1} kg</strong> of mangoes and <strong>${w2} kg</strong> of grapes. What is the total mass of fruits she bought?`,
                    correctAnswer: `${total} kg`,
                    options: [`${total} kg`, `${total + 1} kg`, `${Math.abs(w1 - w2)} kg`, `${total * 2} kg`],
                    solution: `Add the weights of both fruits:<br/>
                               ${w1} kg + ${w2} kg = <strong>${total} kg</strong>.`
                };
            } else {
                ...
                if (i === 6) {
                    ...
                    const empty = 2.5;
                    question = {
                        text: `A box full of books weighs <strong>${total} kg</strong>. If the <strong>empty box</strong> weighs <strong>${empty} kg</strong>, what is the mass of the <strong>books</strong> inside?`,
                        correctAnswer: `${total - empty} kg`,
                        options: [`${total - empty} kg`, `${total + empty} kg`, `${total} kg`, `${empty} kg`],
                        solution: `Subtract the weight of the box from the total weight:<br/>
                                   ${total} kg - ${empty} kg = <strong>${total - empty} kg</strong>.`
                    };
                } else if (i < 9) {
                    const start = 10;
                    const soldCount = 5;
                    const soldWeight = 500;
                    const totalSoldG = soldCount * soldWeight;
                    const totalSoldKg = totalSoldG / 1000;
                    question = {
                        text: `A grocer has <strong>${start} kg</strong> of sugar. He sells <strong>${soldCount} packets</strong> of <strong>${soldWeight} g</strong> each. How much sugar is left?`,
                        correctAnswer: `${start - totalSoldKg} kg`,
                        options: [`${start - totalSoldKg} kg`, `${totalSoldKg} kg`, `${start - 1} kg`, `${start - 5} kg`],
                        solution: `1. Find total weight sold: ${soldCount} × ${soldWeight} g = ${totalSoldG} g.<br/>
                                   2. Convert to kg: ${totalSoldG} g = ${totalSoldKg} kg.<br/>
                                   3. Subtract from original: ${start} kg - ${totalSoldKg} kg = <strong>${start - totalSoldKg} kg</strong>.`
                    };
                } else {
                    const w1 = 1.2;
                    const w2 = 800;
                    const w3 = 500;
                    question = {
                        text: `A parcel has three items weighing <strong>${w1} kg</strong>, <strong>${w2} g</strong>, and <strong>${w3} g</strong>. Find the total mass in <strong>kilograms</strong>.`,
                        correctAnswer: `2.5 kg`,
                        options: [`2.5 kg`, `2500 kg`, `2 kg`, `1.5 kg`],
                        solution: `1. Convert grams to kg: ${w2} g = 0.8 kg, ${w3} g = 0.5 kg.<br/>
                                   2. Add all: ${w1} kg + 0.8 kg + 0.5 kg = <strong>2.5 kg</strong>.`
                    };
                }
            }
            questions.push({
    ...
    if (!currentQuestion && !showResults) return <div className="flex justify-center items-center h-screen text-[#31326F] font-bold tracking-widest text-2xl uppercase italic animate-pulse">Processing Weights...</div>;
        ...
        return (
            <div className="junior-practice-page results-view overflow-y-auto" style={{ fontFamily: '"Open Sans", sans-serif' }}>
                <header className="junior-practice-header results-header relative">
                    ...
                    <div className="title-area"><h1 className="results-title">Calculation Report</h1></div>
                </header>
                <main className="practice-content results-content max-w-5xl mx-auto w-full px-4 text-center">
                    <h2 className="text-4xl font-black text-[#31326F] mb-1">Adventure Complete! ⚖️</h2>
...
export default MassCalculation;
