...
export default function DoesItLookSameAssessmentEngine({ questions: questionPool, sampleSize = 10, title, color, onBack, onSecondaryBack }) {
            ...
            <div style={{ maxWidth: 860, margin: '0 auto', padding: '40px 20px' }}>
                <div style={{ textAlign: 'center', marginBottom: 40 }}>
                    ...
                    <div style={{ display: 'flex', gap: 12, justifyContent: 'center', marginTop: 24 }}>
                         <button className="dils-btn-filled" onClick={() => window.location.reload()} style={{ '--skill-color': color, padding: '12px 32px' }}>Retake</button>
                         <button className="dils-btn-outline" onClick={onBack} style={{ padding: '12px 32px' }}>Back to Skills</button>
                         {onSecondaryBack && <button className="dils-btn-outline" onClick={onSecondaryBack} style={{ padding: '12px 32px' }}>Chapter Home</button>}
                    </div>
                </div>
                            ...
                            <div key={i} style={{
                                ...
                                <div style={{ display: 'flex', gap: 12, alignItems: 'flex-start', marginBottom: 16 }}>
                                    ...
                                    <div style={{ flex: 1 }}>
                                        ...
                                        {(q.svg || q.image) && (
                                            <div style={{ 
                                                marginTop: 16, padding: '12px', background: '#fff', borderRadius: 12, 
                                                border: '1px solid rgba(0,0,0,0.05)', display: 'flex', justifyContent: 'center'
                                            }}>
                                                {q.svg ? (
                                                    <div style={{ width: '100%', maxWidth: 200 }}>{q.svg}</div>
                                                ) : (
                                                    <img src={q.image} alt="Question Diagram" style={{ maxWidth: '100%', maxHeight: 120, borderRadius: 8 }} />
                                                )}
                                            </div>
                                        )}
                                    </div>
                                ...
                                {q.explanation && (
                                    <div style={{ marginTop: 16, fontSize: 14, color: '#64748b', lineHeight: 1.6, background: '#fff', padding: '12px 16px', borderRadius: 12, border: '1px solid rgba(0,0,0,0.05)' }}>
                                        <strong style={{ color: '#4f46e5' }}>💡 Explanation: </strong><MathRenderer text={q.explanation} />
                                    </div>
                                )}
                            </div>
                        );
    ...
    return (
        <div style={{ display: 'flex', gap: 24, padding: '10px 0', maxWidth: 1150, margin: '0 auto' }}>
            <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ background: '#fff', borderRadius: 24, padding: '32px', border: '1px solid rgba(0,0,0,0.05)', boxShadow: '0 12px 30px rgba(0,0,0,0.03)' }}>
                    ...
                    {(q.svg || q.image) && (
                        <div style={{ 
                            marginBottom: 24, padding: '24px', background: '#f8fafc', borderRadius: 20, 
                            border: '1px solid #e2e8f0', display: 'flex', justifyContent: 'center', alignItems: 'center'
                        }}>
                            {q.svg ? (
                                <div style={{ width: '100%', maxWidth: 400 }}>{q.svg}</div>
                            ) : (
                                <img src={q.image} alt="Question Diagram" style={{ maxWidth: '100%', maxHeight: 250, borderRadius: 12 }} />
                            )}
                        </div>
                    )}
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 32 }}>
                    <button className="dils-btn-outline" onClick={() => setCurrent(c => Math.max(0, c - 1))} disabled={current === 0}
                        style={{ opacity: current === 0 ? 0 : 1, padding: '10px 24px' }}>← Previous</button>
                    {current + 1 === questions.length ? (
                        <button className="dils-btn-filled" onClick={handleSubmit} style={{ '--skill-color': '#ef4444', padding: '12px 32px' }}>
                            Submit Assessment
                        </button>
                    ) : (
                        <button className="dils-btn-filled" onClick={() => setCurrent(c => c + 1)} style={{ '--skill-color': color, padding: '12px 32px' }}>
                            Next →
                        </button>
                    )}
                </div>
            ...
            <aside style={{ width: 240, background: '#fff', borderRadius: 24, padding: '20px', border: '1px solid rgba(0,0,0,0.05)', boxShadow: '0 4px 20px rgba(0,0,0,0.05)', position: 'sticky', top: 20, height: 'fit-content' }}>
                <div style={{
                    ...
                    {questions.map((_, i) => {
                        ...
                        return (
                            <button key={i} onClick={() => setCurrent(i)}
                                style={{
                                    aspectRatio: '1/1', borderRadius: 8, fontSize: 13, fontWeight: 800, cursor: 'pointer', transition: 'all 0.2s',
                                    display: 'flex', alignItems: 'center', justifyContent: 'center',
                                    border: isCurrent ? `3px solid #0f172a` : '1px solid #e2e8f0',
                                    background: isAnswered ? color : '#f8fafc',
