...
export default function DoesItLookSamePracticeEngine({ questionPool, sampleSize = 20, title, color, onBack , nodeId}) {
    ...
    if (finished) {
        ...
        return (
            <div style={{ textAlign: 'center', padding: '60px 0', maxWidth: 800, margin: '0 auto' }}>
                ...
                <p style={{ color: '#64748b', fontSize: 16, margin: '0 0 32px' }}>Review your progress and keep mastering the world of symmetry!</p>
                <div style={{ display: 'flex', gap: 12, justifyContent: 'center' }}>
                    <button className="dils-btn-filled" onClick={handleRetry} style={{ '--skill-color': color, padding: '12px 32px' }}>🔀 New Questions</button>
                    <button className="dils-btn-outline" onClick={onBack} style={{ padding: '12px 32px' }}>Return to Skills</button>
                </div>
            </div>
        );
    }
    ...
    return (
        <div style={{ maxWidth: 860, margin: '0 auto', padding: '10px 24px' }}>
            ...
            <div style={{ background: '#fff', borderRadius: 24, padding: '32px', boxShadow: '0 12px 30px rgba(0,0,0,0.03)', border: '1px solid rgba(0,0,0,0.05)' }}>
                ...
                {answered && (
                    <div style={{ marginTop: 24, padding: '20px', borderRadius: 16, background: '#f8fafc', border: '1px solid #e2e8f0', color: '#64748b', fontSize: 14, lineHeight: 1.6 }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'center', marginTop: 32 }}>
                <button onClick={handleNext} disabled={!answered}
                    className="dils-btn-filled"
                    style={{ 
                        '--skill-color': color, padding: '12px 48px', borderRadius: 100, fontSize: 15, background: answered ? color : '#cbd5e1', 
