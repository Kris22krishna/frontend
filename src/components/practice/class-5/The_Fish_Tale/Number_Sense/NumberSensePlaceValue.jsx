...
const NumberSensePlaceValue = () => {
    ...
    const SKILL_ID = "FT-06";
    const SKILL_NAME = "Number sense and place value understanding";
    ...
    const generateQuestions = () => {
        ...
        for (let i = 0; i < TOTAL_QUESTIONS; i++) {
            ...
            const multiplier = Math.pow(10, Math.floor(Math.random() * 3) + 1);

            let qText, correctAns;
            if (isMultiply) {
                qText = `What is ${base} × ${multiplier}?`;
                correctAns = base * multiplier;
            } else {
                const dividend = base * multiplier;
                qText = `What is ${dividend} ÷ ${multiplier}?`;
                correctAns = base;
            }

            let opts = [
                correctAns.toLocaleString('en-IN'),
                (correctAns * 10).toLocaleString('en-IN'),
                (correctAns / 10).toLocaleString('en-IN'),
                (correctAns * 100).toLocaleString('en-IN')
            ];

            let detailedSolution;
            if (isMultiply) {
                const zeros = Math.log10(multiplier);
                detailedSolution = `When we multiply by ${multiplier}, we shift the digits ${zeros} place(s) to the left (add ${zeros} zero${zeros > 1 ? 's' : ''} at the end).\n\n${base} × ${multiplier} = ${correctAns.toLocaleString('en-IN')}\n\nThink of it as: ${base} has ${base.toString().length} digits. Adding ${zeros} zero${zeros > 1 ? 's' : ''} gives us a ${(base.toString().length + zeros)}-digit number: ${correctAns.toLocaleString('en-IN')}.`;
            } else {
                const zeros = Math.log10(multiplier);
                const dividend = base * multiplier;
                detailedSolution = `When we divide by ${multiplier}, we shift the digits ${zeros} place(s) to the right (remove ${zeros} zero${zeros > 1 ? 's' : ''} from the end).\n\n${dividend.toLocaleString('en-IN')} ÷ ${multiplier} = ${correctAns.toLocaleString('en-IN')}\n\nThink of it as: ${dividend.toLocaleString('en-IN')} has ${dividend.toString().length} digits. Removing ${zeros} zero${zeros > 1 ? 's' : ''} gives us: ${correctAns.toLocaleString('en-IN')}.`;
            }
    ...
    useEffect(() => {
        startSession({ nodeId: 'a4051001-0001-0000-0000-000000000000', sessionType: 'practice' });
...
export default NumberSensePlaceValue;
