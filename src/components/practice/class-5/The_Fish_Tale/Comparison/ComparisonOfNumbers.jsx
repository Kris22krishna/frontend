...
const ComparisonOfNumbers = () => {
    ...
    const SKILL_ID = "FT-04";
    const SKILL_NAME = "Comparison of numbers";
    ...
    const generateQuestions = () => {
        ...
        for (let i = 0; i < TOTAL_QUESTIONS; i++) {
            ...
            let qText = `Compare the numbers: ${num1.toLocaleString('en-IN')} ___ ${num2.toLocaleString('en-IN')}`;
            let correctAns = num1 > num2 ? '>' : (num1 < num2 ? '<' : '=');
            let opts = ['>', '<', '='];

            const num1Digits = num1.toString().length;
            const num2Digits = num2.toString().length;
            let comparisonExplanation;
            if (num1Digits !== num2Digits) {
                comparisonExplanation = `Step 1: Count the digits.\n• ${num1.toLocaleString('en-IN')} has ${num1Digits} digits\n• ${num2.toLocaleString('en-IN')} has ${num2Digits} digits\n\nThe number with more digits is always greater.\n\nSo ${num1.toLocaleString('en-IN')} ${correctAns} ${num2.toLocaleString('en-IN')}.`;
            } else {
                ...
                for (let j = 0; j < s1.length; j++) {
                    if (s1[j] !== s2[j]) { diffPos = j; break; }
                }
                const places = ['Lakhs', 'Ten Thousands', 'Thousands', 'Hundreds', 'Tens', 'Ones'];
                const placeName = places[diffPos] || `position ${diffPos + 1}`;
                comparisonExplanation = `Step 1: Both numbers have ${num1Digits} digits, so compare digit by digit from left.\n\nStep 2: The first different digit is in the ${placeName} place.\n• ${num1.toLocaleString('en-IN')} has ${s1[diffPos]} in the ${placeName} place\n• ${num2.toLocaleString('en-IN')} has ${s2[diffPos]} in the ${placeName} place\n• Since ${s1[diffPos]} ${correctAns} ${s2[diffPos]}, we get:\n\n${num1.toLocaleString('en-IN')} ${correctAns} ${num2.toLocaleString('en-IN')}.`;
            }

            generated.push({
                text: qText,
                correctAnswer: correctAns,
                solution: comparisonExplanation,
                options: opts
            });
        }
    ...
    useEffect(() => {
        startSession({ nodeId: 'a4051001-0004-0000-0000-000000000000', sessionType: 'practice' });
...
export default ComparisonOfNumbers;
