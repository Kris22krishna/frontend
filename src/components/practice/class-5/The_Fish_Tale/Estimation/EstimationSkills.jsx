...
const EstimationSkills = () => {
    ...
    const SKILL_ID = "FT-07";
    const SKILL_NAME = "Estimation skills";
    ...
    const generateQuestions = () => {
        ...
        for (let i = 0; i < TOTAL_QUESTIONS; i++) {
            ...
            const est1 = Math.round(num1 / 100) * 100;
            const est2 = Math.round(num2 / 100) * 100;
            const isAdd = Math.random() > 0.5;

            let qText = `Estimate the ${isAdd ? 'sum' : 'difference'} by rounding each number to the nearest 100: ${num1} ${isAdd ? '+' : '-'} ${num2}`;
            let correctAns = isAdd ? est1 + est2 : Math.abs(est1 - est2);

            if (!isAdd && num1 < num2) {
                qText = `Estimate the difference by rounding each number to the nearest 100: ${num2} - ${num1}`;
            }

            let opts = [
                correctAns,
                correctAns + 100,
                Math.max(0, correctAns - 100),
                correctAns + 200
            ];
    ...
    useEffect(() => {
        startSession({ nodeId: 'a4051001-0006-0000-0000-000000000000', sessionType: 'practice' });
...
export default EstimationSkills;
