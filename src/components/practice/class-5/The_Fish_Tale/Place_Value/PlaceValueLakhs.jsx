...
const PlaceValueLakhs = () => {
    ...
    const SKILL_ID = "FT-02";
    const SKILL_NAME = "Place value (up to lakhs)";
    ...
    const generateQuestions = () => {
        ...
        for (let i = 0; i < TOTAL_QUESTIONS; i++) {
            ...
            let opts = [
                correctAns,
                `${digit} ${places[(places.indexOf(place) + 1) % places.length].name}`,
                `${digit} ${places[(places.indexOf(place) + 2) % places.length].name}`,
                `${(digit + 1) % 10} ${place.name}`
            ];

            const placeValueNum = digit * place.val;
            const allPlaces = ['Ones', 'Tens', 'Hundreds', 'Thousands', 'Ten Thousands', 'Lakhs'];
            const placePositions = { 'Ones': '1st', 'Tens': '2nd', 'Hundreds': '3rd', 'Thousands': '4th', 'Ten Thousands': '5th', 'Lakhs': '6th' };
            const posLabel = placePositions[place.name] || '';

            opts = [...new Set(opts)].sort(() => Math.random() - 0.5);
            generated.push({
                text: qText,
                correctAnswer: correctAns,
                solution: `In the number ${number.toLocaleString('en-IN')}, the digit ${digit} is in the ${place.name} place (${posLabel} position from the right).\n\nPlace value = digit × position value\n= ${digit} × ${place.val.toLocaleString('en-IN')}\n= ${placeValueNum.toLocaleString('en-IN')}\n\nSo the answer is ${correctAns}.`,
                options: opts
            });
        }
    ...
    useEffect(() => {
        startSession({ nodeId: 'a4051001-0003-0000-0000-000000000000', sessionType: 'practice' });
...
export default PlaceValueLakhs;
