...
const InterpretingRealLifeData = () => {
    ...
    const SKILL_ID = "FT-08";
    const SKILL_NAME = "Interpreting real-life numerical data";
    ...
    const generateQuestions = () => {
        const scenarios = [
            { text: "A boat catches {{v1}} kg of fish in one trip. If it makes {{v2}} trips in a month, how much fish does it catch in a month?", op: 'mul' },
            { text: "If {{v1}} kg of fresh fish gives {{v2}} kg of dried fish, how much dried fish will we get from {{v3}} kg of fresh fish?", op: 'prop' },
            { text: "A motor boat costs Rs {{v1}} and a long tail boat costs Rs {{v2}}. How much more does the motor boat cost?", op: 'sub' },
            { text: "Women in the bank collected Rs {{v1}} in one month. How much will they collect in {{v2}} months?", op: 'mul' }
        ];
        ...
        for (let i = 0; i < TOTAL_QUESTIONS; i++) {
            ...
            if (scenario.op === 'mul') {
                const v1 = Math.floor(Math.random() * 500) + 100;
                const v2 = Math.floor(Math.random() * 20) + 5;
                qText = scenario.text.replace('{{v1}}', v1).replace('{{v2}}', v2);
                correctAns = v1 * v2;
            } else if (scenario.op === 'prop') {
                const v1 = 6000;
                const v2 = 2000; // 1/3 ratio usually
                const v3 = (Math.floor(Math.random() * 5) + 2) * 3000;
                qText = scenario.text.replace('{{v1}}', v1.toLocaleString('en-IN'))
                    .replace('{{v2}}', v2.toLocaleString('en-IN'))
                    .replace('{{v3}}', v3.toLocaleString('en-IN'));
                correctAns = v3 / (v1 / v2);
            } else if (scenario.op === 'sub') {
                const v1 = Math.floor(Math.random() * 500000) + 800000;
                const v2 = Math.floor(Math.random() * 100000) + 50000;
                qText = scenario.text.replace('{{v1}}', v1.toLocaleString('en-IN'))
                    .replace('{{v2}}', v2.toLocaleString('en-IN'));
                correctAns = v1 - v2;
            }

            const formattedCorrect = `${correctAns.toLocaleString('en-IN')}`;
            let opts = [
                formattedCorrect,
                (correctAns + 1000).toLocaleString('en-IN'),
                (correctAns - 500).toLocaleString('en-IN'),
                (correctAns * 2).toLocaleString('en-IN')
            ];

            let detailedSolution;
            if (scenario.op === 'mul') {
                const v1 = parseInt(qText.match(/(\d[\d,]*)/)?.[1]?.replace(/,/g, '') || '0');
                detailedSolution = `This is a multiplication word problem.\n\nStep 1: Identify the values\n• Amount per unit: given in the question\n• Number of units: given in the question\n\nStep 2: Multiply\nResult = ${formattedCorrect}\n\nThe answer is ${formattedCorrect}.`;
            } else if (scenario.op === 'prop') {
                detailedSolution = `This is a proportion problem.\n\nStep 1: Find the ratio\n• 6,000 kg fresh fish → 2,000 kg dried fish\n• Ratio = 6,000 : 2,000 = 3 : 1\n• So for every 3 kg fresh, we get 1 kg dried.\n\nStep 2: Apply the ratio\n• Divide the fresh fish amount by 3\n• Result = ${formattedCorrect} kg dried fish.`;
            } else {
    ...
    useEffect(() => {
        startSession({ nodeId: 'a4051001-0007-0000-0000-000000000000', sessionType: 'practice' });
...
export default InterpretingRealLifeData;
