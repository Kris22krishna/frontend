...
const ReadingWritingBigNumbers = () => {
    ...
    const SKILL_ID = "FT-03";
    const SKILL_NAME = "Reading and writing big numbers";
    ...
    const generateQuestions = () => {
        ...
        for (let i = 0; i < TOTAL_QUESTIONS; i++) {
            const num = Math.floor(Math.random() * 900000) + 100000; // 1,00,000 to 9,99,999
            const words = numberToWords(num);
            const formattedNum = num.toLocaleString('en-IN');

            const isWordToNum = Math.random() > 0.5;
            let qText, correctAns, opts;

            if (isWordToNum) {
                qText = `Write "${words}" in figures.`;
                correctAns = formattedNum;
                opts = [
                    formattedNum,
                    (num + 10000).toLocaleString('en-IN'),
                    (num - 1000).toLocaleString('en-IN'),
                    (num + 100000).toLocaleString('en-IN')
                ];
            } else {
                qText = `Write the number ${formattedNum} in words.`;
                correctAns = words;
                opts = [
                    words,
                    numberToWords(num + 10000),
            ...
            if (isWordToNum) {
                detailedSolution = `To convert words to figures, we break it down:\n• "${words}" uses the Indian number system\n• ${lakhs > 0 ? lakhs + ' Lakh = ' + (lakhs * 100000).toLocaleString('en-IN') : ''}\n• ${tenThousands > 0 ? tenThousands + ' Ten Thousand = ' + (tenThousands * 10000).toLocaleString('en-IN') : ''}\n• ${thousands > 0 ? thousands + ' Thousand = ' + (thousands * 1000).toLocaleString('en-IN') : ''}\n• Remaining: ${rest}\n\nCombining all: ${formattedNum}`;
            } else {
                detailedSolution = `To write ${formattedNum} in words using the Indian system:\n• The number has ${num.toString().length} digits\n• ${lakhs > 0 ? 'Lakhs place: ' + lakhs : ''}\n• ${tenThousands > 0 ? 'Ten Thousands place: ' + tenThousands : ''}\n• ${thousands > 0 ? 'Thousands place: ' + thousands : ''}\n• Remaining: ${rest}\n\nSo we read it as: ${words}`;
            }
    ...
    useEffect(() => {
        startSession({ nodeId: 'a4051001-0002-0000-0000-000000000000', sessionType: 'practice' });
...
export default ReadingWritingBigNumbers;
