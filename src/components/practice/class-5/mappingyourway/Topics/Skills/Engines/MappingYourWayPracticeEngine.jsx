...
export default function MappingYourWayPracticeEngine({ questionPool, sampleSize = 10, title, color, onBack , nodeId}) {
    ...
    const handleRetry = () => {
        const pool = typeof questionPool === 'function' ? questionPool() : questionPool;
        setQuestions(sample(pool, Math.min(sampleSize, pool.length)));
    ...
    const longestOption = q ? q.options.reduce((max, opt) => Math.max(max, opt.length), 0) : 0;
    ...
    if (finished) {
        ...
        return (
            <div style={{ textAlign: 'center', padding: '60px 0', maxWidth: 800, margin: '0 auto' }}>
                ...
                <p style={{ color: '#64748b', fontSize: 16, margin: '0 0 32px' }}>Review your progress and keep diving into maps and scales!</p>
                <div style={{ display: 'flex', gap: 12, justifyContent: 'center' }}>
                    <button className="myw-btn-primary" onClick={handleRetry} style={{ background: color }}>🔀 New Questions</button>
                    <button className="myw-btn-secondary" onClick={onBack}>Return to Skills</button>
                </div>
            </div>
        );
    }
    ...
    return (
        <div style={{ maxWidth: 860, margin: '0 auto', padding: '10px 24px' }}>
            ...
            <div className="myw-quiz-card" style={{ padding: '24px' }}>
                ...
                {answered && (
                    <div style={{ marginTop: 20, padding: '16px', borderRadius: 12, background: '#f8fafc', border: '1px solid #e2e8f0', color: '#64748b', fontSize: 14, lineHeight: 1.5 }}>
                        <strong style={{ color: '#0f766e' }}>💡 Explanation: </strong><MathRenderer text={q.explanation} />
                    </div>
                )}
            </div>
