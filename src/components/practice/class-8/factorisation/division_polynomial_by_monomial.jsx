...
const DivisionPolynomialByMonomialComponent = () => {
    ...
    useEffect(() => { if (answers[qIndex] !== undefined) { const s = history.current[qIndex]; if (s) { setSelectedOption(s.selectedOption); setIsSubmitted(s.isSubmitted); setIsCorrect(s.isCorrect); } } }, [qIndex, answers]);
    ...
    // EASY 1: (6x² + 12x) ÷ 3x = 2x + 4
    const easy1 = () => { const d = randomInt(2, 5), a = randomInt(2, 5), b = randomInt(2, 5); const t1 = d * a, t2 = d * b; return { text: `<div class='question-container'><p>Divide: $(${t1}x^2 + ${t2}x) \\div ${d}x$</p></div>`, correctAnswer: `$${a}x + ${b}$`, solution: `$\\frac{${t1}x^2 + ${t2}x}{${d}x} = \\frac{${t1}x^2}{${d}x} + \\frac{${t2}x}{${d}x} = ${a}x + ${b}$`, options: [`$${a}x + ${b}$`, `$${a}x - ${b}$`, `$${a}x^2 + ${b}$`, `$${b}x + ${a}$`] }; };

    // EASY 2: (8a² + 4a) ÷ 2a = 4a + 2
    const easy2 = () => { const d = randomInt(2, 4), a = randomInt(2, 6), b = randomInt(1, 5); const t1 = d * a, t2 = d * b; return { text: `<div class='question-container'><p>Divide: $(${t1}a^2 + ${t2}a) \\div ${d}a$</p></div>`, correctAnswer: `$${a}a + ${b}$`, solution: `$\\frac{${t1}a^2}{${d}a} + \\frac{${t2}a}{${d}a} = ${a}a + ${b}$`, options: [`$${a}a + ${b}$`, `$${a}a - ${b}$`, `$${b}a + ${a}$`, `$${a}a^2 + ${b}$`] }; };

    // EASY 3: (15y³ + 10y²) ÷ 5y = 3y² + 2y
    const easy3 = () => { const d = randomInt(2, 5), a = randomInt(2, 5), b = randomInt(2, 5); const t1 = d * a, t2 = d * b; return { text: `<div class='question-container'><p>Divide: $(${t1}y^3 + ${t2}y^2) \\div ${d}y$</p></div>`, correctAnswer: `$${a}y^2 + ${b}y$`, solution: `$\\frac{${t1}y^3}{${d}y} + \\frac{${t2}y^2}{${d}y} = ${a}y^2 + ${b}y$`, options: [`$${a}y^2 + ${b}y$`, `$${a}y^3 + ${b}y^2$`, `$${a}y + ${b}$`, `$${b}y^2 + ${a}y$`] }; };

    // MED 1: Three-term polynomial ÷ monomial
    const med1 = () => { const d = randomInt(2, 4), a = randomInt(2, 4), b = randomInt(2, 4), c = randomInt(1, 4); const t1 = d * a, t2 = d * b, t3 = d * c; return { text: `<div class='question-container'><p>Divide: $(${t1}x^3 + ${t2}x^2 + ${t3}x) \\div ${d}x$</p></div>`, correctAnswer: `$${a}x^2 + ${b}x + ${c}$`, solution: `Divide each term by $${d}x$:<br/><br/>$\\frac{${t1}x^3}{${d}x} + \\frac{${t2}x^2}{${d}x} + \\frac{${t3}x}{${d}x} = ${a}x^2 + ${b}x + ${c}$`, options: [`$${a}x^2 + ${b}x + ${c}$`, `$${a}x^3 + ${b}x^2 + ${c}x$`, `$${a}x^2 - ${b}x + ${c}$`, `$${b}x^2 + ${a}x + ${c}$`] }; };

    // MED 2: With subtraction
    const med2 = () => { const d = randomInt(2, 4), a = randomInt(2, 5), b = randomInt(2, 5); const t1 = d * a, t2 = d * b; return { text: `<div class='question-container'><p>Divide: $(${t1}m^3 - ${t2}m^2) \\div ${d}m^2$</p></div>`, correctAnswer: `$${a}m - ${b}$`, solution: `$\\frac{${t1}m^3}{${d}m^2} - \\frac{${t2}m^2}{${d}m^2} = ${a}m - ${b}$`, options: [`$${a}m - ${b}$`, `$${a}m + ${b}$`, `$${a}m^2 - ${b}m$`, `$${b}m - ${a}$`] }; };

    // MED 3: Two variables
    const med3 = () => { const d = randomInt(2, 4), a = randomInt(2, 5), b = randomInt(2, 5); const t1 = d * a, t2 = d * b; return { text: `<div class='question-container'><p>Divide: $(${t1}x^2y + ${t2}xy^2) \\div ${d}xy$</p></div>`, correctAnswer: `$${a}x + ${b}y$`, solution: `$\\frac{${t1}x^2y}{${d}xy} + \\frac{${t2}xy^2}{${d}xy} = ${a}x + ${b}y$`, options: [`$${a}x + ${b}y$`, `$${a}y + ${b}x$`, `$${a}xy + ${b}$`, `$${a}x - ${b}y$`] }; };

    // HARD 1: Four-term polynomial
    const hard1 = () => { const d = randomInt(2, 3), a = randomInt(2, 4), b = randomInt(1, 3), c = randomInt(1, 3), e = randomInt(1, 3); const t1 = d * a, t2 = d * b, t3 = d * c, t4 = d * e; return { text: `<div class='question-container'><p>Divide: $(${t1}x^4 + ${t2}x^3 - ${t3}x^2 + ${t4}x) \\div ${d}x$</p></div>`, correctAnswer: `$${a}x^3 + ${b}x^2 - ${c}x + ${e}$`, solution: `Divide each term by $${d}x$:<br/><br/>$${a}x^3 + ${b}x^2 - ${c}x + ${e}$`, options: [`$${a}x^3 + ${b}x^2 - ${c}x + ${e}$`, `$${a}x^4 + ${b}x^3 - ${c}x^2 + ${e}x$`, `$${a}x^3 - ${b}x^2 + ${c}x - ${e}$`, `$${a}x^2 + ${b}x - ${c} + ${e}$`] }; };

    // HARD 2: Higher power divisor
    const hard2 = () => { const d = randomInt(2, 4), a = randomInt(2, 5), b = randomInt(2, 5), c = randomInt(1, 4); const t1 = d * a, t2 = d * b, t3 = d * c; return { text: `<div class='question-container'><p>Divide: $(${t1}a^4b^2 - ${t2}a^3b^3 + ${t3}a^2b^2) \\div ${d}a^2b^2$</p></div>`, correctAnswer: `$${a}a^2 - ${b}ab + ${c}$`, solution: `Divide each term by $${d}a^2b^2$:<br/><br/>$\\frac{${t1}a^4b^2}{${d}a^2b^2} - \\frac{${t2}a^3b^3}{${d}a^2b^2} + \\frac{${t3}a^2b^2}{${d}a^2b^2}$<br/><br/>$= ${a}a^2 - ${b}ab + ${c}$`, options: [`$${a}a^2 - ${b}ab + ${c}$`, `$${a}a^2 + ${b}ab + ${c}$`, `$${a}a^4 - ${b}a^3b + ${c}$`, `$${b}a^2 - ${a}ab + ${c}$`] }; };

    // HARD 3: Negative signs
    const hard3 = () => { const d = randomInt(2, 4), a = randomInt(2, 5), b = randomInt(2, 5), c = randomInt(2, 5); const t1 = d * a, t2 = d * b, t3 = d * c; return { text: `<div class='question-container'><p>Divide: $(-${t1}x^3 + ${t2}x^2 - ${t3}x) \\div (-${d}x)$</p></div>`, correctAnswer: `$${a}x^2 - ${b}x + ${c}$`, solution: `Dividing each term by $-${d}x$:<br/><br/>$\\frac{-${t1}x^3}{-${d}x} + \\frac{${t2}x^2}{-${d}x} + \\frac{-${t3}x}{-${d}x}$<br/><br/>$= ${a}x^2 - ${b}x + ${c}$`, options: [`$${a}x^2 - ${b}x + ${c}$`, `$-${a}x^2 + ${b}x - ${c}$`, `$${a}x^2 + ${b}x + ${c}$`, `$${a}x^2 - ${b}x - ${c}$`] }; };

    // BONUS
    const bonus = () => { const d = randomInt(2, 4), a = randomInt(2, 5), b = randomInt(2, 5); const t1 = d * a, t2 = d * b; return { text: `<div class='question-container'><p>If $${d}x$ students share $${t1}x^2 + ${t2}x$ sweets equally, how many does each get?</p></div>`, correctAnswer: `$${a}x + ${b}$`, solution: `Each student gets $\\frac{${t1}x^2 + ${t2}x}{${d}x} = ${a}x + ${b}$ sweets`, options: [`$${a}x + ${b}$`, `$${b}x + ${a}$`, `$${a}x - ${b}$`, `$${a}x^2 + ${b}x$`] }; };
