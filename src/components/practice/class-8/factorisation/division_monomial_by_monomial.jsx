...
const DivisionMonomialByMonomialComponent = () => {
    ...
    // EASY 1: 12x³ ÷ 4x = 3x²
    const easy1 = () => { const c = randomInt(2, 5), d = randomInt(2, 4), coeff = c * d; const ep = randomInt(2, 4); return { text: `<div class='question-container'><p>Divide: $${coeff}x^{${ep + 1}} \\div ${d}x$</p></div>`, correctAnswer: `$${c}x^{${ep}}$`, solution: `$\\frac{${coeff}x^{${ep + 1}}}{${d}x} = \\frac{${coeff}}{${d}} \\times x^{${ep + 1}-1} = ${c}x^{${ep}}$`, options: [`$${c}x^{${ep}}$`, `$${c}x^{${ep + 1}}$`, `$${d}x^{${ep}}$`, `$${c + 1}x^{${ep}}$`] }; };

    // EASY 2: 18a² ÷ 6a = 3a
    const easy2 = () => { const c = randomInt(2, 6), d = randomInt(2, 5), coeff = c * d; return { text: `<div class='question-container'><p>Divide: $${coeff}a^2 \\div ${d}a$</p></div>`, correctAnswer: `$${c}a$`, solution: `$\\frac{${coeff}a^2}{${d}a} = \\frac{${coeff}}{${d}} \\times a^{2-1} = ${c}a$`, options: [`$${c}a$`, `$${c}a^2$`, `$${d}a$`, `$${c + 1}a$`] }; };

    // EASY 3: 20xy ÷ 5x = 4y
    const easy3 = () => { const c = randomInt(2, 6), d = randomInt(2, 5), coeff = c * d; return { text: `<div class='question-container'><p>Divide: $${coeff}xy \\div ${d}x$</p></div>`, correctAnswer: `$${c}y$`, solution: `$\\frac{${coeff}xy}{${d}x} = \\frac{${coeff}}{${d}} \\times \\frac{xy}{x} = ${c}y$`, options: [`$${c}y$`, `$${c}x$`, `$${c}xy$`, `$${d}y$`] }; };

    // MED 1: 24x³y² ÷ 8xy = 3x²y
    const med1 = () => { const c = randomInt(2, 5), d = randomInt(2, 4), coeff = c * d; const e1 = randomInt(2, 3), e2 = randomInt(2, 3); return { text: `<div class='question-container'><p>Divide: $${coeff}x^{${e1 + 1}}y^{${e2}} \\div ${d}xy$</p></div>`, correctAnswer: `$${c}x^{${e1}}y^{${e2 - 1}}$`, solution: `$\\frac{${coeff}x^{${e1 + 1}}y^{${e2}}}{${d}xy} = ${c} \\times x^{${e1 + 1}-1} \\times y^{${e2}-1} = ${c}x^{${e1}}y^{${e2 - 1}}$`, options: [`$${c}x^{${e1}}y^{${e2 - 1}}$`, `$${c}x^{${e1 + 1}}y^{${e2}}$`, `$${d}x^{${e1}}y^{${e2 - 1}}$`, `$${c}x^{${e1 - 1}}y^{${e2}}$`] }; };

    // MED 2: -36a²b³ ÷ 9ab = -4ab²
    const med2 = () => { const c = randomInt(2, 5), d = randomInt(2, 4), coeff = c * d; return { text: `<div class='question-container'><p>Divide: $-${coeff}a^2b^3 \\div ${d}ab$</p></div>`, correctAnswer: `$-${c}ab^2$`, solution: `$\\frac{-${coeff}a^2b^3}{${d}ab} = -${c} \\times a^{2-1} \\times b^{3-1} = -${c}ab^2$`, options: [`$-${c}ab^2$`, `$${c}ab^2$`, `$-${c}a^2b$`, `$-${d}ab^2$`] }; };

    // MED 3: coefficient division with power
    const med3 = () => { const c = randomInt(3, 7), d = randomInt(2, 4), coeff = c * d; const e1 = randomInt(3, 5); return { text: `<div class='question-container'><p>Divide: $${coeff}m^{${e1}} \\div ${d}m^2$</p></div>`, correctAnswer: `$${c}m^{${e1 - 2}}$`, solution: `$\\frac{${coeff}m^{${e1}}}{${d}m^2} = ${c} \\times m^{${e1}-2} = ${c}m^{${e1 - 2}}$`, options: [`$${c}m^{${e1 - 2}}$`, `$${c}m^{${e1}}$`, `$${c}m^{${e1 - 1}}$`, `$${d}m^{${e1 - 2}}$`] }; };

    // HARD 1: Three-variable monomial
    const hard1 = () => { const c = randomInt(2, 4), d = randomInt(2, 3), coeff = c * d; return { text: `<div class='question-container'><p>Divide: $${coeff}x^3y^2z^4 \\div ${d}xyz^2$</p></div>`, correctAnswer: `$${c}x^2yz^2$`, solution: `$\\frac{${coeff}x^3y^2z^4}{${d}xyz^2} = ${c} \\times x^{3-1} \\times y^{2-1} \\times z^{4-2} = ${c}x^2yz^2$`, options: [`$${c}x^2yz^2$`, `$${c}x^3y^2z^4$`, `$${c}xyz^2$`, `$${d}x^2yz^2$`] }; };

    // HARD 2: Negative with higher powers
    const hard2 = () => { const c = randomInt(3, 6), d = randomInt(2, 4), coeff = c * d; return { text: `<div class='question-container'><p>Divide: $-${coeff}p^4q^3 \\div (-${d}p^2q)$</p></div>`, correctAnswer: `$${c}p^2q^2$`, solution: `$\\frac{-${coeff}p^4q^3}{-${d}p^2q} = +${c} \\times p^{4-2} \\times q^{3-1} = ${c}p^2q^2$<br/><br/>(Negative ÷ Negative = Positive)`, options: [`$${c}p^2q^2$`, `$-${c}p^2q^2$`, `$${c}p^4q^3$`, `$${c}pq^2$`] }; };

    // HARD 3: Large coefficients
    const hard3 = () => { const c = randomInt(4, 8), d = randomInt(3, 6), coeff = c * d; return { text: `<div class='question-container'><p>Divide: $${coeff}a^5b^3c^2 \\div ${d}a^2b^2c$</p></div>`, correctAnswer: `$${c}a^3bc$`, solution: `$\\frac{${coeff}a^5b^3c^2}{${d}a^2b^2c} = ${c} \\times a^{5-2} \\times b^{3-2} \\times c^{2-1} = ${c}a^3bc$`, options: [`$${c}a^3bc$`, `$${c}a^2bc$`, `$${c}a^3b^2c$`, `$${d}a^3bc$`] }; };

    // BONUS
    const bonus = () => { const c = randomInt(2, 5), d = randomInt(2, 4), coeff = c * d; return { text: `<div class='question-container'><p>If the area of a rectangle is $${coeff}x^3y^2$ and its breadth is $${d}xy$, find its length.</p></div>`, correctAnswer: `$${c}x^2y$`, solution: `Length $= \\frac{\\text{Area}}{\\text{Breadth}} = \\frac{${coeff}x^3y^2}{${d}xy} = ${c}x^2y$`, options: [`$${c}x^2y$`, `$${c}xy$`, `$${c}x^3y$`, `$${d}x^2y$`] }; };
