...
export default function CQAssessmentEngine({ questionPool, sampleSize = 10, title, color, onBack }) {
    ...
    if (!questions.length) {
        return (
            <div style={{ maxWidth: 860, margin: '0 auto', padding: '20px', textAlign: 'center' }}>
                ...
                <button className="cq-btn-secondary" onClick={onBack}>Back to Skills</button>
            </div>
        );
    }
    ...
    // ── ACTIVE ASSESSMENT ─────────────────────────────────────────────────────
    return (
        <div className="cq-assessment-layout">
            {/* ── LEFT Question Panel ──────────────────── */}
            <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 20 }}>
                    <button className="cq-btn-secondary" onClick={() => setCurrent((c) => Math.max(0, c - 1))} disabled={current === 0}
                        style={{ visibility: current === 0 ? 'hidden' : 'visible' }}>← Previous</button>
                    {current + 1 === questions.length ? (
                        <button className="cq-btn-primary" onClick={handleSubmit} style={{ background: '#ef4444' }}>
                            Submit Assessment
                        </button>
                    ) : (
                        <button className="cq-btn-primary" onClick={() => setCurrent((c) => c + 1)} style={{ background: color }}>
                            Next →
                        </button>
                    )}
                </div>
            ...
            {/* ── RIGHT Sidebar ─────────────────────────── */}
            <div className="cq-assessment-palette">
                <div style={{
                ...
                <button className="cq-btn-primary" onClick={handleSubmit}
