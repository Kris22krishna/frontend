...
export default function ProportionsPracticeEngine({ questionPool, sampleSize = 20, title, color, onBack, nodeId }) {
    ...
    if (finished) {
        ...
        const sub = pct >= 90 ? 'Excellent understanding of proportions!' : 'Review the concepts and try again.';
        return (
            <div style={{ textAlign: 'center', padding: '40px 0' }}>
                ...
                <div style={{ display: 'flex', gap: 12, justifyContent: 'center' }}>
                    <button className="prop-btn-primary" onClick={handleRetry} style={{ background: color }}>🔀 New Questions</button>
                    <button className="prop-btn-secondary" onClick={onBack}>Return to Skills</button>
                </div>
            </div>
        );
    }
    ...
    return (
        <div className="prop-quiz-container">
            <div style={{ marginBottom: 20 }}>
                <div className="prop-score-header">
                    <div>
            ...
            <div className="prop-quiz-card">
                ...
                <div style={{ fontSize: 17, fontWeight: 600, color: '#0f172a', lineHeight: 1.6, marginBottom: 20 }}>{q.question}</div>
                    ...
                    {q.options.map((opt, oi) => {
                        ...
                        return (
                            <button key={oi} onClick={() => handleSelect(oi)} disabled={answered}
                                style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '14px 16px', borderRadius: 12, border: `2.5px solid ${border}`, background: bg, cursor: answered ? 'default' : 'pointer', fontSize: 15, color: txtColor, textAlign: 'left', transition: 'all 0.2s', fontWeight: selected === oi ? 700 : 500 }}>
                ...
                {answered && (
                    <div style={{ marginTop: 20, padding: '14px 18px', borderRadius: 12, background: 'rgba(59,130,246,0.05)', border: '1px solid rgba(59,130,246,0.1)', color: '#64748b', fontSize: 13.5, lineHeight: 1.6 }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'center', marginTop: 16 }}>
                <button onClick={handleNext} disabled={!answered}
                    style={{ padding: '12px 40px', background: answered ? color : '#f1f5f9', color: answered ? '#fff' : '#94a3b8', cursor: answered ? 'pointer' : 'not-allowed', border: 'none', borderRadius: 100, fontSize: 15, fontWeight: 800, boxShadow: answered ? `0 8px 20px ${color}30` : 'none', transition: 'all 0.2s' }}>
