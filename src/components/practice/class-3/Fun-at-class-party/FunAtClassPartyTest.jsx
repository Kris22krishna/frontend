...
import '../../../../pages/juniors/grade3/fun-at-class-partytest.css';
import mascotImg from '../../../../assets/mascot.png';



const SKILL_ID = 3003;
const SKILL_NAME = "Fun at Class Party - Chapter Test";

const FunAtClassPartyTest = () => {
    ...
    const generateQuestions = () => { const qs = []; for (let i = 1; i <= 15; i++) { if (i <= 7) { qs.push({ id: i, text: 'Which string is longer: ' + (i + 5) + ' cm or ' + (i + 8) + ' cm?', options: ['' + (i + 8) + ' cm', '' + (i + 5) + ' cm', 'Both are equal', 'Cannot be determined'], correctAnswer: '' + (i + 8) + ' cm', solution: (i + 8) + ' is greater than ' + (i + 5) + '.' }); } else { qs.push({ id: i, text: 'A table is ' + (i + 1) + ' meters long. How many centimeters is it?', options: ['' + ((i + 1) * 100) + ' cm', '' + ((i + 1) * 10) + ' cm', '' + ((i + 1) * 1000) + ' cm', '' + (i + 1) + ' cm'], correctAnswer: '' + ((i + 1) * 100) + ' cm', solution: '1 meter = 100 cm. So ' + (i + 1) + ' meters = ' + ((i + 1) * 100) + ' cm.' }); } } return qs; };
    useEffect(() => {
...
export default FunAtClassPartyTest;
