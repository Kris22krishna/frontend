...
const SKILL_ID = 3001;
const SKILL_NAME = "Raksha Bandhan - Chapter Test";

const RakshaBandhanTest = () => {
    ...
    const generateQuestions = () => { const qs = []; for (let i = 1; i <= 15; i++) { if (i <= 5) { qs.push({ id: i, text: 'If you have ' + (i + 2) + ' packets of sweets and each contains 5 sweets, how many sweets are there in total?', options: ['' + ((i + 2) * 5), '' + ((i + 2) * 4), '' + ((i + 2) * 6), '' + ((i + 2) * 5 + 1)], correctAnswer: '' + ((i + 2) * 5), solution: 'Multiply ' + (i + 2) + ' by 5 to get ' + ((i + 2) * 5) + '.' }); } else if (i <= 10) { let num = (i % 5 + 2) * 4; qs.push({ id: i, text: 'Divide ' + num + ' rakhis equally among 4 brothers. How many does each brother get?', options: ['' + (num / 4), '' + ((num / 4) + 1), '' + ((num / 4) - 1), '' + (num / 4 + 2)], correctAnswer: '' + (num / 4), solution: 'Dividing ' + num + ' by 4 gives ' + (num / 4) + '.' }); } else { qs.push({ id: i, text: 'A sister bought a gift for Rs ' + (i * 10) + ' and a rakhi for Rs ' + (i * 2) + '. Total cost is?', options: ['Rs ' + (i * 12), 'Rs ' + (i * 10), 'Rs ' + (i * 15), 'Rs ' + (i * 11)], correctAnswer: 'Rs ' + (i * 12), solution: 'Add ' + (i * 10) + ' and ' + (i * 2) + ' to get ' + (i * 12) + '.' }); } } return qs; };
    useEffect(() => {
...
export default RakshaBandhanTest;
