...
const evaluateAnswer = (type, data, state) => {
    ...
    switch (type) {
        ...
        case 'tally_count': return parseInt(state.userAns) === data.answer;
...
function VisualCountQ({ data, color, onHasInput, onAnswer, checkTrigger, saved, isAssessment }) {
    ...
    for (let r = 0; r < data.rows; r++) {
        const items = [];
        for (let c = 0; c < data.perRow; c++) items.push(<span key={c} style={{ fontSize: 18, animation: saved ? 'none' : `dhBounceIn 0.3s ${(r * data.perRow + c) * 0.03}s both` }}>{data.emoji}</span>);
...
// ── TALLY COUNT ──
function TallyCountQ({ data, color, onHasInput, onAnswer, checkTrigger, saved, isAssessment }) {
    const [userAns, setUserAns] = useState(saved?.userAns || '');
    const [result, setResult] = useState(saved?.result || null);
    const checkedRef = useRef(false);
    ...
    const drawTally = (n) => {
        ...
        for (let i = 0; i < bundles; i++) {
            els.push(
                <div key={`b${i}`} style={{ position: 'relative', width: 44, height: 60, display: 'inline-block', margin: '0 12px' }}>
                    <div style={{ position: 'absolute', left: 4, top: 0, width: 5, height: 60, background: '#334155', borderRadius: 3 }}></div>
                    <div style={{ position: 'absolute', left: 16, top: 0, width: 5, height: 60, background: '#334155', borderRadius: 3 }}></div>
                    <div style={{ position: 'absolute', left: 28, top: 0, width: 5, height: 60, background: '#334155', borderRadius: 3 }}></div>
                    <div style={{ position: 'absolute', left: 40, top: 0, width: 5, height: 60, background: '#334155', borderRadius: 3 }}></div>
                    <div style={{ position: 'absolute', left: -4, top: 28, width: 56, height: 5, background: '#ef4444', borderRadius: 3, transform: 'rotate(-25deg)', transformOrigin: 'center' }}></div>
                </div>
            );
        }
        if (singles > 0) {
            ...
            for (let i = 0; i < singles; i++) {
                sEls.push(<div key={`s${i}`} style={{ position: 'absolute', left: 4 + (i * 12), top: 0, width: 5, height: 60, background: '#334155', borderRadius: 3 }}></div>);
            }
            els.push(<div key="singles" style={{ position: 'relative', width: singles * 12 + 4, height: 60, display: 'inline-block', margin: '0 12px' }}>{sEls}</div>);
        }
        return <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10, justifyContent: 'center', margin: '20px 0' }}>{els}</div>;
    };

    return (
        <div style={{ textAlign: 'center' }}>
            <p style={{ fontSize: 24, fontWeight: 900, color: '#1e293b', lineHeight: 1.4, marginBottom: 12 }}>{data.q}</p>
            {data.category && <div style={{ fontSize: 18, color: '#64748b', fontWeight: 800, marginBottom: 16 }}>Category: <span style={{ color }}>{data.category}</span></div>}

            <div style={{ background: '#f8fafc', borderRadius: 20, padding: '24px', marginBottom: 24, border: '1px solid #f1f5f9' }}>
                {drawTally(data.answer)}
            </div>

            <div style={{ display: 'flex', gap: 10, alignItems: 'center', justifyContent: 'center' }}>
                <span style={{ fontSize: 20, fontWeight: 800, color: '#475569' }}>Total =</span>
                <input type="number" value={userAns} onChange={e => setUserAns(e.target.value)} disabled={!!result && !isAssessment}
                    placeholder="?" style={{ width: 100, padding: '12px 18px', borderRadius: 14, fontSize: 24, fontWeight: 900, border: `2px solid ${result && !isAssessment ? (result === 'correct' ? '#10b981' : '#ef4444') : color + '40'}`, outline: 'none', fontFamily: 'Outfit, sans-serif', textAlign: 'center', background: result && !isAssessment ? (result === 'correct' ? '#f0fdf4' : '#fef2f2') : '#fff' }} />
                {!isAssessment && result === 'correct' && <span style={{ fontSize: 32 }}>🎉</span>}
                {!isAssessment && result === 'wrong' && <span style={{ fontSize: 32 }}>😢</span>}
            </div>
            {!isAssessment && <ResultFeedback result={result} expl={data.expl} correctAnswer={data.answer} />}
...
function FillBlankQ({ data, color, onHasInput, onAnswer, checkTrigger, saved, isAssessment }) {
    ...
    return (
        <div>
            ...
            {data.table && (
                <table style={{ margin: '0 auto 24px', borderCollapse: 'separate', borderSpacing: 0, width: '100%', maxWidth: 500, background: '#fff', borderRadius: 12, overflow: 'hidden', border: `2px solid ${color}30` }}>
                    <thead>
                        <tr style={{ background: `${color}15` }}>
                            <th style={{ padding: '16px', borderBottom: `2px solid ${color}30`, borderRight: `2px solid ${color}30`, color, fontFamily: 'Outfit, sans-serif', fontSize: 18 }}>{data.tableHeaders[0]}</th>
                            <th style={{ padding: '16px', borderBottom: `2px solid ${color}30`, color, fontFamily: 'Outfit, sans-serif', fontSize: 18 }}>{data.tableHeaders[1]}</th>
                        </tr>
                    </thead>
                    <tbody>
                        {data.table.map((row, i) => (
                            <tr key={i}>
                                <td style={{ padding: '14px', borderBottom: i < data.table.length - 1 ? `1px solid ${color}20` : 'none', borderRight: `2px solid ${color}30`, textAlign: 'center', fontSize: 18, fontWeight: 700, color: '#334155' }}>{row[0]}</td>
                                <td style={{ padding: '14px', borderBottom: i < data.table.length - 1 ? `1px solid ${color}20` : 'none', textAlign: 'center', fontSize: 18, fontWeight: 700, color: row[1] === '?' ? color : '#334155' }}>{row[1]}</td>
                            </tr>
                        ))}
                    </tbody>
                </table>
...
function GroupMakerQ({ data, color, onHasInput, onAnswer, checkTrigger, saved, isAssessment }) {
    ...
    const allItems = [];
    for (let i = 0; i < data.total; i++) allItems.push(<span key={i} style={{ fontSize: 20, animation: saved ? 'none' : `dhBounceIn 0.3s ${i * 0.03}s both` }}>{data.emoji}</span>);
    const groups = [];
    let arrIdx = 0;
    ...
    return (
        <div style={{ background: '#fff', borderRadius: 24, padding: '28px 32px', boxShadow: '0 8px 30px rgba(0,0,0,0.04)', maxWidth: 800, margin: '0 auto' }}>
            ...
            <div key={idx} style={{ minHeight: 200 }}>
                ...
                {cur.type === 'tally_count' && <TallyCountQ data={cur} color={color} onHasInput={setHasInput} onAnswer={handleAnswer} checkTrigger={checkTrigger} saved={curState.savedState} />}
                            ...
                            <div key={i} style={{ border: '2px solid #f1f5f9', borderRadius: 16, padding: '20px 24px', background: '#fafafa' }}>
                                ...
                                {q.type === 'tally_count' && (
                                    <div style={{ marginLeft: 46, marginBottom: 16 }}>
                                        <div style={{ transform: 'scale(0.6)', transformOrigin: 'left top', marginBottom: -24 }}>
                                            {(() => {
                                                const bundles = Math.floor(q.answer / 5);
                                                const singles = q.answer % 5;
                                                const els = [];
                                                for (let bi = 0; bi < bundles; bi++) {
                                                    els.push(
                                                        <div key={`b${bi}`} style={{ position: 'relative', width: 44, height: 60, display: 'inline-block', margin: '0 12px' }}>
                                                            <div style={{ position: 'absolute', left: 4, top: 0, width: 5, height: 60, background: '#334155', borderRadius: 3 }}></div>
                                                            <div style={{ position: 'absolute', left: 16, top: 0, width: 5, height: 60, background: '#334155', borderRadius: 3 }}></div>
                                                            <div style={{ position: 'absolute', left: 28, top: 0, width: 5, height: 60, background: '#334155', borderRadius: 3 }}></div>
                                                            <div style={{ position: 'absolute', left: 40, top: 0, width: 5, height: 60, background: '#334155', borderRadius: 3 }}></div>
                                                ...
                                                if (singles > 0) {
                                                    const sEls = [];
                                                    for (let si = 0; si < singles; si++) {
                                                        sEls.push(<div key={`s${si}`} style={{ position: 'absolute', left: 4 + (si * 12), top: 0, width: 5, height: 60, background: '#334155', borderRadius: 3 }}></div>);
                                                    }
                                                    els.push(<div key="singles" style={{ position: 'relative', width: singles * 12 + 4, height: 60, display: 'inline-block', margin: '0 12px' }}>{sEls}</div>);
                                                }
                                                return <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10 }}>{els}</div>;
                                            })()}
                                        </div>
                                    </div>
                                )}
                                {q.table && (
                                    <table style={{ margin: '0 0 16px 46px', borderCollapse: 'separate', borderSpacing: 0, width: '100%', maxWidth: 400, background: '#fff', borderRadius: 8, overflow: 'hidden', border: `1px solid #e2e8f0` }}>
                                        <thead>
                                            <tr style={{ background: `#f8fafc` }}>
                                                <th style={{ padding: '8px 12px', borderBottom: `1px solid #e2e8f0`, borderRight: `1px solid #e2e8f0`, color: '#475569', fontSize: 14 }}>{q.tableHeaders[0]}</th>
                                                <th style={{ padding: '8px 12px', borderBottom: `1px solid #e2e8f0`, color: '#475569', fontSize: 14 }}>{q.tableHeaders[1]}</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            {q.table.map((row, ri) => (
                                                <tr key={ri}>
                                                    <td style={{ padding: '8px 12px', borderBottom: ri < q.table.length - 1 ? `1px solid #f1f5f9` : 'none', borderRight: `1px solid #e2e8f0`, textAlign: 'center', fontSize: 14, fontWeight: 700, color: '#334155' }}>{row[0]}</td>
                                                    <td style={{ padding: '8px 12px', borderBottom: ri < q.table.length - 1 ? `1px solid #f1f5f9` : 'none', textAlign: 'center', fontSize: 14, fontWeight: 700, color: row[1] === '?' ? '#ef4444' : '#334155' }}>{row[1]}</td>
                                                </tr>
                                            ))}
                                        </tbody>
                                    </table>
                                ...
                                {q.type === 'visual_count' && (
                                    <div style={{ marginLeft: 46, marginBottom: 16, opacity: 0.8 }}>
                                        {Array.from({ length: q.rows }).map((_, ri) => (
                                            <div key={ri} style={{ display: 'flex', gap: 4, marginBottom: 4 }}>
                                                {Array.from({ length: q.perRow }).map((_, ci) => <span key={ci} style={{ fontSize: 16 }}>{q.emoji}</span>)}
                                            </div>
                                        ))}
                                    </div>
                                )}
                                {q.type === 'picture_problem' && (
                                    <div style={{ marginLeft: 46, marginBottom: 16 }}>
                                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4, opacity: 0.8, maxWidth: 300 }}>
                                            {Array.from({ length: Math.min(q.sceneCount, 20) }).map((_, i) => <span key={i} style={{ fontSize: 16 }}>{q.scene}</span>)}
                                        </div>
                                        {q.perItem && <div style={{ fontSize: 12, color: '#64748b', fontWeight: 700, marginTop: 4 }}>{q.perItem}</div>}
                                    </div>
                                )}
        ...
        <div style={{ display: 'flex', gap: 20, alignItems: 'stretch', maxWidth: 1100, margin: '0 auto', height: 'calc(100vh - 120px)' }}>
            ...
            <div style={{ flex: '1 1 600px', background: '#fff', borderRadius: 24, padding: '28px 32px', boxShadow: '0 8px 30px rgba(0,0,0,0.04)', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
                ...
                <div style={{ flex: 1, overflowY: 'auto' }}>
                    ...
                    {cur.type === 'tally_count' && <TallyCountQ key={idx} data={cur} color={color} onHasInput={handleHasInput} onAnswer={() => { }} checkTrigger={0} saved={curState.savedState} isAssessment={true} />}
                    ...
                    <>
                        <div style={{ textAlign: 'center', marginBottom: 32 }}>
                            <h1 style={{ fontFamily: 'Outfit', fontSize: '2.6rem', fontWeight: 900, margin: '0 0 8px', color: '#0f172a' }}>
                                Master <span style={{ background: 'linear-gradient(135deg, #059669, #0d9488)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>{SKILLS.length} Data Handling Skills</span>
                            </h1>
