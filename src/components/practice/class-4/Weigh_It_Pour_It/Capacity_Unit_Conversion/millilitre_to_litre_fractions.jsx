...
const CORRECT_MESSAGES = [
    "✨ Refreshing! That's correct! ✨",
    "🌟 Splash! You got it right! 🌟",
    "🎉 Correct! You're a capacity captain! 🎉",
    "✨ Fantastic! ✨",
    "🚀 Super! Full to the brim! 🚀"
];

const MillilitreToLitreFractions = () => {
    ...
    const SKILL_ID = 1136;
    const SKILL_NAME = "Weigh It, Pour It - Millilitre to Litre Fractions";
    ...
    useEffect(() => {
        ...
        // Logic based on Page 9 of PDF: "How many 500ml bottles fill a 1l bottle?"
        const questions = [];
        let attempts = 0;
        ...
        while (questions.length < TOTAL_QUESTIONS && attempts < 100) {
            ...
            if (type === 1) { // How many X ml bottles fill a 1 L bottle?
                const volume = [500, 250, 100, 200][randomInt(0, 3)];
                const count = 1000 / volume;

                // Generate bottles visual
                const bottleHtml = `
                    <div style="width: 40px; height: 80px; background: #E6FFFA; border: 2px solid #38B2AC; border-radius: 5px 5px 15px 15px; display: flex; align-items: center; justify-content: center; color: #2C7A7B; font-weight: bold; font-size: 0.8rem; position: relative;">
                        <div style="position: absolute; top: -10px; left: 10px; width: 16px; height: 10px; background: #E6FFFA; border: 2px solid #38B2AC; border-bottom: none;"></div>
                        ${volume}ml
                    </div>
                 `;

                qText = `<div class="flex flex-col items-center gap-4">
                            <span class="text-3xl font-medium">How many <strong>${volume}ml</strong> bottles will fill a <strong>1 Litre</strong> bottle?</span>
                            <div class="flex gap-2 flex-wrap justify-center mt-4 items-end">
                                ${Array(3).fill(0).map(() => bottleHtml).join('')}
                                <span class="text-4xl self-center text-gray-400">...</span>
                                <div style="width: 60px; height: 120px; background: #bee3f8; border: 3px solid #3182ce; border-radius: 8px 8px 20px 20px; display: flex; align-items: center; justify-content: center; color: #2b6cb0; font-weight: bold; position: relative; margin-left: 20px;">
                                    <div style="position: absolute; top: -15px; left: 15px; width: 24px; height: 15px; background: #bee3f8; border: 3px solid #3182ce; border-bottom: none;"></div>
                                    1 L
                                </div>
                            </div>
                          </div>`;
                correct = count.toString();
                sol = `1 Litre = 1000 ml.<br/>
                        $${volume} \\times ${count} = 1000$.<br/>
                        So, <strong>${count}</strong> bottles are needed.`;

                options = [count.toString()];
                while (options.length < 4) {
                    const wrong = randomInt(2, 20).toString();
                    if (!options.includes(wrong)) options.push(wrong);
                }

            } else if (type === 2) { // 1/2 Litre = ? ml
                const fractions = [
                    { num: 1, den: 2, val: 500 },
                    { num: 1, den: 4, val: 250 },
                    { num: 3, den: 4, val: 750 },
                    { num: 1, den: 10, val: 100 },
                    { num: 1, den: 5, val: 200 }
                ];
                const f = fractions[randomInt(0, 4)];

                qText = `<div class="flex flex-col items-center gap-4">
                            <span class="text-3xl font-medium">What is <strong>$\\frac{${f.num}}{${f.den}}$</strong> of a Litre?</span>
                            <div class="mt-4">
                               <div style="width: 100px; height: 150px; background: white; border: 3px solid #3182ce; border-radius: 0 0 10px 10px; position: relative; overflow: hidden;">
                                    <div style="position: absolute; bottom: 0; left: 0; right: 0; height: ${f.val / 10}%; background: #90cdf4; transition: height 1s ease;"></div>
                                    <div style="position: absolute; top: 50%; width: 100%; text-align: center; font-weight: bold; z-index: 10;">? ml</div>
                                    
                                    <!-- Markings -->
                                    <div style="position: absolute; top: 0; right: 0; width: 10px; border-bottom: 1px solid black; opacity: 0.3;"></div>
                                    <div style="position: absolute; top: 25%; right: 0; width: 10px; border-bottom: 1px solid black; opacity: 0.3;"></div>
                                    <div style="position: absolute; top: 50%; right: 0; width: 10px; border-bottom: 1px solid black; opacity: 0.3;"></div>
                                    <div style="position: absolute; top: 75%; right: 0; width: 10px; border-bottom: 1px solid black; opacity: 0.3;"></div>
                               </div>
                            </div>
                          </div>`;
                ...
                correct = f.val + " ml";
                sol = `1 Litre = 1000 ml.<br/>
                       $\\frac{${f.num}}{${f.den}} \\times 1000 = ${f.val}$ ml.`;

                options = [correct];
                options.push((1000 - f.val) + " ml");
                options.push((f.val / 2) + " ml");
                options.push((f.val + 100) + " ml");

            } else { // Addition: 500ml + 250ml = ?
                const v1 = [500, 250, 200, 100][randomInt(0, 3)];
                const v2 = [500, 250, 200, 100][randomInt(0, 3)];
                const total = v1 + v2;

                qText = `<div class="flex flex-col items-center gap-4">
                            <span class="text-3xl font-medium">${v1} ml + ${v2} ml = ?</span>
                          </div>`;
                correct = total + " ml";
                sol = `$${v1} + ${v2} = ${total}$ ml.`;
                options = [total + " ml", (total + 100) + " ml", (total - 50) + " ml", (total * 2) + " ml"];
            }
            ...
            <div className="junior-practice-page results-view overflow-y-auto" style={{ fontFamily: '"Open Sans", sans-serif' }}>
                <header className="junior-practice-header results-header relative">
                    ...
                    <div className="title-area">
                        <h1 className="results-title">Capacity Report</h1>
                    </div>
                </header>

                <main className="practice-content results-content max-w-5xl mx-auto w-full px-4">
                    <div className="results-hero-section flex flex-col items-center mb-8">
                        <h2 className="text-4xl font-black text-[#31326F] mb-2">Volume Victor! 🎉</h2>
                    ...
                    <div className="detailed-breakdown w-full mb-12">
                        <h3 className="text-2xl font-black text-[#31326F] mb-6 px-4">Pouring Log 📜</h3>
                                            ...
                                            {shuffledOptions.map((option, idx) => (
                                                <button
                                                    ...
                                                    style={{
                                                        ...
                                                        fontWeight: '400',
                                                        fontSize: '2rem', // Reduced font size for longer text options
...
export default MillilitreToLitreFractions;
