...
export default function TickingClocksSkills() {
    ...
    const goSkill = (skill, mode) => { setActiveSkill(skill); setView(mode); };
    const backToList = () => { setView('list'); setActiveSkill(null); };

    return (
        <div className="tc-skills-page">
            <nav className="tc-nav">
                ...
                <div className="tc-nav-links">
                    ...
                    <button className="tc-nav-link tc-nav-link--active" onClick={() => navigate('/ticking-clocks/skills')}>🎯 Skills</button>
                </div>
            </nav>

            <div style={{ maxWidth: 1000, margin: '20px auto 40px', padding: '0 24px' }}>
                {view === 'list' && (
                    <>
                        <div style={{ textAlign: 'center', marginBottom: 32 }}>
                            <h1 style={{ fontFamily: 'Outfit', fontSize: '2.6rem', fontWeight: 900, margin: '0 0 8px' }}>
                                Master <span style={{ background: 'linear-gradient(135deg, var(--tc-amber), var(--tc-orange))', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>7 Time Skills</span>
                            </h1>
                        ...
                        <div className="tc-skills-list">
                            {SKILLS.map(skill => (
                                <div key={skill.id} className="tc-skill-card">
                                    <div className="tc-skill-info">
                                        <div className="tc-skill-icon" style={{ background: `${skill.color}15` }}>
                                            <span>{skill.icon}</span>
                                        </div>
                                        <div>
                                            <div style={{ fontFamily: 'Outfit', fontSize: 18, fontWeight: 800, color: '#0f172a' }}>{skill.title}</div>
                                            <div style={{ fontSize: 13, color: '#64748b', marginTop: 2 }}>{skill.desc}</div>
                                        </div>
                                    </div>
                                    <div className="tc-skill-actions">
                                        <button className="tc-btn-secondary" onClick={() => goSkill(skill, 'learn')} style={{ padding: '8px 18px', fontSize: 13 }}>📘 Learn</button>
                                        <button className="tc-btn-primary" onClick={() => goSkill(skill, 'practice')} style={{ padding: '8px 18px', fontSize: 13 }}>✏️ Practice</button>
                                        <button className="tc-btn-primary" onClick={() => goSkill(skill, 'assess')} style={{ padding: '8px 18px', fontSize: 13, background: 'linear-gradient(135deg, #7c3aed, #a855f7)', boxShadow: '0 4px 12px rgba(124,58,237,0.3)' }}>🎯 Assess</button>
                                    </div>
                                </div>
                            ))}
                        </div>
                ...
                {view === 'learn' && activeSkill && <LearnView skill={activeSkill} onBack={backToList} />}

                {view === 'practice' && activeSkill && (
                    <div className="tc-quiz-container">
                        <QuizEngine questions={PRACTICE_QS[activeSkill.id]} title={`${activeSkill.title} — Practice`} color={activeSkill.color} onBack={backToList} />
                    </div>
                )}

                {view === 'assess' && activeSkill && (
                    <div className="tc-quiz-container">
                        <QuizEngine questions={ASSESS_QS[activeSkill.id]} title={`${activeSkill.title} — Assessment`} color={activeSkill.color} onBack={backToList} />
                    </div>
                )}
            </div>
        </div>
