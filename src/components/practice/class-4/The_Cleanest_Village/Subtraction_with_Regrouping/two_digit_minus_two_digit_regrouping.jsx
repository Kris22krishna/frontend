...
const CORRECT_MESSAGES = [
    "✨ Great subtraction skills! ✨",
    "🌟 You're taking away the mess! 🌟",
    "🎉 Correct! The village is cleaner now! 🎉",
    "✨ Fantastic calculation! ✨",
    "🚀 Super! You're making a difference! 🚀",
    "🌿 Perfect! Nature thanks you! 🌿",
    "🎊 Great job! Moving on... 🎊",
    "💎 Spot on! Excellent! 💎"
];

const TwoDigitSubtractionRegrouping = () => {
    ...
    const SKILL_ID = 4005;
    const SKILL_NAME = "The Cleanest Village - Subtraction with Regrouping (2-Digit)";
    ...
    useEffect(() => {
        ...
        while (questions.length < TOTAL_QUESTIONS) {
            // Generate numbers where borrowing is needed
            // A - B
            // Tens of A > Tens of B
            // Ones of A < Ones of B
            let tensA = randomInt(3, 9);
            let tensB = randomInt(1, tensA - 1);

            let onesA = randomInt(0, 8);
            let onesB = randomInt(onesA + 1, 9);

            let num1 = parseInt(`${tensA}${onesA}`);
            let num2 = parseInt(`${tensB}${onesB}`);

            // Just to be safe, ensure num1 > num2
            ...
            if (!seenCombinations.has(comboKey)) {
                ...
                const templates = [
                    `<div class='question-container' style='font-size: 2.5rem; font-weight: bold; text-align: center;'>
                        ${num1} - ${num2} = ?
                     </div>`
                ];
                ...
                while (uniqueOptions.length < 4) {
                    let rand = (total + randomInt(-5, 5)).toString();
                    if (rand !== total.toString() && !uniqueOptions.includes(rand) && parseInt(rand) > 0) uniqueOptions.push(rand);
                }

                questions.push({
                    text: templates[0],
                    correctAnswer: total.toString(),
                    solution: `Write ${num1} above ${num2}.<br/>
                               Since ${onesA} < ${onesB}, borrow 1 ten from ${tensA}.<br/>
                               ${10 + onesA} - ${onesB} = ${total % 10}.<br/>
                               ${tensA - 1} - ${tensB} = ${Math.floor(total / 10)}.<br/>
            ...
            <div className="junior-practice-page results-view overflow-y-auto">
                ...
                <main className="practice-content results-content max-w-5xl mx-auto w-full px-4">
                    ...
                    <div className="detailed-breakdown w-full mb-12">
                        <h3 className="text-2xl font-black text-[#31326F] mb-6 px-4">Subtraction Log 📜</h3>
...
export default TwoDigitSubtractionRegrouping;
