...
import { RefreshCw, Check, Eye, ChevronRight, ChevronLeft, X, Star, Zap, Heart, Smile } from 'lucide-react';
...
const NODE_ID = 'a4041011-0005-0000-0000-000000000000';
const randomInt = (min, max) => Math.floor(Math.random() * (max - min + 1)) + min;

const CORRECT_MESSAGES = [
    "✨ Double magic! ✨",
    "🌟 You're a doubling pro! 🌟",
    "🎉 Twice as nice! 🎉",
    "✨ Great mental math! ✨",
    "🚀 Super! Double score! 🚀",
    "🌿 Excellent! 🌿",
...
// Doubling Visual Component: Shows two identical groups
const DoublingVisual = ({ number, itemType = 'star' }) => {
    return (
        <div className="flex gap-8 items-center justify-center my-6">
            {/* Group 1 */}
            <div className="flex flex-col items-center">
                <div className="flex flex-wrap justify-center gap-2 p-4 bg-blue-50 rounded-2xl border-2 border-blue-200" style={{ maxWidth: '180px' }}>
                    {Array.from({ length: number }).map((_, i) => (
                        ...
                        >
                            {itemType === 'star' && <Star size={24} fill="currentColor" />}
                            {itemType === 'heart' && <Heart size={24} fill="currentColor" className="text-pink-500" />}
                            {itemType === 'zap' && <Zap size={24} fill="currentColor" className="text-amber-500" />}
                        </motion.div>
                    ))}
                </div>
                <div className="mt-2 text-xl font-bold text-blue-600">{number}</div>
            </div>
...
const DoublingNumbers = () => {
  ...
  useEffect(() => {
    ...
    const SKILL_ID = 1147;
    const SKILL_NAME = "Equal Groups - Doubling Numbers";
    ...
    useEffect(() => {
      ...
      v4IsFinishedRef.current = false; setSessionId(sess.session_id);
        ...
        const questions = [];
        const types = ['star', 'heart', 'zap'];

        while (questions.length < TOTAL_QUESTIONS) {
            const index = questions.length;
            let q = {};
            const itemType = types[randomInt(0, 2)];

            // Easy: Visual Doubling (Numbers 1-10)
            if (index < 3) {
                const number = randomInt(1, 6); // Keep it small for visual sanity
                const double = number * 2;

                q = {
                    text: `What is <strong>Double of ${number}</strong>?`,
                    correctAnswer: double.toString(),
                    solution: `Double of ${number} means adding ${number} to itself.<br/>${number} + ${number} = ${double}.`,
                    type: 'visual_basic',
                    visualData: { number, itemType }
                };

                const opts = new Set([double.toString()]);
                opts.add((number + 2).toString());
                opts.add((double + 2).toString());
                opts.add(number.toString());
                q.shuffledOptions = [...opts].sort(() => Math.random() - 0.5);

            }
            // Medium: Number Patterns (10-20), Mental Math
            else if (index < 6) {
                const number = randomInt(7, 25);
                const double = number * 2;

                // Mix of phrasing
                const phrase = Math.random() > 0.5 ? `Double of ${number}` : `2 times ${number}`;

                q = {
                    text: `What is <strong>${phrase}</strong>?`,
                    correctAnswer: double.toString(),
                    solution: `To double ${number}, calculate ${number} + ${number} or ${number} \\times 2.<br/>Result is ${double}.`,
                    type: 'mental_math'
                };
                ...
                opts.add((double + 2).toString());
                opts.add((double - 2).toString());
                opts.add((number + 10).toString()); // Common error
                q.shuffledOptions = [...opts].sort(() => Math.random() - 0.5);
            }
            // Hard: Word Problems involving doubling
            else {
                const scenarios = [
                    { t: "ant", l: 6 },
                    { t: "spider", l: 8 },
                    { t: "dog", l: 4 },
                    { t: "octopus", l: 8 },
                    { t: "bicycle wheel", l: 1 } // Spokes? No, maybe hands on clock
                ];
                const scenario = scenarios[randomInt(0, 3)]; // stick to basics
                const number = scenario.l;
                const double = number * 2;

                q = {
                    text: `An ${scenario.t} has <strong>${number}</strong> legs.<br/>How many legs do <strong>2</strong> ${scenario.t}s have in total?`,
                    correctAnswer: double.toString(),
                    solution: `One ${scenario.t} has ${number} legs.<br/>Two ${scenario.t}s have double that amount.<br/>$${number} \\times 2 = ${double}$.`,
                    type: 'word_problem'
                };

                const opts = new Set([double.toString()]);
                opts.add((number + 2).toString());
                opts.add((double + 4).toString());
                opts.add(number.toString());
                q.shuffledOptions = [...opts].sort(() => Math.random() - 0.5);
            }
            ...
            <div className="junior-practice-page results-view overflow-y-auto" style={{ fontFamily: '"Open Sans", sans-serif' }}>
                <header className="junior-practice-header results-header relative">
                    ...
                    <div className="title-area">
                        <h1 className="results-title">Double Up!</h1>
                    </div>
                </header>
                                ...
                                <div className="question-card-modern" style={{ paddingLeft: '2rem' }}>
                                    <div className="question-header-modern">
                                        ...
                                        {currentQuestion.visualData && (
                                            <DoublingVisual
                                                number={currentQuestion.visualData.number}
                                                itemType={currentQuestion.visualData.itemType}
                                            />
                                        )}
                                    </div>
...
export default DoublingNumbers;
