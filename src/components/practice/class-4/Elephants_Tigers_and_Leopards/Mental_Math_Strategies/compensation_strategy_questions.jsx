...
const CORRECT_MESSAGES = [
    "✨ Fair Trade! You kept it balanced! ✨",
    "🌟 Your mental math is incredible! 🌟",
    "🎉 Correct! The sum remains perfect! 🎉",
    "✨ Fantastic work! ✨",
    "🚀 Super! Keep striving for excellence! 🚀",
    "🌿 Perfect! Nature loves your efficiency! 🌿",
    "🎊 Great job! Moving on... 🎊",
    "💎 Spot on! Excellent! 💎"
];
...
const CompensationStrategy = () => {
    ...
    const generateQuestion = (difficulty, index) => {
        ...
        if (difficulty === 'easy') {
            ...
            explanation = `In subtraction, if you ADD to one number, you must ADD the same amount to the other to keep the difference unchanged.<br/>So, **Add ${adj}** to ${s1} too.`;
            ...
            explanation = `We give ${adj} to ${n} to make it ${nNew}.<br/>We must take ${adj} from ${m} to keep the sum the same.<br/>$$${n} + ${m} = ${nNew} + ${mNew}$$.`;
        }

        const distractors = new Set([correctAnswer]);
        while (distractors.size < 4) {
            if (difficulty === 'easy') {
                let d = parseInt(correctAnswer) + randomInt(-5, 5);
                if (d > 0 && d !== parseInt(correctAnswer)) distractors.add(d.toString());
            } else if (difficulty === 'hard') {
                const parts = correctAnswer.split(' + ');
                const nP = parseInt(parts[0]);
                const mP = parseInt(parts[1]);
                if (distractors.size < 4) distractors.add(`${nP} + ${mP + randomInt(1, 4)}`);
                if (distractors.size < 4) distractors.add(`${nP - randomInt(1, 4)} + ${mP}`);
                if (distractors.size < 4) distractors.add(`${nP + randomInt(1, 4)} + ${mP + randomInt(1, 4)}`);
            }
        }
            ...
            <div className="junior-practice-page results-view overflow-y-auto" style={{ fontFamily: '"Open Sans", sans-serif' }}>
                ...
                <main className="practice-content results-content max-w-5xl mx-auto w-full px-4">
                    <div className="results-hero-section flex flex-col items-center mb-8">
                        <h2 className="text-4xl font-black text-[#31326F] mb-2">Math Balancer! ⚖️</h2>
                    ...
                    <div className="detailed-breakdown w-full mb-12">
                        <h3 className="text-2xl font-black text-[#31326F] mb-6 px-4">Balance Log 📜</h3>
                        ...
                        <AnimatePresence mode="wait">
                            <motion.div key={qIndex} initial={{ x: 50, opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ x: -50, opacity: 0 }} transition={{ duration: 0.4, ease: "easeOut" }} style={{ height: '100%', width: '100%' }}>
                                <div className="question-card-modern" style={{ padding: '0 0 2rem 0' }}>
                                    <div className="w-full bg-[#E3F2FD] p-8 mb-8 rounded-b-[3rem] border-b-4 border-blue-200 flex flex-col items-center">
                                        <div className="flex justify-center gap-4 mb-6">
                                            <div className="bg-white/50 p-4 rounded-full shadow-sm border-2 border-blue-100"><GitMerge size={48} className="text-blue-600" /></div>
                                            <div className="bg-white/30 p-4 rounded-full shadow-sm border-2 border-blue-50"><ArrowLeftRight size={48} className="text-blue-400" /></div>
                                        </div>
