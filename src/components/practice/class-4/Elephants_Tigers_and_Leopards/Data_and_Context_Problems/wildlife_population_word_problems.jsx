            ...
            <main className="practice-content-wrapper flex items-center justify-center min-h-[calc(100vh-200px)] p-4 relative top-[-20px]">
                <div className="w-full max-w-6xl bg-white/90 backdrop-blur-sm rounded-[3rem] shadow-xl border-4 border-[#E0FBEF] p-6 lg:p-10 flex flex-col md:flex-row gap-8 items-stretch">
                    ...
                    <div className="flex-1 flex flex-col justify-start border-b-2 md:border-b-0 md:border-r-2 border-dashed border-gray-200 pb-6 md:pb-0 md:pr-8">
                        <div className="w-full bg-[#E0FBEF] p-4 lg:p-6 mb-6 rounded-3xl border-b-4 border-[#4FB7B3]/20 flex flex-col items-center shadow-sm">
                            <h3 className="text-[#31326F] font-normal text-lg lg:text-xl uppercase tracking-widest mb-4 flex items-center gap-2 text-center">
                                <span className="w-2 h-2 rounded-full bg-[#4FB7B3] hidden sm:block"></span> {currentQuestion && currentQuestion.parkName} Census <span className="w-2 h-2 rounded-full bg-[#4FB7B3] hidden sm:block"></span>
                            </h3>
                            <div className="w-full max-w-xl bg-white rounded-2xl shadow-sm border-2 border-[#4FB7B3]/30 overflow-hidden">
                                <div className="grid grid-cols-[auto_1fr_auto] gap-x-4 gap-y-0 text-base lg:text-lg">
                                    <div className="bg-[#31326F] text-white font-normal p-3 col-span-3 grid grid-cols-[auto_1fr_auto] gap-4 items-center">
                                    ...
                                    {currentQuestion && currentQuestion.tableData.map((item, idx) => (
                                        <div key={idx} className="contents group">
                                            <div className={`p-3 flex items-center justify-center font-normal text-2xl lg:text-3xl ${idx % 2 === 0 ? 'bg-gray-50' : 'bg-white'} border-b border-gray-100 last:border-0`}><span className="transform group-hover:scale-125 transition-transform">{item.icon}</span></div>
                                            <div className={`p-3 flex items-center font-normal text-[#31326F] ${idx % 2 === 0 ? 'bg-gray-50' : 'bg-white'} border-b border-gray-100 last:border-0`}>{item.name}</div>
                                            <div className={`p-3 flex items-center justify-end font-normal text-[#4FB7B3] text-xl lg:text-2xl ${idx % 2 === 0 ? 'bg-gray-50' : 'bg-white'} border-b border-gray-100 last:border-0`}>{item.count}</div>
                                        </div>
                                    ))}
                                </div>
                            </div>
                    ...
                    <div className="flex-1 flex flex-col justify-center items-center">
                        ...
                        {isSubmitted && (
                            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className={`mt-8 font-normal text-xl md:text-2xl text-center px-6 py-3 rounded-2xl ${isCorrect ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                                {isCorrect ? feedbackMessage : "Check your numbers!"}
                            </motion.div>
                        )}
                    </div>
                </div>
