...
const AppleIcon = () => <span className="text-3xl">🍎</span>;
const BananaIcon = () => <span className="text-3xl">🍌</span>;

const CORRECT_MESSAGES = [
    "✨ Brilliant analysis! Great gathering! ✨",
    "🌟 Your calculations are spot on! 🌟",
    "🎉 Correct! The feast is ready thanks to you! 🎉",
    "✨ Fantastic work! ✨",
...
const JungleGatheringAddition = () => {
    ...
    const generateQuestion = (difficulty, index) => {
        ...
        if (difficulty === 'easy') {
            ...
            ans = n1 + n2 + n3;
            items = [
                { name: "Stones", count: n1, icon: "🪨" },
                { name: "Logs", count: n2, icon: "🪵" },
                { name: "Vines", count: n3, icon: "🌿" }
            ];
            ...
            ans = n1 + n2 + n3 + n4;
            items = [
                { name: "North Herd", count: n1, icon: "🐘" },
                { name: "South Herd", count: n2, icon: "🐅" },
                { name: "East Herd", count: n3, icon: "🦓" },
                { name: "West Herd", count: n4, icon: "🦒" }
            ];
            ...
            <div className="junior-practice-page results-view overflow-y-auto" style={{ fontFamily: '"Open Sans", sans-serif' }}>
                ...
                <main className="practice-content results-content max-w-5xl mx-auto w-full px-4">
                    <div className="results-hero-section flex flex-col items-center mb-8">
                        <h2 className="text-4xl font-black text-[#31326F] mb-2">Gathering Complete! 🧺</h2>
                    ...
                    <div className="detailed-breakdown w-full mb-12">
                        <h3 className="text-2xl font-black text-[#31326F] mb-6 px-4">Gathering Log 📜</h3>
                        ...
                        <AnimatePresence mode="wait">
                            <motion.div key={qIndex} initial={{ x: 50, opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ x: -50, opacity: 0 }} transition={{ duration: 0.4, ease: "easeOut" }} style={{ height: '100%', width: '100%' }}>
                                <div className="question-card-modern" style={{ padding: '0 0 2rem 0' }}>
                                    <div className="w-full bg-[#FFF8E1] p-8 mb-8 rounded-b-[3rem] border-b-4 border-orange-200 flex flex-col items-center">
                                        <div className="flex flex-wrap justify-center gap-4 mb-4">
                                            {currentQuestion.items.map((item, idx) => (
                                                <div key={idx} className="bg-white px-4 py-2 rounded-2xl shadow-sm border-2 border-orange-100 flex items-center gap-2">
                                                    <span className="text-2xl">{item.icon}</span>
                                                    <span className="text-[#31326F] font-bold">{item.count}</span>
                                                </div>
                                            ))}
                                        </div>
