...
import { coordinateGeometryConnectomicsData } from './CoordinateGeometryConnectomicsData';
import '../../CoordinateGeometryBranch.css';
import MathRenderer from '../../../../../MathRenderer';

export default function CoordinateGeometryConnectomics() {
        ...
        <div className="rn-page">
            <nav className="rn-nav">
                ...
                <div className="rn-nav-links">
                    ...
                    <button className="rn-nav-link active" onClick={() => navigate('/senior/grade/10/coordinate-geometry/connectomics')}>🔗 Connectomics</button>
                </div>
            </nav>

            <div className="rn-hero" style={{ background: 'linear-gradient(145deg, #0f172a 0%, #1d4ed8 100%)' }}>
                <h1 className="rn-hero-title">Coordinate Geometry <span style={{ color: '#f59e0b' }}>Connectomics</span></h1>
                <p className="rn-hero-sub">Discover how plotting points powers GPS, computer graphics, equations, and architectural design.</p>
            </div>
            ...
            <main className="rn-topic-shell" style={{ maxWidth: '1100px', margin: '0 auto', padding: '40px 24px' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(320px, 100%), 1fr))', gap: '24px', marginBottom: '60px' }}>
                    {coordinateGeometryConnectomicsData.connections.map((conn, idx) => (
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(280px, 100%), 1fr))', gap: '24px' }}>
                    {coordinateGeometryConnectomicsData.realWorld.map((app, idx) => (
                ...
                <div style={{ marginTop: '60px', textAlign: 'center', background: '#f8fafc', padding: '40px', borderRadius: '32px', border: '2px dashed #e2e8f0' }}>
                    ...
                    <h3 style={{ fontSize: '20px', fontWeight: 800, color: '#1e1b4b' }}>One Plane, Many Uses</h3>
                    <p style={{ maxWidth: '600px', margin: '0 auto', color: '#64748b' }}>
                        Coordinate geometry bridges algebra and geometry, giving us the tools to analyze shapes visually and computationally at the same time.
                    </p>
                </div>
            </main>
        </div>
