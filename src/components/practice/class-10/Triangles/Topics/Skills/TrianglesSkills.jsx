...
export default function TrianglesSkills() {
    ...
    if (view === 'list') {
        return (
            <div className="tri-page">
                <nav className="tri-nav">
                    <button className="tri-nav-back" onClick={() => navigate('/senior/grade/10/triangles')}>
                        ← Back to Dashboard
                    </button>
                    <div className="tri-nav-links">
                        <button className="tri-nav-link" onClick={() => navigate('/senior/grade/10/triangles/introduction')}>🌟 Intro</button>
                        <button className="tri-nav-link" onClick={() => navigate('/senior/grade/10/triangles/terminology')}>📖 Terminology</button>
                        <button className="tri-nav-link active" onClick={() => setView('list')}>🎯 Skills</button>
                        <button className="tri-nav-link" onClick={() => navigate('/senior/grade/10/triangles/connectomics')}>🔗 Connectomics</button>
                    </div>
                </nav>

                <div className="tri-hero">
                    <h1 className="tri-hero-title">Core <span style={{ color: '#93c5fd' }}>Skills</span></h1>
                    <p className="tri-hero-sub">Practice makes perfect. Choose a skill to start your journey.</p>
                </div>

                <div className="tri-skills-grid">
                    {SKILLS.map(s => (
                        <div key={s.id} className="tri-skill-card" style={{ '--skill-color': s.color }}>
                            <div className="tri-skill-icon" style={{ background: `${s.color}15`, color: s.color }}>
                                {s.icon}
                            </div>
                            <div className="tri-skill-info">
                                <h3 className="tri-skill-title">{s.title}</h3>
                                <p className="tri-skill-desc"><MathRenderer text={s.desc} /></p>
                            </div>
                            <div className="tri-skill-actions">
                                <button className="tri-btn tri-btn-outline" onClick={() => { setActiveSkillId(s.id); setView('learn'); }}>Learn</button>
                                <button className="tri-btn tri-btn-outline" onClick={() => { setActiveSkillId(s.id); setView('practice'); }}>Practice</button>
                                <button className="tri-btn tri-btn-filled" style={{ '--skill-color': s.color }} onClick={() => { setActiveSkillId(s.id); setView('assess'); }}>Assess</button>
                            </div>
                        </div>
                    ))}
                </div>
    ...
    return (
        <div className="tri-page">
            <nav className="tri-nav">
                <button className="tri-nav-back" onClick={() => {
                ...
                <div className="tri-nav-links">
                    <button className={`tri-nav-link ${view === 'learn' ? 'active' : ''}`} onClick={() => setView('learn')}>📖 Learn</button>
                    <button className={`tri-nav-link ${view === 'practice' ? 'active' : ''}`} onClick={() => setView('practice')}>🎯 Practice</button>
                    <button className={`tri-nav-link ${view === 'assess' ? 'active' : ''}`} onClick={() => setView('assess')}>🏆 Assess</button>
                </div>
            </nav>
...
function LearnView({ skill, onPractice, mobileLearnDetailView, setMobileLearnDetailView, selectedCategory, setSelectedCategory }) {
    ...
    return (
        <>
            {/* Desktop View */}
            <div className="tri-details-window skills-desktop-only">
                ...
                <div style={{ fontSize: 18, color: 'var(--tri-text)', lineHeight: 1.7, marginBottom: 32, whiteSpace: 'pre-line' }}>
                    <MathRenderer text={skill.learn.content} />
                </div>

                <div className="tri-responsive-grid-2">
                    <div>
                        ...
                        {skill.learn.rules.map((rule, idx) => (
                            <div key={idx} style={{ display: 'flex', gap: 12, marginBottom: 16 }}>
                                ...
                                <p style={{ margin: 0, fontSize: 15, color: 'var(--tri-muted)' }}><MathRenderer text={rule} /></p>
                            </div>
                        ))}
                    </div>
                    <div>
                        <h4 style={{ textTransform: 'uppercase', fontSize: 12, letterSpacing: 2, color: 'var(--tri-teal)', marginBottom: 16, fontWeight: 800 }}>Quick Examples</h4>
                        {skill.learn.examples.map((ex, idx) => (
                            <div key={idx} style={{ background: '#f8fafc', padding: 16, borderRadius: 14, border: '1px solid #e2e8f0', marginBottom: 16 }}>
                                <div style={{ color: 'var(--tri-indigo)', fontWeight: 800, fontSize: 14, marginBottom: 4 }}>Q: <MathRenderer text={ex.q} /></div>
                                <div style={{ color: 'var(--tri-text)', fontSize: 15 }}>A: <MathRenderer text={ex.a} /></div>
                            </div>
                        ))}
                    </div>
                </div>
                ...
                <div style={{ marginTop: 40, textAlign: 'center' }}>
                    <button className="tri-btn-filled" style={{ '--skill-color': skill.color, padding: '14px 40px' }} onClick={onPractice}>
                ...
                {!mobileLearnDetailView ? (
                    <div className="sl-selection-grid">
                        <div style={{ textAlign: 'center', marginBottom: '24px' }}>
                            ...
                            <h2 style={{ fontSize: '24px', fontWeight: 900, color: '#0f172a', margin: 0 }}>{skill.learn.title}</h2>
                        </div>
                        {categories.map((cat) => (
                            <div key={cat.id} className="sl-item-card" onClick={() => { setSelectedCategory(cat.id); setMobileLearnDetailView(true); }}>
                                ...
                                <div className="sl-item-info">
                                    <div className="sl-item-title">{cat.title}</div>
                                    <div className="sl-item-desc">{cat.desc}</div>
                                </div>
                                <div className="sl-item-arrow">→</div>
                            </div>
                        ))}
                        <button className="tri-btn-filled" style={{ background: skill.color, width: '100%', marginTop: '20px', padding: '16px', borderRadius: '16px' }} onClick={onPractice}>
                            Start Practice →
                        </button>
                    </div>
                ) : (
...
/* ── Practice View Component (Enhanced Layout) ── */
function PracticeView({ skill, onAssess }) {
    ...
    const handleSelect = (optIdx) => {
        ...
        setResponses(prev => ({
            ...prev,
            [current]: { selected: optIdx, isCorrect: optIdx === q.correct }
        }));
    };
    ...
    if (finished) {
        ...
        return (
            <div className="tri-report-card" style={{
                ...
                <div style={{ display: 'flex', gap: '12px', justifyContent: 'center', marginTop: '24px' }}>
                    <button className="tri-btn tri-btn-outline" onClick={resetPractice}>Try Again</button>
                    <button className="tri-btn tri-btn-filled" style={{ '--skill-color': color }} onClick={onAssess}>Take Assessment🏆</button>
                </div>
            </div>
        );
    }
    ...
    return (
        <div className="tri-quiz-engine-practice" style={{ maxWidth: '900px', margin: '0 auto', padding: '10px 0' }}>
            <div className="tri-assess-main" style={{
                ...
                <div className="tri-responsive-grid-2" style={{ marginBottom: '20px' }}>
                    {q.options.map((opt, i) => {
...
/* ── Assess View Component (Exact Layout Sync) ── */
function AssessView({ skill, onComplete }) {
    ...
    const handleSelect = (optIdx) => {
        ...
        setResponses(prev => ({
            ...prev,
            [qIdx]: { selected: optIdx, isCorrect: optIdx === q.correct }
        }));
    };
    ...
    if (finished) {
        ...
        return (
            <div className="tri-report-container" style={{ maxWidth: '1000px', margin: '0 auto', padding: '20px' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: '15px', marginBottom: '30px' }}>
                    <div style={{ background: '#EFF6FF', border: '1px solid #DBEAFE', padding: '15px', borderRadius: '16px', textAlign: 'center' }}>
                    ...
                    <div style={{ background: '#F0FDF4', border: '1px solid #DCFCE7', padding: '15px', borderRadius: '16px', textAlign: 'center' }}>
                    ...
                    <div style={{ background: '#FEF2F2', border: '1px solid #FEE2E2', padding: '15px', borderRadius: '16px', textAlign: 'center' }}>
                    ...
                    <div style={{ background: '#F8FAFC', border: '1px solid #E2E8F0', padding: '15px', borderRadius: '16px', textAlign: 'center' }}>
                    ...
                    {questions.map((question, idx) => {
                        ...
                        return (
                            <div key={idx} style={{
                                ...
                                <div className="tri-responsive-grid-2" style={{ marginBottom: '20px', marginLeft: '40px' }}>
                ...
                <div style={{ textAlign: 'center', marginTop: '40px', display: 'flex', gap: '16px', justifyContent: 'center' }}>
                    <button className="tri-btn tri-btn-outline" onClick={() => window.location.reload()}>Retake Test</button>
                    <button className="tri-btn tri-btn-filled" onClick={onComplete} style={{ '--skill-color': skill.color, padding: '12px 40px' }}>Return to Skills</button>
                </div>
            </div>
        );
    }
    ...
    return (
        <div className="tri-quiz-engine-exam" style={{ maxWidth: '1300px', margin: '0 auto', padding: '10px 0' }}>
            ...
            <div className="tri-assess-layout">
                <div className="tri-assess-main">
                    <div style={{ display: 'flex', flexDirection: 'column' }}>
                        ...
                        <div className="tri-responsive-grid-2" style={{ marginBottom: '16px' }}>
                            {q.options.map((opt, i) => {
                                const isSelected = selected === i;
                                let borderColor = '#f1f5f9';
                                let bgColor = '#fff';
                ...
                <div className="tri-assess-sidebar">
                    <div className="tri-palette-window" style={{ position: 'sticky', top: '100px' }}>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: '8px', marginBottom: '20px' }}>
                            {questions.map((_, idx) => {
                                ...
                                const isVisited = visited[idx] && !isAnswered && idx !== qIdx;
                                ...
                                if (isCurrent) {
                                    bgColor = '#EFF6FF'; borderColor = color; textColor = color;
                                } else if (isAnswered) {
