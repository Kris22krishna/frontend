        ...
        return (
            <div className="exam-report-container" style={{ maxWidth: '1000px', margin: '0 auto', padding: '20px' }}>
                ...
                <div style={{ textAlign: 'center', marginTop: '40px' }}>
                    <button className="prob-btn-primary" onClick={onBack} style={{ padding: '15px 40px', background: color }}>
                        Return to Skills
                    </button>
                </div>
            </div>
        ...
        <div className="quiz-engine-exam" style={{ maxWidth: isAssessment ? '1300px' : '900px', margin: '0 auto', padding: '10px 0' }}>
            ...
            <div className="quiz-main-layout" style={{ display: 'flex', gap: '20px', flexDirection: isAssessment ? 'row' : 'column', alignItems: 'flex-start' }}>
                ...
                <div style={{ flex: 1, width: '100%' }}>
                    <div className="prob-quiz-card" style={{
...
export default function ProbabilitySkills() {
            ...
            <div className="skills-page" style={{ paddingTop: '80px' }}>
                <nav className="intro-nav">
                    ...
                    <div className="intro-nav-links">
                        <button className="intro-nav-link" onClick={() => navigate('/senior/grade/10/probability/introduction')}>🌟 Intro</button>
                ...
                <div style={{ padding: '0 24px 60px' }}>
                    {view === 'learn' ? (
                        <div style={{ maxWidth: 1000, margin: '0 auto' }}>
                            ...
                            <div className="prob-learn-grid">
                                <aside className="prob-learn-sidebar" style={{ background: '#fff', padding: 10, borderRadius: 20, border: '1.5px solid #e2e8f0', display: 'flex', flexDirection: 'column', gap: 6 }}>
                                ...
                                <main className="prob-details-window" key={selectedLearnIdx} style={{ background: '#fff', borderRadius: 20, padding: '24px 32px', border: '1.5px solid #e2e8f0', boxShadow: '0 10px 40px rgba(0,0,0,0.03)' }}>
                                    ...
                                    <div style={{ marginTop: 24, display: 'flex', gap: 12 }}>
                                        <button className="prob-btn-primary" style={{ background: skill.color, padding: '10px 24px', fontSize: 14 }} onClick={() => setView('practice')}>Start Practice →</button>
                                        <button className="prob-btn-secondary" style={{ padding: '10px 24px', fontSize: 14 }} onClick={() => setSelectedLearnIdx((selectedLearnIdx + 1) % skill.learn.rules.length)}>Next Rule</button>
                                    </div>
                                </main>
                            </div>
                        </div>
    ...
    return (
        <div className="skills-page">
            <nav className="intro-nav">
                <button className="intro-nav-back" onClick={() => navigate('/senior/grade/10/probability')}>← Back to Hub</button>
                <div className="intro-nav-links">
                    <button className="intro-nav-link" onClick={() => navigate('/senior/grade/10/probability/introduction')}>🌟 Introduction</button>
                    <button className="intro-nav-link" onClick={() => navigate('/senior/grade/10/probability/terminology')}>📖 Terminology</button>
                    <button className="intro-nav-link intro-nav-link--active">🎯 Skills</button>
                </div>
            </nav>
            ...
            <div style={{ maxWidth: 1100, margin: '10px auto', padding: '0 16px' }}>
                <div style={{ textAlign: 'center', marginBottom: 12 }}>
                    <h1 style={{ fontFamily: 'Outfit, sans-serif', fontSize: '1.4rem', fontWeight: 900, color: '#0f172a', marginBottom: 4 }}>
                        Introduction to <span style={{ color: "var(--prob-primary)" }}>Probability Skills</span>
                    </h1>
                </div>

                <div className="prob-skills-list">
                    {SKILLS.map((s, idx) => (
                        <div key={s.id} className="prob-skill-card">
                            <div className="prob-skill-info">
                                <div className="prob-skill-icon" style={{ background: `${s.color}15`, color: s.color }}>{s.icon}</div>
                                <div>
                                    <div style={{ fontSize: 11, fontWeight: 800, color: '#3b82f6', textTransform: 'uppercase', letterSpacing: 1.5, marginBottom: 6 }}>{s.subtitle}</div>
                                    <h3 style={{ margin: '0 0 6px', fontSize: 22, fontWeight: 800, color: '#0f172a' }}>{s.title}</h3>
                                    <p style={{ margin: 0, fontSize: 15, color: '#64748b', lineHeight: 1.5 }}>{s.desc}</p>
                                </div>
                            </div>
                            <div className="prob-skill-actions">
                                <button className="prob-btn-secondary" onClick={() => { setActiveSkillIdx(idx); setView('learn'); }}>Learn</button>
                                <button className="prob-btn-secondary" onClick={() => { setActiveSkillIdx(idx); setView('practice'); }}>Practice</button>
                                <button className="prob-btn-primary" style={{ background: s.color }} onClick={() => { setActiveSkillIdx(idx); setView('assessment'); }}>Assess</button>
                            </div>
                        </div>
                    ))}
                </div>

                <div style={{ marginTop: 24, padding: 24, textAlign: 'center', background: '#f8fafc', borderRadius: 20, border: '1px solid #e2e8f0' }}>
                    <p style={{ margin: 0, fontSize: 15, color: '#64748b', fontWeight: 600 }}>
                        Completed all sessions? You're a <span style={{ color: 'var(--prob-primary)', fontWeight: 800 }}>Probability Master!</span> 🧗‍♂️
                    </p>
                </div>
            </div>
        </div>
