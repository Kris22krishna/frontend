...
import { Timer, Star, ChevronLeft, RefreshCw, FileText, Check, X, Eye, ChevronRight } from 'lucide-react';
...
// ─── Vertical Subtraction UI ───
const VerticalSubtraction = ({ num1, num2, isSubtraction = true }) => {
    ...
    return (
        <div style={{
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px', width: '100%', textAlign: 'center' }}>
                ...
                <span style={{ color: '#F56565', fontSize: '3.5rem', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    −
                </span>
                <div style={{ display: 'flex', gap: '15px' }}>
...
const DynamicVisual = ({ type, data }) => {
    if (type === 'vertical') {
        return <div style={{ display: 'flex', justifyContent: 'center', width: '100%' }}><VerticalSubtraction num1={data.num1} num2={data.num2} isSubtraction={true} /></div>;
    }
    // No visual for word problems as per user preference
    return null;
};
...
const Grade2GiveAndTake = () => {
    ...
    const getTopicInfo = () => {
        ...
        return { topicName: 'Give and Take', skillName: 'Subtraction Practice' };
    };

    const { topicName, skillName } = getTopicInfo();

    const generateSubtractionNoBorrow = () => {
        const qs = [];
        for (let i = 0; i < totalQuestions; i++) {
            ...
            qs.push({
                ...
                correct: String(ans),
                type: 'vertical',
                visualData: { num1: n1, num2: n2 },
                explanation: `${n1} - ${n2} = ${ans}. Ones: ${n1 % 10} - ${n2 % 10} = ${n1 % 10 - n2 % 10}. Tens: ${Math.floor(n1 / 10)} - ${Math.floor(n2 / 10)} = ${Math.floor(n1 / 10) - Math.floor(n2 / 10)}.`
            });
        }
        return qs;
    };

    const generateSubtractionBorrow = () => {
        const qs = [];
        for (let i = 0; i < totalQuestions; i++) {
            let n1, n2;
            do {
            ...
            qs.push({
                ...
                type: 'vertical',
                visualData: { num1: n1, num2: n2 },
                explanation: `Since ${n1 % 10} < ${n2 % 10}, borrow 1 ten from ${Math.floor(n1 / 10)}. Now ones place is ${n1 % 10 + 10}. ${n1 % 10 + 10} - ${n2 % 10} = ${(n1 % 10 + 10) - n2 % 10}. Tens: ${Math.floor(n1 / 10) - 1} - ${Math.floor(n2 / 10)} = ${Math.floor(n1 / 10) - 1 - (Math.floor(n2 / 10))}. Result: ${ans}.`
            });
        }
        return qs;
    };
    ...
    const generateWordProblems = () => {
        const problems = [
            { t: "Rina has {n1} cookies. She gives {n2} to Arjun. How many are left?", verb: "gives" },
            { t: "There are {n1} birds on a tree. {n2} fly away. How many stay?", verb: "fly away" },
            { t: "Ali collected {n1} stamps. He lost {n2}. How many does he have now?", verb: "lost" },
            { t: "A shop had {n1} toys. {n2} were sold. How many toys remain?", verb: "sold" }
        ];
        const qs = [];
        for (let i = 0; i < totalQuestions; i++) {
            ...
            const ans = n1 - n2;
            qs.push({
                text: p.t.replace('{n1}', n1).replace('{n2}', n2),
                options: [...new Set([ans, ans + 10, ans - 1, n1 + n2])].sort(() => 0.5 - Math.random()).slice(0, 4).map(String),
                correct: String(ans),
                type: 'word-problem',
                visualData: { num1: n1, num2: n2 },
                explanation: `Total was ${n1}. ${n2} were ${p.verb}. Remaining = ${n1} - ${n2} = ${ans}.`
            });
        }
        return qs;
    };
    ...
    const generateQuestions = (sid) => {
        if (sid === '1034') return generateSubtractionNoBorrow();
        if (sid === '1035') return generateSubtractionBorrow();
        if (sid === '1036') return generateWordProblems();

        const pool = [
            ...generateSubtractionNoBorrow().slice(0, 4),
            ...generateSubtractionBorrow().slice(0, 3),
            ...
            <div className="grade1-practice-page results-view overflow-y-auto">
                ...
                <main className="results-content">
                    ...
                    <div className="results-actions">
                        <button className="action-btn-large play-again-btn" onClick={() => window.location.reload()}><RefreshCw size={24} /> New Subtraction Adventure</button>
...
const Grade2GiveAndTakeWithBoundary = () => <ErrorBoundary><Grade2GiveAndTake /></ErrorBoundary>;
export default Grade2GiveAndTakeWithBoundary;
