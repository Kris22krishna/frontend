...
import { TERMS, CHART_TYPES, VOCAB_QUIZ } from './Mensuration6TerminologyData.jsx';

export default function Mensuration6Terminology() {
    ...
    return (
        <div className={styles['men-terminology-page']} style={{
            fontFamily: "'Inter', system-ui, sans-serif",
            background: "#f8fafc",
            minHeight: "100vh"
        }}>
            <style>{`
                .men-details-window-anim {
                ...
                .men-term-btn-mini:hover {
                    transform: translateY(-2px);
                    box-shadow: 0 6px 15px rgba(0,0,0,0.08);
                }
                .men-term-btn-mini:hover::before {
                    opacity: 0.9;
                }
                .men-term-btn-mini.active {
                    transform: translateY(-1px);
                    box-shadow: 0 8px 20px rgba(0,0,0,0.1);
                    z-index: 2;
                }
                .men-term-btn-mini.active::before {
                    opacity: 0;
                }
                .men-term-btn-mini > * {
                    position: relative;
            ...
            <nav className={styles['men-intro-nav']}>
                <button className={styles['men-intro-nav-back']} onClick={() => navigate('/middle/grade/6/mensuration-6')}>← Back to Perimeter & Area Dashboard</button>
                <div className={styles['men-intro-nav-links']}>
                    <button className={styles['men-intro-nav-link']} onClick={() => navigate('/middle/grade/6/mensuration-6/introduction')}>🌟 Introduction</button>
                    <button className={`${styles['men-intro-nav-link']} ${styles['men-intro-nav-link--active']}`}>📖 Terminology</button>
                    <button className={styles['men-intro-nav-link']} onClick={() => navigate('/middle/grade/6/mensuration-6/skills')}>🎯 Skills</button>
                </div>
            </nav>

            {/* ── MAIN CONTENT ──────────────────────────────── */}
            <div className="men-lexicon-container" style={{ maxWidth: 1100, margin: '40px auto 20px', padding: '0 24px' }}>

                {/* Heading Stack */}
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', marginBottom: 20 }}>
                    <h1 style={{ fontFamily: 'Outfit, sans-serif', fontSize: '2.8rem', fontWeight: 900, color: '#0f172a', margin: '0 0 8px' }}>
                        Measurement <span style={{ background: 'linear-gradient(135deg, #0891b2, #6366f1)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>Vocabulary</span>
                    </h1>
                ...
                <div style={{ display: 'flex', justifyContent: 'center', gap: 8, marginBottom: 16 }}>
                    <button className={`${styles['men-tab']} ${activeTab === 'terms' ? styles.active : ''}`} onClick={() => setActiveTab('terms')}>📚 Terminology</button>
                    <button className={`${styles['men-tab']} ${activeTab === 'rules' ? styles.active : ''}`} onClick={() => setActiveTab('rules')}>📏 Key Formulas</button>
                    <button className={`${styles['men-tab']} ${activeTab === 'quiz' ? styles.active : ''}`} onClick={() => setActiveTab('quiz')}>🧪 Test Prep</button>
                </div>

                {activeTab !== 'quiz' ? (
                    /* ── MASTER-DETAIL VIEW (Terms or Rules) ── */
                    <div className="men-lexicon-grid" style={{ display: 'grid', gridTemplateColumns: 'minmax(300px, 360px) 1fr', gap: 16, alignItems: 'start' }}>
                        <aside className="men-selector-container" style={{
                            ...
                            {activeTab === 'terms' ? (
                                TERMS.map((term, i) => {
                                    ...
                                    return (
                                        <button key={i} className={`men-term-btn-mini ${isActive ? 'active' : ''}`} onClick={() => setSelectedIdx(i)}
                                            style={{
                                ...
                                CHART_TYPES.map((rule, i) => {
                                    ...
                                    return (
                                        <button key={i} className={`men-term-btn-mini ${isActive ? 'active' : ''}`} onClick={() => setSelectedRuleIdx(i)}
                        ...
                        <main className="men-details-window-anim" key={activeTab === 'terms' ? selectedIdx : selectedRuleIdx} style={{
                            ...
                            {activeTab === 'terms' ? (
                                ...
                                <>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
                                        ...
                                        <h2 style={{ fontFamily: 'Outfit, sans-serif', fontSize: 28, fontWeight: 900, color: activeRule.color, margin: 0 }}>Formula: {activeRule.title}</h2>
                                    </div>
                    ...
                    /* ── VOCABULARY TEST TAB ── */
                    <div className="men-details-window-anim" style={{ maxWidth: 800, margin: '0 auto', background: '#fff', borderRadius: 24, padding: '32px 40px', boxShadow: '0 15px 40px rgba(0,0,0,0.04)', border: '1px solid rgba(0,0,0,0.04)' }}>
                        {!quizFinished ? (
                            <>
                            ...
                            <div style={{ textAlign: 'center', padding: '20px 0' }}>
                                ...
                                <div style={{ display: 'flex', gap: 12, justifyContent: 'center' }}>
                                    ...
                                    <button style={{ padding: '12px 30px', background: '#fff', color: '#0891b2', border: '2px solid #0891b2', borderRadius: 100, fontWeight: 600, cursor: 'pointer' }} onClick={() => navigate('/middle/grade/6/mensuration-6/skills')}>Go to Skills 🎯</button>
                                </div>
                            </div>
                        )}
                    </div>
                ...
                <div style={{ marginTop: 16, textAlign: 'center' }}>
                    <button style={{ padding: '12px 30px', background: '#0891b2', color: '#fff', borderRadius: 100, border: 'none', fontWeight: 600, cursor: 'pointer' }} onClick={() => navigate('/middle/grade/6/mensuration-6/skills')}>Ready to Solve! 🎯</button>
                </div>
            </div>
        </div>
    );
