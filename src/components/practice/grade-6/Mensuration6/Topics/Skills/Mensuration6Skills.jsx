...
import QuizEngine from '../../Mensuration6QuizEngine.jsx';
import AssessmentEngine from '../../Mensuration6AssessmentEngine.jsx';
import { SKILLS } from './Mensuration6SkillsData.jsx';

export default function Mensuration6Skills() {
    ...
    console.log("Loaded Mensuration Skills:", SKILLS.length);

    if (view !== 'list' && skill) {
        return (
            <div className={styles['men-page']} style={{ background: '#f8fafc', minHeight: '100vh', paddingBottom: '60px' }}>
                <nav className={styles['men-intro-nav']}>
                    <button className={styles['men-intro-nav-back']} onClick={() => { setView('list'); setSelectedLearnIdx(0); }}>← Back to Skills</button>
                    <div className={styles['men-intro-nav-links']}>
                        <button className={styles['men-intro-nav-link']} onClick={() => navigate('/middle/grade/6/mensuration-6/introduction')}>🌟 Intro</button>
                        <button className={styles['men-intro-nav-link']} onClick={() => navigate('/middle/grade/6/mensuration-6/terminology')}>📖 Terminology</button>
                        <button className={`${styles['men-intro-nav-link']} ${styles['men-intro-nav-link--active']}`}>🎯 Skills</button>
                    </div>
                </nav>
            ...
            <div style={{ padding: '20px 24px 80px', maxWidth: 1100, margin: '0 auto' }}>
                ...
                <div style={{ textAlign: 'center', marginBottom: 40 }}>
                    <h1 style={{ fontFamily: 'Outfit, sans-serif', fontSize: '2.8rem', fontWeight: 900, color: '#0f172a', margin: '0 0 8px' }}>
                        Mensuration <span style={{ background: 'linear-gradient(135deg, #0369a1, #0891b2)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>Skills</span>
                    </h1>
