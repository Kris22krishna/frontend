...
export default function QuizEngine({ questions, title, onBack, onSecondaryBack, color, prefix = 'dh' , nodeId }) {
    ...
    useEffect(() => {
        ...
        } else if (answersMap[current] && ['perimeter-draw', 'area-draw', 'composite-draw'].includes(q.type)) {
            setDraftCustomAnswer(answersMap[current].customAnswer ?? null);
        } else if (!answersMap[current]) {
            setDraftTextAnswer('');
            if (q?.type === 'perimeter-draw') setDraftCustomAnswer([]);
            else if (q?.type === 'area-draw') setDraftCustomAnswer([]);
            else if (q?.type === 'composite-draw') setDraftCustomAnswer([]);
    ...
    const handleCustomSubmit = () => {
        if (isAnswered) return;
        if (q.type === 'perimeter-draw' || q.type === 'area-draw' || q.type === 'composite-draw') {
            const arrStr = JSON.stringify([...(draftCustomAnswer || [])].sort());
            if (q.type === 'area-draw' && q.targetArea) {
                isCorrect = (draftCustomAnswer || []).length === q.targetArea;
            } else if (q.targetAnswers) {
                isCorrect = q.targetAnswers.some(tgt => JSON.stringify([...tgt].sort()) === arrStr);
            } else {
                const tgtStr = JSON.stringify([...(q.targetAnswer || [])].sort());
    ...
    return (
        <div style={{ maxWidth: 860, margin: '0 auto', padding: '10px 24px' }}>
            ...
            <div style={{ background: '#fff', borderRadius: 20, padding: '32px', border: '1px solid #e2e8f0', boxShadow: '0 4px 20px rgba(0,0,0,0.06)', marginBottom: 24 }}>
                ...
                {['perimeter-draw', 'area-draw', 'composite-draw'].includes(q.type) ? (
                    <div style={{ marginBottom: 24 }}>
                        {q.type === 'perimeter-draw' && (
                            <PerimeterDrawInteractive question={q} answered={isAnswered} userAnswer={draftCustomAnswer} onChange={setDraftCustomAnswer} color={color} prefix={prefix} />
                        )}
                        {q.type === 'area-draw' && (
                            <AreaDrawInteractive question={q} answered={isAnswered} userAnswer={draftCustomAnswer} onChange={setDraftCustomAnswer} color={color} prefix={prefix} />
                        )}
                        {q.type === 'composite-draw' && (
                            <CompositeDrawInteractive question={q} answered={isAnswered} userAnswer={draftCustomAnswer} onChange={setDraftCustomAnswer} color={color} prefix={prefix} />
                        )}
