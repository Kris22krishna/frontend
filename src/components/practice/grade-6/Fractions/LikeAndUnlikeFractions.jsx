...
const LikeAndUnlikeFractions = () => {
    ...
    const SKILL_ID = 9308;
    const SKILL_NAME = "Like and Unlike Fractions";
    ...
    const generateQuestion = (index) => {
        ...
        // Q1-Q3: Conceptual
        // Q4-Q7: Identifying Like Fractions
        // Q8-Q10: Identifying Unlike Fractions
        if (index < 3) {
            const concepts = [
                {
                    q: "Fractions with the same denominators are called:",
                    c: "Like Fractions", opts: ["Like Fractions", "Unlike Fractions", "Equivalent Fractions", "Proper Fractions"],
                    e: "Like fractions are fractions that have the same denominator, meaning they are divided into the exact same number of equal parts."
                },
                {
                    q: "Unlike fractions are those which have:",
                    c: "Different denominators", opts: ["Different denominators", "The same denominator", "Different numerators", "The same numerator"],
                    e: "Unlike fractions have different numbers in the denominator, meaning their wholes are divided into a different number of parts."
                },
                {
                    q: `Are ${fractionMap['1/5']} and ${fractionMap['3/5']} like or unlike fractions?`,
                    c: "Like Fractions", opts: ["Like Fractions", "Unlike Fractions", "Equivalent Fractions", "Mixed Fractions"],
                    e: `Since both fractions share the denominator 5, they are like fractions.`
                }
            ];
            ...
            const denom = randomInt(4, 9);
            const num1 = randomInt(1, denom - 1);
            const num2 = randomInt(1, denom - 1);

            const baseFrac = formatFraction(`${num1}/${denom}`);
            const likeFrac = formatFraction(`${num2}/${denom}`);

            const diffDenom1 = denom + 1;
            const diffDenom2 = denom > 4 ? denom - 1 : denom + 2;
            const diffDenom3 = denom + 3;

            const wrong1 = formatFraction(`${randomInt(1, diffDenom1 - 1)}/${diffDenom1}`);
            const wrong2 = formatFraction(`${randomInt(1, diffDenom2 - 1)}/${diffDenom2}`);
            const wrong3 = formatFraction(`${randomInt(1, diffDenom3 - 1)}/${diffDenom3}`);

            const isFindLike = randomInt(0, 1) === 0;

            if (isFindLike) {
                qText = `Which of the following fractions is a LIKE fraction to ${baseFrac}?`;
                correct = likeFrac;
                options = [likeFrac, wrong1, wrong2, wrong3];
                explanation = `Like fractions must have the exact same denominator. The fraction ${likeFrac} has a denominator of ${denom}, just like ${baseFrac}.`;
            } else {
                qText = `Which pair of fractions are LIKE fractions?`;
                correct = `${baseFrac} and ${likeFrac}`;
                const pairWrong1 = `${baseFrac} and ${wrong1}`;
                const pairWrong2 = `${likeFrac} and ${wrong2}`;
                const pairWrong3 = `${wrong1} and ${wrong3}`;
                options = [correct, pairWrong1, pairWrong2, pairWrong3];
                explanation = `Like fractions have identical denominators. Both fractions in the pair ${correct} share the denominator ${denom}.`;
            }
        } else {
            const baseDenom = randomInt(3, 8);
            const diffDenom = baseDenom + randomInt(1, 3);

            const baseNum = randomInt(1, baseDenom - 1);
            const diffNum = randomInt(1, diffDenom - 1);
            const likeNum = randomInt(1, baseDenom - 1);

            const baseFrac = formatFraction(`${baseNum}/${baseDenom}`);
            const unlikeFrac = formatFraction(`${diffNum}/${diffDenom}`);
            const likeFrac = formatFraction(`${likeNum}/${baseDenom}`);

            const isFindUnlike = randomInt(0, 1) === 0;

            if (isFindUnlike) {
                qText = `Which of the following fractions is an UNLIKE fraction to ${baseFrac}?`;
                correct = unlikeFrac;
                const wrong1 = likeFrac;
                const wrong2 = formatFraction(`${randomInt(1, baseDenom - 1)}/${baseDenom}`);
                const wrong3 = formatFraction(`${randomInt(1, baseDenom - 1)}/${baseDenom}`);
                options = [correct, wrong1, wrong2, wrong3];
                explanation = `Unlike fractions have different denominators. ${unlikeFrac} has a denominator of ${diffDenom}, which is different from the denominator ${baseDenom} of ${baseFrac}.`;
            } else {
                qText = `Which pair of fractions are UNLIKE fractions?`;
                correct = `${baseFrac} and ${unlikeFrac}`;
                const pairWrong1 = `${baseFrac} and ${likeFrac}`;
                const pairWrong2 = `${likeFrac} and ${formatFraction(`${randomInt(1, baseDenom - 1)}/${baseDenom}`)}`;
                const pairWrong3 = `${baseFrac} and ${formatFraction(`${baseNum}/${baseDenom}`)}`;
                options = [correct, pairWrong1, pairWrong2, pairWrong3];
                explanation = `Unlike fractions do not share the same denominator. In the pair ${correct}, the denominators are ${baseDenom} and ${diffDenom}.`;
            }
        }

        const optSet = new Set(options);
        while (optSet.size < 4) {
            const fallback = `Option ${randomInt(10, 99)}`;
            if (!optSet.has(fallback)) optSet.add(fallback);
        }
    ...
    return (
            ...
            <header className="junior-practice-header">
                <div className="header-left">
                    <span className="text-[#31326F] font-normal text-lg sm:text-xl">Like and Unlike Fractions</span>
                </div>
...
export default LikeAndUnlikeFractions;
