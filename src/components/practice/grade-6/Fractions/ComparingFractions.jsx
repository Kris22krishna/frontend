...
const ComparingFractions = () => {
    ...
    const SKILL_ID = 9309;
    const SKILL_NAME = "Comparing Fractions";
    ...
    const generateQuestion = (index) => {
        ...
        // Q1-Q3: Compare Like Fractions
        // Q4-Q6: Compare Same Numerator Fractions
        // Q7-Q10: Compare Unlike Fractions
        if (index < 3) {
            const denom = randomInt(4, 9);
            const num1 = randomInt(1, denom - 2);
            const num2 = randomInt(num1 + 1, denom - 1);

            const frac1 = formatFraction(`${num1}/${denom}`);
            const frac2 = formatFraction(`${num2}/${denom}`);

            const wantsGreater = randomInt(0, 1) === 0;
            if (wantsGreater) {
                qText = `Which fraction is GREATER: ${frac1} or ${frac2}?`;
                correct = frac2;
                options = [frac1, frac2, "They are equal", "Cannot be compared"];
                explanation = `For like fractions (same denominator), the fraction with the larger numerator is greater. Since ${num2} > ${num1}, ${frac2} is greater.`;
            } else {
                qText = `Which fraction is SMALLER: ${frac1} or ${frac2}?`;
                correct = frac1;
                options = [frac1, frac2, "They are equal", "Cannot be compared"];
                explanation = `For like fractions (same denominator), the fraction with the smaller numerator is smaller. Since ${num1} < ${num2}, ${frac1} is smaller.`;
            }
        } else if (index < 6) {
            const num = randomInt(1, 4);
            const denom1 = randomInt(num + 1, 6);
            const denom2 = randomInt(denom1 + 1, 9);

            const frac1 = formatFraction(`${num}/${denom1}`);
            const frac2 = formatFraction(`${num}/${denom2}`);
            ...
            if (wantsGreater) {
                ...
                correct = frac1;
                options = [frac1, frac2, "They are equal", "Cannot be compared"];
                explanation = `When fractions have the same numerator, the fraction with the SMALLER denominator is actually greater. Since ${denom1} < ${denom2}, the pieces are larger, so ${frac1} is greater.`;
            } else {
                qText = `Which fraction is SMALLER: ${frac1} or ${frac2}?`;
                correct = frac2;
                options = [frac1, frac2, "They are equal", "Cannot be compared"];
                explanation = `When fractions have the same numerator, the fraction with the LARGER denominator is smaller. Since ${denom2} > ${denom1}, the pieces are smaller, so ${frac2} is smaller.`;
            }
        } else {
            const num1 = randomInt(1, 3);
            const den1 = randomInt(3, 5);
            const num2 = randomInt(2, 4);
            const den2 = randomInt(5, 7);
            ...
            qText = `Compare the fractions: ${frac1} and ${frac2}. Which symbol makes this true? \n ${frac1} _ ${frac2}`;

            if (eq) {
                correct = "=";
                options = ["<", ">", "=", "None of these"];
                explanation = `If we cross-multiply, ${num1} × ${den2} = ${num1 * den2}, and ${num2} × ${den1} = ${num2 * den1}. Since they are equal, the fractions are equivalent (${frac1} = ${frac2}).`;
            } else if (val1 > val2) {
                correct = ">";
                options = ["<", ">", "=", "None of these"];
                explanation = `If we cross-multiply, ${num1} × ${den2} = ${num1 * den2}, and ${num2} × ${den1} = ${num2 * den1}. Since ${num1 * den2} > ${num2 * den1}, ${frac1} is greater than ${frac2}.`;
            } else {
                correct = "<";
                options = ["<", ">", "=", "None of these"];
                explanation = `If we cross-multiply, ${num1} × ${den2} = ${num1 * den2}, and ${num2} × ${den1} = ${num2 * den1}. Since ${num1 * den2} < ${num2 * den1}, ${frac1} is less than ${frac2}.`;
            }
        }
    ...
    return (
            ...
            <header className="junior-practice-header">
                <div className="header-left">
                    <span className="text-[#31326F] font-normal text-lg sm:text-xl">Comparing Fractions</span>
                </div>
