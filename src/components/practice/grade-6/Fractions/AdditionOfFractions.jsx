...
const AdditionOfFractions = () => {
    ...
    const SKILL_ID = 9310;
    const SKILL_NAME = "Addition of Fractions";
    ...
    const generateQuestion = (index) => {
        ...
        // Q1-Q4: Adding Like Fractions
        // Q5-Q7: Adding Fractions and Whole Numbers
        // Q8-Q10: Adding Unlike Fractions
        if (index < 4) {
            ...
            qText = `Add the fractions: ${frac1} + ${frac2}`;
            correct = getSimpStr(sumNum, denom);

            const wrong1 = getSimpStr(sumNum + 1, denom);
            const wrong2 = getSimpStr(sumNum, denom + denom); // Common mistake add denominators
            const wrong3 = getSimpStr(sumNum - 1, denom) || `1`;
            options = [correct, wrong1, wrong2, wrong3];
            explanation = `To add like fractions, add the numerators and keep the denominator the same: ${num1} + ${num2} = ${sumNum}. The fraction is ${sumNum}/${denom}, which simplifies to ${correct}.`;
        } else if (index < 7) {
            ...
            qText = `Add the whole number and fraction: ${whole} + ${frac}`;
            correct = `${whole} ${frac}`;

            const wrong1 = `${whole + num}/${denom}`;
            const wrong2 = `${whole + 1} ${frac}`;
            const wrong3 = `${whole * num}/${denom}`;
            options = [correct, wrong1, wrong2, wrong3];
            explanation = `Adding a whole number to a proper fraction simply results in a mixed fraction. ${whole} whole + ${num}/${denom} parts is just written as ${correct}.`;
        } else {
            const num1 = randomInt(1, 3);
            const den1 = randomInt(num1 + 1, 5); // 2 to 5

            const num2 = randomInt(1, 3);
            const den2 = randomInt(num2 + 1, 5); // 2 to 5

            // Re-roll if denom1 == denom2
            let finalDen2 = den2;
            if (den1 === finalDen2) finalDen2 = den1 < 5 ? den1 + 1 : den1 - 1;

            const sumNum = num1 * finalDen2 + num2 * den1;
            const sumDenom = den1 * finalDen2;

            const frac1 = formatFraction(`${num1}/${den1}`);
            const frac2 = formatFraction(`${num2}/${finalDen2}`);

            qText = `Add the unlike fractions: ${frac1} + ${frac2}`;
            correct = getSimpStr(sumNum, sumDenom);

            const wrong1 = getSimpStr(num1 + num2, den1 + finalDen2); // Common mistake: top+top, bot+bot
            const wrong2 = getSimpStr(sumNum + 1, sumDenom);
            const wrong3 = getSimpStr(sumNum - 1, sumDenom);
            options = [correct, wrong1, wrong2, wrong3];
            explanation = `To add unlike fractions, find a common denominator (like ${den1} × ${finalDen2} = ${sumDenom}). Convert ${frac1} to ${num1 * finalDen2}/${sumDenom} and ${frac2} to ${num2 * den1}/${sumDenom}. Now add: (${num1 * finalDen2} + ${num2 * den1}) / ${sumDenom} = ${sumNum}/${sumDenom}, which simplifies to ${correct}.`;
        }
    ...
    return (
            ...
            <header className="junior-practice-header">
                <div className="header-left">
                    <span className="text-[#31326F] font-normal text-lg sm:text-xl">Addition of Fractions</span>
                </div>
