...
const SubtractionOfFractions = () => {
    ...
    const SKILL_ID = 9311;
    const SKILL_NAME = "Subtraction of Fractions";
    ...
    const generateQuestion = (index) => {
        ...
        // Q1-Q4: Subtracting Like Fractions
        // Q5-Q7: Subtracting Fractions from Whole Number 1
        // Q8-Q10: Subtracting Unlike Fractions
        if (index < 4) {
            const denom = randomInt(4, 9);
            const num1 = randomInt(2, denom - 1);
            const num2 = randomInt(1, num1 - 1); // num1 > num2

            const diffNum = num1 - num2;
            const frac1 = formatFraction(`${num1}/${denom}`);
            const frac2 = formatFraction(`${num2}/${denom}`);

            qText = `Subtract the fractions: ${frac1} - ${frac2}`;
            correct = getSimpStr(diffNum, denom);

            const wrong1 = getSimpStr(diffNum + 1, denom);
            const wrong2 = getSimpStr(num1 + num2, denom); // Added instead
            const wrong3 = getSimpStr(diffNum, denom * 2);
            options = [correct, wrong1, wrong2, wrong3];
            explanation = `To subtract like fractions, subtract the second numerator from the first and keep the denominator: ${num1} - ${num2} = ${diffNum}. The fraction is ${diffNum}/${denom}, which simplifies to ${correct}.`;
        } else if (index < 7) {
            const whole = 1;
            const num = randomInt(1, 4);
            const denom = randomInt(num + 1, 6);

            const frac = formatFraction(`${num}/${denom}`);
            const diffNum = denom - num;

            qText = `Subtract the fraction from the whole number: ${whole} - ${frac}`;
            correct = getSimpStr(diffNum, denom);

            const wrong1 = getSimpStr(diffNum + 1, denom);
            const wrong2 = getSimpStr(num, denom);
            const wrong3 = getSimpStr(denom + num, denom);
            options = [correct, wrong1, wrong2, wrong3];
            explanation = `To subtract from 1, remember that 1 whole can be written as a fraction where the numerator equals the denominator (${denom}/${denom}). Then ${denom}/${denom} - ${num}/${denom} = ${diffNum}/${denom}, which simplifies to ${correct}.`;
        } else {
            const num1 = randomInt(2, 4);
            const den1 = randomInt(num1 + 1, 5); // 3 to 5

            const num2 = randomInt(1, 2);
            let finalDen2 = randomInt(num2 + 1, 5); // 2 to 5

            if (den1 === finalDen2) finalDen2 = den1 < 5 ? den1 + 1 : den1 - 1;

            const val1 = num1 / den1;
            const val2 = num2 / finalDen2;

            let fNum1 = num1; let fDen1 = den1;
            let fNum2 = num2; let fDen2 = finalDen2;

            if (val2 > val1) {
                // swap so first is bigger
                fNum1 = num2; fDen1 = finalDen2;
                fNum2 = num1; fDen2 = den1;
            }

            const diffNum = fNum1 * fDen2 - fNum2 * fDen1;
            const sumDenom = fDen1 * fDen2;

            const frac1 = formatFraction(`${fNum1}/${fDen1}`);
            const frac2 = formatFraction(`${fNum2}/${fDen2}`);

            qText = `Subtract the unlike fractions: ${frac1} - ${frac2}`;
            correct = getSimpStr(diffNum, sumDenom);

            const wrong1 = getSimpStr(fNum1 - fNum2, fDen1 - fDen2); // Common mistake: top-top, bot-bot (might be div/0 so safeguard)
            const safeWrong1 = fDen1 - fDen2 !== 0 ? getSimpStr(Math.abs(fNum1 - fNum2), Math.abs(fDen1 - fDen2)) : getSimpStr(diffNum + 1, sumDenom);
            const wrong2 = getSimpStr(fNum1 * fDen2 + fNum2 * fDen1, sumDenom); // Added instead
            const wrong3 = getSimpStr(Math.abs(diffNum - 1), sumDenom);
            options = [correct, safeWrong1, wrong2, wrong3];
            explanation = `To subtract unlike fractions, find a common denominator (like ${fDen1} × ${fDen2} = ${sumDenom}). Convert ${frac1} to ${fNum1 * fDen2}/${sumDenom} and ${frac2} to ${fNum2 * fDen1}/${sumDenom}. Now subtract: (${fNum1 * fDen2} - ${fNum2 * fDen1}) / ${sumDenom} = ${diffNum}/${sumDenom}, which simplifies to ${correct}.`;
        }

        const optSet = new Set(options);
        while (optSet.size < 4) {
            const fallbackVal = randomInt(1, 4);
            const fallbackDen = randomInt(5, 9);
            const fallbackStr = formatFraction(`${fallbackVal}/${fallbackDen}`);
    ...
    return (
            ...
            <header className="junior-practice-header">
                <div className="header-left">
                    <span className="text-[#31326F] font-normal text-lg sm:text-xl">Subtraction of Fractions</span>
                </div>
...
export default SubtractionOfFractions;
