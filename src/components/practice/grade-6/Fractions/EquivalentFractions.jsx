...
const EquivalentFractions = () => {
    ...
    const SKILL_ID = 9306;
    const SKILL_NAME = "Equivalent Fractions";
    ...
    const generateQuestion = (index) => {
        ...
        // Q1-Q3: Meaning & Property
        // Q4-Q7: Generating Equivalent Fractions
        // Q8-Q10: Identifying Equivalent Fractions
        if (index < 3) {
            const concepts = [
                {
                    q: "Equivalent fractions are fractions that:",
                    c: "Represent the same value", opts: ["Represent the same value", "Have the same numerator only", "Have the same denominator only", "Are always proper fractions"],
                    e: "Equivalent fractions may look different but they have exactly the same value when simplified."
                },
                {
                    q: "How can you find an equivalent fraction?",
                    c: "Multiply or divide numerator and denominator by the same number", opts: ["Multiply or divide numerator and denominator by the same number", "Add the same number to numerator and denominator", "Subtract the same number from both", "Only multiply by 2"],
                    e: "To get an equivalent fraction, you must multiply or divide both the numerator and the denominator by the exact same non-zero number."
                },
                {
                    q: `Are ${fractionMap['1/2']} and ${formatFraction('2/4')} equivalent fractions?`,
                    c: "Yes", opts: ["Yes", "No", "Sometimes", "They are reciprocals"],
                    e: `Yes, because if you multiply the numerator and denominator of ½ by 2, you get 2/4. They represent the same amount.`
                }
            ];
            ...
            const dens = randomInt(num + 1, 6);
            const multiplier = randomInt(2, 5);

            const baseFrac = formatFraction(`${num}/${dens}`);
            const eqFrac = formatFraction(`${num * multiplier}/${dens * multiplier}`);

            const isMissingNum = randomInt(0, 1) === 0;

            if (isMissingNum) {
                qText = `Find the missing numerator to make the fractions equivalent: ${baseFrac} = ?/${dens * multiplier}`;
                correct = `${num * multiplier}`;
                const wrong1 = `${num * multiplier + 1}`;
                const wrong2 = `${num * multiplier - 1}`;
                const wrong3 = `${num + multiplier}`;
                options = [correct, wrong1, wrong2, wrong3];
                explanation = `To get from ${dens} to ${dens * multiplier}, we multiplied by ${multiplier}. Whatever we do to the bottom, we must do to the top. So, ${num} × ${multiplier} = ${num * multiplier}.`;
            } else {
                qText = `Find the missing denominator to make the fractions equivalent: ${baseFrac} = ${num * multiplier}/?`;
                correct = `${dens * multiplier}`;
                const wrong1 = `${dens * multiplier + 1}`;
                const wrong2 = `${dens * multiplier - 1}`;
                const wrong3 = `${dens + multiplier}`;
                options = [correct, wrong1, wrong2, wrong3];
                explanation = `To get from ${num} to ${num * multiplier}, we multiplied by ${multiplier}. Whatever we do to the top, we must do to the bottom. So, ${dens} × ${multiplier} = ${dens * multiplier}.`;
            }
        } else {
            const num = randomInt(1, 3);
            const dens = randomInt(num + 1, 5);
            const mult = randomInt(2, 4);
            const wrongMult = mult + 1;

            const targetFrac = formatFraction(`${num}/${dens}`);
            const rightFrac = formatFraction(`${num * mult}/${dens * mult}`);
            const wrong1 = formatFraction(`${num * wrongMult}/${dens * mult}`);
            const wrong2 = formatFraction(`${(num + 1) * mult}/${dens * mult}`);
            const wrong3 = formatFraction(`${num}/${dens * mult}`);

            qText = `Which of the following is an equivalent fraction of ${targetFrac}?`;
            correct = rightFrac;
            options = [correct, wrong1, wrong2, wrong3];
            explanation = `To find an equivalent fraction, we multiply the numerator and denominator by the same number. ${num} × ${mult} = ${num * mult}, and ${dens} × ${mult} = ${dens * mult}. So ${rightFrac} is equivalent to ${targetFrac}.`;
        }

        const optSet = new Set(options);
        while (optSet.size < 4) {
            const randomVal = `${randomInt(10, 99)}`;
    ...
    return (
            ...
            <header className="junior-practice-header">
                <div className="header-left">
                    <span className="text-[#31326F] font-normal text-lg sm:text-xl">Equivalent Fractions</span>
                </div>
