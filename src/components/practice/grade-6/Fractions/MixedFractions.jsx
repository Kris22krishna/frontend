...
const MixedFractions = () => {
    ...
    const SKILL_ID = 9305;
    const SKILL_NAME = "Mixed Fractions";
    ...
    const generateQuestion = (index) => {
        ...
        // Q1-Q3: Definition & Meaning
        // Q4-Q7: Converting Mixed to Improper
        // Q8-Q10: Converting Improper to Mixed
        if (index < 3) {
            const concepts = [
                {
                    q: "A mixed fraction is made up of:",
                    c: "A whole number and a proper fraction",
                    opts: ["A whole number and a proper fraction", "Two proper fractions", "An improper fraction and a proper fraction", "Two whole numbers"],
                    e: "A mixed fraction is a combination of a natural number (whole number) and a proper fraction. For example, 1 ½."
                },
                {
                    q: "Which of the following is an example of a mixed fraction?",
                    c: "2 ½",
                    opts: ["2 ½", "5/2", "½", "3"],
                    e: "2 ½ consists of a whole number (2) and a proper fraction (½), making it a mixed fraction."
                },
                {
                    q: "The value of a mixed fraction is always:",
                    c: "Greater than 1",
                    opts: ["Greater than 1", "Less than 1", "Equal to 1", "Less than 0"],
                    e: "Since a mixed fraction includes at least 1 whole and a proper fraction along with it, its value is always greater than 1."
                }
            ];
            ...
            const wholes = randomInt(2, 6);
            const dens = randomInt(3, 8);
            const num = randomInt(1, dens - 1);

            const fracStr = formatFraction(`${num}/${dens}`);
            const mixedStr = `${wholes} ${fracStr}`;

            const improperNum = wholes * dens + num;
            const improperFrac = formatFraction(`${improperNum}/${dens}`);

            qText = `Convert the mixed fraction ${mixedStr} into an improper fraction.`;
            correct = improperFrac;

            const wrong1 = formatFraction(`${improperNum - 1}/${dens}`);
            const wrong2 = formatFraction(`${wholes + num}/${dens}`);
            const wrong3 = formatFraction(`${improperNum}/${dens + 1}`);
            options = [correct, wrong1, wrong2, wrong3];

            explanation = `To convert a mixed fraction to an improper fraction: Multiply the whole number by the denominator (${wholes} × ${dens} = ${wholes * dens}). Then add the numerator (${wholes * dens} + ${num} = ${improperNum}). The denominator remains the same (${dens}). So, ${mixedStr} = ${improperFrac}.`;
        } else {
            const wholes = randomInt(2, 6);
            const dens = randomInt(3, 8);
            const num = randomInt(1, dens - 1);
            ...
            qText = `Convert the improper fraction ${improperFrac} into a mixed fraction.`;
            correct = mixedStr;

            const wrong1 = `${wholes + 1} ${formatFraction(`${1}/${dens}`)}`;
            const wrong2 = `${wholes - 1} ${formatFraction(`${dens - 1}/${dens}`)}`;
            const wrong3 = `${wholes} ${formatFraction(`${num + 1}/${dens}`)}`;
            options = [correct, wrong1, wrong2, wrong3];

            explanation = `To convert an improper fraction to a mixed fraction: Divide the numerator by the denominator (${improperNum} ÷ ${dens} = ${wholes} with a remainder of ${num}). The quotient (${wholes}) becomes the whole number, the remainder (${num}) becomes the new numerator, and the denominator stays the same (${dens}). So, ${improperFrac} = ${mixedStr}.`;
        }
    ...
    return (
            ...
            <header className="junior-practice-header">
                <div className="header-left">
                    <span className="text-[#31326F] font-normal text-lg sm:text-xl">Mixed Fractions</span>
                </div>
