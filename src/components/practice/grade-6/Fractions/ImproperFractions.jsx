...
const ImproperFractions = () => {
    ...
    const SKILL_ID = 9304;
    const SKILL_NAME = "Improper Fractions";
    ...
    const generateQuestion = (index) => {
        ...
        // Q4-Q7: Identifying improper fractions
        // Q8-Q10: Finding wholes
        if (index < 3) {
            const concepts = [
                {
                    q: "In an improper fraction, the numerator is:",
                    c: "Greater than or equal to the denominator", o: ["Greater than or equal to the denominator", "Less than the denominator", "Always 1", "Always 0"],
                    e: "An improper fraction has a numerator that is greater than or equal to its denominator."
                },
                {
                    q: "Which of the following describes the value of an improper fraction?",
                    c: "1 or more than 1", o: ["1 or more than 1", "Exactly 1", "Less than 1", "Always 0"],
                    e: "Because the numerator is greater than or equal to the denominator, an improper fraction represents one full whole or more."
                },
                {
                    q: "If I eat 5 slices from a pizza cut into 4 slices, what kind of fraction represents my eating?",
                    c: "Improper Fraction", o: ["Improper Fraction", "Proper Fraction", "Like Fraction", "Unit Fraction"],
                    e: "Eating 5 out of 4 means the fraction is 5/4. Since numerator > denominator, it's an improper fraction."
                }
            ];
            ...
            const improperDens = randomInt(2, 6);
            const improperNum = randomInt(improperDens + 1, 10);

            const properFrac1 = formatFraction(`${properNum}/${properDens}`);
            const properFrac2 = formatFraction(`1/${randomInt(3, 9)}`);
            const properFrac3 = formatFraction(`2/${randomInt(4, 9)}`);
            const impFrac = formatFraction(`${improperNum}/${improperDens}`);

            if (isIdentify) {
                qText = `Which of the following is an IMPROPER fraction?`;
                correct = impFrac;
                options = [impFrac, properFrac1, properFrac2, properFrac3];
                explanation = `An improper fraction has a numerator equal to or greater than its denominator. In ${impFrac}, ${improperNum} > ${improperDens}.`;
            } else {
                qText = `Which of the following is NOT an improper fraction?`;
                correct = properFrac1;
                options = [properFrac1, impFrac, formatFraction(`${randomInt(5, 9)}/4`), formatFraction(`${randomInt(7, 9)}/6`)];
                explanation = `A proper fraction has a numerator strictly less than its denominator. Thus, ${properFrac1} is NOT an improper fraction.`;
            }
        } else {
            const dens = randomInt(2, 5);
            const wholes = randomInt(2, 4);
            const extra = randomInt(1, dens - 1);
            const improperNum = wholes * dens + extra;

            const scenarios = [
                {
                    q: `How many exact wholes are hidden inside the improper fraction ${formatFraction(`${improperNum}/${dens}`)}?`,
                    c: `${wholes}`,
                    opts: [`${wholes}`, `${wholes + 1}`, `${wholes - 1}`, `${extra}`],
                    e: `To find the number of exact wholes, divide the numerator by the denominator. ${improperNum} ÷ ${dens} = ${wholes} with a remainder of ${extra}. So there are ${wholes} wholes.`
                },
                {
                    q: `If an improper fraction has numerator ${improperNum} and denominator ${dens}, what is its value as a mixed phrase?`,
                    c: `${wholes} and ${extra} over ${dens}`,
                    opts: [`${wholes} and ${extra} over ${dens}`, `${extra} and ${wholes} over ${dens}`, `${dens} and ${extra} over ${wholes}`, `${wholes} and ${dens} over ${improperNum}`],
                    e: `Dividing ${improperNum} by ${dens} gives ${wholes} with a remainder of ${extra}. So it's ${wholes} wholes and ${extra} parts left over out of ${dens}.`
                },
                {
                    q: `Is the fraction ${formatFraction(`${dens}/${dens}`)} proper or improper?`,
                    c: "Improper",
                    opts: ["Improper", "Proper", "Neither", "Both"],
                    e: `A fraction where the numerator equals the denominator forms exactly 1 whole. By definition, if numerator is equal to or greater than the denominator, it is improper.`
                }
            ];
            const item = scenarios[index - 7];
            qText = item.q;
            correct = item.c;
            options = item.opts;
            explanation = item.e;
        ...
        while (optSet.size < 4) {
            const randomVal = `Option ${randomInt(10, 99)}`;
    ...
    return (
            ...
            <header className="junior-practice-header">
                <div className="header-left">
                    <span className="text-[#31326F] font-normal text-lg sm:text-xl">Improper Fractions</span>
                </div>
...
export default ImproperFractions;
