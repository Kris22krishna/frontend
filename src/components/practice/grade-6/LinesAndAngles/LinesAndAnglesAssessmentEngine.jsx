...
export default function AssessmentEngine({ questions, title, onBack, onSecondaryBack, color, prefix = 'alg' , nodeId }) {
    const getQuestionType = (question) => {
        ...
        if (question?.type === 'protractor') return 'protractor';
        if (question?.type === 'geometry-draw') return 'geometry-draw';
        return 'mcq';
    };
    ...
    const isAnswerComplete = (question, answer) => {
        ...
        if (type === 'geometry-draw') {
            try {
                const user = typeof answer === 'string' ? JSON.parse(answer) : answer;
                return !!(user && user.p1 && user.p2);
            } catch(e) { return false; }
        }
        if (type === 'protractor') return answer !== null && answer !== undefined && answer !== '';
    ...
    const isAnswerCorrect = (question, answer) => {
        ...
        if (type === 'protractor') {
            if (question.range) {
                return Number(answer) >= question.range[0] && Number(answer) <= question.range[1];
            }
            const allowedError = question.tolerance || 5;
            return Math.abs(Number(answer) - Number(question.answer)) <= allowedError;
        }
        if (type === 'text') {
    ...
    const getCorrectAnswerLabel = (question) => {
        const type = getQuestionType(question);
        if (type === 'geometry-draw') return `Connect ${question.answer.p1} to ${question.answer.p2}`;
        if (type === 'protractor') {
            return question.range ? `Range: ${question.range[0]}° - ${question.range[1]}°` : `${question.answer}°`;
        }
    ...
    const getUserAnswerLabel = (question, answer) => {
        ...
        if (type === 'geometry-draw') {
            try {
                const user = typeof answer === 'string' ? JSON.parse(answer) : answer;
                return `Connected ${user.p1} to ${user.p2}`;
            } catch(e) { return 'Invalid'; }
        }
        if (type === 'protractor') return `${answer}°`;
        ...
        return (
            <div className={`${prefix}-quiz-finished`} style={{ maxWidth: 900, margin: '0 auto', padding: '40px 20px' }}>
                <div style={{ textAlign: 'center', marginBottom: 40 }}>
                    ...
                    <div style={{ fontSize: 14, color: `var(--${prefix}-muted)`, fontWeight: 600, marginTop: 4 }}>Total Time: {formatTime(questionTimes.reduce((a, b) => a + b, 0))}</div>
        ...
        <div className={`${prefix}-quiz-active ${prefix}-assessment-layout`}>
            <div style={{ flex: 1 }} ref={topRef}>
                ...
                <div className={`${prefix}-quiz-card`}>
                    ...
                    {getQuestionType(q) === 'geometry-draw' ? (
