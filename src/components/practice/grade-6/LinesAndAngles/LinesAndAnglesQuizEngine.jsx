...
export default function QuizEngine({ questions, title, onBack, onSecondaryBack, color, prefix = 'alg' }) {
    ...
    const handleTextSubmit = () => {
        ...
        // geometry-draw: need two points selected
        if (type === 'geometry-draw') {
            if (!textAnswer) return;
            try {
                const user = JSON.parse(textAnswer);
        ...
        // protractor: numeric tolerance or range check
        if (type === 'protractor') {
            if (!textAnswer) return;
            setAnswered(true);
            const val = Number(textAnswer);
            let isCorrect = false;
            if (q.range) {
        ...
        // text: simple string comparison
        if (!textAnswer.trim()) return;
        setAnswered(true);
        if (textAnswer.trim().toLowerCase() === String(q.answer).trim().toLowerCase()) {
            setScore(s => s + 1);
        }
    };
            ...
            <div className={`${prefix}-quiz-card`}>
                ...
                {q.type === 'geometry-draw' ? (
                    <GeometryDrawInteractive 
                        ...
                        onChange={(val) => !answered && setTextAnswer(val)}
                        ...
                        <div style={{ display: 'flex', gap: 10 }}>
                            <input
                                ...
                                style={{
                                    ...
                                    border: `2px solid ${answered ? (textAnswer.trim().toLowerCase() === String(q.answer).trim().toLowerCase() ? `var(--${prefix}-teal)` : `var(--${prefix}-red)`) : 'rgba(0,0,0,0.08)'}`,
                            ...
                            {!answered && (
                                <button
                                    ...
                                    style={{
                                        ...
                                        background: textAnswer.trim() ? color : '#e2e8f0',
                                        color: textAnswer.trim() ? '#fff' : '#94a3b8',
                                        fontWeight: 700, cursor: textAnswer.trim() ? 'pointer' : 'not-allowed'
                                    }}
                                >Check</button>
                            )}
                        </div>
                        {answered && (
                            <div style={{ fontSize: 14, fontWeight: 700, color: textAnswer.trim().toLowerCase() === String(q.answer).trim().toLowerCase() ? `var(--${prefix}-teal)` : `var(--${prefix}-red)` }}>
                                {textAnswer.trim().toLowerCase() === String(q.answer).trim().toLowerCase() ? '✅ Correct!' : `❌ Incorrect. The correct answer is ${q.answer}.`}
                            </div>
                        )}
                    </div>
                ) : (
