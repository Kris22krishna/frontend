...
export default function PatternsTerminology() {
    ...
    return (
        <div className={styles['pat-terminology-page']} style={{
            fontFamily: "'Inter', system-ui, sans-serif",
            background: "#f8fafc",
            minHeight: "100vh"
        }}>
            <style>{`
                .pat-details-window-anim {
                ...
                .pat-term-btn-mini:hover {
                    transform: translateY(-2px);
                    box-shadow: 0 6px 15px rgba(0,0,0,0.08);
                }
                .pat-term-btn-mini:hover::before {
                    opacity: 0.9;
                }
                .pat-term-btn-mini.active {
                    transform: translateY(-1px);
                    box-shadow: 0 8px 20px rgba(0,0,0,0.1);
                    z-index: 2;
                }
                .pat-term-btn-mini.active::before {
                    opacity: 0;
                }
                .pat-term-btn-mini > * {
                    position: relative;
            ...
            <nav className={styles['pat-intro-nav']}>
                <button className={styles['pat-intro-nav-back']} onClick={() => navigate('/middle/grade/6/patterns-in-mathematics')}>← Back to Patterns</button>
                <div className={styles['pat-intro-nav-links']}>
                    <button className={styles['pat-intro-nav-link']} onClick={() => navigate('/middle/grade/6/patterns-in-mathematics/introduction')}>🌟 Introduction</button>
                    <button className={`${styles['pat-intro-nav-link']} ${styles['pat-intro-nav-link--active']}`} onClick={() => navigate('/middle/grade/6/patterns-in-mathematics/terminology')}>📖 Terminology</button>
                    <button className={styles['pat-intro-nav-link']} onClick={() => navigate('/middle/grade/6/patterns-in-mathematics/skills')}>🎯 Skills</button>
                </div>
            </nav>

            {/* ── MAIN CONTENT ──────────────────────────────── */}
            <div className="pat-lexicon-container" style={{ maxWidth: 1100, margin: '40px auto 20px', padding: '0 24px' }}>

                {/* Heading Stack */}
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', marginBottom: 20 }}>
                    <h1 style={{ fontFamily: 'Outfit, sans-serif', fontSize: '2.8rem', fontWeight: 900, color: '#0f172a', margin: '0 0 8px' }}>
                        Patterns <span style={{ background: 'linear-gradient(135deg, #0891b2, #6366f1)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>Vocabulary</span>
                    </h1>
                ...
                <div style={{ display: 'flex', justifyContent: 'center', gap: 8, marginBottom: 16 }}>
                    <button className={`${styles['pat-tab']} ${activeTab === 'terms' ? styles.active : ''}`} onClick={() => setActiveTab('terms')}>📚 Terminology</button>
                    <button className={`${styles['pat-tab']} ${activeTab === 'rules' ? styles.active : ''}`} onClick={() => setActiveTab('rules')}>📏 4 Core Properties</button>
                    <button className={`${styles['pat-tab']} ${activeTab === 'quiz' ? styles.active : ''}`} onClick={() => setActiveTab('quiz')}>🧪 Test Prep</button>
                </div>

                {activeTab !== 'quiz' ? (
                    /* ── MASTER-DETAIL VIEW (Terms or Rules) ── */
                    <div className="pat-lexicon-grid" style={{ display: 'grid', gridTemplateColumns: 'minmax(300px, 360px) 1fr', gap: 16, alignItems: 'start' }}>
                        <aside className="pat-selector-container" style={{
                            ...
                            {activeTab === 'terms' ? (
                                TERMS.map((term, i) => {
                                    ...
                                    return (
                                        <button key={i} className={`pat-term-btn-mini ${isActive ? 'active' : ''}`} onClick={() => setSelectedIdx(i)}
                                            style={{
                                ...
                                FOUR_RULES.map((rule, i) => {
                                    ...
                                    return (
                                        <button key={i} className={`pat-term-btn-mini ${isActive ? 'active' : ''}`} onClick={() => setSelectedRuleIdx(i)}
                                        ...
                                        >
                                            ...
                                            <div style={{ display: 'flex', flexDirection: 'column' }}>
                                                <span style={{ fontWeight: 800, fontSize: 16, color: isActive ? '#fff' : '#0f172a', lineHeight: 1 }}>Rule {rule.num}</span>
                                                <span style={{ fontSize: 11, fontWeight: 700, color: isActive ? 'rgba(255,255,255,0.8)' : '#64748b', textTransform: 'uppercase', letterSpacing: 0.5, marginTop: 4 }}>{rule.title}</span>
                                            </div>
                        ...
                        <main className="pat-details-window-anim" key={activeTab === 'terms' ? selectedIdx : selectedRuleIdx} style={{
                    ...
                    /* ── VOCABULARY TEST TAB ── */
                    <div className="pat-details-window-anim" style={{ maxWidth: 800, margin: '0 auto', background: '#fff', borderRadius: 24, padding: '32px 40px', boxShadow: '0 15px 40px rgba(0,0,0,0.04)', border: '1px solid rgba(0,0,0,0.04)' }}>
                        {!quizFinished ? (
                            <>
                            ...
                            <div style={{ textAlign: 'center', padding: '20px 0' }}>
                                ...
                                <div style={{ display: 'flex', gap: 12, justifyContent: 'center' }}>
                                    ...
                                    <button style={{ padding: '12px 30px', background: '#fff', color: '#0891b2', border: '2px solid #0891b2', borderRadius: 100, fontWeight: 600, cursor: 'pointer' }} onClick={() => navigate('/middle/grade/6/patterns-in-mathematics/skills')}>Go to Skills 🎯</button>
                                </div>
                            </div>
                        )}
                    </div>
                ...
                <div style={{ marginTop: 16, textAlign: 'center' }}>
                    <button style={{ padding: '12px 30px', background: '#0891b2', color: '#fff', borderRadius: 100, border: 'none', fontWeight: 600, cursor: 'pointer' }} onClick={() => navigate('/middle/grade/6/patterns-in-mathematics/skills')}>Ready to Solve! 🎯</button>
                </div>
            </div>
        </div>
    );
