...
import QuizEngine from '../../PlayingWithConstructions8QuizEngine.jsx';
import AssessmentEngine from '../../PlayingWithConstructions8AssessmentEngine.jsx';
import { SKILLS } from './ConstructionsSkillsData.jsx';

export default function ConstructionsSkills() {
    ...
    if (view !== 'list' && skill) {
        return (
            <div className={styles['frac-page']} style={{ background: '#f8fafc', minHeight: '100vh', paddingBottom: '60px' }}>
                <nav className={styles['frac-intro-nav']}>
                    <button className={styles['frac-intro-nav-back']} onClick={() => { setView('list'); setSelectedLearnIdx(0); }}>← Back to Skills</button>
                    <div className={styles['frac-intro-nav-links']}>
                        <button className={styles['frac-intro-nav-link']} onClick={() => navigate('/middle/grade/6/playing-with-constructions/introduction')}>🌟 Intro</button>
                        <button className={styles['frac-intro-nav-link']} onClick={() => navigate('/middle/grade/6/playing-with-constructions/terminology')}>📖 Terminology</button>
                        <button className={`${styles['frac-intro-nav-link']} ${styles['frac-intro-nav-link--active']}`}>🎯 Skills</button>
                    </div>
                </nav>
            ...
            <div style={{ padding: '20px 24px 80px', maxWidth: 1100, margin: '0 auto' }}>
                ...
                <div style={{ textAlign: 'center', marginBottom: 40 }}>
                    <h1 style={{ fontFamily: 'Outfit, sans-serif', fontSize: '2.8rem', fontWeight: 900, color: '#0f172a', margin: '0 0 8px' }}>
                        Construction <span style={{ background: 'linear-gradient(135deg, #0ea5e9, #10b981)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>Skills</span>
                    </h1>
