...
import { terminologyDocs, terminologyQuiz } from './ConstructionsTerminologyData.jsx';

export default function ConstructionsTerminology() {
    ...
    return (
        <div style={{
            ...
            <style>{`
                ...
                .cons-term-btn-mini:hover {
                    transform: translateY(-2px);
                    box-shadow: 0 6px 15px rgba(0,0,0,0.08);
                }
                .cons-term-btn-mini:hover::before {
                    opacity: 0.9;
                }
                .cons-term-btn-mini.active {
                    transform: translateY(-1px);
                    box-shadow: 0 8px 20px rgba(0,0,0,0.1);
                    z-index: 2;
                }
                .cons-term-btn-mini.active::before {
                    opacity: 0;
                }
                .cons-term-btn-mini > * {
                    position: relative;
            ...
            <nav className={styles['frac-intro-nav']}>
                <button className={styles['frac-intro-nav-back']} onClick={() => navigate('/middle/grade/6/playing-with-constructions')}>← Back to Constructions Dashboard</button>
                <div className={styles['frac-intro-nav-links']}>
                    <button className={styles['frac-intro-nav-link']} onClick={() => navigate('/middle/grade/6/playing-with-constructions/introduction')}>🌟 Introduction</button>
                    <button className={`${styles['frac-intro-nav-link']} ${styles['frac-intro-nav-link--active']}`}>📖 Terminology</button>
                    <button className={styles['frac-intro-nav-link']} onClick={() => navigate('/middle/grade/6/playing-with-constructions/skills')}>🎯 Skills</button>
                </div>
            </nav>
            ...
            <div style={{ maxWidth: 1100, margin: '40px auto 20px', padding: '0 24px' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', marginBottom: 20 }}>
                    <h1 style={{ fontFamily: 'Outfit, sans-serif', fontSize: '2.8rem', fontWeight: 900, color: '#0f172a', margin: '0 0 8px' }}>
                        Construction <span style={{ background: 'linear-gradient(135deg, #0ea5e9, #8b5cf6)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>Vocabulary</span>
                    </h1>
                ...
                {activeTab !== 'quiz' ? (
                    ...
                    <div className="cons-lexicon-grid" style={{ display: 'grid', gridTemplateColumns: 'minmax(280px, 340px) 1fr', gap: 16, alignItems: 'start' }}>
                        <aside className="cons-selector-container" style={{
                            ...
                            {terminologyDocs.map((term, i) => {
                                ...
                                return (
                                    <button key={i} className={`cons-term-btn-mini ${isActive ? 'active' : ''}`} onClick={() => setSelectedIdx(i)}
                                        style={{
                        ...
                        <main className="cons-details-window-anim" key={selectedIdx} style={{
                    ...
                    /* ── VOCABULARY TEST TAB ── */
                    <div className="cons-details-window-anim" style={{ maxWidth: 800, margin: '0 auto', background: '#fff', borderRadius: 24, padding: '32px 40px', boxShadow: '0 15px 40px rgba(0,0,0,0.04)', border: '1px solid rgba(0,0,0,0.04)' }}>
                        {!quizFinished ? (
                            <>
                            ...
                            <div style={{ textAlign: 'center', padding: '20px 0' }}>
                                ...
                                <div style={{ display: 'flex', gap: 12, justifyContent: 'center' }}>
                                    ...
                                    <button style={{ padding: '12px 30px', background: '#fff', color: '#0ea5e9', border: '2px solid #0ea5e9', borderRadius: 100, fontWeight: 600, cursor: 'pointer' }} onClick={() => navigate('/middle/grade/6/playing-with-constructions/skills')}>Go to Skills 🎯</button>
                                </div>
                            </div>
                        )}
                    </div>
                ...
                <div style={{ marginTop: 16, textAlign: 'center' }}>
                    <button style={{ padding: '12px 30px', background: '#0ea5e9', color: '#fff', borderRadius: 100, border: 'none', fontWeight: 600, cursor: 'pointer' }} onClick={() => navigate('/middle/grade/6/playing-with-constructions/skills')}>Ready to Construct! 🎯</button>
                </div>
            </div>
        </div>
    );
