...
import QuizEngine from '../../Symmetry6QuizEngine.jsx';
import AssessmentEngine from '../../Symmetry6AssessmentEngine.jsx';
import { SKILLS } from './Symmetry6SkillsData.jsx';
import { generateSymmetryQuestions } from './symmetryQuestions.jsx';

export default function Symmetry6Skills() {
    ...
    if (view !== 'list' && skill) {
        return (
            <div className={styles['sym-page']} style={{ background: '#f8fafc', minHeight: '100vh', paddingBottom: '60px' }}>
                <nav className={styles['sym-intro-nav']}>
                    <button className={styles['sym-intro-nav-back']} onClick={() => { setView('list'); setSelectedLearnIdx(0); }}>← Back to Skills</button>
                    <div className={styles['sym-intro-nav-links']}>
                        <button className={styles['sym-intro-nav-link']} onClick={() => navigate('/middle/grade/6/symmetry-6/introduction')}>🌟 Intro</button>
                        <button className={styles['sym-intro-nav-link']} onClick={() => navigate('/middle/grade/6/symmetry-6/terminology')}>📖 Terminology</button>
                        <button className={`${styles['sym-intro-nav-link']} ${styles['sym-intro-nav-link--active']}`}>🎯 Skills</button>
                    </div>
                </nav>
                <div style={{ padding: '40px 24px 0' }}>
                    {view === 'learn' ? (
                        ...
                        <QuizEngine
                            skillId={skill.id}
                            questions={() => generateSymmetryQuestions(skill.id, 10)}
                        ...
                        <AssessmentEngine
                            skillId={skill.id}
                            questions={() => generateSymmetryQuestions(skill.id, 20)}
                            title={skill.title}
                            color={skill.color}
                            onBack={() => setView('list')}
                            prefix="dh"
            ...
            <div style={{ padding: '20px 24px 80px', maxWidth: 1100, margin: '0 auto' }}>
                <div style={{ textAlign: 'center', marginBottom: 40 }}>
                    <h1 style={{ fontFamily: 'Outfit, sans-serif', fontSize: '2.8rem', fontWeight: 900, color: '#0f172a', margin: '0 0 8px' }}>
                        Symmetry <span style={{ background: 'linear-gradient(135deg, #0ea5e9, #6366f1)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>Skills</span>
                    </h1>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                    {SKILLS.map((s, i) => (
                        <div key={s.id} style={{ display: 'flex', alignItems: 'center', background: '#fff', padding: '24px 32px', borderRadius: 20, gap: 24, border: '1px solid rgba(0,0,0,0.04)', boxShadow: '0 4px 12px rgba(0,0,0,0.02)', transition: 'all 0.3s ease' }}>
                            ...
                            <div style={{ display: 'flex', gap: 14, alignItems: 'center', flexWrap: 'wrap' }}>
                                ...
                                <button style={{ padding: '10px 24px', borderRadius: 100, fontSize: 14, fontWeight: 700, cursor: 'pointer', background: s.color, color: '#fff', border: 'none', boxShadow: `0 4px 12px ${s.color}30` }} onClick={() => { setActiveSkill(i); setView('assessment'); }}>Assess</button>
                            </div>
                        </div>
                    ))}
                </div>
