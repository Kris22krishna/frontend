...
export default function AssessmentEngine({ questions, title, onBack, onSecondaryBack, color, prefix = 'pt' , nodeId }) {
    ...
    const getQuestionType = (question) => {
        ...
        if (question?.type === 'factor-tree') return 'factor-tree';
        if (question?.type === 'division-table') return 'division-table';
        return 'mcq';
    };
    ...
    const isAnswerComplete = (question, answer) => {
        ...
        if (type === 'factor-tree' || type === 'division-table') return !!(answer && typeof answer.finalAnswer === 'string' && normalizeTextAnswer(answer.finalAnswer).length > 0);
    ...
    const getUserAnswerLabel = (question, answer) => {
        ...
        if (type === 'factor-tree' || type === 'division-table') return answer.finalAnswer;
    ...
    if (finished) {
        ...
        return (
            <div className={styles[`${prefix}-quiz-finished`]} style={{ maxWidth: 900, margin: '0 auto', padding: '40px 20px', fontFamily: "'Inter', system-ui, sans-serif" }}>
                    ...
                    {questionSet.map((question, index) => {
                        ...
                        return (
                            ...
                            >
                                ...
                                {getQuestionType(question) === 'factor-tree' && answers[index] && (
                                    <FactorTreeInteractive question={question} value={answers[index]} readOnly={true} />
                                )}
                                {getQuestionType(question) === 'division-table' && answers[index] && (
                                    <DivisionTableInteractive question={question} value={answers[index]} readOnly={true} />
                                )}
        ...
        <div className={`${styles[`${prefix}-quiz-active`]} ${styles[`${prefix}-assessment-layout`]}`} style={{ fontFamily: "'Inter', system-ui, sans-serif", maxWidth: 1000, margin: '0 auto', display: 'flex', gap: 24, padding: '0 24px' }}>
            <div style={{ flex: 1, minWidth: 0 }} ref={topRef}>
                ...
                <div style={{ background: '#fff', borderRadius: 24, padding: 32, border: '1px solid rgba(0,0,0,0.05)', boxShadow: '0 10px 30px rgba(0,0,0,0.03)' }}>
                    ...
                    ) : getQuestionType(q) === 'factor-tree' ? (
                        <div style={{ display: 'grid', gap: 12 }}>
                            <FactorTreeInteractive question={q} value={answers[current]} onChange={handleAnswerChange} readOnly={false} />
                        </div>
                    ) : getQuestionType(q) === 'division-table' ? (
                        <div style={{ display: 'grid', gap: 12 }}>
                            <DivisionTableInteractive question={q} value={answers[current]} onChange={handleAnswerChange} readOnly={false} />
                        </div>
                    ) : (
