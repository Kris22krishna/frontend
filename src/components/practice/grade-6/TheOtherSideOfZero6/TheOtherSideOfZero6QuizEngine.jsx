...
export default function QuizEngine({ questions, title, onBack, onSecondaryBack, color, prefix = 'osz' , nodeId }) {
    ...
    const [draftNLAnswer, setDraftNLAnswer] = useState(null);
    ...
    useEffect(() => {
        ...
        } else if (answersMap[current] && q.type === 'number-line-click') {
            setDraftNLAnswer(answersMap[current].nlAnswer ?? null);
    ...
    const handleNLSubmit = () => {
        if (isAnswered) return;
        if (draftNLAnswer === null) return;
        const isCorrect = draftNLAnswer === q.targetAnswer;
        setAnswersMap(prev => ({ ...prev, [current]: { nlAnswer: draftNLAnswer, isCorrect } }));
    };
    ...
    return (
        <div className={`${prefix}-quiz-active ${prefix}-quiz-container`}>
            ...
            <div style={{ background: '#fff', borderRadius: 20, padding: '32px', border: '1px solid #e2e8f0', boxShadow: '0 4px 20px rgba(0,0,0,0.06)', marginBottom: 24 }}>
                ...
                {q.type === 'number-line-click' ? (
                    <div style={{ marginBottom: 24 }}>
                        <NumberLineInteractive
                            question={q}
                            answered={isAnswered}
                            userAnswer={isAnswered ? savedAnswer.nlAnswer : draftNLAnswer}
                            correctAnswer={q.targetAnswer}
                            onChange={setDraftNLAnswer}
                            color={color}
                        ...
                        {!isAnswered && (
                            <div style={{ marginTop: 16, textAlign: 'center' }}>
                                <button
                                    ...
                                    disabled={draftNLAnswer === null}
                                    style={{
                                        padding: '14px 40px', borderRadius: 12, border: 'none',
                                        background: draftNLAnswer !== null ? color : '#e2e8f0',
                                        color: draftNLAnswer !== null ? '#fff' : '#94a3b8',
                                        fontSize: 16, fontWeight: 800,
                                        cursor: draftNLAnswer !== null ? 'pointer' : 'not-allowed',
                                        boxShadow: draftNLAnswer !== null ? `0 4px 12px ${color}40` : 'none'
                                    }}
                                >Check Answer</button>
                            </div>
                        )}
                        {isAnswered && (
                            <div style={{ marginTop: 12, textAlign: 'center', fontSize: 16, fontWeight: 800, color: savedAnswer.isCorrect ? '#10b981' : '#ef4444' }}>
                                {savedAnswer.isCorrect ? '✅ Correct! Brilliant work.' : `❌ Incorrect. The correct answer is ${q.targetAnswer}.`}
                            </div>
                        )}
                    </div>
                ) : q.type === 'text' ? (
