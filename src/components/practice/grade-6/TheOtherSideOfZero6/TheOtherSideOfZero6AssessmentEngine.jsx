...
import NumberLineInteractive from './Topics/Skills/NumberLineInteractive';
import { useSessionLogger } from '@/hooks/useSessionLogger';
import './theOtherSideOfZero6.module.css';

export default function AssessmentEngine({ questions, title, onBack, onSecondaryBack, color, prefix = 'osz' , nodeId }) {
    const normalizeTextAnswer = (value) => String(value ?? '').replace(/\s+/g, ' ').trim().toLowerCase();

    const getQuestionType = (question) => {
        if (question?.type === 'text') return 'text';
        if (question?.type === 'number-line-click') return 'number-line-click';
        return 'mcq';
    };
    ...
    const isAnswerComplete = (question, answer) => {
        ...
        if (type === 'number-line-click') return answer !== null && answer !== undefined;
    ...
    const isAnswerCorrect = (question, answer) => {
        ...
        if (type === 'number-line-click') return answer === question.targetAnswer;
    ...
    const getCorrectAnswerLabel = (question) => {
        ...
        if (type === 'number-line-click') return String(question.targetAnswer);
    ...
    const getUserAnswerLabel = (question, answer) => {
        ...
        if (type === 'number-line-click') return String(answer);
    ...
    const palette = (
        <div className={`${prefix}-assessment-palette ${paletteOpen ? 'is-open' : ''}`} style={{ fontFamily: "'Inter', system-ui, sans-serif" }}>
            <div className={`${prefix}-assessment-mobile-head`} style={{ marginBottom: '24px' }}>
                <div>
            ...
            >
                <span style={{ fontSize: 13, textTransform: 'uppercase', letterSpacing: 1, marginRight: 8, opacity: 0.8 }}>Time Left</span> {formatTime(timeLeft)}
            </div>

            <div className={`${prefix}-palette-mobile-stats`} style={{ display: 'flex', gap: '12px', marginBottom: '24px' }}>
                <div style={{ flex: 1, background: '#f8fafc', padding: '12px', borderRadius: '12px', border: '1px solid #e2e8f0' }}>
                    <span style={{ display: 'block', fontSize: '10px', fontWeight: 800, color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: '4px' }}>Answered</span>
                    <strong style={{ fontSize: '18px', fontWeight: 800, color: '#0f172a' }}>{answeredCount} / {questionSet.length}</strong>
                </div>
                <div style={{ flex: 1, background: '#f8fafc', padding: '12px', borderRadius: '12px', border: '1px solid #e2e8f0' }}>
                    <span style={{ display: 'block', fontSize: '10px', fontWeight: 800, color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: '4px' }}>Current</span>
                    <strong style={{ fontSize: '18px', fontWeight: 800, color: '#0f172a' }}>Q{current + 1}</strong>
                </div>
            </div>
            ...
            <div style={{ marginTop: 24, fontSize: 13, color: '#64748b' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
                    <div className={`${prefix}-palette-cell is-answered`} style={{ width: 16, height: 16, borderRadius: 4, cursor: 'default' }} /> Answered
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
                    <div className={`${prefix}-palette-cell`} style={{ width: 16, height: 16, borderRadius: 4, cursor: 'default' }} /> Not Answered
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    <div className={`${prefix}-palette-cell is-marked`} style={{ width: 16, height: 16, borderRadius: 4, cursor: 'default' }} /> Marked for Review
                </div>
            </div>

            <button onClick={handleSubmit} className={`${prefix}-submit-btn`}>
                Submit Assessment
            </button>
        </div>
    );
        ...
        <div className={`${prefix}-quiz-active ${prefix}-assessment-layout`} style={{ fontFamily: "'Inter', system-ui, sans-serif", maxWidth: 1000, margin: '0 auto', display: 'flex', gap: 24, padding: '0 24px' }}>
            <div style={{ flex: 1, minWidth: 0 }} ref={topRef}>
                ...
                <div className={`${prefix}-quiz-card`} style={{ background: '#fff', borderRadius: 24, padding: 32, border: '1px solid rgba(0,0,0,0.05)', boxShadow: '0 10px 30px rgba(0,0,0,0.03)' }}>
                    ...
                    {getQuestionType(q) === 'number-line-click' ? (
                        <div style={{ marginBottom: 8 }}>
                            <NumberLineInteractive
                                ...
                                onChange={(val) => handleTextAnswerChange(val)}
                                color={color}
                                showResult={false}
                            />
                        </div>
