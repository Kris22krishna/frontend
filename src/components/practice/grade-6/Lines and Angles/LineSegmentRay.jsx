...
const CORRECT_MESSAGES = [
    "✨ Spot on! You can clearly see the endpoints! ✨",
    "🌟 Geometry Genius! Parallel paths identified! 🌟",
    "🎉 Correct! You're mastering the fundamental strokes! 🎉",
    "✨ Perfect! You know exactly where things begin and end! ✨",
    "🚀 Way to go! You're navigating these geometric paths brilliantly! 🚀"
];

const LineSegmentRay = () => {
    ...
    const SKILL_ID = 2002;
    const SKILL_NAME = "Line, Line Segment and Ray";
    ...
    const generateQuestion = (index, retryCount = 0) => {
        const types = ["define_ray", "define_segment", "define_line", "endpoints_count"];
        ...
        if (type === "define_ray") {
            qText = "Identify the figure shown below:";
            correct = "Ray";
            options = ["Ray", "Line", "Line Segment", "Point"];
            explanation = "A Ray starts at one fixed point and goes on forever in exactly one direction, represented by one arrow.";
            diagram = (
                <svg width="200" height="60" viewBox="0 0 200 60">
                    <circle cx="40" cy="30" r="4" fill="#31326F" />
                    <text x="35" y="50" fontSize="12" fill="#31326F">A</text>
                    <line x1="40" y1="30" x2="160" y2="30" stroke="#31326F" strokeWidth="2" />
                    <polygon points="160,25 170,30 160,35" fill="#31326F" />
                    <circle cx="120" cy="30" r="3" fill="#31326F" />
                    <text x="115" y="50" fontSize="12" fill="#31326F">B</text>
                </svg>
            );
        } else if (type === "define_segment") {
            qText = "Which of the following has a fixed length?";
            correct = "Line Segment";
            options = ["Line Segment", "Line", "Ray", "Infinite Line"];
            explanation = "A line segment is a part of a line with two fixed endpoints. Because it has ends, we can measure its exact length.";
            diagram = (
                <svg width="200" height="60" viewBox="0 0 200 60">
                    <circle cx="50" cy="30" r="4" fill="#31326F" />
                    <circle cx="150" cy="30" r="4" fill="#31326F" />
                    <line x1="50" y1="30" x2="150" y2="30" stroke="#31326F" strokeWidth="2" />
                    <text x="45" y="50" fontSize="12" fill="#31326F">P</text>
                    <text x="145" y="50" fontSize="12" fill="#31326F">Q</text>
                </svg>
            );
        } else if (type === "define_line") {
            qText = "Look at the arrows on both sides. This figure represents a:";
            correct = "Line";
            options = ["Line", "Line Segment", "Ray", "Curve"];
            explanation = "A line goes on forever in both directions. The two arrows indicate that it has no endpoints.";
            diagram = (
                <svg width="200" height="60" viewBox="0 0 200 60">
                    <line x1="30" y1="30" x2="170" y2="30" stroke="#31326F" strokeWidth="2" />
                    <polygon points="30,25 20,30 30,35" fill="#31326F" />
                    <polygon points="170,25 180,30 170,35" fill="#31326F" />
                    <circle cx="70" cy="30" r="3" fill="#31326F" />
                    <circle cx="130" cy="30" r="3" fill="#31326F" />
                    <text x="65" y="50" fontSize="12" fill="#31326F">X</text>
                    <text x="125" y="50" fontSize="12" fill="#31326F">Y</text>
                </svg>
            );
        } else {
            const count = randomInt(0, 2);
            const targets = ["Line", "Ray", "Line Segment"];
            const answers_map = [0, 1, 2];
            const target = targets[count];
            qText = `How many endpoints does a **${target}** have?`;
            correct = answers_map[count].toString();
            options = ["0", "1", "2", "Infinite"];
            explanation = target === "Line" ? "A line has 0 endpoints (goes forever both ways)." : target === "Ray" ? "A ray has 1 endpoint (the starting point)." : "A line segment has 2 endpoints (start and stop).";
        }
