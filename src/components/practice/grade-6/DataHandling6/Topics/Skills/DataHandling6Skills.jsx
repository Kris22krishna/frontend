...
import QuizEngine from '../../DataHandling6QuizEngine.jsx';
import AssessmentEngine from '../../DataHandling6AssessmentEngine.jsx';
import { SKILLS } from './DataHandling6SkillsData.jsx';

export default function DataHandling6Skills() {
    ...
    if (view !== 'list' && skill) {
        return (
            <div className={styles['dh-page']} style={{ background: '#f8fafc', minHeight: '100vh', paddingBottom: '60px' }}>
                <nav className={styles['dh-intro-nav']}>
                    <button className={styles['dh-intro-nav-back']} onClick={() => { setView('list'); setSelectedLearnIdx(0); }}>← Back to Skills</button>
                    <div className={styles['dh-intro-nav-links']}>
                        <button className={styles['dh-intro-nav-link']} onClick={() => navigate('/middle/grade/6/data-handling-6/introduction')}>🌟 Intro</button>
                        <button className={styles['dh-intro-nav-link']} onClick={() => navigate('/middle/grade/6/data-handling-6/terminology')}>📖 Terminology</button>
                        <button className={`${styles['dh-intro-nav-link']} ${styles['dh-intro-nav-link--active']}`}>🎯 Skills</button>
                    </div>
                </nav>
            ...
            <div style={{ padding: '20px 24px 80px', maxWidth: 1100, margin: '0 auto' }}>
                ...
                <div style={{ textAlign: 'center', marginBottom: 40 }}>
                    <h1 style={{ fontFamily: 'Outfit, sans-serif', fontSize: '2.8rem', fontWeight: 900, color: '#0f172a', margin: '0 0 8px' }}>
                        Data <span style={{ background: 'linear-gradient(135deg, #0369a1, #0891b2)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>Skills</span>
                    </h1>
