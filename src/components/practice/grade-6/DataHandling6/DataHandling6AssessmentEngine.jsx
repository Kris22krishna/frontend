...
export default function AssessmentEngine({ questions, title, onBack, onSecondaryBack, color, prefix = 'dh' , nodeId }) {
    ...
    const getQuestionType = (question) => {
        ...
        if (['tally-draw', 'pictograph-draw', 'bar-graph-draw'].includes(question?.type)) return question.type;
        return 'mcq';
    };
    ...
    const isAnswerComplete = (question, answer) => {
        ...
        if (['tally-draw', 'pictograph-draw', 'bar-graph-draw'].includes(type)) return answer !== null && answer !== undefined;
    ...
    const isAnswerCorrect = (question, answer) => {
        ...
        if (type === 'tally-draw') return answer === question.targetCount;
        if (['pictograph-draw', 'bar-graph-draw'].includes(type)) return JSON.stringify(answer) === JSON.stringify(question.targetCounts);
    ...
    const getCorrectAnswerLabel = (question) => {
        ...
        if (['tally-draw', 'pictograph-draw', 'bar-graph-draw'].includes(type)) return '[Visual Drawing Activity]';
    ...
    const getUserAnswerLabel = (question, answer) => {
        ...
        if (['tally-draw', 'pictograph-draw', 'bar-graph-draw'].includes(type)) return '[Visual Drawing Input]';
                    ...
                    {questionSet.map((question, index) => {
                        ...
                        return (
                            ...
                            >
                                ...
                                {['tally-draw', 'pictograph-draw', 'bar-graph-draw'].includes(getQuestionType(question)) ? (
                                    <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1fr) minmax(0,1fr)', gap: 16 }}>
                                        <div style={{ background: '#fff', padding: 16, borderRadius: 12, border: '1px solid #e2e8f0', pointerEvents: 'none', overflow: 'hidden' }}>
                                            ...
                                            <div style={{ transform: 'scale(0.85)', transformOrigin: 'top center', display: 'flex', justifyContent: 'center' }}>
                                                {getQuestionType(question) === 'tally-draw' && (
                                                    <TallyDrawInteractive question={question} answered={true} userAnswer={question.targetCount} onChange={() => {}} color="#10b981" prefix={prefix} />
                                                )}
                                                {getQuestionType(question) === 'pictograph-draw' && (
                                                    <PictographDrawInteractive question={question} answered={true} userAnswer={question.targetCounts} onChange={() => {}} color="#10b981" prefix={prefix} />
                                                )}
                                                {getQuestionType(question) === 'bar-graph-draw' && (
                                                    <BarGraphDrawInteractive question={question} answered={true} userAnswer={question.targetCounts} onChange={() => {}} color="#10b981" prefix={prefix} />
                                                )}
                                            </div>
                                        </div>
                                        <div style={{ background: '#fff', padding: 16, borderRadius: 12, border: '1px solid #e2e8f0', pointerEvents: 'none', overflow: 'hidden' }}>
                                            ...
                                            {userOptText === 'Not Answered' ? (
                                                ...
                                                <div style={{ transform: 'scale(0.85)', transformOrigin: 'top center', display: 'flex', justifyContent: 'center' }}>
                                                    {getQuestionType(question) === 'tally-draw' && (
                                                        <TallyDrawInteractive question={question} answered={true} userAnswer={answers[index]} onChange={() => {}} color={statusColor} prefix={prefix} />
                                                    )}
                                                    {getQuestionType(question) === 'pictograph-draw' && (
                                                        <PictographDrawInteractive question={question} answered={true} userAnswer={answers[index]} onChange={() => {}} color={statusColor} prefix={prefix} />
                                                    )}
                                                    {getQuestionType(question) === 'bar-graph-draw' && (
                                                        <BarGraphDrawInteractive question={question} answered={true} userAnswer={answers[index]} onChange={() => {}} color={statusColor} prefix={prefix} />
                                                    )}
                                                </div>
                                            )}
                                        </div>
        ...
        <div className={`${prefix}-quiz-active ${prefix}-assessment-layout`} style={{ fontFamily: "'Inter', system-ui, sans-serif", maxWidth: 1000, margin: '0 auto', display: 'flex', gap: 24, padding: '0 24px' }}>
            <div style={{ flex: 1, minWidth: 0 }} ref={topRef}>
                ...
                <div className={`${prefix}-quiz-card`} style={{ background: '#fff', borderRadius: 24, padding: 32, border: '1px solid rgba(0,0,0,0.05)', boxShadow: '0 10px 30px rgba(0,0,0,0.03)' }}>
                    ...
                    {['tally-draw', 'pictograph-draw', 'bar-graph-draw'].includes(getQuestionType(q)) ? (
                        <div style={{ marginBottom: 24 }}>
                            {getQuestionType(q) === 'tally-draw' && (
                                <TallyDrawInteractive question={q} answered={false} userAnswer={answers[current]} onChange={handleTextAnswerChange} color={color} prefix={prefix} />
                            )}
                            {getQuestionType(q) === 'pictograph-draw' && (
                                <PictographDrawInteractive question={q} answered={false} userAnswer={answers[current]} onChange={handleTextAnswerChange} color={color} prefix={prefix} />
                            )}
                            {getQuestionType(q) === 'bar-graph-draw' && (
                                <BarGraphDrawInteractive question={q} answered={false} userAnswer={answers[current]} onChange={handleTextAnswerChange} color={color} prefix={prefix} />
                            )}
                        </div>
