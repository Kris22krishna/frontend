...
import { TERMS, FOUR_RULES, VOCAB_QUIZ } from './NumberPlay6TerminologyData.jsx';

export default function NumberPlay6Terminology() {
    ...
    return (
        <div className={styles['np6-terminology-page']} style={{
            fontFamily: "'Inter', system-ui, sans-serif",
            background: "#f8fafc",
            minHeight: "calc(100vh - 80px)"
        }}>
            <style>{`
                .np6-details-window-anim {
                ...
                .np6-term-btn-mini:hover {
                    transform: translateY(-2px);
                    box-shadow: 0 6px 15px rgba(0,0,0,0.08);
                }
                .np6-term-btn-mini:hover::before {
                    opacity: 0.9;
                }
                .np6-term-btn-mini.active {
                    transform: translateY(-1px);
                    box-shadow: 0 8px 20px rgba(0,0,0,0.1);
                    z-index: 2;
                }
                .np6-term-btn-mini.active::before {
                    opacity: 0;
                }
                .np6-term-btn-mini > * {
                    position: relative;
            ...
            <nav className={styles['np6-intro-nav']}>
                ...
                <div className={styles['np6-intro-nav-links']}>
                    <button className={styles['np6-intro-nav-link']} onClick={() => navigate('/middle/grade/6/number-play-chapter/introduction')}>🌟 Introduction</button>
                    <button className={`${styles['np6-intro-nav-link']} ${styles['np6-intro-nav-link--active']}`}>📖 Terminology</button>
                    <button className={styles['np6-intro-nav-link']} onClick={() => navigate('/middle/grade/6/number-play-chapter/skills')}>🎯 Skills</button>
                </div>
            </nav>

            {/* ── MAIN CONTENT ──────────────────────────────── */}
            <div className="np6-lexicon-container" style={{ maxWidth: 1100, margin: '40px auto 20px', padding: '0 24px' }}>

                {/* Heading Stack */}
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', marginBottom: 20 }}>
                    <h1 style={{ fontFamily: 'Outfit, sans-serif', fontSize: '2.8rem', fontWeight: 900, color: '#0f172a', margin: '0 0 8px' }}>
                        Number Play <span style={{ background: 'linear-gradient(135deg, #0891b2, #6366f1)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>Vocabulary</span>
                    </h1>
                ...
                <div style={{ display: 'flex', justifyContent: 'center', gap: 8, marginBottom: 16 }}>
                    <button className={`${styles['np6-tab']} ${activeTab === 'terms' ? styles.active : ''}`} onClick={() => setActiveTab('terms')}>📚 Terminology</button>
                    <button className={`${styles['np6-tab']} ${activeTab === 'rules' ? styles.active : ''}`} onClick={() => setActiveTab('rules')}>📏 Number Rules</button>
                    <button className={`${styles['np6-tab']} ${activeTab === 'quiz' ? styles.active : ''}`} onClick={() => setActiveTab('quiz')}>🧪 Test Prep</button>
                </div>

                {activeTab !== 'quiz' ? (
                    /* ── MASTER-DETAIL VIEW (Terms or Rules) ── */
                    <div className="np6-lexicon-grid" style={{ display: 'grid', gridTemplateColumns: 'minmax(300px, 360px) 1fr', gap: 16, alignItems: 'start' }}>
                        <aside className="np6-selector-container" style={{
                            ...
                            {activeTab === 'terms' ? (
                                TERMS.map((term, i) => {
                                    ...
                                    return (
                                        <button key={i} className={`np6-term-btn-mini ${isActive ? 'active' : ''}`} onClick={() => setSelectedIdx(i)}
                                            style={{
                                ...
                                FOUR_RULES.map((rule, i) => {
                                    ...
                                    return (
                                        <button key={i} className={`np6-term-btn-mini ${isActive ? 'active' : ''}`} onClick={() => setSelectedRuleIdx(i)}
                        ...
                        <main className="np6-details-window-anim" key={activeTab === 'terms' ? selectedIdx : selectedRuleIdx} style={{
                            ...
                            {activeTab === 'terms' ? (
                                <>
                                    ...
                                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginTop: 8 }}>
                                        <div>
                                            ...
                                            <div style={{ background: `${activeTerm.color}05`, padding: 16, borderRadius: 16, border: `1px solid ${activeTerm.color}10`, overflowX: 'auto' }}>
                                                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                                                    {activeTerm.examples.map((ex, j) => (
                                                        <code key={j} style={{ background: '#fff', border: `1px solid ${activeTerm.color}20`, color: activeTerm.color, padding: '4px 10px', borderRadius: 8, display: 'inline-flex', alignItems: 'center', whiteSpace: 'nowrap' }}>
                                ...
                                <>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
                                        ...
                                        <h2 style={{ fontFamily: 'Outfit, sans-serif', fontSize: 28, fontWeight: 900, color: activeRule.color, margin: 0 }}>Rule: {activeRule.title}</h2>
                                    </div>
                                    ...
                                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 20 }}>
                                        <div>
                                            ...
                                            <div style={{ background: '#f8fafc', padding: 16, borderRadius: 16, border: '1px solid rgba(0,0,0,0.04)', overflowX: 'auto' }}>
                                                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                                                    {activeRule.examples.map((ex, j) => (
                                                        <div key={j} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                                                            <div style={{ width: 6, height: 6, borderRadius: '50%', background: activeRule.color, flexShrink: 0 }} />
                                                            <span style={{ fontSize: 15, background: '#fff', padding: '3px 8px', borderRadius: 6, color: '#0f172a', fontWeight: 600, whiteSpace: 'nowrap' }}>
                                                                <MathRenderer text={ex} />
                                                            </span>
                                                        </div>
                                                    ))}
                    ...
                    /* ── VOCABULARY TEST TAB ── */
                    <div className="np6-details-window-anim" style={{ maxWidth: 800, margin: '0 auto', background: '#fff', borderRadius: 24, padding: '32px 40px', boxShadow: '0 15px 40px rgba(0,0,0,0.04)', border: '1px solid rgba(0,0,0,0.04)' }}>
                        {!quizFinished ? (
                            <>
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 28 }}>
                                    <div>
                                        <div style={{ fontSize: 13, fontWeight: 800, color: '#0369a1', textTransform: 'uppercase', letterSpacing: 1 }}>Question {quizIdx + 1} of 10</div>
                                        <h3 style={{ fontFamily: 'Outfit, sans-serif', fontSize: 24, fontWeight: 800, color: '#0f172a', margin: 0 }}>Vocabulary Check</h3>
                                    </div>
                                    <div style={{ width: 70, height: 70, borderRadius: '50%', border: '5px solid #f1f5f9', borderTopColor: '#0369a1', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20, fontWeight: 900, color: '#0369a1' }}>{quizIdx + 1}/10</div>
                                </div>
                                {activeQuiz.svg && (
                                ...
                                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 28 }}>
                                    {activeQuiz.options.map((opt, oi) => {
                                        ...
                                        } else if (quizSelected === oi) { bCol = '#0369a1'; bgCol = 'rgba(3,105,161,0.05)'; }
                                        return (
                                ...
                                {quizAnswered && (
                                    <div style={{ background: 'rgba(3,105,161,0.05)', padding: 16, borderRadius: 12, border: '1px solid rgba(3,105,161,0.2)', marginBottom: 24 }}>
                                        <p style={{ margin: 0, fontSize: 14, color: '#64748b', lineHeight: 1.6 }}><strong style={{ color: '#0369a1' }}>Solution: </strong><MathRenderer text={activeQuiz.explanation} /></p>
                                    </div>
                                )}
                                <div style={{ display: 'flex', justifyContent: 'center' }}>
                                    <button onClick={nextQuiz} disabled={!quizAnswered} style={{ padding: '12px 40px', background: quizAnswered ? '#0369a1' : '#f1f5f9', color: quizAnswered ? '#fff' : '#94a3b8', borderRadius: 100, border: 'none', cursor: quizAnswered ? 'pointer' : 'not-allowed', fontWeight: 600 }}>{quizIdx + 1 === 10 ? 'Finish Test' : 'Next Question →'}</button>
                                </div>
                            </>
                        ) : (
                            <div style={{ textAlign: 'center', padding: '20px 0' }}>
                                ...
                                <p style={{ color: '#64748b', fontSize: 18, marginBottom: 32 }}>Your Vocabulary Score: <span style={{ color: '#0369a1', fontWeight: 900 }}>{quizTotalScore} / 10</span></p>
                                <div style={{ display: 'flex', gap: 12, justifyContent: 'center' }}>
                                    <button style={{ padding: '12px 30px', background: '#0891b2', color: '#fff', borderRadius: 100, border: 'none', fontWeight: 600, cursor: 'pointer' }} onClick={resetQuiz}>Try Again</button>
                                    <button style={{ padding: '12px 30px', background: '#fff', color: '#0891b2', border: '2px solid #0891b2', borderRadius: 100, fontWeight: 600, cursor: 'pointer' }} onClick={() => navigate('/middle/grade/6/number-play-chapter/skills')}>Go to Skills 🎯</button>
                                </div>
                            </div>
                        )}
                    </div>
                ...
                <div style={{ marginTop: 16, textAlign: 'center' }}>
                    <button style={{ padding: '12px 30px', background: '#0891b2', color: '#fff', borderRadius: 100, border: 'none', fontWeight: 600, cursor: 'pointer' }} onClick={() => navigate('/middle/grade/6/number-play-chapter/skills')}>Ready to Solve! 🎯</button>
                </div>
            </div>
        </div>
    );
