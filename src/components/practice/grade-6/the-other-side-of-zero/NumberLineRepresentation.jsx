...
import { Check, Eye, ChevronRight, ChevronLeft, ArrowRight, HelpCircle, X } from 'lucide-react';
    ...
    const generateQuestion = (index, retryCount = 0) => {
        ...
        if (type === "point_identification") {
            ...
            if (a > 0) {
                ...
                optSet.add(correct);
                optSet.add(`$${a}$ units to the left of $0$`);
                optSet.add(`At the origin`);
                optSet.add(`$${a + 1}$ units to the right`);
            } else if (a < 0) {
                correct = `$${Math.abs(a)}$ units to the left of $0$`;
                optSet.add(correct);
                optSet.add(`$${Math.abs(a)}$ units to the right of $0$`);
                optSet.add(`At the origin`);
                optSet.add(`$${Math.abs(a) + 1}$ units to the left`);
            } else {
                correct = `At the point zero ($0$)`;
                optSet.add(correct);
                optSet.add(`1 unit to the right`);
                optSet.add(`1 unit to the left`);
                optSet.add(`Cannot be plotted`);
            }
            ...
            correct = `${dist} units`;
            const optSet = new Set([correct, `${Math.abs(a + b)} units`, `${Math.abs(a)} units`, `${Math.abs(b)} units`]);
            while (optSet.size < 4) optSet.add(`${dist + randomInt(1, 5)} units`);
            options = Array.from(optSet);
            explanation = `Distance is the absolute difference between the two numbers: $|${a} - (${b})| = ${dist}$.`;
            uniqueId = `nl_dist_${a}_${b}`;
        } else {
            const a = randomInt(1, 10);
            qText = `If point A is at $${a}$, where is its opposite point located on the number line?`;
            correct = `At $-${a}$`;
            const optSet = new Set([correct, `At $${a}$`, `At $0$`, `At $${a + 1}$`]);
            while (optSet.size < 4) optSet.add(`At $${-a + randomInt(-2, 2)}$`);
            options = Array.from(optSet);
